...
By a $\Vbb$-module $\Wbb$, we always assume that it is a (grading-restricted) ordinary $\Vbb$-module with vertex operators $Y_\Mbb(v,z)=\sum_{n\in\Zbb}Y_\Mbb(v)_nz^{-n-1}$ and Virasoro operators  $L_n=Y_\Mbb(\cbf)_{n+1}$. In particular, we assume that $\Wbb$ is a \emph{finite} direct sum of weak $\Vbb$-modules $\Wbb=\oplus_i\Wbb_i$, where each $\Wbb_i$ has \textbf{simple $L_0$-grading}.  The latter means that each $\Wbb_i$ has $L_0$-grading $\Wbb_i=\oplus_{n\in\Nbb}\Wbb_{i,(n+a)}$ for some $a\in\Cbb$, and each $\Wbb_{i,(n+a)}$ is finite dimensional. By adding a constant to $\wtd L_0$, we have a diagonal $\wtd L_0$ on $\Wbb_i$ whose eigenvalues are all in $\Nbb$. Combining all these $\wtd L_0$ on $\Wbb_i$ together, we have a diagonalizable $\wtd L_0$ on $\Wbb$ which gives grading  \index{L0@$\wtd L_0$}  \index{W@$\Wbb(n),\Wbb_{(n)}$}
\begin{align*}
\Wbb=\bigoplus_{n\in E+\Nbb}\Wbb_{(n)}
\end{align*}
where each $\Wbb_{(n)}$ is the eigenspace of $L_0$ with eigenvalue $n$, which is finite-dimensional.  

The $\wtd L_0$-grading makes $\Wbb$ an admissible $\Vbb$-module. To be more precise, since $\wtd L_0-L_0\in\End_\Vbb(\Wbb)$, we  have $[\wtd L_0,Y_\Wbb(v)_n]=[L_0,Y_\Wbb(v)_n]=Y_\Wbb(L_0 v)_n-(n+1)Y_\Wbb(v)_n$, and hence
\begin{align}
...
\begin{cv}\label{lb31}
Suppose an $\wtd L_0$-grading of $\Wbb$ is chosen. We choose the $\wtd L_0$-grading of the contragredient module $\Wbb'$ to be $\Wbb'=\bigoplus_{n\in\Nbb}\Wbb(n)^*$. Thus
\begin{align*}
\wtd L_0^\tr=\wtd L_0,
\end{align*}
...
Theorem \ref{lb21} is the complex-analytic analogue of \cite{DGT19b} Thm. 8.2.1, which says that for an algebraic family of complex curves, the sheaf of covacua is coherent (assuming that $\Vbb$ is $C_2$-cofinite and $\Vbb(0)=\Cbb\id$). The key ideas in our proof are similar to theirs.
...
\end{align*}
Recall $\wtd L_0^\tr=\wtd L_0$ by Convention \ref{lb31}. Define
\begin{align*}
...
\end{align}
$\mc S\uppsi$ is defined similarly, except that the normalized energy operator $\wtd L_0$ is replaced by the standard one $L_0$. When $\Mbb$ and hence $\Mbb'$ have simple $L_0$-grading, $\wtd{\mc S}\upphi$ differs from $\mc S\upphi$ by a factor $q^\lambda$ for some $\lambda\in\Cbb$. 
...
We continue our discussions and assume the setting in Section \ref{lb35}. Recall that $\mc B$ equals $\mc D_{r\rho}\times\wtd{\mc B}$. Then $\mc B$ is Stein if  $\wtd{\mc B}$ is so. We also assume,  unless otherwise stated, that $\Mbb$ and hence $\Mbb'$ have simple $L_0$-grading. So $\wtd{\mc S}\uppsi$ and $\mc S\uppsi$ differ by a scalar multiplication of $q^\lambda$.  We identify $\scr W_{\fk X}(\Wbb_\blt)$ with $\Wbb_\blt\otimes_\Cbb\scr O_{\mc B}$ via $\mc U(\eta_\blt)$.
...
In the remaining part of this section, \emph{we assume $\Vbb$ is $C_2$-cofinite and $\Vbb(0)=\Cbb\id$, and the $\Vbb$-modules $\Wbb_1,\dots,\Wbb_N$ are finitely-generated}. 
...
Let $\mc E$ be a complete list of mutually inequivalent simple $\Vbb$-modules. ``Complete" means that any simple $\Vbb$-module is equivalent to an object of $\mc E$. Assume $\Vbb$ is $C_2$-cofinite and $\Vbb(0)=\Cbb\id$. Then by Theorems \ref{lb47} and \ref{lb48}, for each $q\in\mc D_{r\rho}^\times$, we can define a \index{Sq@$\fk S_q,\wtd{\fk S}_q$} linear map
\begin{gather}
...
Associate to $\sgm_1,\dots,\sgm_N$ finitely-generated $\Vbb$-modules $\Wbb_1,\dots,\Wbb_N$. Associate to $\sgm_1',\dots,\sgm_M'$ $\Vbb$-modules $\Mbb_1,\dots,\Mbb_M$, whose contragredient modules $\Mbb_1',\dots,\Mbb_M'$ are associated to $\sgm_1'',\dots,\sgm_M''$. We understand $\Wbb_\blt\otimes\Mbb_\blt\otimes\Mbb_\blt'$ as
\begin{align*}
