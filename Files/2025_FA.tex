...
\subsection{Perturbation of self-adjoint operators}
...
\end{gather}
\end{enumerate}
The number $\alpha$ is called \index{00@Bounding constant} a \textbf{bounding constant}.
\end{df}


Very often, to verify condition (2), it suffices to verify the inequality \eqref{eq271} for all $\xi$ in a core for $A$:
...
\subsubsection{Perturbation of unbounded operators with bounded inverse}

\begin{thm}\label{lb575}
Let $A,B$ be unbounded operators on $\MH$. Assume that $A$ has an everywhere-defined bounded inverse $A^{-1}\in\fk L(\MH)$. Assume also that $B$ satisfies conditions (a) and (b) of Def. \ref{lb571} with $0\leq\alpha<1$ and $\beta=0$. Then $A+B$ has an everywhere-defined bounded inverse.
\end{thm}
...
Let $A$ be a self-adjoint operator on $\MH$, and let $B$ be a Hermitian operator on $\MH$. Suppose that $B$ is $A$-bounded with bounding constant $0\leq\alpha<1$. Then $A+B$ is a self-adjoint operator on $\MH$ having the same cores as $A$.
\end{thm}
...
For each $y\in\Rbb_{>0}$, observe that $A+B$ is self-adjoint iff $y^{-1}(A+B)$ is self-adjoint. Therefore, by Cor. \ref{lb420}, it suffices to show that there exists $y\in\Rbb_{>0}$ such that $\Rng(A\pm y\im+B)=\MH$. We shall prove this by showing that $A\pm y\im$ and $B$ satisfy the assumptions in Thm. \ref{lb575}.

Since $y^{-1}A$ is self-adjoint for each $y$, Cor. \ref{lb420} implies that $A+y\im$ is surjective. By \eqref{eq197}, we have
\begin{align}\label{eq273}
\Vert (A\pm y\im)\xi\Vert^2=\Vert A\xi\Vert^2+y^2\Vert\xi\Vert^2
\end{align}
and hence $\Vert A\xi+y\im\xi\Vert\geq y\Vert\xi\Vert$. We conclude that $A+y\im$ has an everywhere-defined bounded inverse $(A+y\im)^{-1}\in\fk L(\MH)$. (This also follows immediately from the representation of $A$ as a multiplication operator.)

Note that $\Dom(B)\subset\Dom(A)=\Dom(A\pm y\im)$. By \eqref{eq271} and \eqref{eq273}, for each $\xi\in\Dom(A\pm y\im)$ we have
\begin{align*}
\Vert B\xi\Vert\leq\alpha\Vert A\xi\Vert+\beta\Vert\xi\Vert\leq \alpha\Vert (A\pm y\im)\xi\Vert+y^{-1}\beta\Vert (A\pm y\im)\xi\Vert
\end{align*}
Since $0\leq\alpha<1$, there exists $y\in\Rbb_{>0}$ such that $0\leq\alpha+y^{-1}\beta<1$. For such $y$, we conclude from Thm. \ref{lb575} that $\Rng(A\pm y\im+B)=\MH$. \\[-1ex]

Step 2. To show that $A$ and $A+B$ share the same cores, it suffices to show that the graph inner products on $\Dom(A)$ given by $A$ and $A+B$ are equivalent (in the sense of Def. \ref{lb481}). This follows from the inequalities for each $\xi\in\Dom(A)$:
\begin{align*}
\Vert (A+B)\xi\Vert\leq \Vert(1+\alpha)A\xi\Vert+\beta\Vert\xi\Vert
\end{align*}
and
\begin{align*}
...
\begin{proof}
Recall from Cor. \ref{lb544} that $\ovl\Delta$ is self-adjoint. Write $V=V_1+V_2$ where $V_1\in L^\infty$ and $V_2\in L^2$ are real-valued. Then $\Mbf_{V_1}$ is $\Delta$-bounded with bounding constant $0$. By Lem. \ref{lb576}, for each $\alpha>0$ there exists $\beta>0$ such that for any $f\in C_c^\infty(\Rbb^N)$ we have
\begin{align*}
...
\end{align*}
Thus, $\Mbf_{V_2}$ is $\Delta$-bounded with arbitrarily small bounding constant. The same is true for $\Mbf_V$. Hence, by Prop. \ref{lb574}, $\Mbf_V$ is $\ovl\Delta$-bounded with arbitrarily small bounding constant. In particular, $\Dom(\ovl\Delta)\subset\Dom(\Mbf_V)$. By the Kato-Rellich Thm \ref{lb572}, $T$ is self-adjoint and shares the same cores with $\ovl\Delta$. In particular, $C_c^\infty(\Rbb^N)$ is a core for $T$.
\end{proof}
...
\subsubsection{Application to partial differential equations}


\begin{pp}\label{lb578}
Let $A,B$ be unbounded operators on $\MH$. Assume that $A$ is positive and self-adjoint, and that $B$ is $\sqrt A$-bounded. Then for each $\alpha\in\Rbb_{>0}$, $B$ is $A$-bounded with bounding constant $\alpha$. 
\end{pp}


\begin{proof}
It suffices to show that $\sqrt A$ is $A$-bounded with arbitrarily small bounding constant. For each $\xi\in\Dom(A)$, we compute that
\begin{align*}
...
\begin{thm}
Let $A,B$ be Hermitian operators on $\MH$. Assume that $A$ is positive and self-adjoint, and that $B$ is $\sqrt A$-bounded. Then $A+B$ is a lower-bounded self-adjoint operator on $\MH$ and shares the same cores with $A$.
\end{thm}

\begin{proof}
By assumption, $\sqrt A$ and $B$ satisfy conditions (a) and (b) in Def. \ref{lb571} with $\alpha<1$. Therefore, for each $\xi\in\Dom(A)\subset\Dom(\sqrt B)\subset B$, we have
\begin{align*}
2|\bk{\xi|B\xi}|\leq 2\Vert B\xi\Vert\cdot\Vert\xi\Vert \leq\Vert B\xi\Vert^2+\Vert\xi\Vert^2\leq \alpha\Vert\sqrt A\xi\Vert^2+(1+\beta)\Vert\xi\Vert^2
\end{align*}
and hence
\begin{align*}
2\bk{\xi|(A+B)\xi}\geq& 2\bk{\xi|A\xi}-2|\bk{\xi|B\xi}|\\
&\geq (2-\alpha)\bk{\xi|A\xi}-(1+\beta)\Vert\xi\Vert^2\geq -(1+\beta)\Vert\xi\Vert^2
\end{align*}
This proves that $A+B$ is lower-bounded.
 
By Prop. \ref{lb578}, $B$ is $A$ is bounded with arbitrarily small bounding constant. Therefore, by the Kato-Rellich Thm. \ref{lb572}, $A+B$ is self-adjoint and share the same cores with $A$. 
\end{proof}
