...
\hyperlink{current}{Current writing}\qquad \hyperlink{page.2}{Last page of TOC}\qquad
...
\end{align}
holds for any simple function $f:X\rightarrow\Rbb_{\geq0}$, and hence (by MCT) for any measurable $f:X\rightarrow\Rbb_{\geq0}$. If $Y$ is a topological space and $\fk N$ contains $\fk B_Y$, we call
\begin{align*}
...
\begin{enumerate}
\item Prove that $\dps\mu(E)\leq\liminf_\alpha\mu_\alpha(E)$ if $E$ is open, and that $\dps\mu(E)\geq\limsup_\alpha\mu_\alpha(E)$ if $E$ is closed.
\item Suppose that $\Cl_X(E)\setminus\Int_X(E)$ is $\mu$-null. Prove that $\mu(E)=\lim_\alpha\mu_\alpha(E)$.
...
\begin{proof}[Hint]
Part 1. Recall \eqref{eq47}. Part 3. Reduce the problem to one concerning the convergence of distribution functions.
\end{proof}
...
Although von Neumann did not make this explicit, part of his motivation for studying Thm. \ref{lb343} may have been to reduce the study of the functional calculus for several operators to that of a single operator: indeed, in Thm. \ref{lb343} one has $g(T_1,T_2,\dots)=g\circ (f_1,f_2,\dots)(S)$ for $g\in\Borb(\Rbb\times\Rbb\times\cdots)$. See \cite{vN32} (especially Sec. II.10, III.1, III.3) for von Neumann's use of this result in developing mathematical interpretations of quantum mechanics.
...
For modern readers familiar with the multiplication-operator version of the spectral theorem (Thm. \ref{lb330}), the most suitable version for unbounded operators is that any such operator $T$ is unitarily equivalent to a direct sum of multiplication operators $\Mbf_f$ where $f$ is a real-valued Borel function that is not necessarily bounded (cf. Exp. \ref{lb366}).


\subsubsection{Measurement with absolute precision vs. measurement with prescribed accuracy}
...
To see how this rigorous viewpoint arises, one must understand von Neumann's interpretation of projections in \cite{vN27}, later expanded and refined in \cite{vN32}.\footnote{\cite{vN32} may be regarded as a greatly expanded version of \cite{vN27}. There, von Neumann develops and clarifies his earlier treatment of the mathematical foundations of quantum mechanics.} A projection $E$ on $\MH$ represents a ``proposition'' (or ``event'') in a quantum system, i.e., an observable that can take only the values $0$ and $1$, interpreted as ``false'' and ``true'', respectively. Thus, $\Rng(E)$ is the subspace of states in which the proposition holds, and $\Ker(E)$ is the subspace of states in which it fails. It follows that for any unit vector $\xi\in \MH$, the value $\bk{\xi|E\xi}$ gives the probability that the proposition is true in the state $\xi$.
...
Following von Neumann’s line of thought, the next section introduces the first such notion: the notion of a closed Hermitian operator with zero deficiency indices. This is not yet the final definition of unbounded self-adjoint operators; indeed, in modern textbooks it often appears as a criterion for self-adjointness rather than a definition. Nevertheless, we will see that checking that a Hermitian operator has zero deficiency indices is frequently more convenient than checking self-adjointness directly, and that many central ideas in the theory of unbounded operators---such as the Cayley transform and the concept of closed operators---are more naturally understood from the perspective of deficiency indices.
...
\begin{cv}
Unless otherwise stated, we assume that the domain $\Dom(T)$ of an unbounded operator $T:\MH\rightarrow\MK$ is dense. If this assumption is dropped, we will say that $T$ is an \textbf{unbounded operator not necessarily densely-defined}.
\end{cv}
...
Let $S,T:\MH\rightarrow\MK$ be (not necessarily densely-defined) unbounded operators. The addition $\pmb{S+T}$ \index{00@Addition of unbounded operators} is defined to be
\begin{align*}
...
Let $S:\MK\rightarrow\ML$ and $T:\MH\rightarrow\MK$ be (not necessarily densely-defined) unbounded operators. The composition \index{00@Composition of unbounded operators} $\pmb {ST}$ is defined to be
\begin{gather*}
...
\begin{pp}
Let $A,B,C$ be (not necessarily densely-defined) unbounded operators $\MH\rightarrow\MK$. Then $(A+B)+C=A+(B+C)$.
\end{pp}
...
\begin{pp}
Let $A:\ML\rightarrow\MM$, $B:\MK\rightarrow\ML$, and $C:\MH\rightarrow\MK$ be (not necessarily densely-defined) unbounded operators. Then $(AB)C=A(BC)$.
\end{pp}
...
\begin{pp}
Let $A,B:\MK\rightarrow\ML$ and $C,D\in\MH\rightarrow\MK$ be (not necessarily densely-defined) unbounded operators. Then
\begin{gather}
...
Let $T:\MH\rightarrow\MK$ be a (not necessarily densely-defined) unbounded operator. Assume that $T$ is injective, equivalently, that the kernel \index{00@Kernel of an unbounded operator}
\begin{align*}
\pmb{\Ker(T)}:=\{\xi\in\Dom(T):T\xi=0\}
\end{align*}
is zero. The \textbf{inverse} \index{00@Inverse of an unbounded operator} of $T$ is the (not necessarily densely-defined) unbounded operator $\MK\rightarrow\MH$ defined by
\begin{gather*}
\pmb{T^{-1}}:\Rng(T)\rightarrow\MH\qquad T\xi\mapsto \xi
\end{gather*}
with $\Dom(T^{-1})=\Rng(T)$. It is clear that $T^{-1}$ is injective and $(T^{-1})^{-1}=T$.
\end{df}
...
Let $(X,\fk M)$ be a measurable space. Let $(\mu_\alpha)_{\alpha\in\SI}$ be a family of $\sigma$-finite measures on $\fk M$. Let $f:X\rightarrow\Cbb$ be measurable. Let $\MH=\bigoplus_{\alpha\in\SI}L^2(X,\mu_\alpha)$. Then
...
\subsection{Cayley transform and extensions of Hermitian operators}
...
\begin{align*}
U=\frac{T-\im}{T+\im}\qquad T=-\im\frac{U+\idt}{U-\idt}
\end{align*}
...
Let $T$ be a Hermitian operator on $\MH$. The \textbf{Cayley transform} \index{00@Cayley transforms of Hermitian operators} the (not necessarily densely-defined) unbounded operator on $\MH$ defined by
\begin{align*}
...
\begin{align*}
U_T=\frac{T-\im}{T+\im}:\Rng(T+\im)\xlongrightarrow{\simeq}\Rng(T-\im)
\end{align*}
...
The map $U_T-\idt$ sends each $(T+\im)\xi$ (where $\xi\in\Dom(T)$) to $(T-\im)\xi-(T+\im)\xi=-2\xi$. Hence $\Rng(U_T-\idt)=\Dom(T)$.
\end{proof}
...
Let $U:\Dom(U)\rightarrow\Rng(U)$ be a unitary map between two linear subspaces of $\MH$. Suppose that $\Rng(U-\idt)$ is dense in $\MH$. Then $\Ker(U-\idt)=\{0\}$.
\end{lm}
...
\item[(2)] A unitary map $U:\Dom(U)\rightarrow\Rng(U)$ where $\Dom(U)$ and $\Rng(U)$ are linear subspaces of $\MH$, and $\Rng(U-\idt)$ is dense in $\MH$.
\end{enumerate}
Viewing each $U$ in (2) as a (not necessarily densely-defined) unbounded operator on $\MH$, the correspondence is given by
\begin{align*}
...
Note that $(U-\idt)^{-1}$ is well-defined by Lem. \ref{lb378}. As in Def. \ref{lb379}, it is customary to write $-\im(U+\idt)(U-\idt)^{-1}$ as $-\im\frac{U+\idt}{U-\idt}$ and call it the \textbf{inverse Cayley transform} of $U$. \index{00@Inverse Cayley transform} It is the unique map satisfying
\begin{align}\label{eq193}
-\im\frac{U+\idt}{U-\idt}:\Rng(U-\idt)\rightarrow\MH\qquad (U-\idt)\eta\mapsto -\im(U+\idt)\eta
\end{align}
where $\eta\in\Dom(U)$.
...
Step 2. For each Hermitian $T$, the operator $U_T$ sends its domain vectors $T\xi+\im\xi$ to $T\xi-\im\xi$ (where $\xi\in\Dom(T)$). For each $U$ in (2), the operator $T_U$ sends its domain vectors $U\eta-\eta$ to $-\im(U\eta+\eta)$ (where $\eta\in\Dom(U)$). Thus 
\begin{align*}
T_{U_T}:U_T\eta-\eta\mapsto -\im(U_T\eta+\eta)
\end{align*}
where $\eta\in\Dom(U_T)$. Recall from \eqref{eq192} that $\Dom(U_T)=\Rng(T+\im)$. Thus, such $\eta$ can be described equivalently by $T\xi+\im\xi$ where $\xi\in\Dom(T)$. Thus
\begin{align*}
T_{U_T}:U_T(T\xi+\im\xi)-(T\xi+\im\xi)\mapsto -\im(U_T(T\xi+\im\xi)+(T\xi+\im\xi))
\end{align*}
By \eqref{eq192}, the above correspondence is
\begin{align*}
T_{U_T}:-2\im\xi\mapsto -2\im T\xi
\end{align*}
This proves that $T_{U_T}=T$.\\[-1ex]

Step 3. By the beginning of Step 2, we have
\begin{align*}
U_{T_U}:T_U\xi+\im\xi\mapsto T_U\xi-\im\xi
\end{align*}
where $\xi\in\Dom(T_U)$. By \eqref{eq193}, we have $\Dom(T_U)=\Rng(U-\idt)$. Thus, such $\xi$ can be described equivalently by $U\eta-\eta$ where $\eta\in\Dom(U)$. Thus
\begin{align*}
U_{T_U}:T_U(U\eta-\eta)+\im(U\eta-\eta)\mapsto T_U(U\eta-\eta)-\im(U\eta-\eta)
\end{align*}
By \eqref{eq193}, the above correspondence equals
\begin{align*}
U_{T_U}:-2\im\eta\mapsto -2\im U\eta
\end{align*}
This proves $U_{T_U}=U$.\\[-1ex]

Step 4. If $T\subset\wtd T$, then clearly $U_T\subset U_{\wtd T}$. This proves the direction ``$\Rightarrow$'' in \eqref{eq194}. If $U\subset\wtd U$, then clearly $T_U\subset T_{\wtd U}$. This proves the direction ``$\Leftarrow$'' in \eqref{eq194}.
\end{proof}
