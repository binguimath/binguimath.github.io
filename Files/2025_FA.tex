...
(\lambda-\Delta)f=g
\end{align*}
subject to the Dirichlet boundary condition $f|_{\partial\Omega}=0$  (resp. the Neumann boundary condition $\nabla f|_{\partial\Omega}=0$) admits a weak solution, i.e. a solution $f\in H^1(\Omega)$.  \hqed
\end{rem}
...
\begin{gather*}
\Big(\lambda-\Delta+\sum_j a_j\partial_{x_j}+b\Big)f=g\qquad \nabla f|_{\partial\Omega}=0
\end{gather*}
...
\item Give an example of a positive self-adjoint $A$ on $\MH$ with core $\Dom_0$ such that $\Dom_0\subset\Dom(A^2)$, but $\Dom_0$ is not a core for $\Dom(A^2)$.
\end{enumerate}
\end{prob}
