...
The idea for approaching this question is as follows. Note that $\Dom(\ovl{U_T})$ consists of vectors $\eta\in\MH$ that are limits of elements of $\Dom(U_T)=\Rng(T+\im)$. Thus, $\eta\in\MH$ belongs to $\Dom(\ovl{U_T})$ iff there exists a sequence $(\xi_n)$ in $\Dom(T)$ such that $\eta=\lim_n (T+\im)\xi_n$. In view of \eqref{eq197}, the fact that $(T\xi_n+\im\xi_n)$ is a Cauchy sequence is equivalent to that both $(\xi_n)$ and $(T\xi_n)$ are Cauchy sequences. This suggests that $\xi:=\lim_n\xi_n$ should be in the domain $\Dom(\ovl T)$, and that such $\xi$ form the whole domain $\Dom(\ovl T)$---that is, elements of $\Dom(\ovl T)$ are precisely those of the form $\xi=\lim_n\xi_n$ where $(\xi_n)$ is a sequence in $\Dom(T)$ such that both $(\xi_n)$ and $(T\xi_n)$ are Cauchy sequences. Moreover, it is easy to guess how $\ovl T$ is defined on such $\xi$: the element $\ovl T\xi$ should equal $\lim_n T\xi_n$. 
...
\subsubsection{A basic criterion for positive self-adjoint operators}
...
The following immediate consequence of Thm. \ref{lb458} is also due to \cite{vN32b}.
...
\end{align*}
Therefore, by Lem. \ref{lb368}, there exits a linear isometry $U$ from the closure of $H\Dom(T^*T)$ (which equals $\ovl{\Rng(H)}$ due to the first paragraph) to $\MK$ satisfying $T\xi=UH\xi$ for each $\xi\in\Dom(T^*T)$. Thus
\begin{align*}
T|_{\Dom(T^*T)}=UH|_{\Dom(T^*T)}
\end{align*}
Since $\Dom(T^*T)$ is a core for $T$ and $H$, the closures of the two sides above are equal to $T$ and $UH$ respectively. This proves \eqref{eq241}. 
\end{proof}
