...
Choose any $\xi\in\Rng(T+\im)$. For each $h\in C_c^\infty(\Rbb)$, by Step 2 we have $\xi(h)\in\Dom(T)$ and $T\xi(h)=\im\xi(h')$. Therefore
\begin{align*}
...
\begin{align*}
0=\partial_t\bk{e^{\im s\ovl T}\xi|e^{-\im t\ovl T}U(t)U(s)\eta}\big|_{t=0}=\partial_t\bk{\xi|e^{-\im t\ovl T}U(t)\xi}\big|_{t=s}
\end{align*}
