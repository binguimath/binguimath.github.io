...
\begin{df}
For each linear subspace $\fk G\subset\MH\oplus\MK$, the \textbf{adjoint} \index{00@Adjoint of a subset of $\MH\oplus\MK$} of $\fk G$ is defined to be \footnote{We do not use the notation $\fk G^*$ to denote the adjoint, since it is already reserved for dual spaces.} \index{AdG@$\Ad(\fk G)$}
\begin{align*}
...
\begin{proof}
The first relation follows from $\Jbb_{\MH,\MK}^*=\Jbb_{\MK,\MH}$. The third one follows from the unitarity of the diagonal reflection map $\fk r:\MH\oplus\MK\rightarrow\MK\oplus\MH,\xi\oplus\eta\mapsto\eta\oplus\xi$. The second one follows from $\Jbb\fk r=-\fk r\Jbb$ and the fact that $-\fk V=\fk V$ for any linear subspace $\fk V$.
\end{proof}
...
\end{enumerate}
Moreover, if one of (1) and (2) is true, then $T^*$ is a closed operator, and $\ovl T=T^{**}$.
\end{thm}

Consequently, if $T$ is closable, then $T^{***}=\ovl{T^*}=T^*$.

\begin{proof}
The equivalence of (1) and (2) is due to Cor. \ref{lb401}. If (1) or (2) is true, then $\ovl T=T^{**}$ by Thm. \ref{lb400}-1, and the closedness of $T^*$ is due to the closedness of $\SG(T^*)=(\Jbb\SG(T))^\perp$. (Note that any orthogonal complement is closed; cf. Rem. \ref{lb150}.)
\end{proof}
...
Let $T:\MH\rightarrow\MK$ be an injective closed operator with dense range. Then $T^*:\MK\rightarrow\MH$ is also an injective closed operator with dense range, and
...
\begin{proof}
By Thm. \ref{lb402}, $T^*:\MK\rightarrow\MH$ is a closed operator. By Prop. \ref{lb403}, $\Ker(T^*)=\Rng(T)^\perp=\{0\}$ and $\ovl{\Rng(T^*)}=\Ker(T)^\perp=\MH$. Therefore $T^*$ is injective with dense range. Eq. \eqref{eq204} follows immediately from Prop. \ref{lb399}.
\end{proof}
