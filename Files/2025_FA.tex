...
\begin{proof}
We view $T_n:=E_nTE_n$ as a linear operator on $V_n$. By Rem. \ref{lb173}, $T_n$ is Hermitian. Therefore, by linear algebra, $V_n$ has an orthonormal basis $e_{n,0},e_{n,1},\dots$ such that
\begin{align}
Te_{n,i}=\lambda_{n,i}e_{n,i}\qquad\text{for all }i
\end{align}
...
Let $\MV,\MW$ be normed vector spaces. Let $(T_\alpha)$ be a collection in $\fk L(\MV,\MW)$ satisfying $\sup_\alpha \Vert T_\alpha\Vert<+\infty$ and converging pointwise to some $T\in\fk L(\MV,\MW)$. Let $K$ be a precompact subset of $V$. Then $(T_\alpha)$ converges uniformly on $K$ to $T$. That is,
\begin{align*}
\lim_\alpha \sup_{\xi\in K}\Vert T\xi-T_\alpha\Vert=0
\end{align*}
\end{thm}
...
Recall \eqref{eq100} for the meaning of $\Rr(I)$. Let $(\rho_n)_{n\in\Nbb}$ be the sequence described in Prop. \ref{lb174}. As in Thm. \ref{lb183}, we let $f_n(z)$ be the Stieltjes transform of $\rho_n$. Since $\rho_n$ has finite range, $f_n(z)$ is a rational function. 


As shown in Thm. \ref{lb183}, the sequence $(f_n)$ forms a Pad\'e approximation to a holomorphic function $f$ representing the series $\sum_{n\in\Nbb}c_nz^{-n-1}$. The goal of this section is to provide an elementary description of $f_n$ in terms of orthogonal polynomials. This description not only offers insight into the historical development of Pad\'e approximation but is also essential for connecting Pad\'e approximation to continued fractions, as we will see in the next section.



\subsubsection{Expressing $f_n(z)$ as a quotient of determinants}

Let $V_n$ and $E_n$ be as in Def. \ref{lb189}. By Rem.\ref{lb173},
\begin{align*}
...
\begin{proof}
We use the notation in the proof of Prop. \ref{lb174}. Then $(z-T_n)^{-1}e_{n,i}=(z-\lambda_{n,i})^{-1}e_{n,i}$. Hence, the relation $\Omega=\sum_{i=0}^n a_{n,i}e_{n,i}$ implies
\begin{align*}
\bigbk{\Omega\big|(z-T_n)^{-1}\Omega}=\sum_i |a_{n,i}|^2\cdot (z-\lambda_{n,i})^{-1}=\int_I\frac{d\rho_n(x)}{z-x}
\end{align*}
\end{proof}


Therefore, if we extend the unit vector $\Omega$ to an orthonormal basis of $V_n$, then by Cramer's rule, $f_n(z)$ can be expressed as a quotient in which the denominator is $\det(z-T_n)$, and the numerator is a minor of $z-T_n$ of order $n$. In particular, the denominator and the numerator are monic polynomials of degree $n+1$ and $n$, respectively.

Remarkably, the sequence of denominators $(\det(z-T_n))_{n\in\Nbb}$ turns out to be a sequence of orthogonal polynomials, as we will explain below.



\subsubsection{Orthogonal polynomials and the Jacobi matrix}
