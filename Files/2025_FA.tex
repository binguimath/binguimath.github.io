...
\begin{proof}
Recall from Cor. \ref{lb506} the compatibility between the two versions of the Borel functional calculus for $T_1,T_2$. Suppose that $T_1$ and $T_2$ commutes. Then by Cor. \ref{lb308}, $f_1,f_2\in\Borb(\Rbb,\Rbb)$, both $f_1(T_1)$ and $f_2(T_2)$ can be viewed as Borel functional calculus of $T_\blt=(T_1,T_2)$. Therefore, by Thm. \ref{lb306}, these two bounded linear operators commute. (This can also be proved by Lem. \ref{lb295}.) Thus $T_1$ commutes strongly with $T_2$ by Thm. \ref{lb489}.



Conversely, assume that $T_1$ and $T_2$ commute adjointly. By Thm. \ref{lb306}, $(x\chi_K)(T_1)$ commutes with $(x\chi_K)(T_2)$ where $K:=\Sp(T_1)\cup\Sp(T_2)$. This means precisely that $T_1$ commutes with $T_2$.
\end{proof}
...
\begin{align*}
\Sp(g_1(T_\blt),\dots,g_N(T_\blt))\subset(g_1,\dots,g_L)(\Sp(T_\blt))
\end{align*}
\end{exe}
...
\begin{df}
Let $\omega:\MH\times\MH\rightarrow\Cbb$ be a bounded Hermitian form. Define $n_\pm$ to be the largest dimension of closed linear subspaces $\MK\subset\MH$ such that $\pm\omega|_\MK$ is positive-definite (i.e. an inner product). The numbers $n_+$ and $n_-$ are called the \textbf{positive index of inertia} and \textbf{negative index of inertia} of $\omega$, respectively.
\end{df}
...
\begin{prob}
Let $\omega$ be a bounded Hermitian form on $\MH$. Let $n_+,n_-$ be its indices of inertia.
\begin{enumerate}
...
\end{align*}
\item For any decomposition as in Part 1, prove that $n_+=\dim\MH_+$ and $n_-=\dim\MH_-$ where $\dim\MH_\pm$ is understood as in Pb. \ref{lb605}.
\end{enumerate}
\end{prob}
...
\begin{prob}
Let $T$ be a closed Hermitian operator on $\MH$. Define a sesquilinear form $\omega$ on $\Dom(T^*)$ by $\omega=(\omega_{T^*}-(\omega_{T^*})^*)/{2\im}$, that is,
\begin{align*}
...
\subsection{The historical context of compact operators}
...
\subsubsection{The modern approach to the Dirichlet problem}
...
\sup_{x\in\Rbb^N\setminus\{y\}}|x-y|^{2-N}|\bk{\nabla\Phi(x-y),\nbf_y}|<+\infty
\end{align}
...
Since there are no charges inside $\partial'\Omega$, it follows from the above discussion that $-\Delta\MT f|_{\Omega}=0$. Eq. \eqref{eq257} expresses the fact that $\MT f$ exhibits a jump discontinuity of magnitude $-\frac12f$ when moving from inside $\partial'\Omega$ to midway between $\partial'\Omega$ and $\partial\Omega$. This gives a heuristic explanation of why the function $u$ defined in \eqref{eq280} is continuous on $\ovl\Omega$ and harmonic on $\Omega$. \hqed
\end{rem}
...
Let $\MH,\MK$ be Hilbert spaces. Let $\MV,\MW$ be inner product spaces. Recall Def. \ref{lb606} for the notion of weak topology in inner product spaces.


\begin{df}
A bounded sesquilinear form $\omega:\MW\times\MV\rightarrow\Cbb$ is called \textbf{completely continuous} \index{00@Completely continuous sesquilinear forms} if its restriction to $\ovl B_\MW(0,1)\times\ovl B_\MV(0,1)\rightarrow\Cbb$ is weakly continuous, i.e., continuous with respect to the product topology of the weak topologies of $\ovl B_\MW(0,1)$ and $\ovl B_\MV(0,1)$.
\end{df}


\begin{rem}\label{lb607}
A completely continuous form $\omega:\MW\times\MV\rightarrow\Cbb$ is clearly (norm-)continuous when restricted to $B_\MW(0,1)\times B_\MV(0,1)$, and hence is (norm-)continuous at $(0,0)$. Therefore, $\omega\in\Ses(\MW|\MV)$ by Prop. \ref{lb35}. 
\end{rem}

\begin{comment}
To check that a multilinear map is bounded, it suffices to check the continuity at the origin. The same is true for complete continuity.

\begin{pp}
Let $\omega:\MW\times\MV\rightarrow\Cbb$ be a sesquilinear form. Then the following are equivalent.
\begin{enumerate}
\item[(1)] $\omega$ is completely continuous.
\item[(2)] For each bounded nets $(\xi_\alpha)_{\alpha\in I}$ in $\MV$ and $(\eta_\alpha)_{\alpha\in I}$ in $\MW$ both converging weakly to $0$, we have $\lim_\alpha\omega(\eta_\alpha|\xi_\alpha)=0$.
\end{enumerate}
\end{pp}

\begin{proof}
(1)$\Rightarrow$(2): By sesquilinearity, if $\omega$ is completely continuous, then the restriction of $\omega$ to any bounded subsets of $\MW\times\MV$ is weakly continuous. Thus (2) follows.

(2)$\Rightarrow$(1): Assume (2). Suppose that $(\xi_\alpha)_{\alpha\in I}$ and $(\eta_\alpha)_{\alpha\in I}$ are nets in $\ovl B_\MV(0,1)$ and $\ovl B_\MW(0,1)$ converging weakly to $\xi\in \ovl B_\MV(0,1)$ and $\eta\in\ovl B_\MW(0,1)$. Then
\begin{align*}
|\omega(\eta|\xi)-\omega(\eta_\alpha|\xi_\alpha)|=
\end{align*}
\end{proof}


Any good property in functional analysis must be preserved when passing from a dense subspace to the whole space. This calls for the following property.



\begin{pp}
Let $\MV_0$ and $\MW_0$ be dense linear subspaces of $\MV$ and $\MW$ respectively. There there is a one-to-one correspondence between:
\begin{enumerate}
\item[(1)] A completely continuous sesquilinear form $\varpi:\MW\times\MV\rightarrow\Cbb$.
\item[(2)] A completely continuous sesquilinear form $\omega:\MW_0\times\MV_0\rightarrow\Cbb$. 
\end{enumerate}
The correspondence is given by $\omega=\varpi|_{\MW_0\times\MV_0}$.
\end{pp}


\begin{proof}
Any $\varpi$ in (1) clearly restricts to $\pi$ in (2). Moreover, since $\varpi\in\Ses(\MW|\MV)$ by Rem. \ref{lb607}, the restriction map is injective. Therefore, it remains to shows that any $\omega$ in (2) can be extended to a completely continuous sesquilinear form $\varpi$.

Note that it suffices to show that $\omega$ can be extended to a completely continuous sesquilinear form on $\MW_0\times\MV$. Then a similar process provides the extension to $\MW\times\MV$. Therefore, in the following, it suffices to assume that $\MW=\MW_0$.

Choose $\omega$ as in (2). By Rem. \ref{lb607}, $\omega\in\Ses(\MW|\MV_0)$. Therefore, by Thm. \ref{lb31}, $\omega$ can be extended to a unique $\varpi\in\Ses(\MW|\MV)$. To prove that $\varpi$ is completely continuous, we choose a net $(\xi_\alpha)_{\alpha\in I}$ in $\ovl B_\MV(0,1)$ converging weakly to $\xi\in\ovl B_\MV(0,1)$, and choose a net $(\eta_\alpha)_{\alpha\in I}$ in $\ovl B_\MW(0,1)$ converging weakly to $\eta\in\ovl B_\MW(0,1)$. We shall prove $\lim_\alpha \varpi(\eta_\alpha|\xi_\alpha)=\varpi(\eta|\xi)$.

By Rem. \ref{lb29}, $\ovl B_{\MV_0}(0,1)$ is dense in $\ovl B_\MV(0,1)$ and $\ovl B_\MW(0,1)$ is dense in $\ovl B_\MW(0,1)$. Therefore, for each $n\in\Zbb_+$, we can choose $\xi_{\alpha,n}\in\ovl B_{\MV_0}(0,1)$ and $\eta_{\alpha,n}\in\ovl B_{\MW,0}(0,1)$ such that
\begin{align*}
\Vert \xi_\alpha-\xi_{\alpha,n}\Vert<\frac 1n\qquad \Vert\eta_\alpha-\xi_{\alpha,n}\Vert<\frac 1n
\end{align*}
Then $\lim_{\alpha,n}\xi_\alpha$ converges weakly to $\xi$, 
\end{proof}
\end{comment}
...
\end{align*}
Prove that there exists $T\in\fk L(\MV)$ (where $\MV=C(X)$) satisfying
\begin{align*}
(Tf)(x)=\int_X K(x,y)f(y)dm(y)\qquad\text{for each }x\in X
\end{align*}
and that $T$ is a compact operator.
\end{prob}

\begin{proof}[Hint]
For each $\eps>0$, let $h_\eps:\Rbb_{\geq0}\rightarrow[0,1]$ be a continuous function satisying $h_\eps|_{[0,\eps]}=0$ and $h_\eps|_{[2\eps,+\infty)}=1$. Let $T_\eps:\MV\rightarrow\MV$ be defined by the integral kernel $K_\eps(x,y)=K(x,y)\cdot h_\eps(|x-y|)$. Explain why $T_\eps$ is a compact operator, and show that $\lim_{\eps\rightarrow0}T_\eps$ converges to $T$ in the norm operator topology.
\end{proof}
