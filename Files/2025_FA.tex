...
\begin{align*}
\lim_{A\in\fin(2^X)}\bk{\Psi_A(f_A),g}=\bk{\varphi,g}
\end{align*}
for each $g$ of the form $\chi_{\{x\}}$ where $x\in X$, and hence for each $g\in C_c(X,\Fbb)$. Moreover, the net $(\Psi_A(f_A))_{A\in\fin(2^X)}$ is uniformly bounded, since
\begin{align*}
\Vert\Psi_A(f_A)\Vert_{l^p}=\Vert\varphi|_{l^q(A,\Fbb)}\Vert\leq\Vert\varphi\Vert
\end{align*}
Therefore, by Thm. \ref{lb80}, the net $(\Psi_A(f_A))_{A\in\fin(2^X)}$ converges weak-* to $\varphi$. In other words, $(f_A)_{A\in\fin(2^X)}$ is a uniformly $l^p$-bounded net in $C_c(X,\Fbb)$ converging weak-* to $\varphi$.\\[-1ex]
