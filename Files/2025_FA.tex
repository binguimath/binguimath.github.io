...
Let $T$ be a positive self-adjoint operator on $\MH$. Let $x\in\fk L(H)$ be positive. Then $x+T$ is a positive self-adjoint operator on $\MH$. In particular, for each $\lambda\geq0$, $\lambda+T$ is a positive self-adjoint operator on $\MH$.
\end{eg}
...
T=U\sqrt{T^*T}
\end{align}
We call \eqref{eq241} the \textbf{(left) polar decomposition} of $T$. \index{00@Polar decompositions of closed operators}
\end{thm}

According to this theorem, the closed operator $T$ and the positive self-adjoint operator $\sqrt{T^*T}$ share the same domain, and the graph inner products on this common domain defined by $T$ and $\sqrt{T^*T}$ are equal. 
...
\end{align*}
We call $\pmb{A_\Frm}:=T^*\ovl T\equiv \ovl T^*\ovl T$ the \textbf{Friedrichs extension} of $A$. \index{00@Friedrichs extension}
\end{df}
...
\begin{enumerate}
\item[(1)] $B$ is the Friedrichs extension of $A$.
\item[(2)] $\Dom(A)$ is a core for $\sqrt B$.
\end{enumerate}
\end{thm}

Note that a core for an unbounded operator is always assumed to be a linear subspace of its domain.

\begin{proof}
(1)$\Rightarrow$(2): Assume (1). Then $B=T^*\ovl T$ where $T:\MH\rightarrow\MK$ is an abstract gradient operator of $A$. In particular, $\Dom(A)=\Dom(T)$. Clearly $\Dom(T)$ is a core for $\ovl T$. By the polar decompsition of $\ovl T$ (cf. Thm. \ref{lb472}), the operators $\ovl T$ and $\sqrt{\ovl T^*\ovl T}=\sqrt B$ share the same domain and cores. Thus $\Dom(T)$ is a core for $\sqrt B$. This proves (2).
...
\begin{proof}
By Thm. \ref{lb458} or Exe. \ref{lb471}, $\Dom(A)$ is a core for $\sqrt A$. Therefore, by Thm. \ref{lb473}, $A$ is the Friedrichs extension of $A$.
\end{proof}
...
\begin{proof}
By Exp. \ref{lb477}, $x+B$ is a positive self-adjoint operator. Since $A\subset B$, we have $x+A\subset x+B$. By Thm. \ref{lb473}, $\Dom(A)$ is a core for $\sqrt B$, and it suffices to prove that $\Dom(x+A)=\Dom(A)$ is a core for $\sqrt{x+B}$.
...
\end{align*}
Hence, the graph inner products of $\sqrt B$ and $\sqrt{x+B}$ on $\Dom(B)=\Dom(x+B)$ are equal. Therefore, since $\Dom(A)=\Dom(x+A)$ is a core for $\sqrt B$ (and hence for $\sqrt B|_{\Dom(B)}$), it is also a core for $(\sqrt{x+B}|_{\Dom(x+B)})$. By Thm. \ref{lb458} or Exe. \ref{lb471}, $\Dom(x+B)$ is a core for $\sqrt{x+B}$. Thus $\Dom(x+A)$ is a core for $\sqrt{x+B}$.
\end{proof}
