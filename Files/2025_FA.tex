...
\begin{df}
We say that $f:X\rightarrow\ovl\Rbb$ is called \textbf{lower semicontinuous} \index{00@Lower semicontinuous} if $f^{-1}(a,+\infty]$ is open for each $a\in\ovl\Rbb$. We say that $f$ is  \textbf{upper semicontinuous} \index{00@Upper semicontinuous} if $f^{-1}[-\infty,a)$ is open for each $a\in\ovl\Rbb$. 
\end{df}
...
If $\scr A$ is a countable set, and if each $X_\alpha$ is compact and second-countable, the \textbf{diagonal method} \index{00@Diagonal method} can be used in place of Zorn's lemma to prove that $S$ (which is second countable by Prop. \ref{lb51}).
...
Let $X$ be a compact Hausdorff space. Let $\fk S\subset C(X,\Fbb)$. Suppose that $\fk S$ separates the points of $X$. Then the *-$\Fbb$-subalgebra $\Fbb\bk{\fk S\cup\fk S^*}$ generated by $\fk S$ is dense in $C(X,\Fbb)$ in the $l^\infty$-norm.
\end{thm}
...
The Stone-Weierstrass theorem will be used in the direction (c)$\Rightarrow$(d). The equivalence of (a,b,c) does not rely on the Stone-Weierstrass theorem. Moreover, condition (a) will seldom be used in this course: while metrizability is a convenient notion for beginners in analysis, the second countability condition is more flexible and broadly applicable.
...
Let $\Phi:(X,\fk M)\rightarrow (Y,\fk N)$ be a measurable map between two measurable spaces. Let $\mu$ be a measure on $\fk M$. The \textbf{pushforward measure} \index{00@Pushforward measure $\Phi_*\mu$} \index{zz@$\Phi_*\mu$, the pushforward measure of $\mu$ by $\Phi$} of $\mu$ by $\Phi$ is defined to be
\begin{align}
...
\end{align}
holds for any simple function $f:X\rightarrow\Rbb_{\geq0}$, and hence (by MCT) for any measurable $f:X\rightarrow\Rbb_{\geq0}$. We call
\begin{align*}
\pmb{\Ess(\Phi,\mu)}:=\Supp(\Phi_*\mu)
\end{align*}
the essential range of $\Phi$ \index{00@Essential range $\Ess(\Phi,\mu)=\Ess(\Phi)$} \index{Rngess@$\Ess(\Phi,\mu)=\Ess(\Phi)$} and abbreviate it to $\Ess(\Phi)$ when no confusion arises.
\end{df}
...
The above proof shows that if $V$ is separable, then $\ovl B_{V^*}(0,1)$ is second-countable and therefore sequentially compact in the weak-* topology. The converse is also true:
...
\begin{proof}
By Thm. \ref{lb85}, the normed vector space $V=L^q(\Sbb^1,\mu)$ is separable. Therefore, by Thm. \ref{lb76}, the weak-* topology of $\ovl B_{L^q(\Sbb^1,\mu)}(0,1)$ is second countable (and hence first countable). Therefore, by Prop. \ref{lb209} and Rem. \ref{lb221}, to prove the corollary, it suffices to show that $\ovl B_U(0,1)$ is weak-* dense in $\ovl B_{L^q(\Sbb^1,\mu)}(0,1)$. 
...
\begin{aligned}
&\bk{u|v}=\frac 14\sum_{t=0,\frac \pi 2,\pi,\frac{3\pi}2}~ \bk{e^{\im t}u+v|e^{\im t}u+v}e^{\im t}\\
=&\frac 14\Big(\bk{u+v|u+v}-\bk{-u+v|-u+v}+\im\bk{\im u+v|\im u+\im v}-\im\bk{\im u-v|\im u-v}\Big)
\end{aligned}
\end{align}
...
\begin{df}
If $U$ is a linear subspace of $V$, we define the \textbf{orthogonal complement} \index{00@Orthogonal complement} of $U$ (in $V$) to be
\begin{align*}
...
Suppose that $(e_i)_{i\in I}$ is an orthonormal basis of $V$. Then for each $v\in V$, the RHS of the following converges (in the norm of $V$) to the LHS:
\begin{align}
...
Let $V$ be a closed linear subspace of $\MH$. Show that an orthonormal basis of $V$, together with an orthonormal basis of $V^\perp$, forms an orthonormal basis of $\MH$.
\end{exe}
...
\end{align*}
are called \textbf{\pmb{$I\times J$} (complex) matrices}. For each $A\in \Cbb^{X\times Y}$, the \textbf{norm \pmb{$\Vert A\Vert$}} \index{00@Norm of matrices} is defined to be
...
\end{gather}
\end{subequations}
We say that $A$ is \textbf{bounded} \index{00@Bounded matrix} if $\Vert A\Vert<+\infty$.
\end{df}
...
For each $I\in\fin(2^X)$ and $J\in\fin(2^Y)$, let $A_{I,J}\in\Cbb^{X\times Y}$ be the \textbf{truncation of $A$ by $I,J$}. \index{00@Truncation of matrices} In other words, for each $x\in X,y\in Y$,
\begin{align*}
A_{I,J}(x,y)=\left\{
\begin{array}{ll}
...
Let $\MH$ be a Hilbert space with basis $(e_x)_{x\in X}$. Let $(P_I)_{I\in\fin(2^X)}$ be the net of projections where $P_I$ projects $\MH$ onto $V_I=\Span\{e_x:x\in I\}$. By Exp. \ref{lb157}, $\lim_I P_I$ converges in SOT to $1$. Choose any $R,S,T\in\fk L(\MH)$. Then $\lim_I P_IS$ converges in SOT to $S$, and $\lim_I P_IT$ converges in SOT to $T$. Therefore, by Cor. \ref{lb158}, for each $\eta\in\MH$ we have
\begin{align*}
...
\begin{proof}[Hint]
The strategy differs slightly from that of Prop. \ref{lb238}. In particular, apply \eqref{eq145} to the inner product on $V$ at an appropriate step.
\end{proof}



\begin{df}
Let $V$ be an inner product space. A \textbf{Hilbert space complete} \index{00@Hilbert space completion} denotes a linear isometry $\Phi:V\rightarrow\MH$ such that $\MH$ is a Hilbert space and $\Phi(V)$ is dense in $\MH$. Replacing $V$ with $\Phi(V)$, one may view $V$ as a dense inner product subspace of $\MH$
\end{df}
...
\begin{df}
For each increasing function $\rho:I\rightarrow\ovl\Rbb_{\geq0}$, the function $f(z)$ defined by \eqref{eq96} is called the \textbf{Stieltjes transform} \index{00@Stieltjes transform} of $\rho$. If $\mu$ is a finite Borel measure on $I$, we also call the function
\begin{align*}
...
\begin{align*}
\pi:\scr A\rightarrow\fk L(V)
\end{align*}
is a linear map satisfying
\begin{align*}
\pi(xy)=\pi(x)\pi(y)\qquad\bk{\eta|\pi(x)\xi}=\bk{\pi(x^*)\eta|\xi}\qquad\pi(1_{\scr A})=\id_V
\end{align*}
for all $x,y\in\scr A$ and $\xi,\eta\in V$. If moreover $V$ is a Hilbert space, we say that $(\pi,V)$ is \textbf{unitary representation} of $\scr A$. \index{00@Unitary representation of a unital *-algebra}
\end{df}
...
\scr A\rightarrow\Cbb\qquad f\mapsto\bk{\xi|\pi(f)\xi}
\end{align}
...
Let $X$ be a topological space, and let $(\pi,V)$ be a normal pre-unitary representation of $\Borb(X)$. By Prop. \ref{lb252}, the normality of $\pi$ is equivalent to the fact that for each $\xi\in V$, there is a unique finite Borel measure $\pmb{\mu_\xi}$ \index{zz@$\mu_\xi,\mu_{\eta,\xi}$} (called the \textbf{measure associated to $\xi$}) \index{00@Measure $\mu_\xi$ associated to a vector $\xi$} such that
\begin{align*}
...
\end{align*}
We call $\mu_{\eta,\xi}$ the \textbf{complex measure associated to $\eta,\xi$}. \index{00@Complex measure $\mu_{\eta,\xi}$ associated to vectors $\eta,\xi$}
\end{rem}
...
In this subsection, fix a compact Hausdorff space $X$. Our goal is to prove Thm. \ref{lb274}. We first need some preparation.
...
\end{align*}
where the product ranges over all Radon measures $\mu$ on $X$, (equivalently, all finite Borel measures when $X$ is second countable, cf. Thm. \ref{lb64}), and where for each such $\mu$, the map $\Psi_\mu:\Borb(X)\rightarrow L^2(X,\mu)$ sends each $f$ to (the equivalence class of) $f$.
...
\end{align*}
for each Radon measure $\mu$ on $X$, iff
\begin{align*}
...
Suppose that $(f_\alpha)$ is a net in $\Borb(X)$ converging universally-$L^2$ to $f\in\Borb(X)$. Then for each Radon measure $\mu$ and each $g\in\Borb(X)$, we have $\lim_\alpha\int_X f_\alpha g d\mu=\int_Xf gd\mu$ (since $(f_\alpha)$ converges weakly to $f$ in $L^2(X,\mu)$). In other words,
\begin{align}
\lim_\alpha\Lambda(f_\alpha g)=\Lambda(fg)
\end{align}
for each Radon positive linear functional on $\Borb(X)$ and $g\in\Borb(X)$. 
\end{rem}
...
Let $\scr A$ be a unital *-$\Fbb$-subalgebra of $C(X,\Fbb)$ separating points of $X$. Then $\ovl B_{\scr A}(0,1)$ is dense in $\ovl B_{\Borb(X,\Fbb)}(0,1)$ in the universal $L^2$-topology, i.e., 
\begin{align*}
\{f\in\scr A:\Vert f\Vert_{l^\infty}\leq1\}\qquad\text{is dense in}\qquad \{f\in\Borb(X,\Fbb):\Vert f\Vert_{l^\infty}\leq1\}
\end{align*}
\end{lm}
Therefore, for any $f\in\Borb(X,\Fbb)$ there exists a net $(f_\alpha)$ in $\scr A$ with $\sup_\alpha\Vert f_\alpha\Vert_{l^\infty}\leq\Vert f\Vert_{l^\infty}$ satisfying $\lim_\alpha\Vert f-f_\alpha\Vert_{L^2(X,\mu)}=0$ for each Radon measure $\mu$ on $X$.


\begin{proof}
By Stone-Weierstrass, $\scr A$ is $l^\infty$-dense in $C(X,\Fbb)$. Therefore, for each $f\in C(X,\Fbb)$ with $\Vert f\Vert_{l^\infty}\leq 1$ there is a net $(f_\alpha)$ in $\scr A$ converging uniformly to $f$. In particular, $\Vert f_\alpha\Vert_{l^\infty}$ converges to $\Vert f\Vert_{l^\infty}$. Therefore $f_\alpha\cdot \Vert f\Vert_{l^\infty}/\Vert f_\alpha\Vert_{l^\infty}$ converges uniformly to $f$. This proves that the closed unit ball of $\scr A$ is $l^\infty$-dense (and hence universally $L^2$-dense) in the closed unit ball of $C(X,\Fbb)$.

It remains to prove that $\ovl B_{C(X,\Fbb)}(0,1)$ is universally $L^2$-dense in $\ovl B_{\Borb(X,\Fbb)}(0,1)$. Choose any $f\in \ovl B_{C(X,\Fbb)}(0,1)$. Let $\scr I$ be the set of Radon measures on $X$. By Lusin's Thm. \ref{lb82} (together with the Tietze extension Thm. \ref{lb83}), for each finite subset $E\subset\scr I$ and $\eps>0$ there exists $f_{E,\eps}\in C(X,\Fbb)$ such that $\Vert f_{E,\eps}\Vert_{l^\infty}\leq 1$ and
\begin{align*}
\mu\{x\in X:f(x)\neq f_{E,\eps}\}\leq\eps
\end{align*}
...
\end{align*}
Hence, the net $(f_{E,\eps})_{(E,\eps)\in\fin(2^{\scr I})\times\Rbb_{>0}}$ (where $\Rbb_{>0}$ is given by the order $>$ instead of the usual one $\leq$) is a net in $\ovl B_{C(X,\Fbb)}(0,1)$ and converges universally-$L^2$ to $f$.
\end{proof}
...
Let $X$ be a set. Let $\scr A$ be a unital *-subalgebra of $l^\infty(X)$, equipped with the $l^\infty$-norm. Let $\pi:\scr A\rightarrow\fk L(\MH)$ be a positive unitary representation of $\scr A$. Then $\pi$ is bounded with operator norm $\Vert\pi\Vert=1$.
\end{pp}

\begin{proof}
By Prop. \ref{lb204}, for each $f\in\scr A$ and $\xi\in\MH$ we have
\begin{align*}
...
\end{align*}
This proves $\Vert\pi\Vert\leq1$. Since $\Vert\pi(1)\Vert=1$, we have $\Vert\pi\Vert=1$.
\end{proof}
...
For a more faithful presentation of Riesz’s original proof of his spectral theorem in \cite{Rie13}, see Sec. 25.8 and 25.9 of \cite{Gui-A}.  \hqed
\end{rem}
...
\begin{df}
An operator $T\in\fk L(\MH)$ is called \textbf{normal} \index{00@Normal} if it satisfies $T^*T=TT^*$. This is equivalent to its real part $\Real(T)$ \index{00@Real part of an operator} and its imaginary part $\Imag(T)$ \index{00@Imaginary part of an operator} commuting with each other, where
\begin{align*}
...
Let $X$ be a topological space. Let $(\pi,V)$ be a pre-unitary representation of $\Borb(X)$. The \textbf{support} $\pmb{\Supp(\pi)}$ of $\pi$ \index{00@Support of a unitary representation of $\Borb(X)$} is defined to be
\begin{align*}
...
Let $(\pi,V)$ be a pre-unitary representation of $\Borb(X)$ satisfying either (1) or (2) of Rem. \ref{lb302}. Choose any Borel subset $K\subset X$ containing $\Supp(\pi)$. Then for each $f,g\in\Borb(X)$ we have
\begin{align*}
...
Assume that $X$ is second-countable, and let $(\pi,V)$ be a normal pre-unitary representation of $\Borb(X)$. Let $K\subset X$ be a Borel set containing $\Supp(\pi)$. Then there is a unique normal pre-unitary representation
\begin{gather}\label{eq163}
\pmb{\pi|_K}:\Borb(K)\rightarrow\fk L(V)\qquad \pi|_K(f|_K)=\pi(f)
\end{gather}
...
\end{align*}
where the last step is by Rem. \ref{lb301}. In particular, $\pi|_K(1_K)=\pi(1|_X)$. It follows that $\pi|_K$ is a normal pre-unitary representation satisfying \eqref{eq163}.
\end{proof}
...
\subsection{Joint spectrum $\Sp(T_\blt)$ and basic properties of Borel functional calculus}
...
\begin{df}
The support of the Borel functional calculus $\pi_{T_\blt}:\Borb(\Cbb^N)\rightarrow\fk L(\MH)$ is denoted by $\pmb{\Sp(T_1,\dots,T_N)}$ (abbreviated to $\pmb{\Sp(T_\blt)}$) \index{Sp@$\Sp(T_\blt)$} and called the \textbf{joint spectrum} \index{00@Joint spectrum $\Sp(T_\blt)$} of $T_1,\dots,T_N$.
\end{df}
...
\end{gather}
satisfying $(f|_K)(T_\blt)=f(T_\blt)$ for each $f\in\Borb(X)$. The map \eqref{eq164} is also called a \textbf{Borel functional calculus} \index{00@Borel functional calculus for bounded normal operators} of $T_\blt$.
\end{rem}
...
\begin{proof}
If $\xi\in\Ker(T-\lambda)$, then, by \eqref{eq166},
\begin{align*}
0=\Vert (T-\lambda)\xi\Vert^2=\int_\Cbb |z-\lambda|^2d\mu_\xi
\end{align*}
and hence $\Cbb\setminus\lambda$ is $\mu_\xi$-null by Prop. \ref{lb310}. It follows that
\begin{align*}
\Vert \chi_{\{\lambda\}}(T)-\lambda\xi\Vert^2=\int_\Cbb |\chi_{\{\lambda\}}-\lambda|^2d\mu_\xi=\int_{\{\lambda\}} |\chi_{\{\lambda\}}-\lambda|^2d\mu_\xi=0
\end{align*}
and hence $\chi_{\{\lambda\}}(T)\xi=\xi$.

Next, assume that $\xi\perp\Ker(T-\lambda)$. Since
\begin{align*}
T\chi_{\{\lambda\}}(T)\xi=(z\chi_{\{\lambda\}})(T)\xi=\lambda\chi_{\{\lambda\}}(T)
\end{align*}
we have $\chi_{\{\lambda\}}(T)\xi\in\Ker(T-\lambda)$, and hence $\xi\perp\chi_{\{\lambda\}}\xi$. Therefore
\begin{align*}
\Vert\chi_{\{\lambda\}}(T)\xi\Vert^2=\bk{\chi_{\{\lambda\}}(T)\xi|\xi}=0
\end{align*}
This proves $\chi_{\{\lambda\}}(T)\xi=0$.
\end{proof}
...
In this section, we present an example that illustrates how the Borel functional calculus can be applied in practice.
...
\begin{eg}
Let $(X,\fk M,\mu)$ be a \textbf{probability space} \index{00@Probability} space, i.e., a measure space satisfying $\mu(X)=1$. Let $\phi:X\rightarrow X$ be a \textbf{measure preserving transform}. \index{00@Measure preserving transform} This means that $\phi$ is a measurable map satisfying $\phi_*\mu=\mu$, that is, $\mu(\phi^{-1}(A))=\mu(A)$ for each measurable $A\subset X$ (cf. Def. \ref{lb319}). Then
...
\subsection{The multiplication-operator version of the spectral theorem}
...
\begin{df}
Let $(\MH_\alpha)_{\alpha\in\SI}$ be a family of Hilbert spaces. The \textbf{Hilbert space direct sum} \index{00@Hilbert space direct sum} of $(\MH_\alpha)_{\alpha\in\SI}$, denoted by $\pmb{\bigoplus_{\alpha\in\SI}\MH_\alpha}$, the subspace consisting of all
\begin{align*}
...
\subsubsection{Orthogonal decomposition of Hilbert spaces}
...
\begin{rem}
In Exp. \ref{lb323}, let $P_\alpha\in\fk L(\MH)$ be the projection of $\MH$ onto $\MH_\alpha$. Note that by Cor. \ref{lb322}, for each $I\in\fin(2^\SI)$, the sum $\sum_{\alpha\in I}P_\alpha$ is the projection onto the subspace spanned by $(\MH_\alpha)_{\alpha\in\SI}$. Therefore, 
\begin{align*}
...
\subsubsection{Orthogonal decomposition of unitary representations}



Fix a unital $*$-algebra $\SA$.  
...
In other words, there exists densely-spanning family $(\MH_\alpha)_{\alpha\in\SI}$ of $\SA$-invariant Hilbert subspaces of $\MH$ such that each $\MH_\alpha$ is cyclic.

\begin{proof}
Let $\SF$ be of the set of all $\SI\in 2^\MH$ where $\SI$ is a set of mutually-orthogonal non-zero cyclic subrepresentations of $\MH$. Then $(\SF,\subset)$ is a partially ordered set. Clearly every totally ordered subset of $\SF$ has an upper bound in $\SF$ (defined by taking union). By Zorn's lemma, there exists a maximal element $\SI\in\SF$. 
...
\item[(2)] There exists a finite Borel measure $\mu:\fk B_X\rightarrow[0,+\infty]$ together with a unitary map unitary equivalence
\begin{align*}
...
\bk{\pi(f)\psi|\pi(g)\psi}=\bk{\psi|\pi(\ovl fg)\psi}=\int_X\ovl fgd\mu=\bk{1|M_{\ovl f}M_g1}=\bk{M_f1|M_g1}
\end{align}
This shows that we have a linear isometry
\begin{align*}
\Phi: \Borb(X)\psi\rightarrow L^2(X,\mu)\qquad \pi(f)\psi\mapsto f=M_f1
\end{align*}
In particular, this linear map is well-defined, since \eqref{eq180} implies that
\begin{align*}
\Big\Vert\sum_{i=1}^n\pi(f_i)\psi\Big\Vert^2=0\qquad\Longleftrightarrow \qquad \Big\Vert  \sum_{i=1}^n M_{f_i}1 \Big\Vert^2=0
\end{align*}
for each $f_1,\dots,f_n\in\Borb(X)$.

Clearly $\Phi$ interwines the action of $\Borb(X)$, i.e., $\Phi$ is a homomorphism. The range of $\Phi$ is $L^\infty(X,\mu)$, and hence is dense in $L^2(X,\mu)$. Therefore, if we extend $\Phi$ (uniquely) to a bounded linear map
\begin{align*}
\Phi:\MH=\ovl{\Borb(X)\psi}\longrightarrow L^2(X,\mu)
\end{align*}
(which is also an isometry), then the range of $\Phi$ is not only dense but also complete (since $\MH$ is complete). We conclude that $\Phi$ is unitary. For each $f\in\Borb(X)$, the bounded linear operators $\Phi\pi(f)$ and $M_f\Phi$ agree on $\Borb(X)\psi$, and hence agree on $\MH$. Thus $\Phi$ is a homomorphism.
\end{proof}
...
There exists a family $(\mu_\alpha)_{\alpha\in \SI}$ of finite Borel measures on a common compact subset $X\subset\Cbb^N$, together with a unitary map
\begin{gather*}
...
\begin{proof}
Let $\pi:\Borb(X)\rightarrow\fk L(\MH)$ be the Borel functional calculus (cf. Thm. \ref{lb331}) where $X=\Supp(\pi)$ is a compact subset of $\Cbb^N$. Then $(\pi,\MH)$ is a normal unitary representation of $\Borb(X)$. 
...
\end{align*}
Then $T_1,\dots,T_N$ are adjointly-commuting bounded normal operators. Moreover, for each $g\in\Borb(\Cbb^N)$, the Borel fnuctional calculus satisfies
\begin{align}
...
\end{align*}
is clearly a unitary representation supported in $K$ where $K\subset\Cbb^N$ is any compact set containing $\Rng(f_1,\dots,f_N)$. It remains to prove that $(\pi,\MH)$ is normal.

Choose an increasing sequence $(g_n)$ in $\Borb(\Cbb^N,\Rbb_{\geq0})$ converging pointwise to $g\in \Borb(\Cbb^N,\Rbb_{\geq0})$. Let $\xi=\oplus_{\alpha\in\SI}\xi_\alpha$ be an element of $\MH$. By Rem. \ref{lb333}, we can write $\xi=\oplus_{\alpha\in I}\xi_\alpha$ where $I$ is a countable subset of $\SI$. MCT implies
\begin{align*}
&\lim_n\bk{\xi|\pi(g_n)\xi}=\lim_n \sum_{\alpha\in I}\int_X (g_n\circ f)|\xi_\alpha|^2d\mu_{\alpha}=\sum_{\alpha\in I}\lim_n\int_X (g_n\circ f)|\xi_\alpha|^2d\mu_{\alpha}\\
=&\sum_{\alpha\in I}\int_X (g\circ f)|\xi_\alpha|^2d\mu_{\alpha}=\bk{\xi|\pi(g)\xi}
\end{align*}
Thus $(\pi,\MH)$ is normal.
\end{proof}
...
Let $-\infty<a<b<+\infty$. Let $f\in\Cbb[x]$. Prove that $f|_{[a,b]}\geq0$ iff $f$ is can be written as
\begin{align*}
...
\begin{enumerate}
\item Prove that if $(T_\alpha)$ converges in WOT to $T$, then $(T_\alpha^*)$ converges in WOT to $T^*$.
\item Assume that $T$ and each $T_\alpha$ are normal. Prove that if $(T_\alpha)$ converges in SOT to $T$, then $(T_\alpha)$ converges in SOT* to $T$.
\end{enumerate}
\end{prob}
...
\section{Unbounded operators}\label{lb292}


\subsection{Quantum mechanics}\label{lb291}
...
\section{From completely continuous forms to compact operators}
