...
\begin{proof}[Hint]
2.  Show that the linear functional $\xi\in\Dom(A)\rightarrow\bk{\psi|A\xi}$ is bounded in the inner product $\omega$. Then apply the Riesz-Fr\'echet Thm. \ref{lb135}.
...
\begin{prob}
Let $A$ be a strictly positive closed operator on $\MH$. In this problem, following von Neumann's original method in \cite{vN29a}, we give a more operator-theoretic proof that the Krein-von Neumann extension $A_\Krm=\eqref{eq246}$ is positive self-adjoint. Let $P\in\fk L(\MH)$ be the projection onto $\Rng(A)$ (which is a closed subspace by Pb. \ref{lb482}).
\begin{enumerate}
\item Show that
\begin{align*}
...
\end{align*}
Conclude that we have an unbounded operator $B=A_\Krm|_{\Rng(A)}$ on $\Rng(A)$ with domain $P\Dom(A_\Krm)$ defined by sending $P\xi\in P\Dom(A_\Krm)\mapsto A_\Krm P\xi$. 
\item Show that $A$ equals the direct sum operator (cf. Def. \ref{lb483}) of $B$ on $\Rng(A)$ and $0$ on $\Rng(A)^\perp$. (Note that for any linear subspace $V\subset \MH$, we have $V=PV\oplus P^\perp V$ iff $PV\subset V$ iff $P^\perp V\subset V$.) Conclude that $B$ is strictly positive. 
\end{enumerate}
Since $B$ is strictly positive and surjective (due to part 2), by Prop. \ref{lb456}, $B$ is self-adjoint. It follows from Pb. \ref{lb484} that $A_\Krm=B\oplus 0$ is positive self-adjoint.
\end{prob}
