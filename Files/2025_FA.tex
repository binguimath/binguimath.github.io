...
\begin{proof}
For each $\xi\in V$, we have
\begin{align*}
\Supp(\mu_\xi)\subset\Supp(\pi)
\end{align*}
where $\mu_\xi$ is defined in Rem. \ref{lb297}. By Def. \ref{lb298}, $\mu_\xi$ vanishes on $X\setminus\Supp(\mu_\xi)$, and hence on its subset $X\setminus\Supp(\pi)$. Since $\xi$ is arbitrary, we conclude that $\pi$ vanishes on $X\setminus\Supp(\pi)$.
\end{proof}
