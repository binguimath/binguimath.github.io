...
Note that $(\im U-\im)^{-1}$ is well-defined by Lem. \ref{lb378}. As in Def. \ref{lb379}, it is customary to write $(U+\idt)(\im U-\im)^{-1}$ as $\frac{U+\idt}{\im(U-\idt)}$ and call it the \textbf{inverse Cayley transform} of $U$. \index{00@Inverse Cayley transform} It is the unique map satisfying
...
In \cite{vN29a}, von Neumann introduced the notion of the adjoint operator for a Hermitian operator $T$. The idea is straightforward: viewing $T$ as a differential operator, the extended operator $T^*$ should have domain $\Dom(T^*)$ consisting of all $\eta\in\MH$ for which the ``weak derivative'' $T^*\eta$ exists. Concretely, these are the vectors $\eta\in\MH$ such that the linear functional
\begin{align*}
...
This definition of $T^*$ clearly applies to any unbounded operator $T$ on $\MH$. However, without assuming that $T$ is Hermitian, the adjoint operator $T^*$ need not $T$. In fact, $\Dom(T^*)$ is not necessarily densely-defined, so $T^*$ may be only an n.d.d. unbounded operator. The systematic study of adjoints of general (not necessarily Hermitian) unbounded operators was undertaken by von Neumann in \cite{vN32b}. In this course, contrary to the historical order, we will first examine adjoints of general operators before specializing to Hermitian operators. 
