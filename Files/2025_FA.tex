...
\begin{df}
A \textbf{projection operator} (or simply a \textbf{projection}) \index{00@Projection} on an inner product $V$ is an element $P\in\fk L(V)$ such that $\omega_P$ is Hermitian (i.e. $\bk{\xi|P\eta}=\bk{P\xi|\eta}$ for all $\xi,\eta\in V$) and $P^2=\id_V$. It is easy to check that \index{00@$P^\perp=1-P$}
\begin{align*}
P^\perp:=1-P
\end{align*}
...
Suppose that $X$ is a compact Hausdorff space, not necessarily second-countable. A positive linear functional $\Lambda:\Borb(X,\Fbb)\rightarrow\Fbb$ is called \textbf{Radon} \index{00@Radon positive linear functional} if there exists a (necessarily finite) Radon measure $\mu$ on $X$ such that
\begin{align*}
...
For example, if $X$ is a compact Hausdorff space, any pre-unitary representation of $C(X)$ or $\Borb(X)$ is positive.
...
\begin{df}
Let $X$ be a compact Hausdorff space. A pre-unitary representation $(\pi,V)$ of $\Borb(X)$ (which is automatically positive) is called a \textbf{Radon pre-unitary representation} \index{00@Radon pre-unitary representation of a unital *-algebra} if for each $\xi\in V$, the linear functional
\begin{align}\label{eq151}
\scr A\rightarrow\Cbb\qquad f\mapsto\bk{\xi|\pi(f)\xi}
\end{align}
is Radon in the sense of Rem. \ref{lb275}. 

Similarly, pre-unitary representation $(\pi,V)$ of $\Borb(X)$ is called a \textbf{normal pre-unitary representation} \index{00@Normal pre-unitary representation} if for each $\xi\in V$, the linear functional \eqref{eq151} is normal, that is,
\begin{align*}
\lim_n\bk{\xi|\pi(f_n)\xi}=\bk{\xi|\pi(f)\xi}
\end{align*}
for any increasing sequence $(f_n)$ in $\Borb(X)$ converging pointwise to $f\in\Borb(X)$.

Note that by Prop. \ref{lb252}, when $X$ is second countable, a pre-unitary representation of $\Borb(X)$ is Radon iff it is normal.  \hqed
\end{df}
...
Let $\scr A$ be a unital *-subalgebra of $C(X)$ separating points of $X$. Then $\ovl B_{\scr A}(0,1)$ is dense in $\ovl B_{\Borb(X)}(0,1)$ in the universal $L^2$-topology, i.e., 
\begin{align*}
\{f\in\scr A:\Vert f\Vert_{l^\infty}\leq1\}\qquad\text{is dense in}\qquad \{f\in\Borb(X):\Vert f\Vert_{l^\infty}\leq1\}
\end{align*}
\end{lm}
Therefore, for any $f\in\Borb(X)$ there exists a net $(f_\alpha)$ in $\scr A$ with $\sup_\alpha\Vert f_\alpha\Vert_{l^\infty}\leq\Vert f\Vert_{l^\infty}$ satisfying $\lim_\alpha\Vert f-f_\alpha\Vert_{L^2(X,\mu)}=0$ for each Radon measure $\mu$ on $X$.


\begin{proof}
By Stone-Weierstrass, $\scr A$ is $l^\infty$-dense in $C(X)$. Therefore, for each $f\in C(X)$ with $\Vert f\Vert_{l^\infty}\leq 1$ there is a net $(f_\alpha)$ in $\scr A$ converging uniformly to $f$. In particular, $\Vert f_\alpha\Vert_{l^\infty}$ converges to $\Vert f\Vert_{l^\infty}$. Therefore $f_\alpha\cdot \Vert f\Vert_{l^\infty}/\Vert f_\alpha\Vert_{l^\infty}$ converges uniformly to $f$. This proves that the closed unit ball of $\scr A$ is $l^\infty$-dense (and hence universally $L^2$-dense) in the closed unit ball of $C(X)$.

It remains to prove that $\ovl B_{C(X)}(0,1)$ is universally $L^2$-dense in $\ovl B_{\Borb(X)}(0,1)$. Choose any $f\in \ovl B_{C(X)}(0,1)$. Let $\scr I$ be the set of Radon measures on $X$. By Lusin's Thm. \ref{lb82} (together with the Tietze extension Thm. \ref{lb83}), for each finite subset $E\subset\scr I$ and $\eps>0$ there exists $f_{E,\eps}\in C(X)$ such that $\Vert f_{E,\eps}\Vert_{l^\infty}\leq 1$ and
\begin{align*}
\mu\{x\in X:f(x)\neq f_{E,\eps}\}\leq\eps
\end{align*}
...
\end{align*}
Hence, the net $(f_{E,\eps})_{(E,\eps)\in\fin(2^{\scr I})\times\Rbb_{>0}}$ (where $\Rbb_{>0}$ is given by the order $>$ instead of the usual one $\leq$) is a net in $\ovl B_{C(X)}(0,1)$ and converges universally-$L^2$ to $f$.
\end{proof}
...
For a more faithful presentation of Riesz’s original proof of his spectral theorem in \cite{Rie13}, see Sec. 25.8 and 25.9 of \cite{Gui-A}.  
\end{rem}
...
Modern approaches to the spectral theorem completely abandon the finite approximation paradigm. This is because they aim to develop methods that are also applicable to operators on Banach spaces, not just Hilbert spaces. In these modern treatments, the primary non-trivial step---apart from applying the Riesz(-Markov) representation theorem---is proving the positivity of the polynomial functional calculus. The following is a list of mainstream approaches, along with the key theorems used to establish this crucial positivity.
\begin{itemize}
\item The approach using the notion of spectrum
\begin{align*}
\Sp(T)=\{z\in\Cbb:(z-T)\text{ is not invertible in }\fk L(T)\}
\end{align*}
The key theorems are \textbf{Gelfand's spectral radius theorem}
\begin{align*}
\diam(T)=\lim_{n\rightarrow\infty}\Vert T^n\Vert^{1/n}
\end{align*}
(for any $T\in\fk L(\MH)$, not necessarily self-adjoint) and the \textbf{spectral mapping theorem}: $\Sp(f(T))\subset f(\Sp(T))$ for each $f\in\Cbb[x]$. See \cite{Lax,RS-1}.
...
\end{align*}
where $C$ is a suitable closed curve in $\Cbb$ surrounding $[-r,r]$. The key theorem is $(fg)(T)=f(T)g(T)$, and that any holomorphic function $f$, defined and strictly positive on a neighborhood of $[-r,r]$, can be written as $h^*h$ with $h$ holomorphic. This allows the argument of Prop. \ref{lb272} to apply. See \cite{Gui-S}.
\item The approach reducing the spectral theorem for self-adjoint operators to that of unitary operators. The crucial point is that positivity of the polynomial functional calculus is easier to establish for unitary operators: any strictly positive polynomial polynomial $f$ on $\Sbb^1$ can be written as $g^*g$ for some polynomial $g$, again allowing the argument of Prop. \ref{lb272} to go through. See \cite{Xia}.
\end{itemize}
\hqed 
\end{rem}
...
\end{align*}
As in the proof of Thm. \ref{lb283}, in order to apply the operator-valued abstract Hausdorff moment Thm. \ref{lb274}, the main obstacle lies in proving that $\pi_{T_\blt}$ is positive, that is, $f(T_\blt)\geq0$ for each $f\in\Cbb[x_\blt]$ satisfying $f|_X\geq0$. We shall still apply the method of 
