...
Let $\omega\in\Ses(\MH|\MK)$ and $\sigma\in\Ses(\MK|\MM)$. Then the \textbf{composition} \pmb{$\omega\circ\sigma$} \index{00@Composition of sesquilinear forms} is the element of $\Ses(\MM|\MH)$ defined by
\begin{align*}
...
The expression $(z-T)^{-1}$ is called the \textbf{resolvent} of $T$. \index{00@Resolvent}
...
Next, we introduce an elementary yet fundamental property of SOT. This property was first highlighted by Riesz in \cite{Rie11} as a key tool in his proof of the spectral theorem for bounded self-adjoint operators. In Ch. \ref{lb114}, we will also rely on it in our study of spectral theory. This property roughly says that in Cor. \ref{lb158}, if the nets of operators are uniformly bounded, and if $\scr A=\scr B$, then $\lim_\alpha T_\alpha S_\alpha$ converges strongly to $TS$.
...
\begin{align}
\begin{aligned}
\scr R(I)=\{&\text{Increasing right continuous function }\rho:I\rightarrow\Rbb_{\geq0}\\
&\text{satisfying }\lim_{x\rightarrow-\infty}\rho(x)=0\text{ if }I=\Rbb\}
\end{aligned}
\end{align}
Fix a family $(c_n)_{n\in\Nbb}$ in $\Rbb$. Our goal is to study the following polynomial problem:

\begin{problem}[\textbf{Polynomial moment problem}] \index{00@Polynomial moment problem}\label{lb162}
Does there exists $\rho\in\scr R(I)$ such that
\begin{align}\label{eq79}
c_n=\int_I x^nd\rho\qquad\text{for each $n\in\Nbb$}
\end{align}
where the RHS is integrable, i.e., $\int_I |x|^nd\rho<+\infty$? Depending on $I$ is $\Rbb$, $\Rbb_{\geq0}$, or $[0,1]$, this problem is referred to as the \textbf{Hamburger moment problem}, \index{00@Hamburger moment problem} the \textbf{Stieltjes moment problem}, \index{00@Stieltjes moment problem} and the \textbf{Hausdorff moment problem}, \index{00@Hausdorff moment problem} respectively.
\end{problem}

\subsubsection{Preliminary}


We now clarify the meaning of \eqref{eq79}, since we have so far defined $\int_Ifd\rho$ only for $f\in C_c(I)$.

\begin{df}
Let $f$ be a Borel function from $I$ to $\Cbb$ or $\ovl\Rbb_{\geq0}$. Let $\rho\in\scr R(I)$. Let $\mu_\rho$ be the finite Borel measure on $I$ associated to $\rho$ as in the Riesz representation Thm. \ref{lb9}. We define the \textbf{Stieltjes integral} $\int_I fd\rho$ to be \index{00@Stieltjes integral}
\begin{align*}
...
We also let $H^+\in\Cbb^{\Nbb\times \Nbb}$ be the defined by $H^+(m,n)=H(m+1,n+1)$, that is,
\begin{align*}
H^+=\begin{pmatrix}
c_1&c_2&c_3&c_4&\cdots\\
c_2&c_3&c_4&c_5&\cdots\\
...
\item The range of the solution $\rho$ is not a finite set (equivalently, if the associated measure $\mu_\rho$ is not supported in a finite set, cf. Lem. \ref{lb163}).
\end{enumerate}
...
\subsubsection{From the polynomial moment problem to diagonalization}

Assume that $H$ is positive. 
...
\begin{proof}
Recall that $H$ determines a positive sesquilinear form $\bk{\cdot|\cdot}$ on $\Cbb[x]$. By Cor. \ref{lb164}, the null space $\scr N=\{f\in\Cbb[x]:\Vert f\Vert^2=0\}$ is a linear subspace of $\Cbb[x]$, and $\bk{\cdot|\cdot}$ descends to an inner product of
\begin{align*}
V=\Cbb[x]/\scr N
\end{align*}
Let
...
Assume $\dim V<+\infty$. Then $T$ is a Hermitian operator on the finite-dimensional inner product space $V$, and hence can be diagonalized. More precisely, there exists an orthonormal basis $e_1,\dots,e_n$ of $T$ such that $Te_j=\lambda_je_j$ for all $1\leq j\leq n$, where $\lambda_j\in\Rbb$. Write
\begin{align*}
...
\subsubsection{From discrete spectra to continuous spectra}
...
\begin{align*}
P_n\text{ be the projection of $V$ onto }V_n=\Span\{\Omega,T\Omega,\dots,T^n\Omega\}
\end{align*}
...
Since $\omega_T$ and $\omega_{P_n}$ are Hermitian (Cor. \ref{lb171}), for each $n$ and $\xi\in V$ we have
\begin{align*}
\bk{\xi|P_nTP_n\xi}=\bk{TP_n\xi|TP_n\xi}=\omega_T(P_n\xi|P_n\xi)\geq0
\end{align*}
It follows that $\omega_{PTP}$ is Hermitian. Similarly, if $H^+\geq0$ (equivalently, $\omega_T\geq0$), then
\begin{align*}
\bk{\xi|P_nTP_n\xi}=\bk{P_n\xi|TP_n\xi}\geq0
\end{align*}
and hence $\omega_{P_nTP_n}\geq0$. If both $H^+$ and $H-H^+$ are positive, then 
\begin{align*}
\bk{\xi|P_nTP_n\xi}=\bk{P_n\xi|TP_n\xi}\geq0\qquad\bk{\xi|P_n(1-T)P_n\xi}=\bk{P_n\xi|(1-T)P_n\xi}\geq0
\end{align*}
and hence $\omega_{P_nTP_n},\omega_{P_n(1-T)P_n}$ are positive.
\end{rem}
...
\begin{align}
c_{n,m}=\bk{\Omega|(P_nTP_n)^m\Omega}
\end{align}
\end{df}
...
\begin{gather*}
P_nTP_n\Omega=T\Omega\in V_1\qquad (P_nTP_n)^2\Omega=(P_nTP_n)T\Omega=T^2\Omega\in V_2\\
(P_nTP_n)^3\Omega=(P_nTP_n) T^2\Omega=T^3\Omega\in V_3\qquad\cdots\\ (P_nTP_n)^n\Omega=(P_nTP_n)T^{n-1}\Omega=T^n\Omega
\end{gather*}
Thus, for each $m\leq 2n+1$, write $m=a+b$ where $a,b\in\Nbb$ and $a\leq n$ and $b\leq n+1$. Note that $(P_nTP_n)^{n+1}\Omega=P_nTP_nT^n\Omega=P_nT^{n+1}\Omega$, we have
\begin{align*}
&\bk{\Omega|(P_nTP_n)^m\Omega}=\bk{(P_nTP_n)^a\Omega|(P_nTP_n)^b\Omega}=\bk{T^a\Omega|P_nT^b\Omega}=\bk{P_nT^a\Omega|T^b\Omega}\\
=&\bk{T^a\Omega|T^b\Omega}=\bk{\Omega|T^{a+b}\Omega}=c_m
\end{align*}
\end{proof}

\begin{lm}\label{lb174}
For each $n\in\Nbb$, there exists $\rho_n\in\scr R(I)$ such that $\Rng(\rho_n)$ is a finite subset of $[0,c_0]$, and that  for all $m\in\Nbb$ we have
...
\begin{proof}
We view $T_n:=P_nTP_n$ as a linear operator on $V_n$. By Rem. \ref{lb173}, $T_n$ is Hermitian. Therefore, by linear algebra, $V_n$ has an orthonormal basis $e_0,\dots,e_n$ such that $Te_i=\lambda_ie_i$ for all $0\leq i\leq n$, where $\lambda_i\in\Rbb$. Moreover, by Rem. \ref{lb173}, if $I=\Rbb_{\geq0}$ then $T_n\geq0$, and hence $\lambda_i\geq0$; if $I=[0,1]$ then $0\leq T_n\leq\id_{V_n}$, and hence $0\leq\lambda_i\leq 1$. It follows that $\lambda_i\in I$ in all cases.
Write
\begin{align*}
\Omega=\sum_{i=0}^n a_ie_i
\end{align*}
where $a_n\in\Cbb$. (So $a_i=\bk{e_i|\Omega}$.) Thus
\begin{align*}
c_{n,m}=\bk{\Omega|(T_n)^m\Omega}=\sum_{i=0}^m \lambda_i^m\cdot |a_i|^2=\int_I x^md\rho_n
\end{align*}
where $\rho_n$ is the unique element in $\scr R(I)$ whose associated finite Borel measure is $\sum_{i=0}^n|a_i|^2\delta_{\lambda_i}$, that is,
\begin{align*}
\rho_n(x)=\sum_{\lambda_i\leq x}|a_i|^2
\end{align*}
In particular, by Parseval's identity, we have
\begin{align*}
0\leq \rho_n(x)\leq\sum_i |a_i|^2=\Vert\Omega\Vert^2\xlongequal{\eqref{eq86}}c_0
\end{align*}
\end{proof}


\begin{df}\label{lb175}
Let $(\rho_n)_{n\in\Nbb}$ be the uniformly bounded sequence in $\scr R(I)$ described by Lem. \ref{lb174}. By Helly's selection Thm. \ref{lb94}, $(\rho_n)$ has a subsequence $(\rho_{n_k})_{k\in\Nbb}$ converging pointwise to some increasing function $\wtd\rho:I\rightarrow\Rbb_{\geq0}$. Define $\rho\in\scr R(I)$ as follows.
\begin{itemize}
...
Assume Condition \ref{lb178}, and let $\rho\in\scr R(I)$ be as in Def. \ref{lb175}. Then for each $m\in\Nbb$, we have
\begin{align*}
c_m=\int_I x^md\rho
\end{align*}
...
\end{align}
Since $\beta_\lambda\cdot x^m$ is increasing and converging pointwise to $x^m$ as $\lambda\nearrow+\infty$, by MCT, we conclude that $x^m$ is $d\rho$-integrable.



Next, assume that $m$ is odd. Since $|x|^m\leq 1+x^{m+1}$ and since both $1=x^0$ and $x^{m+1}$ have been proved to be $d\rho$-integrable, we conclude that $\alpha_1|x|^m$ is $d\rho$-integrable, we conclude that $x^m$ is $d\rho$-integrable.
