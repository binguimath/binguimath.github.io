...
\end{align*}
holds. These vectors are eigenvectors of $T$, in the sense that $T$ admits the spectral decomposition
\begin{align*}
...
\end{align}
where $E=(E(\lambda))_{\lambda\in\Rbb}$ is an increasing family of projections satisfying
\begin{align*}
...
Applying this interpretation to the spectral projections $E(\lambda)$ in \eqref{eq188}, one finds that for any unit vector $\xi$, the value $\bk{\xi|E(\lambda)}\xi$ is the probability that the observable $T$ takes a value not exceeding $\lambda$ in the state $\xi$. Therefore, if we define the projection
\begin{align*}
