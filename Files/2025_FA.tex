...
\subsection{Unitary representations of the group $\Rbb^N$}\label{lb508}
...
For each $f\in C_c(\Rbb^N)$, one easily shows that the function $t\in\Rbb\rightarrow U(t)f\in\MH$ is continuous by using the uniform continuity of $f$. Since $C_c(\Rbb^N)$ is dense in $L^2(\Rbb^N,m)$ (cf. Thm. \ref{lb14}), it follows from Thm. \ref{lb32} that $U$ is strongly continuous.  \hqed
\end{eg}
...
\end{align*}
We abbreviate $\mk_N$ to $\mk$ when no confusion arises.


\subsubsection{Fourier transforms of $L^1$-functions}
...
\begin{gather*}
\wht f(t)=\int_{\Rbb^N}f(x)e^{-\im tx}d\mk(x)\qquad \wch f(t)=\wht f(-t)=\int_{\Rbb^N}f(x)e^{\im tx}d\mk(x)
\end{gather*}
\end{df}
...
\begin{pp}[\textbf{Riemann-Lebesgue lemma}]\label{lb510} \index{00@Riemann-Lebesgue lemma}
Let $f\in L^1(\Rbb^N,\mk)$. Then $\wht f,\wch f\in C_0(\Rbb^N)$, and
...
\begin{proof}
The inequalities \eqref{eq252} are obvious. Moreover, DCT shows that $\wht f$ and $\wch f$ are continuous. It remains to prove that $\wht f\in C_0(\Rbb^N)$ (and similarly $\wch f\in C_0(\Rbb^N)$).
...
In the general case, by Prop. \ref{lb14}, there exists a sequence $(f_n)$ in $C_c(\Rbb^N)$ such that $\lim_n\Vert f-f_n\Vert_{L^1}=0$. By \eqref{eq252}, we have $\lim_n\Vert \wht f-\wht{f_n}\Vert_{l^\infty}=0$. Since $\wht{f_n}\in C_0(\Rbb^N)$, and since $C_0(\Rbb^N)$ is closed in $l^\infty(\Rbb^N)$, the function $\wht f$ belongs to $C_0(\Rbb^N)$.
\end{proof}



\begin{pp}\label{lb509}
Let $f,g\in L^1(\Rbb^N,\mk)$. The \textbf{convolution} \index{00@Convolution}
\begin{align*}
(f*g)(x)=\int_{\Rbb^N}f(x-y)g(y)d\mk(y)\equiv \int_{\Rbb^N}f(y)g(x-y)d\mk(y)
\end{align*}
exists for almost every $x$. Moreover, we have $f*g\in L^1(\Rbb^N,\mk)$ and $\wht{f*g}=\wht f\cdot\wht g$.
\end{pp}

\begin{proof}
Since $\int |f(x-y)g(y)|d\mk(x)d\mk(y)=\Vert f\Vert_{L^1}\cdot\Vert g\Vert_{L^1}<+\infty$, Fubini's theorem implies that $f*g$ can be defined a.e., that $f*g\in L^1(\Rbb^N,\mk)$. For each $t\in\Rbb^N$, using Fubini's theorem we compute that
\begin{align*}
&\wht{f*g}(t)=\int (f*g)(x)e^{\im tx}d\mk(x)=\int\int f(x-y)g(y)e^{\im tx}d\mk(y)d\mk(x)\\
=&\int\int f(x-y)g(y)e^{\im tx}d\mk(x)d\mk(y)=\int\int f(u)g(y)e^{\im tu}e^{\im ty}d\mk(u)d\mk(y)
\end{align*}
where the RHS equals $\wht f(t)\wht g(t)$.
\end{proof}
...
The set
\begin{align*}
\wht C_c(\Rbb^N):=\{\wht f:f\in C_c(\Rbb^N)\}
\end{align*}
is an $l^\infty$-dense $*$-subalgebra of $C_0(\Rbb^N)$.
\end{co}

\begin{proof}
Since $C_c(\Rbb^N)$ is closed under convolution, by Prop. \ref{lb509}, $\wht C_c(\Rbb^N)$ is closed under multiplication. By Prop. \ref{lb511}, $\wch C_c(\Rbb^N)$ is closed under taking conjugation. Therefore $\wht C_c(\Rbb^N)$ is a $*$-subalgebra of $C_0(\Rbb^N)$.

For each $t\in\Rbb^N$, one can find $f\in C_c(\Rbb^N,\Rbb_{\geq0})$ supported in a neighborhood of $0$ with $\int fd\mk=1$ such that $\wht f(t)=\int f(x)e^{\im tx}d\mk(x)$ is close to $e^{\im t\cdot 0}=1$. Therefore, $\wht C_c(\Rbb^N)$ vanishes nowhere. For each $s,t\in\Rbb^N$ such that $s\neq t$, choose any $y\in\Rbb^N$ such that $e^{\im sy}\neq e^{\im ty}$. Again, one can find $f\in C_c(\Rbb^N,\Rbb_{\geq0})$ supported in a neighborhood of $y$ with $\int fd\mk=1$ such that $\wht f(t)$ is close to $e^{\im ty}$, and that $\wht f(s)$ is close to $e^{\im sy}$. In particular, one can find $f$ such that $\wht f(t)\neq \wht f(s)$. This proves that $\wht C_c(\Rbb^N)$ separates points of $\Rbb^N$. Therefore, by the Stone-Weierstrass Thm. \ref{lb360}, $\wht C_c(\Rbb^N)$ is dense in $C_0(\Rbb^N)$.
\end{proof}
...
\begin{proof}
Clearly $|f|\leq g$, and hence $\int_X|f|d\mu\leq\int_Xgd\mu<+\infty$. Thus $f$ is integrable.  By Prop. \ref{lb493},
\begin{align*}
...
\begin{enumerate}[label=(\alph*)]
\item For each $t\in I$, the following function is integrable.
\begin{align*}
...
\end{align*}
For each $t\in I$, the following function is measurable
\begin{align*}
\partial_I f(-,t):X\rightarrow\MH\qquad x\mapsto\partial_If(x,t)
\end{align*} 
\item There exists a measurable $g:X\rightarrow\Rbb_{\geq0}$ with $\int_Xgd\mu<+\infty$ such that for all $t\in I$ we have
\begin{align*}
|\partial_I f(-,t)|\leq g
\end{align*}
\end{enumerate}
Then $\int_X f(x,t)d\mu(x)$ is differentiable over $t$, and for each $t\in I$ we have
\begin{align}\label{eq250}
\frac d{dt}\int_X f(x,t)d\mu(x)=\int_X\partial_I f(x,t)d\mu(x)
\end{align}
\end{co}

Note that the function $\partial_I f(-,t)$ is integrable due to (b) and (c).
...
\end{align*}
The preorder on $I\setminus\{t\}$ is defined such that larger elements are closer to $t$. By the fundamental theorem of calculus, for each $\eta\in\MH$ we have
\begin{align*}
...
\end{align*}
and hence $|\varphi_s|\leq g$. Therefore Thm. \ref{lb495} is applicable, which shows that $\lim_{s\rightarrow t}\int_X\varphi_sd\mu=\int_X\partial_If(-,t)d\mu$. This proves \eqref{eq250}.
\end{proof}
