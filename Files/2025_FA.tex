...
This finite-approximation paradigm gave way to the linear-extension paradigm, with F. Riesz as its prime mover. In this course we will exhibit two main patterns of the linear-extension paradigm:
\begin{itemize}
...
\begin{co}[\textbf{Abstract Hausdorff moment theorem}]\label{lb261}\index{00@Abstract Hausdorff moment theorem}\label{lb262}
...
\begin{gathered}
\begin{array}{c}
\{\text{normal positive linear functionals $\Borb(F,\Kbb)\rightarrow\Fbb$}\}\\
\raisebox{0.3cm}{\rotatebox{-90}{$\xlongrightarrow{\rotatebox{90}{$\simeq$}}$}}\\
...
\begin{enumerate}
\item From finite approximations to linear extensions.
\item From (muti)linear forms to linear operators.
\item From duality to Cauchy completeness.
\end{enumerate}
...
The principal complexity with the sesquilinear-form and matrix perspectives---which does not arise in finite-dimensional linear algebra---is not their definition but the associativity of multiplication: one must address the Fubini-type issues for infinite sums appearing in \eqref{eq78} and \eqref{eq144}. For bounded sesquilinear forms and bounded matrices, defining products in terms of orthonormal bases introduces many inconveniences that are absent in the finite-dimensional setting, whereas the operator framework avoids this subtlety.
\end{proof}
