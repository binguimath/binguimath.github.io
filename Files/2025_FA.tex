...
As we shall see in the next subsection, in the historical approach to the Dirichlet problem \eqref{eq278}, compact operators play a crucial and central role. In fact, the study of the Dirichlet problem was one of the main motivations for introducing completely continuous forms/operators and compact operators.




\subsubsection{The historical approach to the Dirichlet problem}


The historical approach to the Dirichlet problem \eqref{eq278} is to reduce it to the solution of the associated integral equation by using the method called \textbf{double layer potential}. In what follows we sketch this method.

Recall that given $g\in C(\partial\Omega)$, we want to solve the equation \eqref{eq278} for $u\in C(\ovl\Omega)$ smooth on $\Omega$. Define the \textbf{fundamental solution} \index{00@Fundamental solution}
\begin{align}
\Phi(x)=\left\{
\begin{array}{ll}
...
\right.
\end{align}
where $\sigma_{N-1}$ is the area of the $(N-1)$-dimensional sphere $\Sbb^{N-1}$. For each $f\in C(\partial\Omega)$, define a function $\MD f$ on $\ovl\Omega$ by
\begin{align*}
(\MD f)(x)=\int_{\partial\Omega}\partial_\nu\Phi(x-y)\cdot f(y)dy
\end{align*}
where $\partial_\nu\Phi$ is the outward directional derivative of $\Phi$ on $\partial\Omega$, that is,
\begin{align*}
\partial_\nu\Phi(x-y)=-\frac{\bk{x-y,\nbf_y}}{\sigma_{N-1}|x-y|^N}
\end{align*}
where $\nbf_y$ is the unit normal vector at $y\in\partial\Omega$ pointing outward. Then $\MD f$ is smooth and harmonic on $\Omega$, and has jumping discontinuity near $\partial\Omega$ by $\frac 12f$. That is, the function $u:\ovl\Omega\rightarrow\Cbb$ defined by
\begin{align*}
u(x)=\left\{
\begin{array}{ll}
(\mc Df)(x)&\text{ if }x\in\Omega\\
\frac 12f(x)+(\mc Df)(x)&\text{ if }x\in\partial\Omega
\end{array}
\right.
\end{align*}
is continuous on the whole domain $\ovl\Omega$. Therefore, this function is a continuous function on $\ovl\Omega$, smooth on $\Omega$, and satisfying the differential equation \eqref{eq278} for the boundary condition $g=\frac12f+(\MD f)$.

Consequently, the Dirichlet problem \eqref{eq278} is reduced to solving the integral equation
\begin{gather}
\frac 12f(x)+\int_{\partial\Omega} K(x,y)f(y)dy=g(x)\qquad\text{with }K(x,y)=\partial_\nu\Phi(x-y)
\end{gather}
