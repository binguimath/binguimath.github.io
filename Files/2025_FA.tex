...
\DeclareMathOperator{\Borb}{{\mathscr{B}\text{\kern -2pt {\calligra\large or}}\,}^{\mathrm b}}
...
\newcommand{\LSCb}{\mathrm{LSC}^{\mathrm b}}
...
The above proof shows that if $V$ is separable, then $\ovl B_{V^*}(0,1)$ is second-countable and therefore sequentially compact under the weak-* topology. The converse is also true:
...
The limit on the RHS of \eqref{eq148} converges under the operator norm. Moreover, for each $\xi\in\MH$, we have
...
Let $\scr A$ be a unital *-subalgebra of $C(X)$ separating points of $X$. Then there is a one-to-one correspondence between the following two classes of objects: \footnote{Since these two classes are proper classes rather than sets, we avoid using the term "bijection".}
\begin{itemize}
...
Let $X$ be a set. Let $\scr A$ be a unital *-subalgebra of $l^\infty(X)$, equipped with the $l^\infty$-norm. Let $\pi:\scr A\rightarrow\fk L(\MH)$ be a positive unitary representation of $\scr A$. Then $\pi$ is bounded with operator norm $\Vert\pi\Vert\leq 4$.
\end{pp}
...
\end{align*}
This, together with Prop. \ref{lb238}, finishes the proof.
\end{proof}
...
\begin{align*}
\diam(T)=\lim_{n\rightarrow\infty}\Vert T^n\Vert^{1/n}
\end{align*}
...
\begin{align*}
g(A)=\sum_{i=1}^n c_iE_i\qquad g(B)=\sum_{j=1}^n c_jF_j\qquad \id=\sum_{i=1}^n E_i=\sum_{j=1}^n F_j
\end{align*}
where
\begin{align*}
...
are projections, and
\begin{align*}
E_iE_j=\delta_{i,j}E_i\qquad F_iF_j=\delta_{i,j}F_j\qquad\sum_i E_i=\sum_j F_j=\idt\qquad [E_i,F_j]=0
\end{align*}
...
\end{align*}
Then $\Supp(\pi)$ is a closed subset of $X$, because we have
\begin{align*}
X\setminus\Supp(\pi)=\bigcup_{U\in\MT_X,\pi(\chi_U)=0}U
\end{align*}
By polarization identity, we also clearly have
\begin{align*}
\Supp(\pi)=\bigcap_{\xi\in V}\Supp(\mu_\xi)
\end{align*}
where $\mu_\xi$ is defined in Rem. \ref{lb297}.

We say that $\pi$ is \textbf{compactly supported} (or that $\pi$ has \textbf{compact support}) if the closed set $\Supp(\pi)$ is compact.  \hqed
\end{df}


\begin{rem}\label{lb302}
In the remainder of this course, whenever we speak of $\Supp(\pi)$, we will always assume that one of the following conditions is satisfied:
\begin{enumerate}
...
\begin{proof}
For each $\xi\in V$, we have $\Supp(\mu)\subset\Supp(\pi)$. By Def. \ref{lb298}, $\mu_\xi$ vanishes on $X\setminus\Supp(\mu)$, and hence on its subset $X\setminus\Supp(\pi)$.
\end{proof}


\begin{rem}\label{lb301}
Let $(\pi,V)$ be a pre-unitary representation of $\Borb(X)$. Choose any closed subset $K\subset X$ containing $\Supp(\pi)$. Then for each $f\in\Borb(X)$ we have
\begin{align*}
\pi(f)=\pi(f\cdot\chi_K)
\end{align*}
Therefore, if $f,g\in\Borb(X)$ satisfy $f|_K=g|_K$, then $\pi(f)=\pi(g)$.
\end{rem}

\begin{proof}
By Rem. \ref{lb302}, we have $\pi_{K^c}=0$. Therefore, $\pi(f)=\pi(f\chi_K)+\pi(f\chi_{K^c})+\pi(f\chi_K)+\pi(f)\pi(\chi_{K^c})=\pi(f\chi_K)$.
\end{proof}
...
The map
\begin{align*}
\wtd\pi_i:\Borb(K)\rightarrow\fk L(\MH)\qquad \wtd\pi_i(f)=\pi_i(f)
\end{align*}
(where each $f\in\Borb(K)$ is extended by zero to $\Cbb^N$) is clearly a normal unitary representation of $\Borb(K)$, noting that
\begin{align*}
\wtd\pi_1(1_K)=\pi_1(\chi_K)=\pi_1(\chi_K)+\pi_1(\chi_{\Cbb^N\setminus K})=\pi_1(1_{\Cbb^N})=\id_\MH
\end{align*}
and similarly $\wtd\pi_2(1_K)=\id_\MH$. Since
\begin{align*}
\wtd\pi_1(z_i|_K)=\pi_1(z_i\chi_K)=\pi_1(z_i\chi_K)+\pi_1(z_i)\pi_1(\chi_{\Cbb^N\setminus K})=\pi_1(z_i)=T_i
\end{align*}
and similarly $\wtd\pi_2(z_i|_K)=T_i$, we conclude that $\wtd\pi_1|_{\scr A}=\wtd\pi_2|_{\scr A}$, and hence $\wtd\pi_1=\wtd\pi_2$ by the uniqueness part of Thm. \ref{lb274}. Therefore, for each $f\in\Borb(X)$, we have
\begin{align*}
\pi_1(f)=\pi_1(f\chi_K)+\pi_1(f)\pi_1(\chi_{\Cbb^N\setminus K})=\pi_1(f\chi_K)=\wtd\pi_1(f)
\end{align*}
and similarly $\pi_2(f)=\wtd\pi_2(f)$. This proves $\pi_1=\pi_2$.\\[-1ex]
...
\begin{align*}
\pi:\Borb(\Cbb^N)\rightarrow\fk L(\fk H)\qquad f\mapsto (f|_K)(A_1,B_1,\dots,A_N,B_N)
\end{align*}
...
\begin{rem}
By Rem. \ref{lb301}, for each $f\in\Borb(\Cbb^N)$, we have
\begin{align*}
f(T_\blt)=(f\chi_{\Sp(T_\blt)})(T_\blt)
\end{align*}
Therefore, if $f,g\in\Borb(\Cbb^N)$ are equal outside $\Sp(T_\blt)$, then $f(T_\blt)=g(T_\blt)$.
\end{rem}
