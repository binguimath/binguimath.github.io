...
Let $T:\MH\rightarrow\MK$ be an n.d.d. unbounded operator. The \textbf{graph inner product} (of $T$) \index{00@Graph inner product $\bk{\cdot\vert\cdot}_T$} on $\Dom(T)$ \index{DomT@$\bk{\cdot\vert\cdot}_T$, the graph inner product} is defined by
\begin{gather*}
...
\subsection{Unitary representations of the group $\Rbb^N$}
...
If $T$ is a self-adjoint operator on $\MH$, then as discussed at the beginning of Sec. \ref{lb487}, the family $t\in\Rbb\rightarrow e^{\im tT}\in\fk L(\MH)$ provides the solution for the Schr\"odinger equation with Hamiltonian operator $T$. This family can be viewed as a ``one parameter symmetry'' of the given quantum system. 

In this section, we rigorously define what is meant by an ``$N$ parameter mutually-commuting symmetries" in terms of strongly continuous unitary representations of $\Rbb^N$, and we show that such representations correspond bijectively to tuples $(T_1,\dots,T_N)$ of strongly commuting self-adjoint operators on $\MH$.
...
\subsubsection{From unitary representations to self-adjoint operators}


In this subsection, we prove that the direction (1)$\Rightarrow$(2) in Thm. \ref{lb488} is surjective. We first consider the case that $N=1$.
...
\subsection{Harmonic analysis on the group $\Rbb^N$}


Let $N\in\Zbb_+$. We define the normalized Lebesgue measure on $\Rbb^N$ by \index{m@$\mk_N\equiv \mk=(2\pi)^{-N/2}m_N$, the normalized Lebesgue measure}
\begin{align*}
\pmb{d\mk_N}:=(2\pi)^{-\frac N2}dm_N
\end{align*}
abbreviated to $d\mk$ when no confusion arises. The reason for introducing the factor $(2\pi)^{-N/2}$ is that
\begin{align*}
...
\begin{df}
Let $\mu$ be a finite Borel measure on $\Rbb^N$. The \textbf{Fourier transform} \index{00@Fourier transform} $\pmb{\wht\mu}$ and the \textbf{inverse Fourier transform} \index{00@Inverse Fourier transform} $\pmb{\wch\mu}$ are functions $\Rbb^N\rightarrow\Cbb$ defined by
\begin{gather*}
\wht\mu(t)=\bk{\mu,e_{-t}}\qquad \wch\mu(t)=\bk{\mu,e_t}
\end{gather*}
for each $t\in\Rbb^N$, where $\mu$ is viewed as a bounded linear functional on $\Cb(\Rbb^N)=C(\Rbb^N)\cap l^\infty(\Rbb^N)$. In other words,
\begin{align*}
\wht\mu(t)=\int_{\Rbb^N}e^{-\im tx}d\mu(x)\qquad\wch\mu(t)=\int_{\Rbb^N}e^{\im tx}d\mu(x)
\end{align*}
If $f\in L^1(\Rbb^N,\mk)$, we define \textbf{Fourier transform}  $\pmb{\wht f}$ and the \textbf{inverse Fourier transform} $\pmb{\wch f}$ by
\begin{align*}
\wht f:=\wht{fd\mk}\qquad \wch f:=\wch{fd\mk}
\end{align*}
\end{df}

\begin{rem}
For each finite Borel measure $\mu$ on $\Rbb^N$, we clearly have $\wht\mu,\wch\mu\in\Cb(\Rbb^N)$ by DCT and the obvious fact that
\begin{align}
|\wht\mu|_{l^\infty}\leq \mu(\Rbb^N)\qquad |\wch\mu|_{l^\infty}\leq\mu(\Rbb^N)
\end{align}
In particular, if $f\in L^1(\Rbb^N,\mk)$, then $\wht f,\wch f\in \Cb(\Rbb^N)$ and
\begin{align}
\Vert \wht f\Vert_{l^\infty}\leq\Vert f\Vert_{L^1(\Rbb^N,\mk)}\qquad \Vert \wch f\Vert_{l^\infty}\leq\Vert f\Vert_{L^1(\Rbb^N,\mk)}
\end{align}
\end{rem}
...
\subsection{Integrals of measurable functions valued in Hilbert spaces}



In this appendix section, we let $\MH,\MK$ be a Hilbert space, and let $(X,\fk M,\mu)$ be a measure space.

\subsubsection{A criterion for measurability}
...
Let $E$ be a countable dense subset of $f(X)$. Then $U:=\ovl{\Span_\Cbb E}$ is separable, since $\Span_{\Qbb+\im\Qbb}E$ is a countable dense subspace of $U$. Moreover, $U$ contains $f(X)$. Since the inclusion map $U\hookrightarrow \MH$ is continuous, to show that $f:X\rightarrow \MH$ is measurable, it suffices to show that the restriction $f:X\rightarrow U$ is measurable.

From now on, we view $U$ as the codomain of $f$. We need to show that $f^{-1}(\Omega)$ is measurable for each open set $\Omega\subset U$. Since $\Omega$ is a separable metric space, it is second countable and hence Lindel\"of. Therefore, $\Omega$ is a countable union of open balls. Thus, it suffices to prove that $f^{-1}(B_U(v,r))$ is measurable for each $v\in U$ and $r\in\Rbb_{>0}$.

Let $\xi_1,\xi_2,\dots$ be a dense sequence in the closed unit ball $\ovl B_U(0,1)$. Then $\Vert\eta\Vert=\sup_n|\bk{\xi_n|\eta}|$ for each $\eta\in U$. Therefore, the function
\begin{align*}
|f-v|:X\rightarrow\Rbb_{\geq0}\qquad x\mapsto \Vert f(x)-v\Vert=\sup_n|\bk{\xi_n|f(x)-v}|
\end{align*}
is measurable, because each $\bk{\xi_n|f-v}$ is measurable. The inverse image of $[0,r)$ by the map $|f-v|$ is precisely $f^{-1}(B_U(v,r))$. Therefore $f^{-1}(B_U(v,r))$ is measurable.
\end{proof}
...
\begin{co}
Let $f,g:X\rightarrow \MH$ be measurable functions with separable ranges. Let $\gamma:X\rightarrow\Cbb$ be measurable. Then $f+g$ and $\gamma f$ are measurable.
\end{co}
...
Let $(f_\alpha)_{\alpha\in\SI}$ be a first-countable net of (measurable and) weakly integrable maps $X\rightarrow \MH$ converging pointwise to a measurable map $f:X\rightarrow \MH$. Assume that there exists a measurable $g:X\rightarrow\ovl\Rbb_{\geq0}$ with $\int_X gd\mu<+\infty$ such that $|f_\alpha|\leq g$ holds for each $\alpha\in\SI$. Then $f$ is weakly integrable, and
...
\begin{proof}
Let $\xi_\alpha=\int_X f_\alpha d\mu$. By Prop. \ref{lb493}, we have $\sup_\alpha\Vert\xi\Vert_\alpha\leq \int_Xgd\mu<+\infty$, and hence $(\xi_\alpha)_{\alpha\in\SI}$ is bounded. By Thm. \ref{lb494}, $(\xi_\alpha)$ has a subnet $(\xi_{\alpha_\nu})$ converging weakly to some $\xi$. Then for each $\eta\in \MH$ we have
\begin{align*}
\int_X\bk{\eta|f}d\mu=\lim_\alpha\int_X\bk{\eta|f_\alpha}d\mu=\lim_\alpha\bk{\eta|\xi_\alpha}=\lim_\nu\bk{\eta|\xi_{\alpha_\nu}}=\bk{\eta|\xi}
\end{align*}
where the first equality is due to the DCT for first countable nets (cf. Thm. \ref{lb225}). This proves that $f$ is weakly integrable (with integral $\xi$). By Prop. \ref{lb493},
\begin{align*}
\Big\Vert \int_X fd\mu-\int_X f_\alpha d\mu\Big\Vert\leq\int_X|f-f_\alpha|d\mu
\end{align*}
and the RHS converges under $\lim_\alpha$ to $0$ due to DCT. This proves the limit \eqref{eq249}.
\end{proof}



The weak integrability of Thm. \ref{lb495} also follows from the following Thm. \ref{lb496}.




\subsubsection{A criterion for weak integrability}


\begin{thm}\label{lb496}
Let $f:X\rightarrow\MH$ be \textbf{integrable}, \index{00@Integrable vector-valued functions} that is, $f$ is measurable and satisfies $\Vert f\Vert_{L^1(X,\mu)}<+\infty$. Then $f$ is weakly integrable.
\end{thm}


\begin{proof}
Step 1. Assume WLOG that $\MH=l^2(I)$ where $I$ is a set. We can thus write
\begin{align*}
f(x)=(f_i(x))_{i\in I}
\end{align*}
where each $f_i:X\rightarrow\Cbb$ equals $\bk{\chi_{\{i\}}|f}$ and hence is measurable. Moreover, from
\begin{align}\label{eq248}
\Vert f\Vert_{L^1}=\int_X|f|d\mu=\int_X\sqrt{\sum_{i\in I}|f_i(x)|^2}d\mu(x)
\end{align}
we see that each $f_i$ is integrable. We can thus define
\begin{align*}
\xi=(\xi_i)_{i\in I}\qquad \text{where }\xi_i=\int_X f_id\mu
\end{align*}
which is an element of $\Cbb^I$. Our goal is to show that $\xi\in l^2(I)$, and that $\xi$ is the weak integral of $f$.\\[-1ex]


Step 2. Let $M=\Vert f\Vert_{L^1}$. To prove $\sum_{i\in I}|\xi_i|^2<+\infty$, it suffices to prove $\sum_{i\in I_0}|\xi_i|^2\leq M$ for each finite subset $I_0\subset I$. To prove this inequality, by the Riesz isometry for $l^2(I_0)$, it suffices to prove
\begin{align*}
\Big|\sum_{i\in I_0} c_i\xi_i\Big|\leq \Vert c\Vert_{l^2(I_0)}\cdot M
\end{align*}
for each $c=(c_i)_{i\in I_0}\in l^2(I_0)$. We compute that
\begin{align*}
&\Big|\sum_{i\in I_0} c_i\xi_i\Big|=\Big|\sum_{i\in I_0}\int_Xc_if_i(x)d\mu(x)\Big|\leq\int_X\sum_{i\in I_0}c_i|f_i(x)|d\mu(x)\\
&\leq \int_X\Vert c\Vert_{l^2(I_0)}\cdot\sqrt{\sum_{i\in I_0}|f_i(x)|^2}d\mu(x)\leq \Vert c\Vert_{l^2(I_0)}\cdot M
\end{align*}
where the last inequality is due to \eqref{eq248}. We have finished proving $\xi\in l^2(I)$.\\[-1ex]

Step 3. By Cor. \ref{lb118}, the set $J=\{i\in I:\xi_i\neq 0\}$ is countable. For each $c=(c_i)_{i\in I}\in l^2(I)$, we compute
\begin{align*}
\bk{c|\xi}=\sum_{i\in J}\ovl{c_i}\xi_i=\sum_{i\in J}\int_X \ovl{c_i}f_i(x)d\mu(x)=\lim_{J_0\in\fin(2^J)}\int_X\sum_{i\in J_0}\ovl{c_i}f_i(x)d\mu(x)
\end{align*}
The limit $\lim_{J_0}$ and the integral $\int_X$ can be exchanged due to DCT (for first countable nets, cf. Thm. \ref{lb225}) and the fact that the family of functions $(\sum_{i\in J_0}c_if_i)_{J_0\in\fin(2^J)}$ is dominated by the function $\sum_{i\in J}|c_if_i|$, where the latter is integrable because
\begin{align*}
&\int_X\sum_{i\in J}|c_if_i(x)|d\mu(x)=\int_X\lim_{J_0\in\fin(2^J)}\sum_{i\in J_0}|c_if_i(x)|d\mu(x)\\
&\xlongequal{\text{Thm. \ref{lb224}}}\lim_{J_0\in\fin(2^J)}\int_X\sum_{i\in J_0}|c_if_i(x)|d\mu(x)\leq \limsup_{J_0\in\fin(2^J)}\int_X\Vert c\Vert_{l^2(J_0)}\cdot\sqrt{\sum_{i\in J_0}|f_i(x)|^2}d\mu(x)\\
&\leq \Vert c\Vert_{l^2(I)}\cdot \Vert f\Vert_{L^1(X,\mu)}<+\infty
\end{align*}
Thus
\begin{align*}
\bk{c|\xi}=\int_X\lim_{J_0\in\fin(2^J)}\sum_{i\in J_0}\ovl{c_i}f_i(x)d\mu(x)=\int_X\sum_{i\in J}\ovl{c_i}f_i(x)d\mu(x)=\int_X\bk{c|f}d\mu
\end{align*}
This proves that $f$ is weakly integrable with integral $\xi$.
\end{proof}
