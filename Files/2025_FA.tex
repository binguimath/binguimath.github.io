...
\begin{thm}
Let $V$ be a separable inner product space, and let $\omega\in\Ses(V)$ be Hermitian. Choose $r\geq0$ such that $\Vert\omega\Vert\leq r$. Then for each $\xi\in V$, there is an increasing right-continuous $\rho_\xi:[-r,r]\rightarrow\Rbb_{\geq0}$ such that for $z\in\Cbb$ satisfying $|z|>r$, the resolvent form $(z-\omega)^{-1}\in\Ses(V)$ satisfies
...
\begin{align*}
T_ne_{n,i}=\lambda_{n,i}e_{n_i}\qquad\text{for each }1\leq i\leq n
\end{align*}
...
\end{align}
By the polarization identity (cf. Rem. \ref{lb237}), $\lim_k\bk{\xi|(z-T_{n_k})^{-1}\eta}$ converges for each $\xi,\eta\in\{u_1,u_2,\dots\}$, and hence for all $\xi,\eta\in V_\infty$. We can thus define a $\Kbb$-sesquilinear form
