...
\begin{df}
An \textbf{unbounded operator} \index{00@Unbounded operator} $T$ from $\MH$ to $\MK$ (abbreviated to $T:\MH\rightarrow\MH$) is defined to be a linear map from a dense linear subspace $\Dom(T)$ of $\MH$ to $\MK$. We call $\Dom(T)$ the \textbf{domain} of $T$. \index{00@Domain of an unbounded operator} If $\MH=\MK$, we say that $T$ is an \textbf{unbounded operator on $\pmb\MH$}.
\end{df}

Note that bounded linear maps are unbounded operators. This should not seem contradictory, since "unbounded" actually means "not necessarily bounded".
