...
\begin{align}
f_n(z)=\frac{q_n(z)}{p_n(z)}\qquad\text{where }p_n,q_n\in\Cbb[z],\deg p_n=n+1,\deg q_n=n
\end{align}
...
In the next section, we will see that the approximation of $\rho$ by the sequence $(\rho_n)$ can be interpreted as the approximation of an infinite-order (not necessarily bounded) Hermitian matrix by a sequence of finite-order Hermitian matrices. Since this type of approximation plays a central role in the development of spectral theory by Hilbert and Riesz, it is crucial to understand its origin in the study of divergent series and its connection with Pad\'e approximation.


\subsubsection{The polynomial moment problem}
...
\begin{proof}
We view $T_n:=E_nTE_n$ as a linear operator on $V_n$. By Rem. \ref{lb173}, $T_n$ is Hermitian. Therefore, by linear algebra, $V_n$ has an orthonormal basis $e_0,\dots,e_n$ such that $Te_i=\lambda_ie_i$ for all $0\leq i\leq n$, where $\lambda_i\in\Rbb$. Moreover, by Rem. \ref{lb173}, if $I=\Rbb_{\geq0}$ then $T_n\geq0$, and hence $\lambda_i\geq0$; if $I=[0,1]$ then $0\leq T_n\leq\id_{V_n}$, and hence $0\leq\lambda_i\leq 1$. It follows that $\lambda_i\in I$ in all cases.
Write
\begin{align*}
