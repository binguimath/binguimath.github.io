...
The main reason for introducing the convolution algebra is that it turns a strongly continuous unitary representation of the group $\Rbb^N$ into a unitary representation of $L^1(\Rbb^N,\mk)$, as shown by the following theorem.
...
The negative sign in $U(-x)$ is chosen to ensure compatibility with the Fourier transform convention $\int f(x)e^{-\im tx}dx$.
...
\subsubsection{Criteria for measurability}
...
Assume that the measure $\mu$ is $\sigma$-finite. Let $f,g:X\rightarrow\MH$ be integrable with separable ranges. Then
\begin{align*}
