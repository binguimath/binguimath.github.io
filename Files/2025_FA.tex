...
\begin{prob}
Let $T$ be a positive unbounded operator on $\MH$. Let $\Dom_0$ be a linear subspace of $\Dom(T)$. Prove that $\Dom_0$ is a core for $T$ iff $(\idt+T)\Dom_0$ is dense in $(\idt+T)\Dom(T)$.
\end{prob}

\begin{proof}[Hint]
Show that the graph inner product of $T$ on $\Dom(T)$ is equivalent (cf. Def. \ref{lb481}) to $(\xi,\eta)\mapsto\bk{(\idt+T)\xi|(\idt+T)\eta}$.
\end{proof}
