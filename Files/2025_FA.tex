...
In the rest of this section, we let $V,W$ be inner product spaces.
...
\begin{rem}
By Rem. \ref{lb115}, the inner product space $V$ is equipped with the norm defined by $\vert v\Vert=\sqrt{\bk{v|v}}$. In particular, $V$ is a metric space with metric $d(u,v)=\Vert u-v\Vert$. The topology on $V$ induced by this metric is called the \textbf{norm topology} \index{00@Norm topology} of $V$. 
\end{rem}

\begin{rem}
By the polarization identity, the inner products are determined by the norms, and hence determined by the metrics. Therefore, if $T:V\rightarrow W$ is a linear isometry of normed vector spaces, then
\begin{align*}
\bk{Tu|Tv}=\bk{u|v}\qquad\text{for all }u,v\in V
\end{align*}
In particular, if $T$ is an isomorphism of normed vector sapces, then $T$ is an isomorphism of inner product spaces. In this case, we say that $T$ is a \textbf{unitary map}, \index{00@Unitary maps} and say that $V,W$ are \textbf{isomorphic inner product spaces} (or that $V,W$ are \textbf{unitarily equivalent}). \index{00@Unitarily equivalent} 


Similarly, if $T:V\rightarrow V$ is antilinear, then $T$ is an isometry of metric spaces iff
\begin{align*}
...
\end{align*}
has operator norm, and hence belongs to $\Ses(V)$. Therefore, by Prop. \ref{lb35}, this map is continuous.
\end{eg}
