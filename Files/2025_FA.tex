...
\begin{align*}
\bk{f,g}=\sum_{x\in X} f(x)\ovl{g(x)}\qquad\text{for any }f,g\in l^2(X)
\end{align*} 
\end{eg}
...
\begin{align*}
\bk{f,g}=\int_X f\ovl gd\mu\qquad\text{for any }f,g\in L^2(X,\mu)
\end{align*}
\end{eg}
...
\begin{gather*}
\ovl{au+bv}=\ovl a\cdot\ovl u+\ovl b\cdot\ovl v\\
\bk{\ovl u|\ovl v}_{V^\Co}=\bk{v|u}_V
\end{gather*}
...
\begin{align*}
\wtd\omega:V\times W^\Co\rightarrow\Cbb\qquad (v,\ovl w)\mapsto \bk{v|w}
\end{align*}
Unless otherwise stated, we always view $\omega$ and $\wtd\omega$ as the same.
\end{rem}

\begin{df}
Let $\omega:V\times W\rightarrow\Cbb$ be a sesquilinear form. The \textbf{norm} \index{00@Norm of sesquilinear forms} $\Vert\omega\Vert$ is defined to be the norm of the associated bilinear form $V\times W^\Co\rightarrow\Cbb$. Therefore, 
\begin{align*}
...
\begin{proof}
The LHS of \eqref{eq65} is $\lim_{J\in\fin(2^I)}\sum_{j\in J}|\bk{v|e_j}|^2$. Thus, it suffices to show that for each $J\in\fin(2^I)$ we have $\sum_{j\in J}|\bk{v|e_j}|^2\leq \Vert v\Vert^2$. Let 
\begin{align*}
...
\begin{align}
v=u+w\qquad\text{where }u=\sum_{i=1}^n\bk{v|e_i}e_i\text{ and }w=v-u
\end{align}
...
By assumption,  we can find $J\in\fin(2^I)$ and $(\lambda_j)_{j\in J}$ in $\Cbb$ such that $\Vert v-\sum_{j\in J}\lambda_je_j\Vert$ is small enough. On the other hand, applying Prop. \ref{lb120} to the orthogonal projection $v=u+w$ where $w=\sum_{j\in J}\bk{v|e_j}e_j$ (cf. Exp. \ref{lb124}), we have
\begin{align}
\Big\Vert v-\sum_{j\in J}\bk{v|e_j}e_j\Big\Vert\leq \Big\Vert v-\sum_{j\in J}\lambda_je_j\Big\Vert
\end{align}
...
\begin{proof}
By Thm. \ref{lb123}, $u=\lim_{J\in\fin(2^I)}u_J$ where $u_J=\sum_{j\in J}\bk{u|e_j}\cdot e_j$. By the continuity of $\bk{\cdot|\cdot}:V\times V\rightarrow\Cbb$ (Exp. \ref{lb125}), we have
\begin{align*}
...
\begin{proof}
Parseval's identity shows that $(\bk{v|e_x})_{x\in X}$ has finite $l^2$-norm $\Vert v\Vert$. So the map $\Phi$ defined by \eqref{eq66} is clearly a linear isometry. The density of the range of $\Phi$ follows from the fact that $l^2(X)$ contains all $\chi_{\{x\}}=\Phi(e_x)$, and that $\Span\{\chi_{\{x\}}:x\in X\}$ is dense in $l^2(X)$ (cf. Prop. \ref{lb109}).
\end{proof}
...
\end{align}
converges to some vector $\eta\in \MH$. By the continuity of $\bk{\cdot|\cdot}$ (Exp. \ref{lb125}), we see that $\bk{\eta|e_i}=\bk{\xi|e_i}$ for all $i$, and hence
\begin{align}
...
\begin{align*}
\Phi:\MH\rightarrow l^2(I)\qquad \xi\mapsto \big(\bk{\xi|e_i}\big)_{i\in I}
\end{align*}
...
In this section, we let $V,W$ be inner product spaces.

\subsubsection{Bounded linear maps and bounded sesquilinear forms}
...
\begin{gather*}
\omega_T:V\times W\rightarrow\Cbb \qquad (v,w)\mapsto \bk{Tv|w}
\end{gather*}
\end{df}
...
\begin{align*}
|\omega_T(v,w)|=|\bk{Tv|w}|\leq\Vert Tv\Vert\cdot\Vert w\Vert\leq \Vert T\Vert\cdot\Vert v\Vert\cdot\Vert w\Vert
\end{align*}
Applying $\sup$ over all $v,w$ in the closed unit balls, we get $\Vert\omega_T\Vert\leq\Vert T\Vert$. Moreover,
\begin{align*}
\Vert Tv\Vert^2=\omega_T(v|Tv)\leq\Vert \omega_T\Vert\cdot \Vert v\Vert\cdot\Vert Tv\Vert
\end{align*}
