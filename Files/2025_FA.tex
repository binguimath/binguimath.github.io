...
\subsection{Borel functiona calculus and the joint spectrum $\Sp(T_\blt)$}
...
\end{align*}
In particular, the normality implies (by Prop. \ref{lb252}) that for each $\xi\in\MH$, there is a unique finite Borel measure $\pmb{\mu_\xi}$ on $\Rbb^N$ \index{zz@$\mu_\xi,\mu_{\eta,\xi}$} (called the \textbf{measure associated to $\xi$}) \index{00@Measure $\mu_\xi$ associated to a vector $\xi$} such that
\begin{align}
\bk{\xi|\pi_{T_\blt}(f)\xi}=\int_{\Rbb^N}fd\mu_\xi\qquad\text{for each }f\in\Borb(\Rbb^N)
\end{align}
...
\end{align*}
\item For each $\xi\in\MH$ and $f\in\Bor(\Rbb^N)$, we have
\begin{align}
...
\item $U_1T_2=T_2U_1$.
\item $U_2T_1=T_1U_2$.
\item For each $f_1,f_2\in\Bor(\Rbb,\Rbb)$, the self-adjoint operators $f_1(T_1),f_2(T_2)$ commute strongly.
\item For each $f_1,f_2\in\Borb(\Rbb)$, the bounded normal operators $f_1(T_1),f_2(T_2)$ commute adjointly.
\end{enumerate}
\end{thm}

\begin{proof}
The equivalence of (1), (2), and (3) follow immediately from Prop. \ref{lb438}. The direction (1)$\Rightarrow$(4) follows from Thm. \ref{lb437} and Cor. \ref{lb439}.

(4)$\Rightarrow$(5): Assume (4). The (5) clearly holds in the special case that $f_1,f_2\in\Borb(\Rbb,\Rbb)$. Therefore, if $f_1,f_2\in\Borb(\Rbb)$, then the real and imaginary parts of $f_1(T_1)$ commute with those of $f_2(T_2)$; hence $f_1(T_1)$ commutes adjointly with $f_2(T_2)$.
