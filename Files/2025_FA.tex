...
\end{align*} 
by DCT, it follows that $g_n(T_\blt)\xi$ converges to $\xi$. Since $g_n(T_\blt)\MK\subset\MK$, we have $g_n(T_\blt)\xi\in\MK$. Since $fg_n$ is bounded, by Lemma \ref{lb535} below we have $g_n(T_\blt)\xi\in\Dom(f(T_\blt))$. Therefore, $g_n(T_\blt)\xi=0$ by Step 1, which completes the proof.
\end{proof}
