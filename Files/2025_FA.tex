...
\begin{proof}
Step 1. The uniqueness follows from Lem. \ref{lb497}. Let us prove the existence. Let $\Dom(T)$ be the subspace of all $\xi\in\MH$ such that the limit $\partial_t U(t)\xi|_{t=0}$ exists, called the \textbf{space of differential elements} of $U$. For any $\xi\in\Dom(T)$, let
\begin{align*}
T\xi=-\im\partial_t U(t)\xi\big|_{t=0}
\end{align*}
Then $T$ is an n.d.d. unbounded operator on $\MH$ with domain $\Dom(T)$. Moreover, for each $\xi,\eta\in\Dom(T)$, since $U(t)^*=U(-t)$, we have
\begin{align*}
...
In the following steps, we show that $\Dom(T)$ is dense, that $T$ is essentially self-adjoint (and hence $\ovl T$ is self-adjoint), and $e^{\im t\ovl T}=U(t)$ for each $t\in\Rbb$.\\[-1ex]

Step 2. In this step, we show that $\Dom(T)$ is dense. For each $\xi\in\MH$ and $h\in C_c^\infty(\Rbb)$, let
\begin{align*}
\xi(h)=\int_\Rbb h(x)U(x)\xi dx
\end{align*}
where the integral is understood as in Sec. \ref{lb498}.
\end{proof}
