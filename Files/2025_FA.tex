...
\end{align}
in WOT. This will be proves in Rem. \ref{lb305}.
\end{proof}
...
\begin{proof}
Note that (1) is equivalent to $\Rng(P)\subset\Rng(Q)^\perp$. By \eqref{eq140}, we have $\Rng(Q)^\perp=\Rng(1-Q)$. Thus (1) is equivalent to $\Rng(P)\subset\Rng(1-Q)$, and hence (by Thm. \ref{lb249}) is equivalent to each of the following three conditions: $P(1-Q)=0$,  $(1-Q)P=0$, and $P\leq 1-Q$.
\end{proof}




\begin{rem}\label{lb263}
...
\end{align*}
for some $\eta\in\MH$. Since $E\xi-E_\alpha\eta=(E-E_\alpha)\xi+E_\alpha(\xi-\eta)$ with $(E-E_\alpha)\xi$ orthogonal to $E_\alpha(\xi-\eta)$ (cf. Rem. \ref{lb263}), the Pythagorean identity shows that $(E-E_\alpha)\xi\Vert\leq\Vert E\xi-E_\alpha\eta\Vert$ and hence
\begin{align*}
\Vert (E-E_\alpha)\xi\Vert\leq\eps
\end{align*}
Therefore, for each $\beta\geq\alpha$, we have $E-E_\beta\leq E-E_\alpha$ due to Rem. \ref{lb263}, and hence $\Vert (E-E_\beta)\xi\Vert\leq\eps$. This proves $\lim_\alpha E_\alpha\xi=E\xi$.
\end{proof}
...
Let $(X,\fk M,\mu)$ be a probability space. Let $\phi:X\rightarrow X$ be measure preserving transform. Prove that $\phi$ is ergodic iff the map $U$ defined in \eqref{eq170} satisfies $\Ker(\idt-U)=\Span\{1\}$ .
\end{exe}
...
\subsection{Spectral theorem: the multiplication operator form}
