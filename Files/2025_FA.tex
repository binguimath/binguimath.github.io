...
\end{align*}
on $L^2(\Rbb^N,m)$, defined by the usual polynomial functional calculus of linear operators (cf. Exp. \ref{lb568}), is essentially self-adjoint.
\end{co}
...
By viewing $S_1,\dots,S_N$ as multiplication operators (so that $f(S_\blt)$ is also a multiplication operator, cf. Thm. \ref{lb435}), one concludes from Exe. \ref{lb570} that $f(S_\blt)|_{C_c^\infty(\Rbb^N)}$ equals $A$. Therefore $A$ is essentially self-adjoint.
\end{proof}
...
Let $A$ be a self-adjoint operator on $\MH$, and let $B$ be a Hermitian operator on $\MH$. Suppose that $B$ is $A$-bounded with bounding constant $0\leq\alpha<1$. Then $A+B$ is a self-adjoint operator on $\MH$, and the unbounded operators $A$ and $A+B$ share the same cores.
\end{thm}
...
For each $y\in\Rbb_{>0}$, observe that $A+B$ is self-adjoint iff $y^{-1}(A+B)$ is self-adjoint. Therefore, by Cor. \ref{lb420}, it suffices to show that there exists $y\in\Rbb_{>0}$ such that $\Rng(A\pm y\im+B)=\MH$. We shall prove this by showing that $A\pm y\im$ and $B$ satisfy the assumptions in Prop. \ref{lb575}.
...
\end{align*}
Since $0\leq\alpha<1$, there exists $y\in\Rbb_{>0}$ such that $0\leq\alpha+y^{-1}\beta<1$. For such $y$, we conclude from Prop. \ref{lb575} that $\Rng(A\pm y\im+B)=\MH$. \\[-1ex]
...
With the help of the following proposition, one can derive from Thm. \ref{lb572} a Kato-Rellich type criterion for essential self-adjointness. 
