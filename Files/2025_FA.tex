...
A positive linear functional $\Lambda:\Borb(X,\Kbb)\rightarrow\Kbb$ is called \textbf{normal} \footnote{This terminology is borrowed from the theory of von Neumann algebras.} \index{00@Nomality of positive linear functionals} if it satisfies the monotone convergence theorem, that is, if $(f_n)$ is an increasing sequence in $\Borb(X,\Kbb)$ converging pointwise to $f\in\Borb(X,\Kbb)$, then
\begin{align*}
...
\begin{gathered}
\begin{array}{c}
\{\text{normal positive linear functionals $\Borb(X,\Kbb)\rightarrow\Fbb$}\}\\
\raisebox{0.3cm}{\rotatebox{-90}{$\xlongrightarrow{\rotatebox{90}{$\simeq$}}$}}\\
...
We have seen that Hilbert's spectral theorem (Thm. \ref{lb239}) is stated in the language of bilinear/sesquilinear forms. As noted in Sec. \ref{lb258}, one reason Riesz framed his spectral theorem in the language of linear operators is that projection operators correspond more naturally to linear subspaces of an inner-product space than do projection forms. Another reason---mentioned in Subsec. \ref{lb132}---is that symbolic calculus is easier to manipulate in the operator framework, i.e. one may replace the real or complex variable $x$ in a function $f(x)$ by an operator or a sesquilinear form. We will explore this in more detail in this section and in Sec. \ref{lb259}. For now, we answer some questions that readers might naturally ask.
...
In the rest of this section, we fix a Hilbert space $\MH$. 
...
Neither the continuous functional calculus nor the integral on the RHS of \eqref{eq146} has been defined yet. We will do this in the following subsections.
...
\subsubsection{The Stieltjes integral again increasing projections}
...
\subsubsection{Borel functional calculus, and why the semicontinuous functional calculus is sufficient}
...
For example, if $X$ is a compact Hausdorff space, any pre-unitary representation of $C(X)$ or $\Borb(X)$ are positive.
...
\begin{df}
Let $X$ be a topological space. A pre-unitary representation $(\pi,V)$ of $\Borb(X)$ (which is automatically positive) is called \textbf{normal} \index{00@Normal pre-unitary representation of a unital *-algebra} if for each $\xi\in V$, the linear functional
\begin{align}
\scr A\rightarrow\Cbb\qquad f\mapsto\bk{\xi|\pi(f)\xi}
\end{align}
is normal in the sense of Def. \ref{lb273}.
\end{df}

\begin{thm}[\textbf{Operator-valued Hausdorff moment theorem}]\index{00@Operator-valued Hausdorff moment theorem}\label{lb274}
Let $X$ be a second-countable compact Hausdorff space. Let $\scr A$ be a unital *-subalgebra of $C(X,\Fbb)$ separating points of $X$. Then there is a one-to-one correspondence between the following two classes of objects: \footnote{Since these two classes are proper classes rather than sets, we avoid using the term "bijection".}
\begin{itemize}
\item[(1)] normal unitary representations of $\Borb(X)$;
...
\begin{proof}
Step 1. An object $(\pi,\MH)$ in the first class clearly restricts to an object $(\pi|_{\scr A},\MH)$ in the second class. Moreover, if $(\pi_1,\MH)$ and $(\pi_2,\MH)$ satisfy $\pi_1|_{\scr A}=\pi_2|_{\scr A}$, then for each $\xi\in \MH$, the positive linear functionals $\scr A\rightarrow\Cbb$ defined by $f\mapsto\bk{\xi|\pi_1(f)\xi}$ and by $f\mapsto\bk{\xi|\pi_2(f)\xi}$ are normal and are equal on $\scr A$. Therefore, by Cor. \ref{lb261}, these two linear functionals are equal. This proves $\pi_1=\pi_2$.

We have thus proved that the correspondence $(\pi,\MH)\mapsto(\pi|_{\scr A},\MH)$ from the first to the second class is injective. To prove that it is surjective, let us prove that any positive unitary representation $(\pi,\MH)$ of $\scr A$ can be extend to a normal unitary representation of $\Borb(X)$.\\[-1ex]
...
\end{align*}
For each $\xi,\eta\in \MH$, define
\begin{gather*}
\Phi:\Borb(X)\times \MH\times \MH^\Co\rightarrow\Cbb\\
\Phi(f,\xi,\ovl\eta)=\frac 14\sum_{t=0,\frac \pi 2,\pi,\frac{3\pi}2}\Lambda_{\xi+e^{\im t}\eta}(f)\cdot e^{\im t}
\end{gather*}
\end{proof}
