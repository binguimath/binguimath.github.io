...
\end{align}
Clearly $S_0(\Rbb^N)$ is a linear subspace of $\MS(\Rbb^N)$ invariant under partial derivatives and multiplication by polynomials.
\end{df}

An advantage of working with $\MS_0(\Rbb^N)$ is that the Fourier transform can be explicitly computes in this space.


\begin{pp}\label{lb511}
Let $f$ be defined by \eqref{eq262}. Then
\begin{gather*}
...
Prop. \ref{lb511} implies that the Fourier transform
\begin{align*}
\MF:\MS_0(\Rbb^N)\rightarrow\MS_0(\Rbb^N)\qquad f\mapsto \wht f
\end{align*}
is a unitary map with inverse $f\mapsto\wch f$. To extend $\MF$ to a unitary operator on $L^2(\Rbb^N,\mk)$, we need to show that $\MS_0(\Rbb^N)$ is dense in $L^2(\Rbb^N,\mk)$.




\subsubsection{Density of $\MS_0(\Rbb^N)$}
...
\begin{proof}
By Cor. \ref{lb369}, it suffices to show that any $f\in L^2(\Rbb^N,\mu)$ orthogonal to $\Cbb[x_\blt]\equiv\Cbb[x_1,\dots,x_N]$ must be zero. Let us fix $f\in L^2(\Rbb^N)$ orthogonal to $\Cbb[x_\blt]$.\\[-1ex]

Step 1. We claim that if
\begin{align}\label{eq265}
\int \ovl{f(x)}e^{-\im tx}d\mu(x)=0\qquad\text{for all }t\in\Rbb
\end{align}
then $f=0$. Note that by viewing $1$ as an element of $\MH:=L^2(\Rbb^N,\mu)$, \eqref{eq265} can be rephrased as
\begin{align*}
\bk{f|e^{-\im t\Mbf_x}1}=0\qquad\text{for all }t\in\Rbb
\end{align*}
where $e^{-\im t\Mbf_x}$ denotes $e^{-\im tT_\blt}$ with $T_j=\Mbf_{x_j}$. Therefore, if \eqref{eq265} is true, then $f$ is orthogonal to
\begin{align*}
\MK=\ovl{\Span\{e^{-\im t\Mbf_x}1:t\in\Rbb^N\}}
\end{align*}


Since $\MK$ is $e^{-\im t\Mbf_x}$-invariant, by Thm. \ref{lb532} we have $\beta(\Mbf_x)\MK\subset\MK$ for each $\beta\in\Borb(\Rbb^N)$, and hence $\beta\equiv\beta(\Mbf_x)1$ belongs to $\MK$. Therefore $\MK$ contains $L^\infty(\Rbb^N,\mu)$. Since $L^\infty(\Rbb^N,\mu)$ is dense in $L^2(\Rbb^N,\mu)$, we conclude that $\MK=\MH$, and hence $f=0$. This finishes the proof of the claim.\\[-1ex]
 


Step 2. It remains to prove \eqref{eq265}. Let
\begin{align*}
\Omega=\{\zeta\in\Cbb:|\Real(\zeta)|<r/2\}
\end{align*} 
In this step, we show that the integral in \eqref{eq265}, as a function of $-\im t$, can be extended to a holomorphic function
\begin{align*}
g:\Omega^N\rightarrow\Cbb\qquad g(z)=\int \ovl{f(x)}e^{zx}d\mu(x)
\end{align*}

First, we need to check that the family of functions $x\mapsto \ovl{f(x)}e^{zx}$ (indexed by $z\in\Omega^N$) is dominated by a positive $L^1$ function so that DCT can be applied. Let $\varphi:\Rbb^N\rightarrow\Rbb_{\geq0}$ be
\begin{align*}
...
|e^{zx}|\leq |\varphi(x)|
\end{align*}
the family of functions $\ovl{f(x)}e^{zx}$ is dominated $|f\varphi|$. Therefore, by DCT, $g$ is well-defined and continuous.
...
Step 3. It remains to show that $g=0$ on $\Omega^N$. Since $g$ is holomorphic on each variable, it suffices to show that $g=0$ on the polydisk $B_\Cbb(0,r/2)^N$. For each $z\in B_\Cbb(0,r/2)^N$, since
\begin{align*}
...
\begin{align*}
g(z)=\sum_{n_1,\dots,n_N\in\Nbb}\int\ovl{f(x)}\cdot\frac{(z_1x_1)^{n_1}\cdots(z_Nx_N)^{n_N}}{n_1!\cdots n_N!}d\mu(x)
\end{align*}
where the RHS equals zero by the assumption that $f\perp\Cbb[x_\blt]$.
\end{proof}
