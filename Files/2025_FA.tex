...
\section{Basics of normed vector spaces and inner product spaces}
...
\subsubsection{Equivalence of the first and second columns of Table \ref{tb1}}
...
Indeed, by this principle, for each $\varphi\in V^*$, we can select a sequence $(f_n)$  approximating weak-* to $\varphi$. If we can show that $(f_n)$ also converges to some function $f$ in the sense described in the first column of Table \ref{tb1}, then---by the equivalence of the three modes of convergence in that table---it follows that $(f_n)$ converges weak-* to $f$. Consequently, $\varphi$ is represented by integration against $f$, thereby solving the problem of characterizing the dual space $V^*$. 

The idea outlined in the preceding paragraph is roughly the approach Riesz employed in 1907 to solve the following trigonometric moment problem.
...
\begin{rem}
In this course, the most frequently encountered cases of \eqref{eq26} are $\fk L(V)$, $V^*$, and $\fk L(U\times V,\Fbb)$. In the next chapter, we also consider spaces such as $\fk L(U\times V\times V_*,\Fbb)$, where $V_*$ is a normed vector space with dual space $V$. In such cases, Prop. \ref{lb40} gives isomorphisms
\begin{align*}
...
As discussed in \cite[Sec. 25.8, 25.9]{Gui-A}, and as we will also explore in the next chapter, Riesz’s spectral theorem provides a striking example of how these two advantages can be fruitfully combined.
...
\subsection{Weak-* convergence of $L^p$-functions}
...
\subsubsection{Application: an alternative proof of the Riesz representation Thm. \ref{lb9}}
