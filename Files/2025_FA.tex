...
Let $X$ be a compact Hausdorff space. Let $\fk S\subset C(X,\Fbb)$. Suppose that $\fk S$ separates the points of $X$. Then the *-$\Fbb$-subalgebra $\Fbb[\fk S\cup\fk S^*]$ generated by $\fk S$ is dense in $C(X,\Fbb)$ in the $l^\infty$-norm.
\end{thm}
...
\begin{align*}
\Co:V\rightarrow V^\Co\qquad v\mapsto \xi^\Co\equiv\ovl \xi
\end{align*}
...
\subsubsection{Von Neumann's spectral decomposition for unbounded operators}
...
For modern readers familiar with the multiplication-operator version of the spectral theorem (Thm. \ref{lb330}), the most suitable version for unbounded operators is that any such operator $T$ is unitarily equivalent to a multiplication operator $\Mbf_f$ where $f$ is a real-valued Borel function that is not necessarily bounded.
...
\subsection{Deficiency indices and extensions of Hermitian operators}


Let $\MH,\MK,\ML$ be Hilbert spaces.

\begin{df}
An \textbf{unbounded operator} \index{00@Unbounded operator} $T$ from $\MH$ to $\MK$ (abbreviated to $T:\MH\rightarrow\MH$) is defined to be a linear map from a linear subspace $\Dom(T)$ of $\MH$ to $\MK$. We call $\Dom(T)$ the \textbf{domain} of $T$. \index{00@Domain of an unbounded operator} If $\MH=\MK$, we say that $T$ is an \textbf{unbounded operator on $\pmb\MH$}. 
\end{df}

\begin{cv}
Unless otherwise stated, we assume that the domain $\Dom(T)$ of an unbounded operator $T:\MH\rightarrow\MK$ is dense. When we wish to emphasize this assumption, we say that $T$ is a densely-defined unbounded operator.
\end{cv}
...
\begin{df}
Let $S,T:\MH\rightarrow\MK$ be unbounded operators. The addition $\pmb{S+T}$ \index{00@Addition of unbounded operators} is defined to be
\begin{align*}
...
\begin{df}
Let $S:\MK\rightarrow\ML$ and $T:\MH\rightarrow\MK$ be unbounded operators. The composition \index{00@Composition of unbounded operators} $\pmb ST$ is defined to be
\begin{gather*}
ST:\Dom(ST)\rightarrow\ML\qquad\xi\mapsto ST\xi
\end{gather*}
where $\Dom(ST)$ is the space of all $\xi\in\Dom(T)$ satisfying $T\xi\in\Dom(S)$. Then $ST$
\end{df}
