...
The converse of Rem. \ref{lb582} also holds: any $\lambda\in\Cbb$ satisfying $(\lambda-T)^{-1}\in\fk L(\MH)$ must lie outside $\Sp(T)$. This can be proved by first showing that $\Sp(T)$ equals the set of approximate eigenvalues of $T$; see Pb. \ref{lb579} and \ref{lb580}. We will not need this fact in the sequel.
...
\begin{prob}\label{lb580}
Let $T$ be a self-adjoint operator on $\MH$. Let $\lambda\in\Cbb$. Prove that $\Sp(T)=\sigma(T)$ where \index{zz@$\sigma(T)$}
\begin{align}
\pmb{\sigma(T)}=\{\lambda\in\Cbb:(\lambda-T)^{-1}\notin\fk L(\MH)\}
\end{align}
(See Def. \ref{lb452} for the notation.)
\end{prob}
...
\subsubsection{Self-adjointness of the perturbation}
...
The idea underlying the proof of the Kato-Rellich theorem leads immediately to the following result, Thm. \ref{lb583}, which will be used in this subsection to establish Thm. \ref{lb584} and \ref{lb585} on the existence of weak solutions to certain second-order linear differential equations.

Our approach is operator-theoretic: we work directly with linear operators and their algebraic relations rather than with sesquilinear forms, which are more common in the PDE literature.
For comparison, see Subsec. 6.2.1 and 6.2.2 of \cite{Eva}, where Thm. \ref{lb584} is proved via the method of sesquilinear forms, using the Lax-Milgram theorem in place of Thm. \ref{lb575}.
...
We shall apply Thm. \ref{lb583} to the case that $A$ is the negative Laplacian and $B$ is a first-order differential operator. To establish the $A$-bounds in this situation, we need the following proposition.
