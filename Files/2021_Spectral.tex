...
\end{align*}
Thus, the derivative of $s\mapsto \bk{U_{-s}e^{\im s\ovl H}\xi|\eta}$ is zero everywhere for all $\xi\in\Dom(\ovl H),\eta\in\Dom(H)$, which shows that $U_{-s}e^{\im sH}$ must be constant, which is $1$.
\end{proof}
