...
\date{\footnotesize First edition finished in  2021.9. Last update in 2025.8}
...
If we let $\wht{\mc A}$ be the norm closure of $\mc A$, then $\wht{\mc A}$ is clearly a unital $*$-subalgebra of $\End(\mc H)$. Moreover, its closed unit ball $B_{\wht{\mc A}}$ is norm-dense (and hence $\mc T$-dense) in $B_{\mc M}$. Therefore, replacing $\mc A$ with $\wht{\mc A}$, we may assume WLOG that $\mc A$ is closed under the operator norm. In particular, if $x\in\mc A$ is self-adjoint and $f:\Rbb\rightarrow\Cbb$ is a continuous function, then $f(x)\in\mc A$.
