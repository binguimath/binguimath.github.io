...
\item Let $A$ be any commutative (unital) $C^*$-subalgebra of $\End(\mc H)$, i.e., $A$ is a commutative unital norm-closed $*$-subalgebra of $\End(\mc H)$. Show that is generated by a set $\fk G$ of  mutually-commuting self-adjoint operators. Conclude that any $A$ is equivalent (as a normed unital $*$-algebra) to $C(X)$ for some compact Hausdorff space $X$. (This is the \textbf{Gelfand-Naimark theorem}.)
\end{enumerate}
\end{exe}
