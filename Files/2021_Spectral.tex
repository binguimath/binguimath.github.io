...
\end{align*}
and satisfying $M_f(\oplus_n g_n)=\oplus_n fg_n$ for each $\oplus_n g_n\in\Dom(M_f)$. One checks easily  that $\Dom(M_f)$ is dense,\footnote{If $r>0$ and $\Omega_r=\{x\in X:|f(x)|\leq r\}$, then for each $\xi\in\mc H$ we have $M_{\chi_{\Omega_r}}\xi\in\Dom(M_f)$ and $\lim_{r\rightarrow+\infty}M_{\chi_{\Omega_r}}\xi=\xi$ by the dominated convergence theorem.} and that
\begin{align*}
(M_f)^*=M_{f^*}
\end{align*}
where $f^*(x)=\ovl{f(x)}$.
