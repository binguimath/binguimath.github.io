...
$\pi:f\in B(X)\mapsto f(T_1,\dots,T_N)\in\End(\mc H)$ is the unique unital $*$-homomorphism extending the continuous functional calculus in Thm. \ref{lb17} and satisfying the following property: If $f_\blt$ is a net of bounded Borel functions on $X$ whose $L^\infty(X)$-norms are uniformly bounded (over the directed set), such that $\lim \int_X|f-f_\blt|d\mu=0$ for each finite (positive) Borel measure on $X$, then $f_\blt(T_1,\dots,T_N)$ converges strongly to $f(T_1,\dots,T_N)$.
\end{thm}

\begin{proof}
$\pi:f\mapsto f(T_1,\dots,T_N)$ is clearly a unital $*$-homomorphism. For each $\xi\in\mc H$, write $U\xi=(g_n)_{n\in\fk N}\in\bigoplus_n L^2(X,\mu_n)$. Then
\begin{align*}
...
The following proposition improves Prop. \ref{lb1}.
\begin{pp}\label{lb46}
Let $X$ be a topological space. Assume $\pi:B(X)\rightarrow\End(\mc H)$ is a unital $*$-homomorphism, i.e., preserves the identities, multiplications and linear combinations, and $*$-structures. Then $\lVert \pi(f)\Vert\leq \lVert f\lVert_{L^\infty(X)}$ for each $f\in B(X)$. 
\end{pp}
...
\end{align*}
Applying Prop. \ref{lb70} to the homeomorphism $Y^N\rightarrow X$ sending $(u_1,x_1,\dots,u_N,x_N)$ to $(u_1,(1+t_1)^{-1},\dots,u_N,(1+t_N)^{-1})$, we see that one may assume that $\mc H=\bigoplus_n L^2(Y^N,\omega_n)$ where $\omega_n$ is a finite Borel measure on $Y^N$ and
\begin{gather*}
