...
Our treatment of the general theory of closed and preclosed operators rely on polar decomposition (Thm. \ref{lb22}), which factors a closed operator as the product of a partial isometry and a positive operator. Thus, the spectral theory for strongly commuting normal closed operators follows from that for adjoint commuting partial isometries and bounded positive operators, which are established in Section \ref{lb65}. 

Thus, our approach differs from the usual ones, which first establish spectral theory for self-adjoint closed operators $H$ (which follows from that of the unitary operators $(H-\im)(H+\im)^{-1}$), and then use linear combinations to investigate normal operators. Our reason is that polar decomposition holds and is an important tool in the study of non-normal closed operators, or more generally, non-abelian von Neumann algebras.
...
In this note, we only consider  locally compact Hausdorff spaces $X$ which are also \textit{second countable}. Then local finiteness implies inner and outer regularity \cite[Thm. 2.18]{Rud-R}. Thus, \textit{locally finite (in particular, finite) positive Borel measures on $X$ are automatically Radon measures}.
...
The first question in spectral theory is: what is an appropriate $X$? To answer this question, we need the following crucial result. For each $r>0$, set $B_r=[-r,r]^2$, \index{Br@$B_r=[-r,r]^2$} regarded as a subset of $\Cbb$.

\begin{pp}\label{lb1}
Let $r_j$ be $\lVert T_j\lVert$, the operator norm of $T_j$. Then for each $f\in\mc P_N$, 
\begin{align*}
\lVert \pi(f)\lVert\leq \sup_{x_j,y_j\in[-r_j,r_j]}| f(x_1,y_1,\dots,x_N,y_N)|.	
\end{align*}
\end{pp}
...
Let
\begin{align*}
X=B_{r_1}\times\cdots B_{r_N}.
\end{align*}
We clearly have a $*$-homomorphism $\varphi:\mc P_N\rightarrow C(X)$ defined by sending each $z_j$ to the function indicating the $j$-th complex component $B_{r_j}$ of $X$. Equivalently, $\varphi$ sends each $x_j,y_j$ to the first and second real component of $B_{r_j}$. We identify $f$ with $\varphi(f)$ when no confusion arises. ($\varphi$ is indeed injective, so that we can regard $\mc P_N$ as a $*$-subalgebra of $C(X)$.)
...
\begin{proof}
It is easy to define $\pi$ on $\varphi(\mc P_N)$, which is continuous with respect to the topology of $C(X)$ due to Prop. \ref{lb1}. By Stone-Weierstrass Theorem, $\varphi(\mc P_N)$ is dense in $C(X)$. Thus $\pi$ is extended uniquely to a continuous map satisfying the desired property.
\end{proof}


The crucial step of proving Prop. \ref{lb1} is to extend $\pi$ to a unital $*$-homomorphism from a larger class $\mc A$ of functions, where $\mc A$ contains  $\mc P_N$ and the positive ``square root" (defined in a proper sense) of any $r^2-f^*f$ where $f\in\mc P_N$ and $r>\lVert f\lVert_{L^\infty(X)}$. As we see below, $\mc A$ is the set of analytic functions defined near $X$.
...
Let $\mc A$ be the set of complex analytic functions $ f(x_1,y_1,\dots,x_N,y_N)$, analytic on a connected neighborhood of $(x_1,y_1,\dots,x_N,y_N)\in X$. (Thus, here we understand $x_\blt,y_\blt$ as complex variables.) $\mc A$ is a $*$-algebra, whose involution $*$ is defined by.
\begin{align*}
 f^*(x_1,y_1,\dots,x_N,y_N)=\ovl{ f(\ovl x_1,\ovl y_1,\dots,\ovl x_N,\ovl y_N)}.	
\end{align*}
(Note that $f^*$ is holomorphic, cf. Prop. \ref{lb57}.) In this way, $\mc P_N$ can be identified naturally  as a (unital) $*$-subalgebra of $\mc A$. Note that any $f\in\mc A$ is determined by its values on $X$. This is due to the fact that any single-valued holomorphic function on a connected open subset of $\Cbb$ is determined by its values on a line segment inside this open subset. Thus, we equip $\mc A$ with the topology of $L^\infty(X)$-norm.


Now, we come back to the setting that $T_1,\dots,T_N$ are adjointly commuting normal operators. For each $j$, we choose anticlockwise piecewise-smooth simple closed curves $R_j,R'_j\subset\Cbb$ such that $R_j$ surrounds $[-r_j,r_j]\supset \Sp(\Re(T_j))$, $R_j'$ surrounds $[-r_j,r_j]\supset\Sp(\Im(T_j))$, $ f$ is holomorphic when $(x_1,y_1,\dots,x_N,y_N)$ is holomorphic inside and also on a neighborhood of $R_1\times R'_1\times \cdots\times R_N\times R'_N$. Define
\begin{align}
&\pi( f)\nonumber\\
=&\oint_{x_j\in R_j,y_j\in R'_j}(2\pi\im)^{-2N} f(x_1,y_1,\dots,x_N,y_N)\nonumber\\
&\cdot(x_1-\Re(T_1))^{-1}(y_1-\Im(T_1))^{-1}\cdots(x_N-\Re(T_N))^{-1}(y_N-\Im(T_N))^{-1}\cdot dx_1dy_1\cdots dx_Ndy_N.	\label{eq2}
\end{align}
Note that the integrand is holomorphic in the sense of Sec. \ref{lb55}, thanks to Lemma \ref{lb4}. The integral is defined as in Sec. \ref{lb55}. By complex analysis,  \eqref{eq2} is independent of the choice of $R_1,R_1',\dots,R_N,R_N'$.



\begin{thm}\label{lb2}
$\pi:\mc A\rightarrow\End(\mc H)$ is a continuous $*$-homomorphism and satisfies $\lVert\pi( f)\lVert\leq \lVert  f\lVert_{L^\infty(X)}$ for each $ f\in\mc A$.  Moreover, if $f\in\mc P_N$, then the $\pi( f)$ defined by \eqref{eq2} agrees with the $\pi(f)$ defined in \eqref{eq1}. 
\end{thm}

\begin{proof}
For each connected open set $O$ containing $X$, we let $\mc A_O$ be the set  of bounded analytic functions on $O$. Then $\mc A$ is the union of all $\mc A_O$.

Step 1. Assume $ f=x_1^{m_1}y_1^{n_1}\cdots x_N^{m_N}y_N^{n_N}$, and assume each $R_j,R'_j$ are circles with radii larger than $\lVert T_j\lVert$ so that we can substitute each $(x_j-\Re(T_j))^{-1}=\sum_{n\geq 0}x_j^{-n-1}\Re(T_j)^n$ and  $(y_j-\Im(T_j))^{-1}=\sum_{n\geq 0}y_j^{-n-1}\Im(T_j)^n$ into \eqref{eq2}. One then checks easily that the $\pi( f)$ equals the $\pi(f)$ defined by \eqref{eq1}. 

Step 2. We show that $\pi$ restricts to a unital $*$-homomorphism from each $\mc A_O$ to $\End(\mc H)$. Then $\pi:\mc A\rightarrow\End(\mc H)$ is a unital $*$-homomorphism. That $\pi(1)=1$ follows from step 1. That $\pi(f^*)=\pi(f)^*$ follows by applying successively Prop. \ref{lb57} to each single variable integral of the multiple integral in \eqref{eq2}. (Note that according to the notations in Prop. \ref{lb57}, if $C$ is anticlockwise then $\ovl C$ is clockwise.)

We now show $\pi(f)\pi(g)=\pi(fg)$ for every $f,g\in \mc A_O$. For simplicity, we assume $N=1$ and write $x_1=x,y_1=y,R=R_1=R_1',T=T_1$. (We assume for simplicity that $R_1=R_1'$.)  The general case of $N$ variables follows from the same method. We choose a smooth simple closed anticlockwise $\Gamma$ containing $[-r,r]:=[-r_1,r_1]$, and assume $R$ is small enough so that it is inside the interior of $\Gamma$. Set differential $\di=(2\im\pi)^{-1}d$. Then
\begin{align*}
&\pi(f)\pi(g)\\
=&\int_{x,y\in R}\int_{z,w\in \Gamma}f(x,y)g(z,w)(x-\Re(T))^{-1}(z-\Re(T))^{-1}(y-\Im(T))^{-1}(w-\Im(T))^{-1}\\
&\cdot\di x\di y \di z \di w
\end{align*}
Note that when $z\neq x$,
\begin{align*}
(x-\Re(T))^{-1}(z-\Re(T))^{-1}=(z-x)^{-1}(x-\Re(T))^{-1}-(z-x)^{-1}(z-\Re(T))^{-1}.
\end{align*}
Since $x\in R,z\in\Gamma$, and since $R$ is inside $\Gamma$, by complex analysis, we see that 
\begin{align*}
\int_{x\in R}f(x,y)g(z,w)\cdot (z-x)^{-1}(z-\Re(T))^{-1}\cdot (y-\Im(T))^{-1}(w-\Im(T))^{-1}\di x=0.
\end{align*}
So
\begin{align*}
&\pi(f)\pi(g)\\
=&\int_{x,y\in R}\int_{z,w\in \Gamma}(z-x)^{-1}f(x,y)g(z,w)(x-\Re(T))^{-1}(y-\Im(T))^{-1}(w-\Im(T))^{-1}\\
&\cdot \di x\di y \di z \di w.
\end{align*}
In this integrand, only $(z-x)^{-1}g(z,w)$ depends on $z$, and
\begin{align*}
\int_{z\in\Gamma}	(z-x)^{-1}g(z,w)\cdot \di z= g(x,w) 
\end{align*}
by Cauchy's formula. Thus
\begin{align*}
&\pi(f)\pi(g)\\
=&\int_{x,y\in R}\int_{w\in \Gamma}f(x,y)g(x,w)(x-\Re(T))^{-1}(y-\Im(T))^{-1}(w-\Im(T))^{-1}\cdot \di x\di y \di w\\
=&\int_{x\in R}(x-\Re(T))^{-1}\\
&\cdot\Big(\int_{y\in R}\int_{w\in\Gamma}f(x,y)g(x,w)(y-\Im(T))^{-1}(w-\Im(T))^{-1} \di y\di w\Big)\di x.
\end{align*}
A similar factorization for $(y-\Im(T))^{-1}(w-\Im(T))^{-1}$ shows
\begin{align*}
&\int_{y\in R}\int_{w\in\Gamma}f(x,y)g(x,w)(y-\Im(T))^{-1}(w-\Im(T))^{-1} \di y\di w\\
=&\int_{y\in R}f(x,y)g(x,y)(y-\Im(T))^{-1}\di y.
\end{align*}
This proves $\pi(f)\pi(g)=\pi(fg)$. 
...
\begin{align*}
\lVert \pi( f)\xi\lVert^2=\bk{\pi( f^* f)\xi|\xi}=r^2\lVert\xi\lVert^2-\bk{\pi( g^* g)\xi|\xi}	=r^2\lVert\xi\lVert^2-\lVert \pi( g)\xi\lVert^2\leq r^2\lVert\xi\lVert^2.
\end{align*}
This proves $\lVert\pi( f)\lVert^2\leq r^2=(\delta+\lVert f\lVert_{L^\infty(X)})^2$ for each $\delta>0$, hence finishes the proof.
\end{proof}

Proposition \ref{lb1} follows immediately from the above Theorem.
...
The following exercise outlines a proof of the celebrated Gelfand-Naimark Theorem. This theorem will not be used in rest of this monograph, and hence can be safely skipped. 
...
\item Let $Y=\prod_{T\in\fk G}\Sp(T)$, which is a compact Hausdorff space by Tychonoff's theorem. Use Prop. \ref{lb46} to show that $\pi(f)\leq \lVert f\lVert_{L^\infty(Y)}:=\sup_{t_T\in\Sp(T)}|f((t_T)_{T\in\fk G})|$ for each $f\in\mc P$. Conclude that $\pi$ can be extended uniquely to a unital $*$-homomorphism $\pi:C(Y)\rightarrow\End(\mc H)$.

\item Let $x_T$ be the function indicating the corresponding compenent of $Y$, namely, the function sending each $(x_T)_{T\in\fk G}\in\prod_{T\in\fk G}\Sp(T)$ to $x_T$. Use the method of Thm. \ref{lb8} to show that the action of $\{T\in\fk G\}$ on $\mc H$ is unitarily equivalent to the multiplication of $\{x_{T}\}$ on $\bigoplus_{n\in\fk N}L^2(Y,\mu_n)$, where each $\mu_n$ is a positive finite Borel measure on $Y$. Thus, in the following, we identify $\mc H$ with $\bigoplus_{n\in\fk N}L^2(Y,\mu_n)$ and $T$ with the multiplication of $x_T$.

\item Let $\mc A$ be the smallest unital (norm-)closed $*$-subalgebra of $\End(\mc H)$ containing $\fk G$, called the $C^*$-algebra generated by $\fk G$. Use Stone-Weierstrass theorem to show that $\mc A=\pi(C(Y))$.

\item It is in general not true that $\pi$ is injective. However, we let $X$ be the closure of the union of the supports of all $\mu_n$ (which is compact). Note that $f\in C(Y)$ satisfies $\pi(f)=0$ whenever $f|_X=0$. Conclude that there is a well defined unital $*$-homomorphism $\pi:C(X)\rightarrow\End(\mc H)$, such that for each $f\in C(X)$, $\pi(f)$ is defined by $\pi(\wtd f)$ where $\wtd f\in C(Y)$ extends $f$ (which exists by Tietze extension theorem). Conclude $\mc A=\pi(C(X))$.

\item Show that $\pi$ is injective on $C(X)$. Thus we have a bijective unital $*$-homomorphism $\pi:C(X)\rightarrow\mc A$. Apply the proof of Prop. \ref{lb46} to $\pi$ and $\pi^{-1}$ to show that $\pi$ is isometric, i.e., $\lVert \pi(f)\lVert=\lVert f\lVert_{L^\infty(X)}$ for each $f$.

\item Show that any unital $*$-closed subalgebra $\mc A$ of $\End(\mc H)$ is generated by a set $\fk G$ of self-adjoint commuting operators. Conclude that any $\mc A$ is equivalent (as a normed unital $*$-algebra) to $C(X)$ for some compact Hausdorff space $X$.
\end{enumerate}
\end{exe}
...
\begin{itemize}
	...
	\item $1+T$ is the inverse of a bounded injective positve operator $A$ on $\mc H$. (Note that $A$ has dense range since $A=A^*$.)
\end{itemize}
...
\begin{rem}
For adjointly commuting normal $T_1,\dots,T_N$, we have
\begin{align}
