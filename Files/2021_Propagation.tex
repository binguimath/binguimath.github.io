...
One can define multi-propagation inductively by $\wr^n\fk X=\wr\wr^{n-1}\fk X$. Write
\begin{align*}
...
Now assume $U\subset\mc C\setminus\SX=\wr\mc B$. Then we can equip the family $\wr\fk X_U$ with local coordinates as follows. For the local coordinate at each submanifold $\wr\sgm_j(U)$ of $\wr\mc C_U=\wr\mc C\cap\wr\pi^{-1}(U)$, we choose $\eta_j$ defined by \index{zz@$\wr\eta_j$, $\wr^n\eta_j$}
\begin{gather}
...
\end{align}
of $\wr^n\fk X$ at $x_j\times\Conf^n(C\setminus\SX)$, defined on $W_j\times\Conf^n(C\setminus\SX)$. Suppose $U$ is an open subset of $C\setminus\SX$ which admits an injective $\mu\in\scr O(U)$, then we can define a local coordinate $\triangle_i\mu$ of $(\wr^n\fk X)_U$ at $\sigma_i(U)$ defined by \index{zz@$\triangle\mu$, $\triangle_i\mu$}
\begin{align}
...
\end{align*}
Choose any sections $v_i\in\scr V_C(U_i)$ and any $w\in\scr W_{\fk X}(\Wbb_\blt)$, we write \index{zz@$\wr^n\upphi$}
\begin{align}
