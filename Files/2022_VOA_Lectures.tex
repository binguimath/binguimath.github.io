...
Let $\fk X=(\pi:\Pbb^1\times\Conf^N(\Cbb^\times)\rightarrow\Conf^N(\Cbb^\times);0,\sgm_1,\dots,\sgm_N,\infty)$ where $0,\infty$ as sections sending $x_\blt$ to  $(0,x_\blt)$ and $(\infty,x_\blt)$ respectively, and $\sgm_i$ is as in the previous example. Then $\fk X$ is $(N+2)$-pointed. Moreover, $\fk X$ can be equipped with local coordinates $\zeta,\eta_1,\dots,\eta_N,1/\zeta$ at $0,\sgm_1,\dots,\sgm_N,\infty$ respectively, where $\zeta$ sends $(z,z_\blt)$ to $z$, $1/\zeta$ sends $(z,z_\blt)$ to $1/z$, and each $\eta_i$ sends $(z,z_\blt)$ to $z-z_i$. The fibers are
\begin{align*}
...
\end{align*}
Each $x_i$ is the section sending $q\in\mc D_{\rho}^\times$ to $(x_i,q)$, and $\eta_i$ sends $(x,q)$ to $\eta_i(x)$ when $x$ is close to $x_i$. Modding this data by a suitable holomorphic relation gives the family $\fk X$.
\end{itemize}
\hfill \qedsymbol
\end{eg}
...
As in Example \ref{lb109}, $O=\{(z,\tau)\in\Pbb^1\times\Delta:\beta_\zeta(z)\in\Omega_\tau\}$ is an open set. Let
\begin{align*}
