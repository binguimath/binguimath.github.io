...
\begin{align}
	\alpha(z)=\exp\Big(\sum_{n\geq 1}\tau c_nz^{n+1}\partial_z\Big)(z),\label{eq172}	
\end{align}
and set
\begin{align*}
...
\end{align*}
be the pushforward of differentials, i.e., $(\eta_i)_*=(\eta_i^*)^{-1}$ where the pullback $\eta_i^*$ sends any $f(z)dz\in\omega_{\eta_i(U_i)}$ to $(f\circ\eta_i)d\eta_i$. We denote $\id_\Vbb\otimes(\eta_i)_*:\Vbb\otimes\omega_{U_i}(\star x_i)\xrightarrow{\simeq} \Vbb\otimes\omega_{\eta_i(U_i)}(\star 0)$ also by $(\eta_i)_*$. (Recall that $\eta_i$ sends $x_i$ to $0$.) Then \index{V@$\mc V_\varrho(\eta_i),\mc V_\varrho(\varphi)$}
...
\end{gathered}
\end{gather}
is called the \textbf{pushforward} induced by $\eta_i$.
...
\begin{align*}
\upphi_\tau(w_\blt)=\upphi_0(e^{-\tau\sum_n c_nL_n}w_1\otimes w_2\otimes\cdots\otimes w_N).
\end{align*}
This expression actually converges in certain good cases, e.g. when $c_n=0$ for sufficiently negative $n$. In case that the expression converges, the map $\upphi_0\mapsto\upphi_\tau$ defines a linear map between the spaces of conformal blocks $\scr T^*_{\fk X_0}(\Wbb_\blt)\rightarrow\scr T^*_{\fk X_\tau}(\Wbb_\blt)$, which is an isomorphism since the operator $e^{-\tau\sum_n c_nL_n}$ is invertible. In particular, the dimensions of $\scr T^*_{\fk X_0}(\Wbb_\blt)$ and $\scr T^*_{\fk X_\tau}(\Wbb_\blt)$ are equal.

$\upphi_\tau$ satisfies the differential equation $\partial_\tau+\sum c_n\upphi_\tau\circ(L_n\otimes\id_{\Wbb_2\otimes\cdots\otimes \Wbb_N})$. This fact can be rephrased by saying that
\begin{align}
\nabla_{\partial_\tau}:=\partial_\tau+\sum_n c_n (L_n\otimes\id_{\Wbb_2\otimes\cdots\otimes \Wbb_N})^\tr
\end{align}
...
Example \ref{lb109} can be easily generalized to the case that on each neighborhood of $\Sbf^1$ around $x_i$ a holomorphic vector field $\xk_i$ is associated. The flows generated by these fields define a family.
...
\end{align}
(where $h_n\in\scr O(\Delta)$). Then $h(z,0)\partial_z=\sum_{n\in\Zbb}h_n(0)z^n\partial_z$, which corresponds to $\sum_n h_n(0)L_{n-1}$. This should be the formula of $\partial_\zeta \mc U(\beta_\zeta)$ at $\zeta=0$.
...
\end{align}
Thus, since the derivative of the LHS of \eqref{eq211} is zero, we conclude that $\upphi_\zeta$ is killed by $\partial_\zeta+\sum_n h_n(\zeta)(L_{n-1}\otimes\id_{\Wbb_2\otimes\cdots\otimes\Wbb_N})^\tr$. Equivalently, $\upphi_\zeta$ is parallel under the connection $\nabla$ defined by
\begin{align}
\nabla_{\partial_\zeta}=\partial_\zeta+\sum_{n\in\Zbb} h_n(\zeta)\big(L_{n-1}\otimes\id_{\Wbb_2\otimes\cdots\otimes\Wbb_N}\big)^\tr.
\end{align}
...
If we have $\eta\in\scr O(U)$ univalent on each fiber $U_b=U\cap\mc C_b$ (where $b\in\mc B$) of $U$, then the relationship between $\xk$ and $\yk$ can be written in an explicit way. 
...
\end{align*}
Then $\mc C_{b_\zeta}$ has an open submanifold $\exp_{\zeta\xk}(\mc C_0^+)$ equivalent to $\mc C_0^+$. Note that $b_0=0$.
\begin{align*}
\vcenter{\hbox{{
...
\begin{align}
\exp_{\zeta\xk}(z,\tau_\blt)=(\alpha^i_\zeta(z,\tau_\blt),\exp_{\zeta\yk}(\tau_\blt)).
\end{align}
...
\section{Sheaves of covacua and conformal blocks, and their connections}



\subsection{}

In this section, we study conformal blocks for families of compact Riemann surfaces in a rigorous way. Let
\begin{align*}
...
To define sheaves of covacua and conformal blocks, we first consider the case that local coordinates $\eta_1,\dots,\eta_N$ at $\sgm_1,\dots,\sgm_N$ are chosen and defined on $U_1,\dots,U_N$.

For each open $V\subset\mc B$, let \index{CV@$\mc C_V=\pi^{-1}(V)$}
\begin{align}
\mc C_V=\pi^{-1}(V),
\end{align}
and we have an $\scr O(V)$-linear action of $H^0(\mc C_V,\scr V_{\fk X}\otimes\omega_{\mc C/\mc B}(\star\SX))$ on $\Wbb_\blt\otimes\scr O(V)$ whose restriction to each fiber is the action of $H^0(\mc C_b,\scr V_{\fk X_b}\otimes\omega_{\mc C_b}(\star\SXb))$ on $\Wbb_\blt$ defined by Def. \ref{lb120}. So this action is compatible with the restriction to open subsets of $V$.
...
\end{align}
since $\sgm_i(\mc B)$ is the only one of $\sgm_\blt(\mc B)$ intersecting (and also inside) $U_i$, and $(\eta_i,\pi)$ sends $\sgm_i(\mc B)$ to $\{0\}\times\mc B$ (cf. \eqref{eq220}). Then for each $w_i\in\Wbb_i\otimes\scr O(V)$ (we regard $w_i=w_i(b)$ as a $\Wbb_i$-valued holomorphic functions on $V$), we define 
\begin{align}
\sigma\cdot w_i=\Res_{z=0}~Y\big(\mc V_\varrho(\eta_i)\sigma,z \big)w_i.
\end{align}
...
\begin{align*}
w_\blt(b)=w_1(b)\otimes_\Cbb\cdots\otimes_\Cbb w_N(b)\qquad\in\Wbb_\blt
\end{align*}
and each $w_i$ is an $\Wbb_i$-valued holomorphic function on $V$. Alternatively, 
\begin{align*}
w_\blt=w_1\otimes_{\scr O(V)}\cdots\otimes_{\scr O(V)}w_N
\end{align*}
is in
\begin{align*}
...
\end{align*}
So the expression $w_\blt=w_1\otimes\cdots\otimes w_N$ can be understood in an unambiguous way. The action of any $\sigma\in H^0(\mc C_V,\scr V_{\fk X}\otimes\omega_{\mc C/\mc B}(\star\SX))$ on $w_\blt$ is given by
\begin{align}
...
\end{align}
Define an $\scr O(V)$-module \index{JX@$\scr J_{\fk X}(\Wbb_\blt)$}
\begin{align}
\scr J_{\fk X}(\Wbb_\blt)(V)=H^0\big(\mc C_V,\scr V_{\fk X}\otimes\omega_{\mc C/\mc B}(\star\SX)\big)\cdot H^0\big(V,\scr W_{\fk X}(\Wbb_\blt)\big).
\end{align}
...
\end{align}
(defined by sheafifying the presheaf $V\mapsto \frac{\scr W_{\fk X}(\Wbb_\blt)(V)}{\scr J_{\fk X}(\Wbb_\blt)(V)}$) and its dual $\scr O_{\mc B}$-module $\scr T_{\fk X}^*(\Wbb_\blt)$ are called respectively the \textbf{sheaf of covacua} and the \textbf{sheaf of conformal blocks} associated to $\fk X$ and $\Wbb_\blt$.
\end{df} 
