...
\end{align}
where tangent field $\frac{\partial}{\partial\mu}$ of $\mc C$ is perpendicular to $d\pi$, i.e. tangent to the fibers. Similar to Prop. \ref{lb178}, we have a natural equivalence
\begin{align}
\scr V_{\fk X}^{\leq n}/\scr V_{\fk X}^{\leq n-1}\big|_U\simeq \Vbb(n)\otimes_\Cbb\Theta_{\mc C/\mc B}^{\otimes n}\big|_U
\end{align}
...
To prove this proposition one needs the base change theorem of Grauert \cite[Sec. III.4.2]{GR-b}. See  \cite[Sec. 2.5]{Gui} for a detailed explanation.
...
We give an application of the above equivalence theorem.
...
In view of \eqref{eq231} and \eqref{eq232}, for each $w_\blt\in \Wbb_\blt^{\leq k}\otimes\scr O_{\mc B}$, we can write
\begin{align}
\nabla_{\partial_\tau}w_\blt=\partial_\tau w_\blt+A(\tau)w_\blt\label{eq238}
\end{align}
where 
\begin{align}
A(\tau)w_\blt=-\upnu(\xk)\cdot w_\blt=\sum_{i=1}^N\sum_{k\in\Zbb}h_{i,k}(\tau)w_1\cdots\otimes L_{k-1}w_i\otimes\cdots\otimes w_N
\end{align}
...
\end{align*}
where  $h_{i,k,n}\in\Cbb$ is determined by $h_{i,k}(\tau)=\sum_{n\in\Nbb}h_{i,k,n}\tau^n$ and vanishes for all $i,n$ and $k\leq K$ for some $K\in\Zbb$. So $A(\tau)\in\End(\Wbb)[[\tau]]$.
...
\end{cv}
Let $\Wbb_1,\dots,\Wbb_N,\Mbb$ be an $\Vbb$-module. We associate $\Wbb_1,\dots,\Wbb_N,\Mbb,\Mbb'$ to the marked points $x_\blt,x',x''$ of $\wtd{\fk X}$ and $\Wbb_1,\dots,\Wbb_N$ to $x_1,\dots,x_N$ of $\fk X$. Recall that $\Mbb'$ is the contragredient of $\Mbb$. Identify
\begin{gather*}
...
\wtd{\mc S}\upphi(w_\blt)=\wtd{\mc S}_q\upphi(w_\blt)=\sum_{n\in\Nbb}\upphi(w_\blt\otimes \bowtie_n)q^n,
\end{gathered}
\end{gather}
...
\end{align}
since we can put the projection $P_n$ to the right of $q^{\wtd L_0}$ and take the sum over all $n$, noting that $q^{\wtd L_0}P_n=q^nP_n$. Suppose that the series $\wtd{\mc S}_q\upphi(w_\blt)$ of $q$ converges a.l.u. on $\Dbb_{r\rho}^\times$. Note that for each  $q$, $\fk X_q$ is obtained by scaling either $\xi$ or $\varpi$ by $q$ (or more generally, scaling $\xi$ and $\varpi$ by $q_1,q_2$ such that $q_1q_2=q$) and then perform the sewing as in Subsec. \ref{lb156} along $x'$ and $x''$ using their local coordinates. Then $\wtd{\mc S}_q\upphi(w_\blt)$ is the contraction with respect to this sewing.

We can also use $L_0$ instead of $\wtd L_0$ for scaling. For simplicity, we assume $\Wbb$ is irreducible, then we define the \textbf{(standard) sewing} of $\upphi$ to be \index{S@$\mc S\upphi$, the standard sewing}
\begin{align}
...
\end{align}
$\wtd{\mc S}\upphi$ vanishes on $H^0(\Dbb_{r\rho}^\times,\scr J_{\fk X}(\Wbb_\blt))$.
...
\end{align}
where $\sgm$ sends each $x\in C\setminus x_\blt$ to $x$ and $x_j$ sends $x$ to $x_j$. Clearly the fiber of $\wr\fk X$ at each $x\in C\setminus x_\blt$ is $\wr\fk X_x$. Thus, we can view $\wr\upphi$ as an $\scr O_{C\setminus x_\blt}$-morphism $\scr W_{\wr\fk X}(\Vbb\otimes\Wbb_\blt)\rightarrow\scr O_{C\setminus x_\blt}$. It is a global conformal block since it is so near $x_1,\dots,x_N$. We conclude:

\begin{thm}\label{lb161}
Let $\wr\upphi\in\scr T_{\fk X}^*(\Wbb_\blt)$. Then the $\scr O_{C\setminus x_\blt}$-module morphism $\wr\upphi:\scr W_{\wr\fk X}(\Vbb\otimes\Wbb_\blt)\rightarrow\scr O_{C\setminus x_\blt}$ is a conformal block associated to $\wr\fk X$ and $\Vbb,\Wbb_\blt$, called the \textbf{propagation of $\upphi$}. \index{zz@$\wr\upphi$, propagation of conformal blocs}
\end{thm}
...
\begin{proof}
Let $w_\blt\in\Wbb_\blt$ such that $w_j\in E_j$ for all $j\geq 2$. Clearly $\upphi(Y(u,z)w_1\otimes w_2\cdots\otimes \otimes w_N)$ (which converges a.l.u. when $z\neq 0$ is small) is $0$. So $\wr\upphi(\cdot,w_\blt)$, as a section of $(\scr V_C^{\leq k})^\vee$ on $C\setminus x_\blt$, vanishes near $x_1$ for all $k$. So it vanishes globally on $C\setminus x_\blt$, and  in particular vanishes near $x_2$. This shows that $\upphi(w_1\otimes Y(u,z)w_2\cdots\otimes \otimes w_N)$ vanishes when $z$ is small. By taking residue at $z=0$, we see $\upphi(w_1\otimes Y(u)_nw_2\otimes\cdots\otimes w_N)=0$ for all $u\in\Vbb$ and $n\in\Zbb$. Therefore, as $E_2$ generates $\Wbb_2$, we conclude that $\upphi(w_\blt)=0$ for all $w_1\in\Wbb_1,w_2\in\Wbb_2$ and $w_j\in E_j$ (where $3\leq j\leq N$). Repeating this procedure shows $\upphi=0$.
\end{proof}
...
Let (finitely-admissible) $\Wbb_1,\Wbb_2'$ be associated to the marked points $0,\infty$ of $\fk P=(\Pbb^1;0,\infty;\zeta,1/\zeta)$ where $\zeta$ is the standard coordinate of $\Cbb$. Then it is not hard to check that there is an isomorphism
\begin{gather}
\begin{gathered}
...
\end{gathered}
\end{gather}
(Note that each $Y(u)_n$ acts on $\Wbb_2^\cl$ in an obvious way.) Therefore, by Thm. \ref{lb166}, for any $N$-pointed ($N\geq 2$) sphere such that $\Wbb_1,\Wbb_2'$ are associated to two marked points and $\Vbb$ is associates to the remaining one, the corresponding space of conformal blocks is isomorphic to $\Hom_\Vbb(\Wbb_1,\Wbb_2^\cl)$.
\end{eg}
...
\begin{align}
\Hom_\Vbb(\Vbb,\Wbb^\cl)\simeq\scr T_{\fk P}^*(\Vbb\otimes\Wbb')\simeq\scr T_{(\Pbb^1;\infty;1/\zeta)}^*(\Wbb')
\end{align}
where the corresponding element of $T\in\Hom_\Vbb(\Vbb,\Wbb)$ in $\scr T_{(\Pbb^1;\infty;1/\zeta)}^*(\Wbb')$ is  $T\id\in\Wbb$ as a linear functional on $\Wbb'$. In particular, taking $\Wbb=\Vbb'$, we have
\begin{align}\label{eq256}
\Hom_\Vbb(\Vbb,\Vbb')=\Hom_\Vbb(\Vbb,(\Vbb')^\cl)\simeq \scr T_{(\Pbb^1;\infty;1/\zeta)}^*(\Vbb).
\end{align}
So $\scr T_{(\Pbb^1;\infty;1/\zeta)}^*(\Vbb)$ is trivial if $\Vbb$ is not self-dual.  The first equality of \eqref{eq256} is due to the general fact that
\begin{align}
\Hom_\Vbb(\Wbb_1,\Wbb_2)=\Hom_\Vbb(\Wbb_1,\Wbb_2^\cl)
\end{align}
\end{eg}
...
Suppose we add the nodal curve $\mc C_0=\lim_{q\rightarrow 0}\mc C_q$ to the family $\fk X$ (see the end of Subsec. \ref{lb170}). One can also define sheaves of coinvariants and conformal blocks for $\fk X$. Due to the singularity of $\mc C_0$, we cannot lift $\partial_q$ to $H^0(\mc C,\Theta_{\mc C}(\star\SX))$. But we can lift $q\partial_q$ to an element $\xk\in H^0(\mc C,\Theta_{\mc C}(-\log\mc C_0+\star\SX))$. Here $-\log\mc C_0$ means that $\xi|_{\mc C_0}$ is tangent to $\mc C_0$ and vanishes at the node. (See \cite[Sec. 3.6]{Gui} or \cite[Sec. 11]{Gui20}). 
\begin{align}
\vcenter{\hbox{{
...
\begin{rem}
If we define $\wtd{\fk S}_q$ using the normalized sewing $\wtd{\mc S}_q$,  Using the fact that for each $\Mbb$, $\mc S_q\upphi_\Mbb=q^d\wtd{\mc S}_q\upphi_\Mbb$ for some $d\in\Cbb$, one shows easily that $\wtd{\fk S}_q$ is also injective.
\end{rem}
...
\end{align*}
where $\zeta$ is the standard coordiante of $\Cbb$. Choose $\Vbb$-modules $\Wbb_1,\dots,\Wbb_N,\Wbb_\infty'$ associated to $0,z_1,\dots,z_N,\infty$. Write
\begin{align*}
\Wbb_\blt=\Wbb_1\otimes\cdots\otimes\Wbb_N,
\end{align*}
namely, $\infty$ are not included in the $\blt$. Note that a linear functional on $\Wbb_\blt\otimes\Wbb_\infty'$ is equivalently a linear map $\Wbb_\blt\rightarrow\Wbb_\infty^\cl$.
...
We give a criterion to decide where a linear functional on $\Wbb_\blt\otimes\Wbb_\infty'$ is a conformal block.

\begin{pp}\label{lb176}
A linear map $\mc Y:\Wbb_\blt\rightarrow\Wbb_\infty^\cl$ belongs to $\scr T_{\fk X_{z_\blt}}^*(\Wbb_\blt\otimes\Wbb_\infty')$ if and only if the following condition holds: For each $w_\blt\in\Wbb_\blt,w_\infty'\in\Wbb_\infty'$ and $u\in\Vbb$, the following formal Laurent series
\begin{gather*}
\bigbk{w_\infty,\mc Y(w_1\otimes\cdots\otimes Y(u,z-z_i)w_i\otimes\cdots\otimes w_N)}\qquad \in\Cbb((z-z_i))
\end{gather*}
(for all $i=1,\dots,N$) and the series
\begin{align*}
\bigbk{Y(u,z)^\tr w_\infty,\mc Y(w_\blt)}\qquad\in\Cbb((z^{-1}))
\end{align*}
are expansions at $z_1,\dots,z_\infty,\infty$ of the same function $f\in H^0(\Pbb^1,\scr O_{\Pbb^1}(\star z_\blt+\star\infty))$.
\end{pp}
...
\begin{proof}
The ``if" part can be proved in the same way as Example \ref{lb173}. ``Only if": Let $v\in H^0(C,\scr V_C(\star\infty))$ be $v=\mc U_\varrho(\zeta)^{-1}u$. Then $f=\wr\mc Y(v,w_\blt\otimes w_\infty')$ if we view $\mc Y$ as a linear functional on $\Wbb_\blt\otimes\Wbb_\infty'$.
\end{proof}
...
\begin{eg}
Consider the case that $N=1$. Then $\mc Y:\Wbb_1\rightarrow\Wbb_\infty^\cl$ is a conformal block iff $\bk{w_\blt,\mc YY(u,z)w_1}$ and $\bk{Y(u,z)^\tr w_\infty,\mc Yw_1}$ are expansions at $0,\infty$ of some $f\in H^0(\Pbb^1,\scr O_{\Pbb^1}(\star0+\star\infty))$. Then $(\Res_{z=0}+\Res_{z+\infty})f(z)z^kdz$ shows that $\bk{w_\blt,[Y(u)_k,\mc Y]w_1}=0$. In particular, taking $Y(u,k)$ to be $L_0$ shows that $\mc Y$ preserves the $L_0$-weights
\end{eg}
