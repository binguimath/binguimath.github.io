...
Similarly, assuming $\sigma=1$, then \eqref{eq99} implies that the a.l.u. convergence of \eqref{eq93} on $\Omega_\sigma$ is equivalent to the a.l.u. convergence of \eqref{eq116} as a formal Laurent series of $z_1,z_2/z_1,z_3/z_2,\dots,z_M/z_{M-1}$ on $0<|z_1|<+\infty,0<|z_j/z_{j-1}|<1$. The advantage of taking these variables is that their given regions are annuli, on which one can take Laurent series expansions of holomorphic functions using contour integrals. \hfill\qedsymbol
\end{rem}
...
\end{align*}
and the lower truncation property, we see that the coefficients of $f^1(z_\blt)$ and hence of $\phi(z_\blt)$ vanish if the powers of $z_M$ (resp. $z_1$) is sufficiently negative (resp. positive). Since we can replace $M$ with $\sigma(M)$ and $1$ with $\sigma(1)$, we see that the coefficients of $\phi$ vanish except when the powers $z_1,\dots,z_M$ are all bounded from below and from above by some fixed constants. Namely, $\phi(z_\blt)\in\Cbb[z_1^{\pm1},\dots,z_M^{\pm1}]$. 

By \eqref{eq99}, we can view $f^1(z_\blt)$ as an element
\begin{align}
	f^1(z_\blt)\in\scr R:=\Cbb((z_1,z_2/z_1,\dots,z_M/z_{M-1})).
\end{align}
So, for $\sigma=1$, \eqref{eq113} holds in the ring $\scr R$. So
\begin{align}
f^1(z_\blt)=\phi(z_\blt)\cdot\prod_{1\leq i<j\leq M}(z_i-z_j)^{-N}	\label{eq114}
\end{align}
where the series $\prod_{1\leq i<j\leq M}(z_i-z_j)^{-N}\in\scr R$ is expanded in the region 
\begin{align}\label{eq267}
\{0<|z_1|,0<|z_2/z_1|,|z_3/z_2|\dots,|z_M/z_{M-1}|<1\}
\end{align}
by taking contour integrals with respect to the new set of variables $\zeta_1=z_1,\zeta_2=z_2/z_1,\dots,\zeta_M=z_M/{z_{M-1}}$. Therefore, by complex analysis about Laurent series expansions on (poly)annuli,  one sees that this series, and hence the series on the RHS of \eqref{eq114} (regarded as an element of $\scr R$), converge a.l.u. on $\eqref{eq267}$ to the RHS of \eqref{eq114} regarded as a holomorphic function which we denote by $f_{v,v'}$. Since this statement also holds when the permutation $1$ is replaced by any $\sigma$, our proof is therefore completed.
\end{proof}
