...
By \cite{DSPS19-balanced}, every left exact linear functor from a finite $\Cbb$-linear category to $\Vect$ is representable. Therefore, for each fixed $\Xbb\in\Mod(\Vbb^{\otimes R})$, there exists an $\Mbb$-natural linear isomorphism
...
The following useful fact is proved in Step 2 of the proof of Thm. \ref{lb55}.

\begin{pp}
Let $\mu\in\Cbb$ and $\Xbb=\bigoplus_{\lambda\in\Cbb}(\boxtimes_\fn\Cbb)_{[\lambda,\mu]}$. Then $(\Xbb,Y_\Xbb)$ is an object of $\Mod(\Vbb)$, where $Y_\Xbb$ is the restriction of $Y_{\boxtimes_\fn\Cbb,+}$ to $\Xbb$. Moreover, the functor $\SF$ in Thm. \ref{lb55} sends $(\Xbb,Y_\Xbb)$ to the left $\boxtimes_\fn\Cbb$-module $\boxtimes_\fn\Cbb\diamond\upchi_\mu$.
\end{pp}
