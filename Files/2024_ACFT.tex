...
\end{align*}
The operator $P_0$ is called the \textbf{Hamiltonian} or the \textbf{energy operator}. $P_1,\dots,P_d$ are the momentum operators. $(P_0)^2-(P_1)^2-\cdots-(P_d)^2$ is the mass square operator.
...
\begin{align}
\bk{\Phi(f)\xi|\eta}=\bk{\xi|\Phi^\dagger(\ovl f)\eta}
\end{align}
for each $\xi,\eta$ in this good subspace. 
...
\fk I=\{\Imag\tau\geq0\}
\end{align*}
then $e^{\im \tau P_0}=e^{\im \Real\tau P_0}\cdot e^{-\Imag\tau P_0}$ is bounded. Indeed, $\tau\in\fk I\mapsto e^{\im\tau P_0}$ is continuous, and is holomorphic on $\Int\fk I$. 
...
We shall further explore the question: for which $\xbf_i$ in $\Cbb^{1+d}$ can $\Phi_1(\xbf_1)\cdots\Phi_n(\xbf_n)\Omega$ be reasonably defined as an element of $\mc H$? 
...
\{(\zeta_1,\dots,\zeta_n)\in\Rbb^n:\text{$\xbf_1+\zeta_1\ebf_0,\dots,\xbf_n+\zeta_n\ebf_0$ are mutually spacelike}\}
\end{align}
\end{subequations}
...
\end{align}
Therefore, if $g\mbf e_0=(a_0,\dots,a_d)$, then
\begin{align}
U(g)P_0U(g)^{-1}=a_0P_0+\cdots+a_dP_d
\end{align}
Hence the RHS must be positive. But what are all the possible $g\ebf_0$?

\begin{rem}
One can show that the orbit of $\mbf \ebf_0=(1,0,\dots,0)$ under $\mathrm{SO}^+(1,d)$ is the upper hyperbola with diameter $1$, i.e., the set of all $(a_0,\dots,a_d)\in\Rbb^{1,d}$ satisfying
\begin{align}
...
To simplify the following discussions, we set $1+d=2$ and
\begin{align*}
t=x^0\qquad x=x^1
\end{align*}
We further set
...
(u_1,v_1,\dots,u_n,v_n)\mapsto \wtd\Phi_1(u_1,v_1)\cdots\wtd\Phi_n(u_n,v_n)\Omega
\end{align}
is holomorphic on
...
\begin{itemize}
\item \textbf{{\color{red}T}ime reversal} \index{00@Time reversal}  $t\mapsto -t$.
...
\end{align*}
is not necessarily (anti)unitary. However, as we shall explain, the combination of PCT transformations is actually antiunitary, and hence is a symmetry of the QFT. This is called the PCT theorem.
...
\begin{align*}
e^{\im\pi s K}\wtd\Phi_1(u_1,v_1)\cdots\wtd\Phi_n(u_n,v_n)\Omega=\wtd\Phi_1(e^{-s}u_1,e^sv_1)\cdots\wtd\Phi_n(e^{-s}u_n,e^sv_n)\Omega
\end{align*}
...
\Theta\cdot\wtd\Phi_1(u_1,v_1)\cdots\wtd\Phi_n(u_n,v_n)\Omega=\wtd\Phi_1(-u_1,-v_1)^\dagger\cdots \wtd\Phi_n(-u_n,-v_n)^\dagger\Omega
\end{align}
...
It suffices to prove this in the special case that $\ubf_1,\dots,\ubf_n,\ubf_1',\dots,\ubf_k'$ are mutually spacelike. Then the general case will follow from the fact that both sides of the above relation can be analytically continued to suitable regions as functions of $\ubf_1,\dots,\ubf_n$. For example, the fact that $H_0,H_1\geq0$ implies that
\begin{align*}
...
\begin{align*}
&\bk{\wtd\Phi_1(\ubf_1)\cdots\wtd\Phi_n(\ubf_n)\wtd\Gamma_1(\ubf_1')\cdots\wtd\Gamma_k(\ubf_k')\Omega|\Omega}\\
=&\bk{\wtd\Phi_1(-\ubf_1)\cdots\wtd\Phi_n(-\ubf_n)\wtd\Gamma_1(-\ubf_1')\cdots\wtd\Gamma_k(-\ubf_k')\Omega|\Omega}
\end{align*}
By the PT Thm. \ref{lb8}, this relation is equivalent to
\begin{align*}
&\bk{\wtd\Phi_1(\ubf_1)\cdots\wtd\Phi_n(\ubf_n)\wtd\Gamma_1(\ubf_1')\cdots\wtd\Gamma_k(\ubf_k')\Omega|\Omega}\\
=&\bk{e^{-\pi K}\wtd\Phi_1(\ubf_1)\cdots\wtd\Phi_n(\ubf_n)\wtd\Gamma_1(\ubf_1')\cdots\wtd\Gamma_k(\ubf_k')\Omega|\Omega}
\end{align*}
...
Combining the PT Thm. \ref{lb8} with the PCT Thm. \ref{lb10}, we conclude that $e^{-\pi K}$ is an injective positive operator, $\Theta$ is antiunitary, and
...
\end{align}
Note that since $\mc W$ is local to $-\mc W$, we have $[\scr A(\mc W),\scr A(-\mc W)]=0$. Therefore, $\Theta\scr A(\mc W)\Theta^{-1}$ is a subset of the (in some sense) commutant of $\scr A(\mc W)$.
\end{subequations}
...
\begin{align}
\mc W=\{(a_0,\dots,a_d)\in\Rbb^{1,d}:-a_1<a_0<a_1\}
\end{align}
Then the \textbf{Bisognano-Wichmann theorem} says that $e^{-\pi K}$ is the square root of the modular operator of $(\fk A(\mc W),\Omega)$, and $\Theta U(\rho)$ is the modular conjugation. 
...
\end{align*}
The condition $|\alpha|^2-|\beta|^2=1$ is to ensure that the transform sends $\Sbb^1$ to $\Sbb^1$ and preserves the orientation of $\Sbb^1$. The exact sequence \eqref{eq26} restricts to
\begin{align}
...
We claim that \eqref{eq116} holds if, instead of assuming $J_1,\dots,J_k\subset\Sbb^1_+$, we assume that $J_1,\dots,J_k\subset\Sbb^1_-$. Suppose this claim is true. Then for each $g_\star$, choosing $\delta>0$ such that $\tau_sg_j\in C_c^\infty(J_j)$ for each $j$ and $-\delta<s<\delta$, we have
\begin{align*}
