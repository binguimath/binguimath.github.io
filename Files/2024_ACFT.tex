...
Note that by Conv. \ref{lb48}, $A^\blt(f_\blt)$ is an unbounded operator on $\HV^\infty$ with invariant domain $\HV^\infty$.
...
\item $f$ is continuous, and $z\in\Omega\mapsto\bk{f(z)|v}$ is holomorhic for all $v\in V$.
\item On each closed disk $D\subset O$ with center $z_0$ and radius $r$, $f(z)$ is the limit of a series $\sum_{n\in\Nbb}\xi_n(z-z_0)^n$ where $\xi_n\in\mc H$ and $\sum_{n\in\Nbb}\Vert\xi_n\Vert<+\infty$.
\end{enumerate}
\end{pp}
...
\end{align*} 
Since $\Vert\xi_n\Vert\leq \rho^{-n-1}M$ where $M=\sup_{\zeta\in C}\Vert f(\zeta)\Vert$, we have $\sum_n\Vert\xi_n\Vert<+\infty$. This proves (3).
\end{proof}
...
\end{align}
on $\Cbb$. The \textbf{Virasoro operator associated to \pmb{$X$}} is the element $L_X\in\End(\Vbb)$ defined by \index{LX@$L_X$, the Virasoro operator associated to the vector field $X$}
\begin{align}
...
\begin{co}\label{lb88}
Assume that $\mc V$ is energy-bounded. Choose open intervals $I_1,I_2,\dots$ of $\Sbb^1$. Let $\mc W_{I_\blt}$ be the subspace spanned by vectors of the form \eqref{eq91} as in the Reeh-Schlieder Thm. \ref{lb70}. Let $k\in\Zbb_+$. Let $B^1,\dots,B^k\in\mc V$ and $g_1,\dots,g_k\in C_c^\infty(I_i)$. Then $\mc W_{I_\blt}$ is a core for $B^1(g_1)\cdots B^k(g_k)$.
\end{co}
...
Recall that if $X$ is a topological space, an \textbf{accumulation point in $X$} \index{00@Accumulation point} is an element $x\in X$ belonging to the closure of $X\setminus\{x\}$. 
...
By solving the equation \eqref{eq87}, one sees that $\delta_\tau:=\exp^{\im K}_\tau$ (called a \textbf{dilation}) equals \index{zz@$\delta_\tau$, the dilation}
...
\end{align*}
for each $-\delta<s<\delta$. Using the rotation covariance Cor. \ref{lb76}, one checks that
\begin{align*}
...
\section{The unbounded Tomita-Takesaki theory}
...
\end{gather*}
Clearly $S_{\mc M}=F_{\mc M'}$ and $S_{\mc M'}=F_{\mc M}$. We abbreviate $S_{\mc M},F_{\mc M}$ to $S,F$ when no confusion arises.
\end{df}
...
The significance of the above theorem is indicated by the following property. Assume as usual that $\Omega$ is cyclic and separating for $\mc M$.
...
\begin{enumerate}[label=(\alph*)]
\item There exists a core $\Dom_0$ for both $A,A^*$ such that $\Dom_0\subset\Dom(A_\alpha)\cap\Dom(A_\alpha)^*$ for all $\alpha$.
\item[(b)] For each $\xi\in\Dom_0$ we have $\lim_\alpha A_\alpha\xi=A\xi$ and $\lim_\alpha A_\alpha^*\xi=A^*\xi$.
\end{enumerate}
...
\begin{proof}
Choose any $y\in\mc N'$ and $\xi\in\Dom_0$. Since $A_\alpha$ is affiliated with $\mc N$, we have $yA_\alpha\subset A_\alpha y$ and hence $y\Dom_0\subset y\Dom(A_\alpha)\subset \Dom(A_\alpha)$. Ahus, for all $\eta\in\Dom_0$ we have
\begin{align*}
...
\end{align*}
So $y\xi\in\Dom((A^*|_{\Dom_0})^*)$ and $(A^*|_{\Dom_0})^*y\xi=yA\xi$. Since $\Dom_0$ is a core for $A^*$, we have $y\xi\in\Dom(A)$ and $Ay\xi=yA\xi$. Ahis proves $yA|_{\Dom_0}\subset Ay$. Since $\Dom_0$ is a core for $A$, we get $yA\subset Ay$. Similarly, $y^*A\subset Ay^*$. So $\mc N'\subset\{A\}'$, and hence $A$ is affiliated with $\mc N$.
\end{proof}
...
\begin{df}
We let $S_{\mc M}=\fk J_{\mc M}\Delta_{\mc M}^{\frac 12}$ (or simply $S=\fk J\Delta$) be the left polar decomposition for $S$, where $\Delta=S^*S$.
\end{df}
...
The above theorem is the celebrated \textbf{Tomita-Takesaki theorem}. It's proof is very involved and hard to understand. Therefore, in this course, we will not prove this theorem in full generality. However, in the context of QFT, this theorem can be proved in a more conceptual way. 

In the following, we prove an extremely useful result that will help us prove the Tomita-Takesaki theorem in chiral CFT, the Bisognano-Wichmann theorem, and a lot more. We first need a lemma.
...
\begin{proof}
Step 1. We first note that by  Prop. \ref{lbb24}, if $A\in\scr A$, then $A\Omega\in\Dom(\Delta_{\mc M}^{\frac 12})$, and $\fk J_{\mc M}\Delta_{\mc M}^{\frac 12}A\Omega=A^*\Omega$. The same can be said about $B\in\scr A$. Therefore \eqref{eqb40} is equivalent to
\begin{align}\label{eqb39}
\bk{D^{\frac 12}A\Omega|D^{\frac 12}B\Omega}=\bk{\Delta_{\mc M}^{\frac 12}A\Omega|\Delta_{\mc M}^{\frac 12}B\Omega}
\end{align}

Let $\Dom_0$ be the dense subspace spanned by $\scr A\Omega$. By conditions (a) and (b), for each $t\in\Rbb$ we have $D^{\im t}\Dom_0\subset\Dom_0$. Thus, by Prop. \ref{lbb18}, $\Dom_0$ is a core for $D^{\frac 12}$. If we can prove that $\Dom(D^{\frac 12})$ contains a core for $\Delta_{\mc M}^{\frac 12}$, then by \eqref{eqb39} and the following Lem. \ref{lbb25}, we must have $D^{\frac 12}=\Delta_{\mc M}^{\frac 12}$.\\[-1ex]

Step 2. Let us prove that $\Dom(D^{\frac 12})$ is a core for $S_{\mc M}$ (and hence for $\Delta_{\mc M}^{\frac 12}$). For any $r>0$, define a (clearly bounded linear) operator $x_r$ on $\mc H$ sending each $\xi\in\mc H$ to the improper Riemann integral
\begin{align}
...
\end{align*}
Therefore, by Prop. \ref{lbb16}, we have $x_r\Omega\in\Dom(D^a)$ for each $a\in\Rbb_{\geq0}$. In particular, $x_r\Omega\in\Dom(D^{\frac 12})$. 
...
\begin{co}
In Thm. \ref{lba18}, if $J\mc MJ\subset\mc M'$, then $J\mc MJ=\mc M'$.
\end{co}

\begin{proof}
By Thm. \ref{lba18}, $J=\fk J_{\mc M}$. By assumption, $\mc M\subset (\fk J_{\mc M}\mathcal M \fk J_{\mc M})'$. We compute
\begin{align*}
S_{(\fk J_{\mc M}\mathcal M \fk J_{\mc M})'}=F_{\fk J_{\mc M}\mathcal M \fk J_{\mc M}}=\fk J_{\mc M}F_{\mc M}\fk J_{\mc M}=\fk J_{\mc M}^2\Delta_{\mc M}^{-\half}\fk J_{\mc M}=S_{\mc M}.
\end{align*}
Therefore, by Thm. \ref{lba15} or Cor. \ref{lba16}, $\mathcal M=(\fk J_{\mc M}\mathcal M \fk J_{\mc M})'$.
\end{proof}
