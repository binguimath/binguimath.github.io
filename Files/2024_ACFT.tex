...
Let $d\in\Zbb_+$. We first sketch general picture of an $(1+d)$ dimensional Poincar\'e invariant QFT in the spirit of \textbf{Wightman axioms}. We consider Bosonic theory for simplicity. 
...
\item There is a (strongly continuous) projective unitary representation $U$ of $\Poid$ on $\mc H$. In particular, it restriction to the translation on the $k$-th component (where $k=0,1,\dots,d$) gives a one parameter unitary group $x^k\in\Rbb\mapsto \exp(\im x^k P_k)$ where $P_k$ is a self-adjoint operator on $\mc H$.
\item (Positive energy) The following are positive operators:
\begin{align*}
P_0\geq0\qquad (P_0)^2-(P_1)^2-\cdots-(P_d)^2\geq0
\end{align*}
The operator $P^0$ is called the \textbf{Hamiltonian} or the \textbf{energy operator}. $P^1,\dots,P^d$ are the momentum operators. $(P_0)^2-(P_1)^2-\cdots-(P_d)^2$ is the mass.
...
\fk I=\{\Imag\tau\geq0\}
\end{align*}
then $e^{\im \tau P_0}=e^{\im \Re\tau}\cdot e^{-\Imag\tau}$ is bounded. Indeed, $\tau\in\fk I\mapsto e^{\im\tau P_0}$ is continuous, and is holomorphic on $\Int\fk I$. 
...
The above remark shows that $\Phi_1(0)$, viewed as a smeared operator localized on a small neighborhood of $0$, is definable on $e^{\im\zeta_2P_0}\Phi_2(0)\Omega=\Phi_2(\zeta_2\ebf_0)\Omega$ whenever $\Imag\zeta_2>0$. Thus, heuristically, $(\zeta_1,\zeta_2)\mapsto e^{\im\zeta_1P_0}\Phi_1(\xbf_1)e^{\im\zeta_2P_0}\Phi_2(\xbf_2)\Omega$ should also be holomorphic on
\begin{align*}
\{(\zeta_1,\zeta_2)\in\Cbb^2:\Imag\zeta_1,\Imag\zeta_2>0\}
\end{align*}
More generally, the holomorphicity holds for
\begin{align*}
...
\end{align}
Therefore, if $g\mbf=(a_0,\dots,a_n)$, then
\begin{align}
...
\end{align*}
is not necessarily unitary. However, as we shall explain, the combination of PCT transformations is actually unitary, and hence is a symmetry of the QFT. This is called the PCT theorem.
...
Set $\Gamma_j=\Psi_k^\dagger$. Then the above relation is equivalent to
\begin{align*}
...
\end{align}
for all $s\in\Rbb$. Since the PT transformation sends $\mc W$ to $-\mc W$, the definition of $\Theta$ clearly also implies (noting $\Theta^{-1}=\Theta^*=\Theta$)
\begin{align}\label{eq15c}
\Theta\scr A(\mc W)\Theta=\scr A(-\mc W)
\end{align}
