...
\item The inverse of the canonical braided $*$-functor $(\fk F_\CN,\fk N^\boxtimes):\Rep^0(P)\xrightarrow{\simeq}\RepU$ (where $\Rep^0(P)$ is the category of unitary dyslectic $P$-modules), composed with the canonical one $(\fk F_\VOA,\fk V^\boxdot):\Rep^0(P)\xrightarrow{\simeq}\Repf(\mc B)$, is equal to the CWX functor $\fk F^U_\CWX:\RepU\rightarrow\Repf(\mc B)$ together with a tensorator.
\item This tensorator is equal to the Wassermann tensorator whenever the later can be defined (e.g., when $V$ is one of the following VOAs: unitary  WZW models and discrete series $W$-algebras of type $ADE$, even lattice VOAs, parafermions VOAs of type $ADE$, their tensor products, their strongly-rational cosets, and their unitary VOA extensions).
\end{enumerate}
\end{abstract}
