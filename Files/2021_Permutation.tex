...
%Now, let $\mc W_j$ be a $\otimes$-separable $g_j$-twisted $\Ubb=\Vbb^{\otimes E}$-module, and let $\uppsi:\mc W_\blt=\otimes_{1\leq j\leq N}\mc W_j\rightarrow\Cbb$ be a conformal block associated to $\fk P$ and $\mc W_\blt$ (Subsec. \ref{lb67}). The first main result of this article is that $\uppsi$ is equivalently a conformal block associated to $\fk P$ and certain (untwisted) $\Vbb$-modules.
...
\end{enumerate}
In the above Jacobi identity, let $\mc W'=\bigoplus_{n\in\frac 1{|g|}\Nbb}\mc W(n)^*$, then  $P_n^g$ \index{Pn@$P_n,P_n^g$, the projection onto the $n$-eigenspace of $\wtd L_0$ resp. $\wtd L_0^g$} is defined to be the projection
\begin{align}
...
The above Jacobi identity is equivalent to its well-known algebraic form (cf. \cite[Sec. 10]{Gui21b}). It follows easily from that algebraic Jacobi identity that \eqref{eq12} holds if $\wtd L_0^g$ is replaced by $L_0$. Thus, $\wtd L_0^g-L_0$ commutes with the actions of vertex operators. In particular, if $\mc W$ is an irreducible twisted $\Ubb$-module, then $\wtd L_0^g-L_0$ is a constant.
...
\mc Y(w_1)_{n,k}w_2\in\mc W_3[\log z]\{z\},	\label{eq103}
\end{gather}
and we have
\begin{align}
...
\begin{pp}\label{lb57}
Let $U\subset\Cbb^\times$ be simply-connected with a continuous $\arg$ function $\arg_U$. Let $\mc Y$ be as in Def. \ref{lb51}, but satisfies the two conditions only when $\tipaz\in U$ and $\arg z=\arg_U(z)$ is defined by that of $U$. Then $\mc Y$ is a type $\mc W_3\choose\mc W_1\mc W_2$ intertwining operator.
\end{pp}
...
\begin{proof}
	$\partial_{t_i}f=A_if$ shows $f_{n_1,\dots,n_i+1,\dots,n_N}=(n_i+1)^{-1}Af_{n_1,\dots,n_i,\dots,n_N}$. Therefore all $f_{n_1,\dots,n_N}$ are determined by $f_{0,\dots,0}$.
\end{proof}
