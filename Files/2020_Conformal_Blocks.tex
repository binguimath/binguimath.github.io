...
\usepackage{latexsym}
\usepackage{chngcntr}
\usepackage[colorlinks,linkcolor=blue,anchorcolor=blue,linktocpage,pagebackref]{hyperref}
...
\title{Conformal Blocks:\\ \Large Local Freeness, Sewing, and Factorization}
\author{{\sc Bin Gui}
	%\\
...
\end{align}
The proof will be completed if we can show that $\Psi$ is surjective with kernal $B^1(\fk U,\scr E\otimes\omega_C)$. We divide the proof into several steps.
...
If $C$ is connected, the number $g=\dim H^1(C,\scr O_C)$ is called the (arithmetic) \textbf{genus} of $C$. Thus $\chi(C,\scr O_C)=1-g$. Again, any line bundle $\scr L$ is equivalent to $\scr O_C(D)$ for some divisor $D=k_1x_1+\cdots +k_Nx_N$. (For the nodal curve, we assume none of $x_1,\dots,x_N$ is a node.) The argument is the same as for smooth curves:  By proposition \ref{lb121}, one may find a divisor $D$ such that $H^1(C,\scr L(-D-x))=0$, where $x$ is any smooth point of $C$. Thus, the short exact sequence $0\rightarrow \scr L(-D-x)\rightarrow\scr L(-D)\rightarrow k_x\rightarrow 0$ (where $k_x\simeq\scr L(-D)/\scr L(-D-x)$ is the skyscraper sheaf) yields a surjective $H^0(C,\scr L(-D))\rightarrow H^0(C,k_x)$. So $H^0(C,\scr L(-D))$ is nonzero. Choose any non-zero $s\in H^0(C,\scr L(-D))$. Then $f\mapsto fs$ defines an isomorphism $\scr O_C(D)\rightarrow \scr L$.
...
\end{align}
$d\pi_{r,\rho}$ is surjective at $(\xi,\varpi)$ whenever $\xi\neq 0$ or $\varpi\neq 0$. So if $q\in\mc D_{r\rho}$ is not $0$, the fiber $\pi^{-1}(q)$ is smooth. But $\pi^{-1}(0)$ is singular, which is just a neighborhood of a node of a nodal curve. Denote also by $\xi$ and $\varpi$ the standard coordinates of $\mc D_r$ and $\mc D_\rho$, and set $q=\pi_{r,\rho}$, i.e.,
\begin{align*}
...
We may now glue the two cases together and obtain the locally free $\scr O_{\mc C}$-module $\Theta_{\mc C}(-\log \mc C_\Delta)$  \index{zz@$\Theta_{\mc C}(-\log \mc C_\Delta)$} and a surjective  homomorphism of $\scr O_{\mc C}$-modules $d\pi:\Theta_{\mc C}(-\log \mc C_\Delta)\rightarrow\pi^*\Theta_{\mc B}(-\log\Delta)$, whose kernal is denoted by $\Theta_{\mc C/\mc B}$. \index{zz@$\Theta_{\mc C/\mc B}$} Thus, there is a short exact sequence
\begin{align}
...
\end{align*}
for any $b\in\mc B$. One may use the following fact:  in case II, by \eqref{eq74}, in the $(\xi_1,q_1)$- resp. $(\varpi_1,q_1)$-coordinate, the section $\xi_1\partial_{\xi_1}-\varpi_1\partial_{\varpi_1}$ equals
\begin{align}
...
We first introduce the following notations. If $W$ is a vector space and $z$ is a (formal) variable, we define two sets of \emph{formal series} of $z$: \index{x@$[[z]],[[z^{\pm 1}]],((z)),\{z\}$}
\begin{gather*}
...
\begin{enumerate}[label=(\alph*)]
\item $\scr E|_U$ is equivalent to the cokernal of a homomorphism of (possibly infinite rank) free $\scr O_U$-modules, i.e., there exist cardinalities $I,J$ and a homomorphism of $\scr O_U$-modules $\varphi:\scr O_U^I\rightarrow\scr O_U^J$  such that $\scr E|_U\simeq\mathrm{coker}(\varphi)$.
...
\begin{cv}\label{lb36}
For a $\Vbb$-module $\Wbb$ and its contragredient module $\Wbb'$, we always assume that the actions of $\wtd L_0$ on $\Wbb$ and on $\Wbb'$ satisfy \eqref{eq106}. For the vacuum module $\Vbb$, we set $\wtd L_0=L_0$. Thus $\Vbb(n)=\Vbb_{(n)}$.
\end{cv}
...
\begin{proof}
Choose any $s\in\Cbb$ so that the $L_0$-weight space $\Wbb_{(s)}$ is nontrivial. Note that $\Wbb_{(s)}$ is also finite-dimensional by the definition of ordinary modules. Since $T$ commutes with $L_0$, $T$ preserves the eigenvalues of the eigenvectors of $L_0$. So $T\Wbb_{(s)}\subset\Wbb_{(s)}$. Thus, we can find an eigenvalue $\lambda$ of $T|\Wbb_{(s)}$. It follows that the kernal of $T-\lambda\id_\Wbb$ is a nontrivial $\Vbb$-invariant subspace of $\Wbb$, which must be $\Wbb$. So $T=\lambda\id_\Wbb$.
\end{proof}
...
The following lemma will be used in section \ref{lb50}.

\begin{lm}\label{lb29}
Let $T$ be an open subset of $\Cbb$ containing $0$. Let $\rho:T\rightarrow\Gbb,\zeta\mapsto\rho_\zeta$ be a holomorphic family of transformations satisfying $\rho_0(z)=z$. Then, for any $v\in\Vbb$, 
\begin{align}
\partial_\zeta\mc U(\rho_\zeta)v\Big|_{\zeta=0}=\sum_{n\geq 1}\frac 1{n!}\Big(\partial_\zeta\rho_\zeta^{(n)}(0)\Big|_{\zeta=0}\Big)L_{n-1}\cdot v\label{eq82}
\end{align}
where $\partial_\zeta\rho_\zeta^{(n)}(z)=\partial_z^n\partial_\zeta\rho_\zeta(z)=\partial_\zeta\partial_z^n\rho_\zeta(z)$.
\end{lm}

Note that when $v$ is a (non-necessarily constant) section of $\Vbb\otimes_\Cbb\scr O_T$, one needs to take $\partial_\zeta v$ into account when calculating the left hand side of \eqref{eq82}. Also, as the derivative of $v=\mc U(\rho_\zeta)\mc U(\rho_\zeta)^{-1}v$ is $0$, we obtain
\begin{align}
\partial_\zeta\big(\mc U(\rho_\zeta)^{-1}\big)v\Big|_{\zeta=0}=-\sum_{n\geq 1}\frac 1{n!}\Big(\partial_\zeta\rho_\zeta^{(n)}(0)\Big|_{\zeta=0}\Big)L_{n-1}\cdot v.
\end{align}
...
\begin{align*}
\partial_\zeta\mc U(\rho_\zeta)v\Big|_{\zeta=0}=&\partial_\zeta\Big(\rho_\zeta'(0)^{L_0}\cdot \exp\Big(\sum_{n\geq1}c_n(\zeta) L_n\Big)\Big)v\Big|_{\zeta=0}\\
=&\partial_\zeta\rho_\zeta'(0)L_0v\Big|_{\zeta=0}+\Big(\sum_{n\geq 1}\partial_\zeta c_n(0)L_nv\Big).
\end{align*}
...
\end{align}
where the partial derivative $\partial_\mu$ is defined to be vertical to $d\pi$. From this, we see that $\varrho(\eta|\mu)_p(0)=0$ and  $\partial_z\varrho(\eta|\mu)_p(0)\neq0$. So $\varrho(\eta|\mu)_p$ is an element of $\Gbb$. We thus obtain a family of transformations $\varrho(\eta|\mu):\mc C\rightarrow\Gbb,p\mapsto \varrho(\eta|\mu)_p$, which  is clearly holomorphic.
...
\end{align}
To see this, one compose both sides of \eqref{eq63} with $(\mu,\pi)^{-1}\big(z+\mu(p),\pi(p)\big)$. This relation shows that,   if for $j=1,2,3$ we have $\eta_j\in\scr O(U_j)$ which is univalent on each fiber of $U_j$, then  on $U_1\cap U_2\cap U_3$ we have
\begin{align}
\varrho(\eta_3|\eta_1)=\varrho(\eta_3|\eta_2)\varrho(\eta_2|\eta_1).
\end{align}
...
\mc U_\varrho(\varpi_j)\mc U_\varrho(\xi_j)^{-1}=\mc U_{\varrho}(\varpi_j|\xi_j):\Vbb^{\leq n}\otimes_{\Cbb}\scr O_{W_j'\cap W_j''}\xrightarrow{\simeq} \Vbb^{\leq n}\otimes_{\Cbb}\scr O_{W_j'\cap W_j''}.
\end{align*}
...
The isomorphism in the above proposition can be described in the following way. Choose any open subset $U\subset \mc C-\Sigma$ equipped with a holomorphic $\eta:U\rightarrow\Cbb$ univalent on each fiber. Then we have $\Theta_{\mc C/\mc B}|_U=\scr O_U\otimes \partial_\eta$ where $\partial_\eta$ is defined to be orthogonal to $d\pi$. Consider
\begin{align}
\mc U_\varrho(\eta)\otimes\partial_\eta^{\otimes n}:\scr V_{\fk X}^{\leq n}|_U\xrightarrow{\simeq}\Vbb^{\leq n}\otimes_\Cbb\scr O_U\otimes_\Cbb\partial_\eta^{\otimes n}\qquad( \simeq \Vbb^{\leq n}\otimes_\Cbb \Theta_{\mc C/\mc B}^{\otimes n}|_U)
\end{align}
which maps  $\scr V_{\fk X}^{\leq n-1}|_U$ onto $\Vbb^{\leq n-1}\otimes_\Cbb \Theta_{\mc C/\mc B}^{\otimes n}|_U$. By identifying $\Vbb^{\leq n}/\Vbb^{\leq n-1}$ with $\Vbb(n)$, we get the isomorphism in \eqref{eq153} when restricted to $U$. Moreover, this isomorphism is independent of the choice of $\eta$.
\end{rem}
...
\begin{gather*}
\uplambda:\quad \mc U_\varrho(\eta)^{-1}\cbf\mapsto \partial_\eta^{\otimes 2},\qquad \id\mapsto 0.
\end{gather*}
...
\end{align}
We now define a \textbf{space of covacuua} 
\begin{gather}
\scr T_{\fk X}(\Wbb_\blt)=\frac{\Wbb_\blt}{H^0(C,\scr V_C\otimes\omega_C(\blt S_{\fk X}))\cdot \Wbb_\blt}
\end{gather}
whose dual vector space is denoted by $\scr T_{\fk X}^*(\Wbb_\blt)$ \index{T@$\scr T_{\fk X}(\Wbb_\blt),\scr T_{\fk X}^*(\Wbb_\blt)$} and called a \textbf{space of conformal blocks} or \textbf{space of vacuua}.
...
\end{gather*}
where $u=u(z)\in\Vbb\otimes_\Cbb\scr O(U)$. Then, by \eqref{eq55}, we have
\begin{gather*}
...
Thus, we have a coordinate-independent linear action of $H^0(C,\scr V_C\otimes\omega_C(\blt S_{\fk X}))$ on $\scr W_{\fk X}(\Wbb_\blt)$. Then the space of conformal blocks $\scr T_{\fk X}^*(\Wbb_\blt)$ \index{T@$\scr T_{\fk X}(\Wbb_\blt),\scr T_{\fk X}^*(\Wbb_\blt)$} is the dual space of the space of covacuua
\begin{gather}
...
Define a \textbf{sheaf of covacuua} \index{T@$\scr T_{\fk X}(\Wbb_\blt),\scr T_{\fk X}^*(\Wbb_\blt)$}
\begin{gather}
...
\end{align*}
We show that each fiber of the sheaf $\scr T_{\fk X}(\Wbb_\blt)$ is isomorphic to the space of covacuua $\scr T_{\fk X_b}(\Wbb_\blt)$. Note the obvious $\scr O_{\mc B,b}$-module isomorphism of  stalks
\begin{gather}
...
The \textbf{sheaf of covacuua} $\scr T_{\fk X}(\Wbb_\blt)$ is defined still by \eqref{eq99}, and the \textbf{sheaf of conformal blocks} $\scr T_{\fk X}^*(\Wbb_\blt)$ is its dual sheaf.
...
\begin{align*}
\wtd{\mc S}\uppsi:\scr W_{\fk X}(\Wbb_\blt)=\Wbb_\blt\otimes_\Cbb\scr O(\mc B)\rightarrow\scr O(\wtd{\mc B})[[q_\blt]],
\end{align*}
and the (standard) \textbf{sewing}
\begin{align*}
\mc S\uppsi:\scr W_{\fk X}(\Wbb_\blt)=\Wbb_\blt\otimes_\Cbb\scr O(\mc B)\rightarrow\scr O(\wtd{\mc B})\{q_\blt\},
\end{align*}
...
\end{align}
where the action of $v_{n_\blt}$ on $\Mbb_j\otimes\Mbb_j'$ is defined by \eqref{eq116} and \eqref{eq152} using the  local coordinates $\xi_j,\varpi_j$ of $\wtd{\fk X}$. On the other hand, since $\uppsi$ is  conformal block, for each $n_\blt$, the element $A_{n_\blt}\in\scr O(\wtd{\mc B})$ defined by
\begin{align*}
...
\subsection*{Multiple propagation}
...
\end{align}
where $\zeta$ is another distinct standard complex variable of $\Cbb$. Note that $u,v$ are $\Vbb$-valued meromorphic functions on $W$ with possible poles at $\sgm_1(\mc B),\dots,\sgm_N(\mc B)$. That $L_{udz}v$ has finite poles at $\SX$ follows from the easy calculation
\begin{align}
...
\end{align*}
(Indeed, we will do this for any $\fk y\in \Theta_{\mc B}(-\log\Delta)(\mc B)$.) Then $\nabla_{\fk y_1},\nabla_{\fk y_2},\dots$ extend to a connection $\nabla$ on $\scr T_{\fk X}(\Wbb_\blt)$, whose dual  connection  is the one $\nabla$ on $\scr T_{\fk X}^*(\Wbb_\blt)$.
...
\begin{align}
h_i(z,\tau_\blt)=\sum_{k\in\Zbb}\wht h_i(k,\tau_\blt)z^k,
\end{align}
...
=&\sum_{k\in\Zbb}\wht h_i(k,\tau_\blt) Y_{\Wbb_i}(\cbf)_kw_i=\sum_{k\in\Zbb}\wht h_i(k,\tau_\blt)L_{k-1}w_i.
\end{align}
We conclude:
\begin{align}
...
\begin{pp}\label{lb73}
Suppose that  $\fk y\in\Theta_{\mc B}(-\log\Delta)(\mc B)$ has two lifts $\wtd\yk,\wtd\yk'\in\Theta_{\mc C}(-\log\mc C_\Delta+\blt S_{\fk X})(\mc C)$ which define $\nabla_\yk,\nabla_\yk'$  respectively. Then there exists  $f\in\scr O(\mc B)$ such that 
\begin{align*}
\nabla_\yk-\nabla_\yk' =f\cdot\id
\end{align*}
when acting on sections of $\scr T_{\fk X}(\Wbb_\blt)$. More precisely, for  any section $w$ of $\scr T_{\fk X}(\Wbb_\blt)$, we have $\nabla_\yk w-\nabla_\yk'w =fw$.
\end{pp}

Note that the above relation is not true when the operators act on $\scr W_{\fk X}(\Wbb_\blt)$.
...
\end{gather*}
Let $W=W_1\cup\cdots\cup W_N$. Then $\upnu(\wtd 0)$ is an element of $ \scr V_{\fk X}\otimes\omega_{\mc C/\mc B}(\blt S_{\fk X})(W)$. We shall show that the action of $\upnu(\wtd 0)$ on $\scr W_{\fk X}(\Wbb_\blt)=\Wbb_\blt\otimes_\Cbb\scr O_{\mc B}$ (defined by acting on each component as in \eqref{eq152}) is  an $\scr O_{\mc B}$-scalar multiple. 
...
Assume $\Vbb$ is $C_2$-cofinite. Then there exist $Q\in\Nbb$ and a finite set $\Ebb$ of homogeneous vectors of $\Vbb$   satisfying the following condition: For any weak $\Vbb$-module $\Wbb$ generated by a vector $w$, there exists $L\in\Nbb$ such that $\Wbb$ is spanned by elements of the form 
\begin{align}
Y_\Wbb(v_k)_{-n_k}Y_\Wbb(v_{k-1})_{-n_{k-1}}\cdots Y_\Wbb(v_1)_{-n_1} w\label{eq156}
\end{align}
...
\begin{proof}
Assume without loss of generality that $\Wbb$ is generated by a single $\wtd L_0$-homogeneous vector. Let $T$ be the set of all vectors of the form \eqref{eq156} where $n_k\leq n$. Then, by the above theorem, $T$ is a finite subset of $\Wbb$. Set $\nu(n)=\max\{\wtd\wt(w_0):w_0\in T\}$. If $w\in\Wbb$ is $\wtd L_0$-homogeneous with weight $\wtd\wt(w)>\nu(n)$, then we can also write $w$ as a sum of $\wtd L_0$-homogeneous vectors of the form \eqref{eq156}, but the now $n_k$ must be greater than $\nu$ since such vector is not in $T$. This proves that $w$ is a sum of $\wtd L_0$-homogeneous vectors of the form $Y_\Wbb(v)_{-l}w_0$ where $v\in\Ebb$, $l>n$, and $w_0\in\Wbb$ is $\wtd L_0$-homogeneous. The same is true if $\Wbb$ is assumed finitely generated. By \eqref{eq203}, we have $\wtd\wt(w_0)=\wtd\wt(w)-\wt(v)-l+1$.
\end{proof}
...
Choose any $w_\blt=w_1\otimes\cdots\otimes w_N\in \Wbb_\blt(n)$ such that $w_1,\dots,w_N$ are $\wtd L_0$-homogeneous. Then one of $w_1,\dots,w_N$ must have $\wtd L_0$-weight greater than $\nu(k)$. Assume, without loss of generality, that $\wtd\wt(w_1)>\nu(k)$. Then, by corollary \ref{lb65}, $w_1$ is a sum of non-zero $\wtd L_0$-homogeneous vectors of the form $Y_{\Wbb_1}(u)_{-l}w_1^\circ$ where $v\in\Ebb$, $l>k$, $w_1^\circ\in\Wbb_1$, and $\wtd\wt(w_1^\circ)=\wtd\wt(w_1)-\wt(v)-l+1$. Thus $\wtd\wt(w_1)-\wtd\wt(w_1^\circ)\geq l-1\geq k\geq E+k_0.$
...
Note that $H^0\big(\mc B,\pi_*\scr G\big)$ is exactly $\scr G(\mc C)$. Choose mutually disjoint neighborhoods $W_1,\dots,W_N$ of $\sgm_1(\mc B),\dots,\sgm_N(\mc B)$ respectively. For each $1\leq i\leq N$, identify $\scr V_{\mc C}^{\leq E}\otimes\omega_{\mc C/\mc B}$ with $\Vbb^{\leq E}\otimes_\Cbb\omega_{\mc C/\mc B}$ via $\mc U_\varrho(\eta_i)$, and identify $\eta_i$ with the standard coordinate $z$ by identifying $W_i$ with $(\eta_i,\pi)(W_i)$. Define an element $\upupsilon\in\scr G(\mc C)$ as follows. $\upupsilon|_{W_1}$ is the equivalence class represented by $uz^{-l}dz$, and $\upupsilon|_{\mc C-\sgm_1(\mc B)}=0$. Since the second map in the above exact sequence is surjective, $\upupsilon$ lifts to an element $\wht\upupsilon$ of $H^0\big(\mc B,\pi_*\big(\scr V_{\mc C}^{\leq E}\otimes\omega_{\mc C/\mc B}(lS_{\fk X})\big)\big)$, i.e., of $\scr V_{\mc C}^{\leq E}\otimes\omega_{\mc C/\mc B}(lS_{\fk X})(\mc C)$. Moreover, by the definition of $\scr G$ as a quotient, for each $1\leq i\leq N$ we have an element $v_i$ of $\Vbb^{\leq E}\otimes_\Cbb\scr O_{\mc C}(k_0 S_{\fk X})(W_i)$ (and hence of $\Vbb^{\leq E}\otimes_\Cbb\scr O_{W_i}(k_0 \sgm_i(\mc B))(W_i)$) such that
\begin{gather*}
...
\begin{align}
\theta\cdot w_\blt=\sum_{i=1}^N w_1\otimes\cdots\otimes \upnu(\theta)\cdot w_i\otimes\cdots \otimes w_N
\end{align}
...
\begin{pp}\label{lb80}
Suppose that $\fk X$ has a projective structure $\fk P$. Choose $\theta\in H^0\big(\mc B,\pi_*\Theta_{\mc C/\mc B}(\blt S_{\fk X})\big)$ whose local expression is given by \eqref{eq177}. Then the action of $\theta$ on $\scr T_{\fk X}(\Wbb_\blt)$ (defined by the local coordinates $\eta_\blt$) is the $\scr O(\mc B)$-scalar multiplication  by
\begin{align}
\#(\theta):=\frac{c}{12}\sum_{i=1}^N \Res_{\eta_i=0}~ \Sbf_{\eta_i}\fk P\cdot a_i(\eta_i,\tau_\blt)~d\eta_i.\label{eq179}
\end{align}
\end{pp}
Note that each $\Sbf_{\eta_i}\fk P$ (defined in remark \ref{lb75}) is an element of $\scr O(W_i)$.
...
\end{align*}
Our strategy is to compare the action of $\theta$ defined by the coordinates $\mu_\blt$ (which is trivial by lemma \ref{lb74}) with the one defined by $\eta_\blt$. Set $\wtd\upnu(\theta)\in \svir_c\otimes \omega_{\mc C/\mc B}(\blt S_{\fk X})(W)$ such that $\mc U_\varrho(\mu_i)\wtd\upnu(\theta)|_{W_i}=a_i(\eta_i,\tau_\blt)\cdot(\partial_{\mu_i}\eta_i)^{-1}\cbf~d{\mu_i}$. Then
\begin{align*}
...
\wtd{\fk X}=(\wtd\pi:\wtd{\mc C}\rightarrow\wtd{\mc B};\sgm_1,\dots,\sgm_N;\sgm_1',\dots,\sgm_M';\sgm_1'',\dots,\sgm_M'';\eta_1,\dots,\eta_N;\xi_1,\dots,\xi_M;\varpi_1,\dots,\varpi_M),
\end{align*}
to which the $\Vbb$-modules $\Wbb_1,\dots,\Wbb_N$, $\Mbb_1,\dots,\Mbb_M'$, and their contragredient modules $\Mbb_1',\dots,\Mbb_M'$ are associated.  Also, we assume throughout this section that \emph{$\Vbb$ is $C_2$-cofinite and $\Wbb_1,\dots,\Wbb_N$, $\Mbb_1,\dots,\Mbb_M'$ (and hence their contragredient modules) are semi-simple}, i.e., they are finite direct sums of irreducible modules. 
...
\end{align*}
where $h_i(q,\eta_i,\tau_\blt)\in\scr O(\blt\SX)(\mc D_{r\rho}\times\wtd V_i)$. Write $h_i=\sum_{n}h_{i,n}q^n$. Then by \eqref{eq237},
\begin{align}
...
\begin{align*}
B:=\oint_{\gamma''}\Sbf_\varpi\fk P(\varpi,\tau_\blt)\cdot b(\varpi,q/\varpi,\tau_\blt)\varpi d\varpi
\end{align*}
...
We choose $k_0$ such that the $s_1,s_2,\dots$ found in theorem \ref{lb67} are in $\Wbb_\blt^{\leq k_0}\otimes_\Cbb\scr O(\mc B)$. Fix any $k\geq k_0$. We may  assume that $s_1,s_2,\dots$ form a basis of $\Wbb_\blt^{\leq k_0}$, regarded as constant sections of $\Wbb_\blt^{\leq k_0}\otimes_\Cbb\scr O(\mc B)$. 
...
Assume $\wtd{\mc B}$ is Stein. For each $1\leq k\leq M$, define $\nabla_{q_k\partial_{q_k}}$ on $\scr T_{\fk X}(\Wbb_\blt)$ and hence on $\scr T_{\fk X}^*(\Wbb_\blt)$ using a lift $\wtd\yk$ of $q_k\partial_{q_k}$ as in section \ref{lb84}. Then there exists a projective term $f_k\in\scr O(\mc B)$ such that
\begin{align*}
...
\begin{proof}
Let us fix $q_0\in\mc D_{r\rho}^\times$ and let $q$ denote a complex variable. Let us prove that $\fk S_{q_0}$ is injective. Suppose that the finite sum $\sum_\Mbb\mc S\uppsi_\Mbb(q_0)$ equals $0$. We shall show that $\uppsi_\Mbb=0$ for any $\Mbb\in\mc E$. Suppose this is not true. Let $\mc F$ be the (finite) subset of all $\Mbb\in\mc E$ satisfying $\uppsi_\Mbb\neq0$. Then $\mc F$ is not an empty set. 

We first show that $\upphi:=\sum_\Mbb\mc S\uppsi_\Mbb$ (which is a multivalued holomorphic function on $\mc D_{r\rho}^\times$) satisfies $\upphi(q)=0$ for each $q\in\mc D_{r\rho}^\times$. Choose any large enough  $k\in\Nbb$. Then, by theorem \ref{lb79},  $\upphi^{\leq k}$ satisfies a linear differential equation on $\mc D_{r\rho}^\times$ of the form $\partial_q\upphi^{\leq k}=q^{-1}A\cdot \upphi^{\leq k}$. Moreover, it satisfies the initial condition $\upphi^{\leq k}(q_0)=0$. Thus, by theorem \ref{lb12}, $\upphi^{\leq k}$ is constantly $0$. So is $\upphi$.
...
Note that $f(z)$ can be written as $z^{\alpha_1}f_1(z)+\cdots+z^{\alpha_n}f_n(z)$ where $f_1,\dots,f_n\in\Cbb[[z]]$, and any two of $\alpha_1,\dots,\alpha_n$ do not differ by an integer. It is easy to see that $f(z)$ converges absolutely on $\mc D_\epsilon^\times$ if and only if $f_1,\dots,f_n\in\scr O(\mc D_\epsilon)$. Indeed, the if part is obvious; the only if part follows from the root test. Moreover, it is clear that that $f_1,\dots,f_n\in\scr O(\mc D_\epsilon)$ implies that $f(z)$ converges a.l.u. on $\mc D_\epsilon^\times$, i.e., that for compact subset $K\subset\mc D_\epsilon^\times$, there is $C>0$ such that \eqref{eq190} holds for any $z\in K$.
\end{rem}
...
\end{align*}
Using \eqref{eq242}, it is easy to see that $Y(v)_n\Omega(\Wbb)\subset\Omega(\Wbb)$ when $\wt(v)=n+1$.  If $\Wbb$ is admissible, then the lowest $\wtd L_0$-weight space of $\Wbb$ is a subspace of $\Omega(\Wbb)$. In particular, $\Omega(\Wbb)$ is non-trivial if $\Wbb$ is so. Also, if $\{\Wbb_i\}$ is a set of weak $\Vbb$-modules, then
\begin{align}
...
It will be interested to know if a weak module has non-trivial lowest weight subspace. The following lemma provides a criterion.
...
If $\Vbb$ satisfies one of the three conditions of proposition \ref{lb132}, we say that $\Vbb$ is \textbf{almost rational}.
\end{df}


Note that by corollary \ref{lb99}, if $\Vbb$ is $C_2$-cofinite and almost rational, each lowest weight weak  $\Vbb$-module $\Wbb$  with finite dimensional $\Omega(\Wbb)$ is a finite direct sum of irreducible (ordinary) $\Vbb$-modules.
...
\begin{proof}
Since $A\curvearrowright\mbf W$ is strongly and completely reducible, by corollary \ref{lb107}, we can find a set of mutually inequivalent strongly irreducible $A$-modules $\{\mbf U_i\}$ such that the equivalence
\begin{align}
...
Let $\Mbb_1,\Mbb_2$ be irreducible $\Vbb$-modules, and choose $\wtd L_0$ such that the lowest weights (with non-trivial weight subspaces $\Mbb_1(0),\Mbb_2(0)$) are both $0$. Choose any $\upphi\in\scr T_{\fk X}^*(\Wbb_\blt\otimes\Mbb_1\otimes\Mbb_2)$. Then there is a natural linear map
\begin{gather*}
...
The following result is claimed in remark \ref{lb111}. We are now ready to prove it. Note that neither $C_2$-cofiniteness nor almost rationality is assumed here.
...
\end{gather}
where $\Psi_\upphi$ is defined in example \ref{lb112}. Then $\Psi$ is injective. If $\Vbb$ is $C_2$-cofinite and almost rational, then $\Psi$ is an isomorphism.
\end{thm}
...
When $\Vbb$ is $C_2$-cofinite and almost rational, the surjectivity of $\Psi$ follows from proposition \ref{lb109} and lemma \ref{lb106}.
\end{proof}
...
We assume  that $\Vbb$ is both $C_2$-cofinite and almost rational. For each irreducible $\Vbb$-module $\Mbb$, we choose $\wtd L_0$ such that its lowest weight (with nontrivial weight space) is $0$. Thus $\Omega(\Mbb)=\Mbb(0)$.
...
Assume that $\Vbb$ is $C_2$-cofinite and almost rational. Then $\wtd{\fk S}_q$ is an isomorphism for each $q\in\mc D_{r\rho}$, and $\fk S_q$ is an isomorphism for each $q\in\mc D_{r\rho}^\times$.
\end{thm}
...
\begin{align}
\sum_{\Mbb_1,\dots,\Mbb_M\in\mc E} \dim\scr T_{\wtd{\fk X}}^*(\Wbb\otimes\Mbb_\blt\otimes\Mbb_\blt')\label{eq243}
\end{align}
...
\begin{thm}
Assume that $\Vbb$ is $C_2$-cofinite and almost rational. Let $\fk X=(\pi:\mc C\rightarrow\mc B;\sgm_1,\dots,\sgm_N)$ be a family of $N$-pointed complex curves. Then $\scr T_{\fk X}(\Wbb_\blt)$ and (hence) $\scr T_{\fk X}^*(\Wbb_\blt)$ are locally free. Consequently, the function $b\in\mc B\mapsto \dim\scr T_{\fk X_b}^*(\Wbb_\blt)$ is locally constant.
\end{thm}
...
%We let $\mc E$ be a complete set of mutually inequivalent irreducible $\Vbb$-modules. The word ``complete" means that any irreducible $\Vbb$-module is equivalent to an element of $\mc E$.
