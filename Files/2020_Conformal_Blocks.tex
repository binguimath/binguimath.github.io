...
\begin{cv}\label{lb36}
For a $\Vbb$-module $\Wbb$ and its contragredient module $\Wbb'$, we always assume that the actions of $\wtd L_0$ on $\Wbb$ and on $\Wbb'$ satisfy \eqref{eq106}.  If $\Wbb$ is semi-simple, i.e., a finite direct some of irreducible $\Vbb$-modules, except when $\Wbb=\Vbb$, we assume  each irreducible submodule $\Mbb$ of $\Wbb$  is $\wtd L_0$-invariant, and the lowest eigenvalue of $\wtd L_0|_{\Mbb}$ (with non-trivial eigenspace) is $0$. These two assumptions are compatible. For the vacuum module $\Vbb$, we set $\wtd L_0=L_0$. Thus $\Vbb(n)=\Vbb_{(n)}$.
\end{cv}
...
We assume  that $\Vbb$ is both $C_2$-cofinite and almost rational. By convention \ref{lb36}, for each irreducible $\Vbb$-module $\Mbb$,  its lowest $\wtd L_0$-weight (with nontrivial weight space) is $0$. Thus $\Omega(\Mbb)=\Mbb(0)$.
