...
\vspace{-0.5cm}
\blfootnote{First version: August 2020. Last update: October 2020}
...
\end{align}
Let $\alpha_i'(0)\in\scr O(\mc B)$ whose value at each $b\in\mc B$ is  $\partial_z\alpha_{i,b}(z)|_{z=0}$. Assume $\Vbb$ has central charge $c$, and $\Wbb_1,\dots,\Wbb_N$ are simple $\Vbb$-modules. For each $\Wbb_i$, we let $\varDelta_i\in\Cbb$ be the unique number such that $\Wbb_i=\bigoplus_{n\in\Nbb}(\Wbb_i)_{(\varDelta_i+n)}$ and $(\Wbb_i)_{(\varDelta_i)}$ is non-trivial.   Thus, according to Convention \ref{lb36}, $L_0-\wtd L_0=\varDelta_i\id$ on $\Wbb_i$. $\varDelta_i$ is called the \textbf{conformal weight} of $\Wbb_i$.

\begin{thm}\label{lb144}
Let $f\in\scr O(\mc B)$ be 
\begin{align}\label{eq245}
f=\sum_{i=1}^N\Big(\Res_{\eta_i=0} ~\frac c{12}h_i(\eta_i,\tau_\blt)\Sbf_{\eta_i}\mu_i d\eta_i-\varDelta_i\sum_{j=1}^m g_j(\tau_\blt)\partial_{\tau_j}\log\alpha_i'(0)  \Big).
\end{align}
...
Thus
\begin{align}
\mc U(\mu_\blt)\nabla_\yk^{(\eta_\blt)} s=\sum_i \mc U(\alpha_1)w_1\otimes\cdots\otimes \mc U(\alpha_i)\wtd w_i\otimes\cdots\otimes\mc U(\alpha_N^{-1}) w_N.\label{eq251}
\end{align}
By \eqref{eq169},
\begin{align*}
...
\begin{align*}
&\partial_{\tau_j}\mc U(\alpha_i)w_i=\Res_{z=0}~(\partial_{\tau_j}\alpha_i)(\alpha_i^{-1}(z)) Y_\Wbb(\cbf,z) \mc U(\alpha_i)w_i dz-\varDelta_i\partial_{\tau_j}\log\alpha_i'(0)\mc U(\alpha_i)w_i\nonumber\\
=&\Res_{\mu_i=0}~\partial_{\tau_j/\eta_i}\mu_i\cdot Y_\Wbb(\cbf,\mu_i) \mc U(\alpha_i)w_i d\mu_i-\varDelta_i\partial_{\tau_j}\log\alpha_i'(0)\mc U(\alpha_i)w_i.
\end{align*}
Thus
\begin{align}
\upupsilon_i=-\Res_{\mu_i=0}~h_i\partial_{\eta_i}\mu_i Y_{\Wbb_i}(\cbf,\mu_i)\mc U(\alpha_i)w_id\mu_i-\varDelta_i\sum_j g_j\partial_{\tau_j}\log\alpha_i'(0)\mc U(\alpha_i)w_i.\label{eq254}
\end{align}
...
Assume $\Wbb_1,\dots,\Wbb_N$ are simple $\Vbb$-modules. We explain how to obtain a flat connection associated to sheaves of covacuua and conformal blocks of $\Vbb$. Let
\begin{align*}
\fk X=(\pi:\mc C\rightarrow\mc B;\sgm_1,\dots,\sgm_N;\nu_1,\dots,\nu_N)
\end{align*}
be a \textbf{family of $N$-framed compact Riemann surfaces}. This means that $(\pi:\mc C\rightarrow\mc B;\sgm_1,\dots,\sgm_N)$ is $N$-pointed, and 
\begin{align*}
...
\begin{eg}
If $\fk X$ is a family of $N$-pointed compact Riemann surfaces with local coordinates $\eta_\blt$, then $\fk X$ has a natural choice of framing: let $\nu_i=\sgm_i^*d\eta_i$.
\end{eg}


\begin{eg}
Assume $\fk X=(\pi:\mc C\rightarrow\mc B;\sgm_\blt)$ is $N$-pointed. For each $i$, we let $\mc B^i$ be the open subset of non-zero vectors of the line bundle $\sgm_i^*\omega_{\mc C/\mc B}$.  Let $p_i:\mc B^i\rightarrow\mc B$ be the projection sending the vectors to their initial points. Using these projections, we define the relative product $\mc B^\triangle=\mc B^1\times_{\mc B}\mc B^2\times_{\mc B}\cdots \times_{\mc B}\mc B^N$, i.e., the closed submanifold of all $(\gamma^1,\dots,\gamma^N)\in \mc B^1\times\cdots\times \mc B^N$ satisfying $p_1(\gamma^1)=\cdots= p_N(\gamma^N)$. Let $p:\mc B^\triangle\rightarrow\mc B$ be the natural projection defined by $p_1,\dots,p_N$. Then we may pull back $\fk X$ along $p:\mc B^\triangle\rightarrow\mc B$ to obtain an $N$-pointed $\fk X^\triangle=(\pi:\mc C^\triangle\rightarrow\mc B^\triangle;\sgm_\blt^\triangle)$. More precisely: we let $\mc C^\triangle=\mc C\times_{\mc B}\mc B^\triangle$ which can be considered as a submanifold of $\mc C\times\mc B^\triangle$. $\sgm_i^\triangle$ is determined by $\sgm_i^\triangle(b^\triangle)=(\sgm_i(p(b^\triangle)),b^\triangle)$ for every $b^\triangle\in\mc B^\triangle$. Then $\fk X^\triangle$ has a natural framing $\nu_\blt$ such that for each $b\in\mc B^\triangle$, if we consider $b^\triangle=(\gamma^1,\dots,\gamma^N)$  as an element of $\mc B^1\times\cdots\times\mc B^N=\sgm_1^*\omega_{\mc C/\mc B}\times\cdots\times\sgm_N^*\omega_{\mc C/\mc B}$ and set $b=p(b^\triangle)$, then $\nu_i(b^\triangle)$, a cotangent vector of $\mc C^\triangle_{b^\triangle}=\mc C_b\times b^\triangle\simeq \mc C_b$ at $\sgm_i^\triangle(b^\triangle)=(\sgm_i(b),b^\triangle)\simeq \sgm_i(b)$, is $\gamma^i$.
\end{eg}


\subsection*{(Logarithmic) flat connections depending on the framing $\nu_\blt$}

Let $\fk X$ be $N$-framed as above with framing $\nu_\blt$. Fix a $C_2$-cofinite almost rational VOA $\Ubb$ with non-zero central charge $c_\Ubb$. We assume that $\Ubb$ is holomorphic, i.e., $\Ubb$ has only one simple module which is $\Ubb$ itself. For instance, one can take $\Ubb$ to be the VOA associated to an even self-dual lattice, or the moonshine VOA. By factorization, any space of conformal block associated to $\Ubb$ and a pointed curve has dimension one. Thus the sheaves of conformal blocks of $\Ubb$ are line bundles. We fix the sheaf of conformal blocks of $\Ubb$ associated to $\fk X$ and the $\Ubb$-modules $\Ubb,\dots,\Ubb$:
\begin{align*}
\scr L_{\fk X}^\Ubb=\scr T_{\fk X}^*(\Ubb\otimes\cdots\otimes\Ubb)
\end{align*}
and consider it as a line bundle on $\mc B$. Then $\mc B$ can be considered as a closed submanifold of $\scr L_{\fk X}^\Ubb$ consisting of zero vectors. Note that we have a natural projection 
\begin{align*}
p:\scr L_{\fk X}^\Ubb\rightarrow\mc B
\end{align*}
sending each vector to its initial point. By pulling back $\scr T^*_{\fk X}(\Wbb_\blt)$ along $p$ to $\scr L_{\fk X}^\Ubb$, we have an $\scr O_{\mc B}$-module $p^*\scr T^*_{\fk X}(\Wbb_\blt)$.

When $\frac c{c_\Ubb}$ is an integer, it is not hard to see that the connection on $\scr T_{\fk X}^*(\Wbb_\blt)\otimes(\scr L_{\fk X}^\Ubb)^{-\frac c{c_\Ubb}}$ over $\mc B$ is flat and (after adding certain projective terms) is independent of the choice of local coordinates and the lifts of tangent fields of $\mc B$. However, when $\frac c{c_\Ubb}$ is not an integer, $(\scr L_{\fk X}^\Ubb)^{-\frac c{c_\Ubb}}$ may not be well-defined. To overcome this difficult, we shall define a flat  connection $\nabla$ on $p^*\scr T^*_{\fk X}(\Wbb_\blt)$  over the complex manifold $\scr L_{\fk X}^\Ubb$ with logarithmic singularities at $\mc B$. 


To define such $\nabla$, it suffices to consider when $\mc B$ is Stein and small enough such that $\fk X$ admits local coordinates $\eta_\blt$ and show that $\nabla$ is independent of lifting and local coordinates. Then the general case can be easily treated by gluing. For each $i$, $\sgm_i^*d\eta_i\in\sgm_i\omega_{\mc C/\mc B}(\mc B)$ is nowhere zero. Thus $\sgm_i^*d\eta_i/\nu_i\in\scr O(\mc B)$. 
...
Choose a nowhere zero section $\uptheta\in\scr L_{\fk X}^\Ubb(\mc B)$. We have a trivialization
\begin{gather*}
\mc F_{\uptheta}:\Cbb\times\mc B\xrightarrow{\simeq} \scr L_{\fk X}^\Ubb,\qquad (z^\uptheta,b)\mapsto z^\uptheta\cdot \uptheta(b).
\end{gather*}
Assume this trivialization, we define $\nabla$ determined by the following descriptions. Choose any open $U\subset\scr L_{\fk X}^\Ubb$. Then any section of $p^*\scr T_{\fk X}^*(\Wbb_\blt)(U)$ is a linear combination of $fp^*\upphi=f\otimes p^*\upphi$ where $f\in\scr O(U)$ and $\upphi\in\scr T_{\fk X}^*(\Wbb_\blt)(p(U))$. For  $\yk\in\Theta(\mc B)(p(U))$, let $\nabla_\yk^{(\eta_\blt)}$ be the differential operator on $\scr T_{\fk X}^*(\Wbb_\blt)$ defined by $\eta_\blt$ and a lift $\wtd\yk$ as in Section \ref{lb84}. Let $\Delta_i$ be the conformal dimension of the simple module $\Wbb_i$ as in Section \ref{lb142}. Then $\nabla$ is determined by
\begin{gather}
\nabla_{\yk^\uptheta}(p^*\upphi)=p^*\nabla_\yk^{(\eta_\blt)}\upphi-\frac c{c_\Ubb}\cdot \frac{\nabla_\yk^{(\eta_\blt)}\uptheta}{\uptheta}\cdot p^*\upphi-\sum_{i=1}^N \Delta_i\cdot \yk\Big(\log\frac{\sgm_i^*d\eta_i}{\nu_i}\Big)\cdot p^*\upphi,\label{eq256}\\
\nabla_{z^\uptheta\partial_{z^\uptheta}}(p^*\upphi)=-\frac c{c_\Ubb}\cdot p^*\upphi \label{eq257}
\end{gather}
where $\yk^\uptheta$  is the tangent field of $\scr L_{\fk X}^\Ubb\simeq\Cbb\times\mc B$ defined by pulling back $\yk$ on $\mc B$ along the projection $\Cbb\times\mc B\rightarrow\mc B$. Note that $\yk^\theta$ depends on $\mc F_\uptheta$ and hence on $\uptheta$. 

\begin{thm}
The logarithmic connection $\nabla$ of $p^*\scr T_{\fk X}^*(\Wbb_\blt)$  is flat. Moreover, $\nabla$ depends on the framing $\nu_\blt$ but not on $\wtd\yk$, $\eta_\blt$, or $\uptheta$.
\end{thm}

\begin{proof}
By Proposition \ref{lb73} and Remark \ref{lb143}, $\nabla_{\yk^\vartheta}$ is independent of $\wtd\yk$. Suppose $\upvartheta$ is another nowhere zero section of $\scr L_{\fk X}^\Ubb$. Let $f=\upvartheta/\uptheta$. Then $\mc F_{\uptheta}^{-1}\mc F_{\upvartheta}:(z,b)\mapsto (zf(b),b)$. Thus $z^\uptheta=fz^\upvartheta$. Using change of coordinate formulas for tangent fields, we have $\yk^{\upvartheta}=\yk^\uptheta+\yk(f)z^\upvartheta\partial_{z^\uptheta}$ and $\partial_{z^\upvartheta}=f\partial_{z^\uptheta}$. So
\begin{gather*}
\yk^{\upvartheta}=\yk^\uptheta+\frac{\yk(f)}{f}\cdot z^\uptheta\partial_{z^\uptheta},\qquad  z^\upvartheta\partial_{z^\upvartheta}=z^\uptheta\partial_{z^\uptheta}.
\end{gather*}
Thus, the definitions of $\nabla_{z^\uptheta\partial_{z^\uptheta}}=\nabla_{z^\upvartheta\partial_{z^\upvartheta}}$ using $\vartheta$ and $\upvartheta$ agree and are clearly independent of $\wtd\yk$ or $\eta_\blt$. Let $A$ be the summation on the right hand side of \eqref{eq257}.  If we define $\nabla$ using $\uptheta$, then
\begin{align*}
&\nabla_{\yk^{\upvartheta}}(p^*\upphi)=\nabla_{\yk^\uptheta}(p^*\upphi)+\frac{\yk(f)}{f}\cdot\nabla_{z^\uptheta\partial_{z^\uptheta}}(p^*\upphi)\\
=&p^*\nabla_\yk^{(\eta_\blt)}\upphi-\frac c{c_\Ubb}\cdot \frac{\nabla_\yk^{(\eta_\blt)}\uptheta}{\uptheta}\cdot p^*\upphi-A-\frac c{c_\Ubb}\cdot \frac{\yk(f)}{f}\cdot p^*\upphi\\
=&p^*\nabla_\yk^{(\eta_\blt)}\upphi-\frac c{c_\Ubb}\cdot \frac{\nabla_\yk^{(\eta_\blt)}(f\uptheta)}{f\uptheta}\cdot p^*\upphi-A,
\end{align*}
which clearly equals $\nabla_{\yk^{\upvartheta}}(p^*\upphi)$ defined using $\upvartheta$.
...
\begin{rem}
The definition of $\nabla$ on $p^*\scr T_{\fk X}(\Wbb_\blt)$ is similar, except that the minus signs in \eqref{eq256} and \eqref{eq257} should be positive. 
\end{rem}



\subsection*{Flat connections depending on $\nu_\blt$ and a global section $\uptheta$ of $\scr L_{\fk X}^\Ubb$}



The disadvantage of the above construction is that $\nabla$ is not defined on $\mc B$ but on  a line bundle $\scr L_{\fk X}^\Ubb$ over $\mc B$ (more precisely, on the open subset of non-zero vectors of $\scr L_{\fk X}^\Ubb$). However, if we assume there is a nowhere zero section $\uptheta\in\scr L_{\fk X}^\Ubb$, then one can define a flat connection $\nabla^\uptheta$ of $\scr T_{\fk X}^*(\Wbb_\blt)$ on $\mc B$. Indeed, consider $\uptheta$ as a map from $\mc B$ to $\scr L_{\fk X}^\Ubb$, then $\nabla^\uptheta$ is the pullback of $\nabla$ along $\uptheta$. Its explicit expression is similar to \eqref{eq256}.

To be more precise, assume without loss of generality that $\mc B$ is Stein and small enough so that we can choose local coordinates $\eta_\blt$. For each section $\yk$ of $\Theta_{\mc B}$, choose a lift $\wtd\yk$. Define $\nabla^{(\eta_\blt)}$ using $\eta_\blt$ and $\wtd\yk$ as in Section \ref{lb84}.  Then for each section $\upphi$ of $\scr T_{\fk X}^*(\Wbb_\blt)$, 
\begin{align}
\nabla^\uptheta_\yk \upphi=\nabla_\yk^{(\eta_\blt)}\upphi-\frac c{c_\Ubb}\cdot \frac{\nabla_\yk^{(\eta_\blt)}\uptheta}{\uptheta}\cdot \upphi-\sum_{i=1}^N \Delta_i\cdot \yk\Big(\log\frac{\sgm_i^*d\eta_i}{\nu_i}\Big)\cdot \upphi.
\end{align}
$\nabla^\uptheta$ is flat, and  is independent of the choice of $\eta_\blt$ and the lift $\wtd\yk$. So it can be defined when $\mc B$ is not Stein or small.



\subsection*{Connections depending on $\nu_\blt$ and a projective structure $\fk P$}

Suppose $\fk X$ has a projective structure $\fk P$ and a framing $\nu_\blt$,  one can define a connection $\nabla^{\fk P}$ as follows. Choose a lift $\wtd\yk$ of the tangent field $\yk$, and let $h_i$ be as in \eqref{eq259}. Then
\begin{align}
\nabla^{\fk P}_\yk \upphi=\nabla_\yk^{(\eta_\blt)}\upphi-\sum_{i=1}^N\Res_{\eta_i=0} ~\frac c{12}h_i(\eta_i,\tau_\blt)\Sbf_{\eta_i}\fk P d\eta_i-\sum_{i=1}^N \Delta_i\cdot \yk\Big(\log\frac{\sgm_i^*d\eta_i}{\nu_i}\Big)\cdot \upphi.
\end{align}
...
\begin{proof}
To compare the definition of $\nabla$ using two sets of local coordinates $\eta_\blt$ and $\mu_\blt$, it suffices to assume $\mu_1,\dots,\mu_N$ belong to $\fk P$. Then the coincidence of follows from Theorem \ref{lb144}. When the local coordinates belong to $\fk P$, $\nabla$ is independent of the choice of lift by Remark \ref{lb143} and Lemma \ref{lb74} (or equation \eqref{eq179}). Thus, for a general $\eta_\blt$, the independence on $\wtd\yk$ is also true.
\end{proof}



Note that unlike the previous two connections, $\nabla^{\fk P}$ might not be flat. 
