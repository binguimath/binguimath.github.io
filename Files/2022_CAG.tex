...
\begin{proof}
If $x$ belongs to an associated prime $\Ann_{\mc A}(\xi)$ then $\xi\neq0$ and $x\xi=0$. So $x$ is a zero-divisor of $\mc E$. Conversely, if $x$ is a zero-divisor of $\mc E$, then $x\xi=0$ for some non-zero $\xi\in\mc E$. By Prop. \ref{lb290}, we may find $a\in\mc A$ so that $\Ass_{\mc A}(a\xi)$ is prime, which clearly contains $x$.
\end{proof}
