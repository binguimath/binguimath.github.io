...
Assume Thm. \ref{lb109}. Assume $X$ is reduced at $x$. Then $\sqrt{\mc I}$ is coherent and its stalk at $x$ is $0$. So its stalk is zero on a neighborhood $U$ of $x$. Then $X$ is reduced everywhere on $U$.
...
\begin{proof}
It suffices to show that, after shrinking $X$ to a neighborhood of $x$, $X_{i,k}\cap X_{j,l}$ is nowhere dense in $X_{i,k}$ if $(i,k)\neq (j,l)$. By Thm. \ref{lb125}, we may shrink $X$ so that this is true whenever $i=j$. So let us assume $i\neq j$. Suppose that $X_{i,k}\cap X_{j,l}$ contains a non-empty open subset $U\subset X_{i,k}$. Since $X_{i,k}\cap (\bigcup_{k'\neq k}X_{i,k'})$ is nowhere dense in $X_{i,k}$, it does not contain $U$. Thus $U$ intersects $X_{i,k}\setminus\bigcup_{k'\neq k}X_{i,k'}$, where the latter equals $X_i\setminus\bigcup_{k'\neq k}X_{i,k'}$ and is hence an open subset of $X_i$. This shows that $X_{i,k}\cap X_{j,l}$ contains an open subset of $X_i$, contradicting the fact that $X_i\cap X_j$ is nowhere dense in $X_i$.
\end{proof}
...
\begin{proof}
Since reducedness is a local property, we may assume that $X$ and $Y$ are small enough. Choose any $x\in X$ and $y\in Y$. Since $X,Y$ are reduced, by Prop. \ref{lb181}, we may shrink $X,Y$ to  neighborhoods of $x,y$ respectively so that we can find $f\in\scr I_{\Sg(X)}(X)$ which is a non zero-divisor of $\scr O_{X,x}$, and find $g\in\scr I_{\Sg(Y)}(Y)$ which is a non zero-divisor of $\scr O_{Y,y}$. Since $f$ takes value zero on $\Sg(X)$, $f\otimes 1$ takes value zero on $\Sg(X)\times Y$, and similarly $1\otimes g$ takes value zero on $X\times\Sg(Y)$. Thus $f\otimes g=(f\otimes 1)(1\otimes g)$ vanishes on
\begin{align}
...
Now we have $f\otimes g\in\scr I_{\Sg(X\times Y)}(X\times Y)$. By Prop. \ref{lb180}, $f\otimes g$ is a non zero-divisor of $\scr O_{X\times Y,x\times y}$. So by \ref{lb181}, $X\times Y$ is reduced at $x\times y$.
\end{proof}
...
By Thm. \ref{lb74}, $\varphi_*\scr O_X$ is $\scr O_V$-coherent. Therefore $\scr O_{X,x}$ is a finitely-generated $\scr O_{V,0}$-module since it is a direct summand of $(\varphi_*\scr O_X)_x$ (cf. Prop. \ref{lb56}). Thus, as $\scr O_{X,x}$ is Noetherian, the germ of $f$ in $\scr O_{X,x}$ is integral over $\scr O_{V,0}$ (see the argument for \eqref{eq40}), i.e.
\begin{align*}
...
Recall that each $Y_i=\varphi(X_i)$ is an analytic subset (i.e. reduced complex subspace) of $Y$, cf. Exe. \ref{lb155}. By Def. \ref{lb76} and Exe. \ref{lb155},
\begin{align*}
\scr O_{Y_i,y}=\big(\scr O_Y/\sann(\varphi_*\scr O_{X_i})\big)_y=\scr O_{Y,y}/\Ann_{\scr O_{Y,y}}(\scr O_{X_i,x}),
\end{align*}
where we have used Prop. \ref{lb56} in the last equality.  Thus $\varphi^\#:\scr O_{Y,y}\rightarrow\scr O_{X_i,x}$ restricts to an injective map $\varphi^\#:\scr O_{Y_i,y}\rightarrow \scr O_{X_i,x}$. Therefore $\scr O_{Y_i,y}$ is an integral domain since $\scr O_{X_i,x}$ is so.

By Cor. \ref{lb143}, $\dim_y Y_i>0$. Therefore $\fk m_{Y,y}\nsubset\scr I_{Y_i,y}$ for each $i$. By Lemma \ref{lb145}, there exists $g\in\fk m_{Y,y}$ which is a non-zero in $\scr O_{Y_i,y}$ for all $i$. Thus $g\circ\varphi$ is non-zero in $\scr O_{X_i,x}$. Shrink $Y$ and shrink $X$ to $\varphi^{-1}(X)$ so that $g\in\scr O(Y)$. Then, by Active lemma, $\dim_y N(g)\cap Y_i=\dim Y_i-1$ and $\dim_y N(g\circ\varphi)\cap X_i=\dim_xX_i-1$. Thus, by Prop. \ref{lb146}, $\dim_y N(g)\cap \varphi(X)=\dim_y \varphi(X)-1$ and
\begin{align*}
\dim_x N(g\circ\varphi)=\dim_xX-1.
\end{align*}
By assumption (1), $\varphi(X)$ contains a neighborhood of $y\in Y$. So
\begin{align*}
\dim_y N(g)=\dim_y Y-1.
\end{align*}
Clearly the restriction $\varphi:N(g\circ\varphi)\rightarrow N(g)$ is finite and open. Therefore, if (2) holds for this restriction then (2) holds for $\varphi:X\rightarrow Y$. This proves (2) by induction on $\dim_xX$.
\end{proof}
...
If $\mc A$ a commutative ring, we let \index{Nzd@$\Nzd(\mc A)$, the set of non zero-divisors of $\mc A$}
\begin{align}
...
\begin{enumerate}
\item $x\in\mc B$ is integral over $\mc A$ if and only if $x$ is contained in an $\mc A$-subring $\mc C\subset\mc B$  which is a finitely-generated $\mc A$-module.

\item Let $\fk A\subset \mc B$ be an $\mc A$-subring of $\mc B$. Assume that $\fk A$ is a finitely-generated $\mc A$-module. Then an element $x\in \mc B$ is integral over $\mc A$ if and only if $x$ is integral over $\fk A$.

\item If $x_1,\dots,x_n\in\mc B$ are integral over $\mc A$, then $\mc A[x_1,\dots,x_n]$ (the $\mc A$-subring of $\mc B$ generated by $x_1,\dots,x_n$) is a finitely generated $\mc A$-module.
\end{enumerate}
\end{pp}

Note that an $\mc A$-subring of $\mc B$ is a subset of $\mc B$ closed under multiplications and $\mc A$-linear combinations.

\begin{proof}
1. Let $\mc X$ be the $\mc A$-subring generated by $x$, namely $\mc X=\mc A[x]$. Then $x$ being integral means precisely that $\mc X$ is a finitely-generated $\mc A$-module. Then part 1 is obvious, because $\mc A$ is Noetherian.
...
\begin{proof}
Let $\ovl\Kbb$ be a field extension of $\Kbb$ in which $p(z)$ splits as $p(z)=(z-b_1)\cdots(z-b_n)$ where each $b_i\in\ovl\Kbb$. Since $p(b_i)=0$, each $b_i$ is integral over $\mc A$. The coefficients of $p_i(z)$ are contained in the $\mc A$-subring of $\ovl\Kbb$ generated by $b_1,\dots,b_n$, and hence are integral over $\mc A$ due to Prop. \ref{lb188}. So these coefficients are in $\wht{\mc A}$.
\end{proof}
...
Let $\ovl\Kbb$ be a field extension of $\Kbb$ in which $f\neq 0$ splits. Then $D(f)=0$ if and only if each zero of $f$ in $\ovl\Kbb$ has multiplicity $1$.
\end{co}
...
Let $X\rightarrow S$ be a branched covering where $X$ is reduced and $S$ is a connected complex manifold. Assume that there is a primitive element $e\in\scr O(X)$ over $\scr O(S)$. Then $\scr O(X)$ contains an element whose stalk at each $x\in X$ is  a universal denominator of $\Owht_{X,x}$.
\end{thm}

\begin{proof}
Let $\pi$ be $b$-sheeted, and let $\Delta\subset S$ be a branch locus. For each $t\in S\setminus\Delta$, write $\pi^{-1}(t)=\{x_1,\dots,x_b\}$, and let
...
\begin{proof}
By Thm. \ref{lb217} and local decomposition (Thm. \ref{lb125}), it suffices to assume that $X$ is irreducible at $x$. Thus, we can shrink $X$ to a neighborhood of $x$ so that (by Thm. \ref{lb170}) $X$ has pure dimension $n$ and that (by Prop. \ref{lb141}) there is a finite map $\pi:X\rightarrow S$ where $S$ is a connected open subset of $\Cbb^n$. By Thm. \ref{lb158}, $\pi$ is an open map. Thus, we may replace $S$ by $\pi(X)$ so that $\pi$ is surjective (and clearly still finite). By Thm. \ref{lb198}, $\pi$ is a branched covering. By Prop. \ref{lb54}, we may shrink $S$ further and shrink $X$ to $\pi^{-1}(S)$ so that $X$ is biholomorphic to a model space. Therefore, by Lemma \ref{lb218}, there is a primitive element $e\in\scr O(X)$ over $\scr O(S)$. So by Thm. \ref{lb222}, there is a universal denominator of $\scr O_{X,x}$. This proves the theorem with the help of Lemma \ref{lb212}.
\end{proof}
...
\begin{align*}
\frac{\send_{\scr O_X}(\scr I_{\Sg(X)})}{\send_{\scr O_X}(\scr I_{\Sg(X)})\cap\scr O_X}
\end{align*}
\end{thm}


The following proof actually shows that the theorem still holds if we replace $\scr I_{\Sg(X)}$ by $\scr I_A$ where $A$ is an arbitrary nowhere dense analytic subset of $X$ containing $\Sg(X)$.

\begin{proof}
Choose any $x\in X$. We need to show that 
\begin{align}
\Owht_{X,x}=\scr O_{X,x} \qquad\Longleftrightarrow\qquad \End_{\scr O_{X,x}}(\scr I_{\Sg(X),x})\subset \scr O_{X,x} \label{eq95}
\end{align}

Part 1. Assume $\Owht_{X,x}\neq\scr O_{X,x}$. Choose $f\in\Owht_{X,x}$ not in $\scr O_{X,x}$. Shrink $X$ to a neighborhood of $x$ so that $f\in\Owht_X(X)\subset\scr M_X(X)$, and that $\scr I_{\Sg(X)}$ is generated by finitely many sections $g_1,g_2,\dots\in\scr I_{\Sg(X)}(X)$. The polar set $P(f)$ is contained in $\Sg(X)$ due to First Riemann extension Thm. \ref{lb190}.  By Prop. \ref{lb187}, $\scr O_Xf$ is a coherent $\scr O_X$-submodule of $\scr M_X$. So $\scr O_Xf/(\scr O_Xf\cap\scr O_X)$ is coherent, and its support is $P(f)$ (cf. \eqref{eq94}) on which the functions $g_1,g_2,\dots$ vanish.

By applying Nullstellensatz (Rem. \ref{lb115}-3) to $\scr O_Xf/(\scr O_Xf\cap\scr O_X)$, we see that there is $n\in\Zbb_+$ such that $(g_i^nf)_x\in\scr O_{X,x}$ for all $i$, thus $f\scr I_{\Sg(X),x}^n\in\scr O_{X,x}$. Since $f\notin\scr O_{X,x}$, we can find the smallest $n\in\Zbb_+$ such that $f\scr I_{\Sg(X),x}^n\in\scr O_{X,x}$. Choose any $\wtd f\in f\scr I_{\Sg(X),x}^{n-1}$ not in $\scr O_{X,x}$. (But note that $\wtd f$ belongs to $\Owht_{X,x}$ since $f$ does.) Then $\wtd f\scr I_{\Sg(X),x}\in\scr O_{X,x}$. We claim that $\wtd f\scr I_{\Sg(X),x}\in\scr I_{\Sg(X),x}$. Then the multiplication of $\wtd f$ gives an element of $\End_{\scr O_{X,x}}(\scr I_{\Sg(X),x})$ not inside $\scr O_{X,x}$, which disproves the RHS of \eqref{eq95}.

Shrink $X$ further so that $\wtd f\in\Owht_X(X)$ and that $\wtd fg_i\in\scr O(X)$ for each $i$. By Thm. \ref{lb190} again, $\wtd f$ belongs to $\scr O(S\setminus\Sg(X))$ and is locally bounded on $X$. Therefore, each $\wtd fg_i$ vanishes on $\Sg(X)$, and hence must belong to $\scr I_{\Sg(X)}(X)$. This proves the claim.




Part 2. Assume $\Owht_{X,x}=\scr O_{X,x}$. Choose any $\scr O_{X,x}$-module endomorphism $\alpha$ of $\scr I_{\Sg(X),x}$. Then $\alpha$ is the multiplication by $f$ for some $f\in\scr M_{X,x}$. Indeed, since $\Sg(X)$ is thin in $X$ (Thm. \ref{lb179}), by Prop. \ref{lb123} we can find $g\in\scr I_{\Sg(X),x}$ which is a non zero-divisor of $\scr O_{X,x}$ and hence of $\scr M_{X,x}$. Set $f=\frac{\alpha(g)}{g}$. Then for each $h\in\scr I_{\Sg(X),x}$, since $\alpha$ is a homomorphism, we have
\begin{align*}
...
Since $\scr O_{X,x}$ is Noetherian and $\End_{\scr O_{X,x}}(\scr I_{\Sg(X),x})$ is a finitely-generated $\scr O_{X,x}$-module, the $\scr O_{X,x}$-submodule generated by $1,\alpha,\alpha^2,\dots$ is finitely-generated. Therefore $\alpha$ is integral over $\scr O_{X,x}$. Thus, a monic $\scr O_{X,x}$-polynomial of $f$ multiplied by the non zero-divisor $g$ is zero, which implies that $f$ is integral over  $\scr O_{X,x}$. Thus $f\in\Owht_{X,x}=\scr O_{X,x}$. Therefore $\alpha$ is the multiplication of an element of $\scr O_{X,x}$. This proves the RHS of \eqref{eq95}.
\end{proof}
...
\begin{proof}
Let $Y=\Sg(X)$ and fix $x\in X$. By Prop. \ref{lb245}, after shrunk to a neighborhood of $x$, $X$ has pure dimension $n$. Since $Y$ is nowhere dense, $Y$ has dimension $\leq n-1$ at $x$ by Ritt's lemma \ref{lb147}. Let us assume that $\dim_xY=n-1$ and find a contradiction.

By Prop. \ref{lb231}, $Y$ has a smooth point with dimension $n-1$. For the purpose of finding a contradiction, we may assume $x$ is that point. Namely, we assume $Y$ is smooth at $x$ and $\dim_xY=n-1$. \\

Step 1. Let $\{f_1,\dots,f_N\}$ be a set of non-zero generators of the ideal $\scr I_{Y,x}$. Note that they are non zero-divisors of $\scr O_{X,x}$ because $X$ is irreducible everywhere. We claim that for each $i$, the germ $(Y,x)$ is a component in the local irreducible decomposition $(N(f_i),x)=\bigcup_k (Z_k,x)$. Indeed, since $(Y,x)$ is irreducible (because $\scr O_{Y,x}\simeq\scr O_{\Cbb^n,0}$) and has decomposition $(Y,x)=\bigcup_k (Y\cap Z_k,x)$, by Rem. \ref{lb128} we have $(Y,x)=(Y\cap Z_k,x)$ for some $k$, and hence $(Y,x)\subset (Z_k,x)$. If $(Y,x)\neq (Z_k,x)$ then there is a element of $\scr I_{Y,x}\setminus\scr I_{Z_k,x}$. This element, when restricted to $(Z_k,x)$, is non-zero and hence a non zero-divisor of the integral domain $\scr O_{Z_k,x}$. Thus, by Active lemma,
\begin{align*}
\dim_x Y\leq\dim_x Z_k-1\leq \dim_x N(f_i)-1=\dim_x X-2=n-2
\end{align*} 
which is impossible. So $(Y,x)=(Z_k,x)$.

After shrinking $X$ to a neighborhood of $x$, we have that $f_i\in\scr I_Y(X)$, that (by Prop. \ref{lb113} and Rem. \ref{lb17}) $f_{1,p},\dots,f_{N,p}$ are non zero-divisors generating $\scr O_{X,p}$ for each $p\in X$, and that (by Thm. \ref{lb125}) all the germs not equal to $(Y,x)$ in the irreducible decomposition of $(N(f_i),x)$  are analytic subsets of $X$ whose intersections with $Y$ are nowhere dense in $Y$. Thus, we may pick a point $p\in Y$ close to $x$ such that $(N(f_i),p)=(Y,p)$.\\

Step 2. We assume that for some $L\leq N$, the germs of $f_1,\dots,f_L$ form a minimal set of generators of $\scr I_{Y,p}$. We claim that $L=1$. If this can be proved, then by Prop. \ref{lb178}, we have $\emb_p Y\geq \emb_pX-1$ since, in general, $\dim_\Cbb d_p(\mc I_p)$ is bounded by the number of generators of an ideal $\mc I_p$ of $\scr O_{X,p}$. Recall that in general the embedding dimensions are no less than the dimensions. Since $Y$ is smooth at $p$ and $X$ is singular at $p$ (because $p\in Y=\Sg(X)$), by Prop. \ref{lb184} we have $\emb_pY=\dim_pY=n-1$ and $\emb_p X>\dim_pX=n$. This gives a contradiction, and hence finishes the proof.\\

Step 3. Let us prove $L=1$. Assume $L\geq 2$. Then by the assumption of minimality on $\{f_1,\dots,f_L\}$, the stalks of $f_1/f_2$ and $f_2/f_1$ in $\scr M_{X,p}$ are not inside $\scr O_{X,p}$. We assume that there is a sequence $(p_n)_{n\in\Zbb_+}$ in $X\setminus Y$ converging to $p$ such that
\begin{align}
\sup_n|f_1(p_n)/f_2(p_n)|<+\infty,  \label{eq96}
\end{align}
otherwise there must exist such a sequence for $f_2/f_1$.

Clearly, there exists $k\in\Zbb_+$ such that $(f_{1,p}/f_{2,p})\cdot\scr I_{Y,p}^k\subset\scr O_{X,p}$. We let $k$ be the smallest such number, and find $g=f_{i_1,p}\cdots f_{i_{k-1},p}$ (where $1\leq i_1,\dots,i_{k-1}\leq L$) such that $h=(f_{1,p}/f_{2,p})\cdot g\notin\scr O_{X,p}$. For each $f_i$,  by \eqref{eq96}, $hf_i\in\scr O_{X,p}$ vanishes on $p$. But $N(hf_i)\subset (Y,p)$ because $(N(f_j),p)=(Y,p)$ for each $j$. Thus, $N(hf_i)$ is a germ of analytic subset passing through $p$. Since (by Active lemma) both $(N(hf_i),p)$ and $(Y,p)$ have dimension $n-1$, these two germs must be equal. (Otherwise there is a non-zero element of the integral domain $\scr O_{Y,p}$ vanishing on $(N(hf_i),p)$, which contradicts Active lemma.) We conclude that $hf_{i,p}\in\scr I_{Y,p}$ for all $i$. Therefore $h\scr I_{Y,p}\subset\scr I_{Y,p}$. As in the proof of Thm. \ref{lb227}, we have $h\in\Owht_{X,p}=\scr O_{X,p}$, which is impossible. 
\end{proof}
...
Assume that $X$ is irreducible. Then any analytic subset $A$ of $X$ is either nowhere dense in $X$ or $A=X$.
\end{pp}

\begin{proof}
First assume that $X$ is a connected complex manifold. Then by Lemma \ref{lb176}, either $\scr I_A=0_X$ (namely, $A=X$) or $\scr I_{A,x}\neq 0$ for each $x\in X$. In the latter case, $A$ is clearly nowhere dense in $X$.

Now assume that $X$ is irreducible but not necessarily smooth. Since $X\setminus\Sg(X)$ is connected, the smooth case implies that either $A\supset X\setminus\Sg(X)$ or $A\setminus\Sg(X)$ is nowhere dense in $X\setminus\Sg(X)$. In the former case, clearly $A=X$. In the latter case, one checks easily that $A$ is nowhere dense in $X$.
\end{proof}




\begin{co}\label{lb240}
Assume that $X$ is irreducible, and let $\mc I$ be a coherent ideal of $\scr O_X$. If $\mc I\neq 0_X$, then $\mc I_x$ contains a non zero-divisor of $\scr O_{X,x}$ for every $x\in X$.
\end{co}

In the special case that $\mc I=f\scr O_X$ where $f\in\scr O(X)$, this corollary implies that if $f$ is not constantly zero on $X$ then $f$ is not zero when restricted to any non-empty open subset of $X$. Thus, this corollary generalizes Lemmas \ref{lb68} and \ref{lb176}.


\begin{proof}
Let $A=N(\mc I)$. If $\mc I\neq 0_X$, then $A\neq X$. So by Prop. \ref{lb239}, $A$ is nowhere dense in $X$. So by Prop. \ref{lb123}, $\mc I_x\cap\Nzd(\scr O_{X,x})\neq\emptyset$ for each $x\in X$.
\end{proof}


\index{00@Open mapping theorem}
\begin{thm}[\textbf{Open mapping theorem}] \label{lb242} 
Assume that $X$ is irreducible and $f\in\scr O(X)$ is not a constant function. Then the map $f:X\rightarrow\Cbb$ is open. In particular, the absolute value function $|f|:X\rightarrow[0,+\infty)$ does not achieve maximum when restricted to any open subset of $X$.
\end{thm}

\begin{proof}
Since $f$ is not constant, by Cor. \ref{lb240}, for each $x\in X$, $f_x$ is not constant in $\scr O_{X,x}$. Thus $f$ is open at $x$ by Cor. \ref{lb241}. 
\end{proof}
...
\begin{enumerate}[label=(\arabic*)]
\item For each $n\in\Nbb$, $\Phi^n$ is functorial (natural). Namely, for each morphism $\mc E\rightarrow\mc F$ of $\fk A$-objects, the diagram commutes
\begin{equation*}
\begin{tikzcd}
H^n(\mc E) \arrow[r] \arrow[d,"\Phi^n"',"\simeq"] & H^n(\mc F) \arrow[d,"\Phi^n","\simeq"'] \\
...
\begin{equation*}
\begin{tikzcd}
H^n(\mc G) \arrow[r,"\delta^n"] \arrow[d,"\Phi^n"',"\simeq"] & H^{n+1}(\mc E) \arrow[d,"\Phi^n","\simeq"'] \\
...
\begin{df}
A $\delta$-functor $(H^\blt,\delta^\blt)$ from $\fk A$ to $\fk B$ is called \textbf{universal} \index{00@Universal $\delta$-functors} if for any other $\delta$-functor $(\wch H^\blt,\delta^\blt)$, any functorial morphism of functors $\Phi^0:H^0\rightarrow\wch H^0$ can be extended uniquely to a morphism of $\delta$-functors $\Phi^\blt:(H^\blt,\delta^\blt)\rightarrow(\wch H^\blt,\delta^\blt)$. Universal cohomological contravariant functors are defined in a similar way.

A homological covariant $\delta$-functor $(H_\blt,\delta_\blt)$ from $\fk A$ to $\fk B$ is called \textbf{universal} if for any other homological covariant $\delta$-functor $(\wch H_\blt,\delta_\blt)$, any functorial morphism of functors $\Phi_0:\wch H_0\rightarrow H_0$ can be extended uniquely to a morphism of homological $\delta$-functors $\Phi_\blt:(\wch H_\blt,\delta_\blt)\rightarrow (H_\blt,\delta_\blt)$.   \hfill\qedsymbol
\end{df}
...
\begin{proof}
We prove the theorem only for cohomological covariant $\delta$-functors, since the other cases can be treated in a similar way. Choose a $\delta$-functor $(\wch H^\blt,\delta^\blt)$. We construct $\Phi^n$ and verifies the desired properties by induction on $n$. The case $n=0$ is obvious. Assume the unique functorial morphisms $\Phi^0,\dots,\Phi^n$ intertwinined by $\delta^0,\dots,\delta^{n-1}$ are constructed. Let us construct a unique functorial $\Phi^{n+1}$ such that $\delta^n$ intertwines $\Phi^n$ and $\Phi^{n+1}$. \\

Step 1. For each $\mc E$, find  a monomorphism $\mc E\hookrightarrow\mc E^0$ such that $H^{>0}(E^0)=0$. Then we have an exact sequence $0\rightarrow\mc E\rightarrow\mc E^0\rightarrow \mc E^0/\mc E\rightarrow 0$. Since $H^{n+1}(\mc E^0)=0$, by Rem. \ref{lb47}, there is a unique morphism $\Phi^{n+1}:H^{n+1}(\mc E)\dashrightarrow\wch H^{n+1}(\mc E)$ which yields a morphism of long exact sequences
...
\end{tikzcd}
\end{equation}
where the horizontal arrows are injective and $H^{>0}(\mc E^0)=H^{>0}(\mc F^0)=0$. By Rem. \ref{lb47} again, this diagram can be extended uniquely to a morphism of exact sequences
\begin{equation*}
\begin{tikzcd}
...
\end{tikzcd}
\end{equation}
By the assumption on case $n$, the left front and the left back rectangles commute, and the left and the middle vertical cells commute. The right front and the right back rectangles commute due to the construction of $\Phi^{n+1}$ in Step 1. The top and the bottom four horizontal cells commute by the definition of $\delta$-functors. Since $H^{n+1}(\mc E^0)=0$, the morphism $H^n(\mc E^0/\mc E)\rightarrow H^{n+1}(\mc E)$ on the top is surjective. Therefore the rightmost vertical (green) parallelogram commutes. To summarize, we have a commutative diagram
...
\end{tikzcd}
\end{equation}
This proves that $\Phi^{n+1}$ is functorial, once we have shown that $\Phi^{n+1}$ is independent of the choice of inclusions $\mc E\hookrightarrow\mc E^0$. 
...
Due to the functoriality of $\Phi^n$ and $\Phi^{n+1}$, the left front and the back front rectangle commute, and the three vertical cells commute. By the definition of $\delta$-functors, the top and the bottom four horizontal cells commute. By the construction of $\Phi^n$ in Step 1, the right front rectangle commutes. Therefore, since $\wch H^{n+1}(\mc E)\rightarrow\wch H^{n+1}(\mc E)$ is the identity, the right back (green) rectangle commutes.
\end{proof}
...
Thus, by \eqref{eq105},
\begin{align}
R^1(\mc E)\simeq\frac{T(\Ker\varphi^1)}{\Imag\big(T(\mc E^0)\xrightarrow{T(\varphi^0)} T(\Ker\varphi^1) \big)}\label{eq107}
\end{align}
...
\end{align*}
Therefore, $T(\iota^1)$ sends $T(\Ker\varphi^1)$ isomorphically to $\Ker\big(T(\mc E^1)\rightarrow T(\mc E^2)\big)$, and sends the bottom of the RHS of \eqref{eq107} isomorphically to the image of $T(\iota^1\circ\varphi^1):T(\mc E^0)\rightarrow T(\mc E^1)$. Thus, by \eqref{eq107} we obtain an isomorphism
\begin{align}
...
\begin{align}
R^nT(\mc E)\simeq \frac{\Ker\big(T(\mc E^n)\rightarrow T(\mc E^{n+1})\big)}{\Imag\big(T(\mc E^{n-1})\rightarrow T(\mc E^n)\big)} 
\end{align}
...
Suppose that $\mc E$ has a left resolution \eqref{eq110} such that $L_\blt T$ vanishes on $\mc E_\blt$. Then we have a short exact sequence
\begin{align*}
...
\end{gather}
Thus $L_1(\mc E)\simeq\Ker\big(T(\Imag(\varphi_1))\xrightarrow{T(\iota_1)} T(\mc E_0)\big)$. Since $L$ is right exact and $\varphi_1:\mc E_1\twoheadrightarrow\Imag\varphi_1$ is surjective, the first morphism in the following non-necessarily exact sequence is surjective
\begin{equation*}
\begin{tikzcd}
...
Step 1 of the following proof is important: it gives an explicit way of constructing derived functors using resolutions.
...
\end{align*}
Each $T\in\mc W$ gives rise to a canonical $\mc A$-module morphism $\Hom_\Cbb(\mc W,\mc V)\rightarrow \mc V$, whose composition with $\Phi$ is the desired $S$.
\end{proof}


\begin{rem}
The above lemma can be easily generalized: assume $\mc A$ is a $\mc B$-ring, namely, $\mc A,\mc B$ are rings and a ring homomorphism $\mc B\rightarrow\mc A$ is fixed. Let $\mc M,\mc N$ be $\mc A$-modules and $\mc V$ be $\mc B$-modules. Then we have a canonical $\mc A$-module isomorphism
\begin{align*}
...
\end{tikzcd}
\end{equation*}
By Subsec. \ref{lb254} or Step 1 in Subsec \ref{lb256}, $\Ext^\star(\mc M,\mc F)=\mc H^\star(\Hom(\mc M,\mc F^\blt))$ and the same relation holds if we replace $\mc M$ with $\mc N$. Thus $\mc H^\star$ acting on the above morphism of complexes defines a morphism $\Ext^n(\mc M,\mc F)\leftarrow\Ext^n(\mc N,\mc F)$ for all $n\in\Nbb$.
...
It is not hard to check that taking direct limit is an exact functor from the category of direct systems of $\mc A$-modules to $\Mod(\mc A)$. Namely, if $(\mc E_i)_{i\in I},(\mc F_i)_{i\in I},(\mc G_i)_{i\in I}$ are direct systems in $\Mod(\mc A)$, an if we have an exact sequence of morphisms of direct systems
\begin{align*}
...
\begin{proof}
We only prove the first relation, since the other two can be proved in a similar way. Choose an projective resolution $\mc F_\blt\rightarrow\mc F\rightarrow 0$. Then we have a chain complex of systems in $\mc A$
\begin{align*}
...
\begin{align*}
\Tor_1^{\mc A}(\mc E,\mc M)\rightarrow \mc E\otimes_{\mc A}\mc M\rightarrow\mc E\otimes_{\mc A}\mc N\rightarrow\mc E\otimes_{\mc A}\mc P\rightarrow0
\end{align*}
where the first term is $0$. So (1) follows.

(3)$\Rightarrow$(4): Obvious.

(4)$\Rightarrow$(3): Assume (3). Since any $\mc A$-module is a union (i.e. direct limit) of its finitely-generated $\mc A$-submodules, by Prop. \ref{lb259}, it suffices to show that $\Tor_1^{\mc A}(\mc E,\mc F)=0$ whenever $\mc F$ is finitely generated. We prove this by induction on $n$, the minimal number of elements generating $\mc F$. The case $n=0$ is trivial. Assume case $\leq n-1$ is proved. Let $\mc F$ be $\mc A$-generated by $n$ elements, and let $\mc F'$ be its submodule generated by the first $n-1$ elements. Denote the last element by $x$. Then we have an exact sequence
\begin{align*}
...
\begin{eg}
Let $\varphi:X\rightarrow Y$ and $Y\rightarrow Z$ be holomorphic maps of complex spaces. Let $\scr E$ be an $\scr O_X$-module. Let $x\in X$. Suppose that $\psi$ is flat at $\varphi(x)$ and $\scr E$ is $\varphi$-flat at $x$. The $\scr E$ is clearly $(\psi\circ\varphi)$-flat at $x$.
\end{eg}


\begin{eg}
Let $\mc I$ be an idea of $\scr O_Y$, and let $X=\Specan(\scr O_Y/\mc I)$. Suppose $x\in X$ (i.e. $x\in N(\mc I)$) and $\mc I_x\neq 0$, then $\varphi$ is not flat at $x$.

Indeed, if $\varphi$ is flat at $x$, then $\scr O_{X,x}=\scr O_{Y,x}/\mc I_x$ is not $\scr O_{Y,x}$-flat. By Exp. \ref{lb262},
\begin{align*}
0=\Tor^{\scr O_{Y,x}}_1(\scr O_{Y,x}/\mc I_x,\scr O_{Y,x}/\mc I_x)=\mc I_x/\mc I_x^2.
\end{align*}
Therefore $\mc I_x^2=\mc I_x$, and hence $\mc I_x^n=\mc I_x$ for all $n\in\Zbb_+$. Thus $\mc I_x\subset\bigcap_{n\in\Zbb_+}\fk m_{Y,x}^n=0$ by Krull's intersection Thm. \ref{lb162}. This contradicts the assumption $\mc I_x\neq0$. \hfill\qedsymbol
\end{eg}
...
\begin{proof}
We shall prove $\Tor_1^{\mc B}(\mc E,\mc B/J)=0$ for each ideal $J\subset\mc B$. Namely (Exp. \ref{lb265}), we shall prove that $\mc E\otimes_{\mc B}J\rightarrow\mc E\otimes_{\mc B}\mc B$ is injective. The natural idea is to tensor it by $\mc B/\fk n^k$, where we choose $k\geq k_0$. But this is not a good choice, since $J\rightarrow\mc B$ tensored by $\mc B/\fk n^k$ is not even injective. Indeed, by Exp. \ref{lb262}, if we tensor $\mc B/\fk n^k$ with the short exact sequence $0\rightarrow J\rightarrow\mc B\rightarrow\mc B/J$, we get a long one
\begin{align}
...
Let $\varphi:X\rightarrow S$ and $\psi:Y\rightarrow S$ be holomorphic maps, and let $\scr E$ be a finite-type $\scr O_X$-module. Consider the Cartesian product
\begin{equation*}
\begin{tikzcd}
...
\begin{proof}
We first consider the special case that $\psi$ is finite. By Thm. \ref{lb75}, we can shrink $X,S$ to neighborhoods of $x,t$ respectively and replace $Y$ by $\psi^{-1}(S)$, so that $x$ is the single point of the set $\varphi^{-1}(t)$. Then $(x,y)$ is the single point of the set $\pr_Y^{-1}(t)$. By Prop. \ref{lb56}, we have isomorphisms of $\scr O_{S,t}$-modules and of $\scr O_{Y,y}$-modules
\begin{align*}
...
\begin{proof}
Indeed, since $\tau$ is a non zero-divisor of $\mc B$, we have a short exact sequence
\begin{align*}
0\rightarrow\mc B\xrightarrow{\times\tau}\mc B\rightarrow\mc B/\tau\mc B\rightarrow 0
\end{align*}
which, tensored by $\mc M$, gives a commutative diagram where the horizontal lines are exact
\begin{equation*}
\begin{tikzcd}[sep=small]
0 \arrow[r] & \Tor_1^{\mc B}(\mc M,\mc B/\tau\mc B) \arrow[r] & \mc M\otimes_{\mc B}\mc B \arrow[r,] \arrow[d, "\simeq"'] & \mc M\otimes_{\mc B}\mc B \arrow[r] \arrow[d, "\simeq"'] & \mc M\otimes_{\mc B}(\mc B/\tau\mc B) \arrow[r] \arrow[d, "\simeq"'] & 0 \\
            &             & \mc M \arrow[r,"\times\tau"]                      & \mc M \arrow[r]                      & \mc M/\tau\mc M                               &  
\end{tikzcd}
\end{equation*}
This shows
\begin{align}
...
Cor. \ref{lb276} can be easily generalized to a criterion on the flatness of a holomorphic map from a complex space to a complex manifold (Cor. \ref{lb279}).
...
We are mainly interested in the case that $(\mc A,\mk)$ is a Noetherian local ring and $a_1,\dots,a_n\in\mk$. In this case, if $\mc E$ is a non-zero finitely-generated $\mc A$-module, then $\mk\mc E\neq\mc E$ by Krull's intersection Thm. \ref{lb162}. Then condition (2) is redundant.


\begin{co}\label{lb279}
Let $\scr E$ be an $\scr O_X$-module, and let $f_1,\dots,f_n\in\scr O(X)$. Let $x\in X$. Set $F=(f_1,\dots,f_n):X\rightarrow\Cbb^n$. Then $\scr E$ is $F$-flat at $x$ if and only if the germs at $x$ of $f_1-f_1(x),\dots,f_n-f_n(x)$ form an $\scr E_x$-regular sequence.
\end{co}



\begin{proof}
By induction on $n$. The case $n=0$ is obvious. Assume case $n-1$ holds where $n\in\Zbb_+$. Now consider case $n$. Let $(z_1,\dots,z_n)$ be the standard coordinates of $\Cbb^n$. Then $\Cbb^{n-1}\simeq 0\times\Cbb^{n-1}$ is $\Specan(\scr O_{\Cbb^n}/z_1\scr O_{\Cbb^n})$. Note that that $z_1$ is a non zero-divisor of $\scr E_x$ is the same as that $f_1$ is a non zero-divisor of $\scr E_x$. Thus, by Slicing criterion \eqref{lb264}, $\scr E$ is $F$-flat at $x$ if and only if $f_1$ is a non zero-divisor of $\scr E_x$ and $\scr E|_{F^{-1}(\Cbb^{n-1})}\simeq\scr E/z_1\scr E$ is flat over $\Cbb^{n-1}$ at $x$. By induction, the second condition is equivalent to that $f_2-f_2(x),\dots,f_n-f_n(x)$ is an $(\scr E_x/z_1\scr E_x)$-regular sequence.
\end{proof}


We close this section with another interesting application of Thm. \ref{lb274}. In this application, the $(\mc B,\nk)$ and $(\mc A,\mk)$ in Thm. \ref{lb274} are set to be equal.
...
\begin{proof}
Clearly (1)$\Rightarrow$(2). By Thm. \ref{lb253}, if $\mc E$ is projective then $\Tor_{>0}^{\mc A}(\mc E,-)=0$. Therefore   (2)$\Rightarrow$(3).

Assume (3). Choose $x_1,\dots,x_n\in\mc E$ forming a basis of the $(\mc A/\mk)$-vector space $\mc E\otimes_{\mc A}(\mc A/\mk)$. By Nakayama's lemma \ref{lb281}, the morphism $\mc A^n\rightarrow\mc M$ sending the $j$-th basis of $\mc A^n$ to $x_j$ is surjective. Let $\mc N$ be the kernel of this morphism. Then the short exact sequence $0\rightarrow\mc N\rightarrow\mc A^n\rightarrow\mc E\rightarrow0$ gives a long one
\begin{align*}
...
\end{align*}
That the second last morphism is an isomorphism is due to the fact that $x_1,\dots,x_n$ are a basis. Since $\mc E$ is flat, $\Tor_1^{\mc A}(\mc A/\mk,\mc E)=0$. So $\mc N\otimes_{\mc A}(\mc A/\mk)=0$. Hence $\mc N=0$ by Nakayama's lemma. Therefore $\mc A^n\rightarrow\mc M$ is an isomorphism. This proves (1).
\end{proof}
...
By Rem. \ref{lb148}, we may shrink $Y$ to a neighborhood of $y$ and shrink $X$ to $\varphi^{-1}(Y)$ so that there exists $\tau\in\scr O_Y$ which is a non zero-divisor of $\scr O_{Y,y}$. Let $Y'=\Specan(\scr O_Y/\tau\scr O_Y)$ and $X'=\varphi^{-1}(Y')=\Specan(\scr O_X/\tau\scr O_X)$. By Active lemma, 
\begin{align*}
\dim_y Y'=\dim_yY-1.
\end{align*}
...
\begin{proof}
By Prop. \ref{lb121}, we may shrink $X$ to a neighborhood of $x$ so that $Z$ is nowhere dense in $X$. We claim that $\sqrt{0_{X,x}}\subset f_x\sqrt{0_{X,x}}$. Then $\sqrt{0_{X,x}}\subset \mk_{X,x}\sqrt{0_{X,x}}$, and hence $\sqrt{0_{X,x}}\subset \mk_{X,x}^k\sqrt{0_{X,x}}$ for all $k\in\Nbb$. By Krull's intersection Thm. \ref{lb162}, we conclude $\sqrt{0_{X,x}}=0$. Hence $X$ is reduced at $x$.

To prove the claim, choose any $g\in\sqrt{0_{X,x}}$. By shrinking $X$ further, we have $g\in\scr O(X)$ and $g$ takes value $0$ at any point of $X$. In particular, $g$ vanishes on $N(f)$.  So $g_x\in f_x\scr O_{X,x}$ because $\scr O_{X,x}/f_x\scr O_{X,x}$ is reduced by assumption. Shrink $X$ so that $g=fh$ where $h\in\scr O(X)$. Since $N(f)$ is nowhere dense in $X$ and $f$ vanishes at every point, $h$ also vanishes at every point. Thus $h_x\in\sqrt{0_{X,x}}$. 
\end{proof}
...
\begin{df}
Let $\mc E\in\Mod(\mc A)$. An \textbf{associated prime} \index{00@Associated primes} of $\mc E$ is a prime ideal $\pk$ of the form $\Ann_{\mc A}(\xi)$ where $\xi\in\mc E$. The set of associated primes is denoted by $\Ass(\mc E)$. \index{Ass@$\Ass(\mc E)$, the set of associated primes}
\end{df}
...
By Cor. \ref{lb291}, we know that if $\mc A$ is reduced, then the prime ideals in the prime decomposition of $0\subset\mc A$ form the set $\Ass(\mc A)$. More generally, if $I$ is an ideal of $\mc A$ such that $I=\sqrt I$, and we let $I=\pk_1\cap\cdots\cap\pk_N$ be the prime decomposition of $I\subset\mc A$, then $\pk_1,\dots,\pk_N$ are precisely the associated primes of the $\mc A$-module $\mc A/I$.
\end{eg}
...
\begin{proof}
Clearly $\Ann_{\mc A}(a\xi)\supset \Ann_{\mc A}(\xi)$ in general. Suppose that $\Ann_{\mc A}(\xi)$ is prime. If $b\in \Ann_{\mc A}(a\xi)$, then $ab\xi=0$, and hence $ab$ belongs to the prime ideal $\Ann_{\mc A}(\xi)$. Since $a\xi\neq 0$, $a\notin\Ann_{\mc A}(\xi)$. So $b\in\Ann_{\mc A}(\xi)$. This proves (2).

Conversely, if $\Ann_{\mc A}(\xi)$ is not prime, then there is $a,b\in\mc A$ not in $\Ann_{\mc A}(\xi)$ such that $ab\in\Ann_{\mc A}(\xi)$. Namely, $a\xi\neq0$, $b\xi\neq 0$, and $ab\xi=0$. Then $b$ belongs to $\Ann_{\mc A}(a\xi)$ but not to $\Ann_{\mc A}(\xi)$.
\end{proof}
...
\begin{proof}
The Noether property of $\mc A$ implies that any chain of the partially ordered set $\{\Ann_{\mc A}(a\xi):a\in\mc A,a\xi\neq0\}$ must be stationary, and hence has an upper bound. By Zorn's lemma, this set contains a maximal element, which we denote by $\Ann_{\mc A}(a\xi)$. If $b\in\mc A$ and $ab\xi\neq 0$, then the maximality shows that $\Ann_{\mc A}(ab\xi)=\Ann_{\mc A}(\xi)$. Therefore $\Ann_{\mc A}(a\xi)$ is prime by Lemma \ref{lb289}.
\end{proof}


\begin{co}\label{lb294}
Let $\mc E\in\Mod(\mc A)$. Let $x\in\mc A$. Then the following are equivalent.
\begin{enumerate}[label=(\arabic*)]
\item $x$ is a non zero-divisor of $\mc E$.
\item $x$ does not belong to any associated prime of $\mc E$.
\end{enumerate}
\end{co}

Note that this property generalizes Cor. \ref{lb120}.

\begin{proof}
If $x$ belongs to an associated prime $\Ann_{\mc A}(\xi)$ then $\xi\neq0$ and $x\xi=0$. So $x$ is a zero-divisor of $\mc E$. Conversely, if $x$ is a zero-divisor of $\mc E$, then $a\xi=0$ for some non-zero $\xi\in\mc E$. By Prop. \ref{lb290}, we may find $a\in\mc A$ so that $\Ass_{\mc A}(a\xi)$ is prime, which clearly contains $\xi$.
\end{proof}


According to the above corollary, associated primes are useful for studying the zero-divisors of modules. But why are we interested in those $\Ann_{\mc A}(\xi)$ that are prime? A main reason is that any finitely generated $\mc A$-module has finitely many associated primes. To prove this important fact,  we first need a lemma.

\begin{lm}\label{lb293}
Let $0\rightarrow\mc E'\rightarrow\mc E\rightarrow\mc E''\rightarrow0$ be an exact sequence in $\Mod(\mc A)$. Then $\Ass(\mc E)\subset\Ass(\mc E')\cup\Ass(\mc E'')$.
\end{lm}

\begin{proof}
Choose $\xi\in\mc E$ such that $\Ann_{\mc A}(\xi)\in\mc E$. We view $\mc E''$ as $\mc E/\mc E'$, and let $[\xi]$ be the residue class of $\xi$ in $\mc E/\mc E''$. Then $\Ann_{\mc A}([\xi])=\{a\in\mc A:a\xi\in\mc E'\}$. Clearly $\Ann_{\mc A}(\xi)\subset\Ann_{\mc A}([\xi])$. Assume that $\Ann_{\mc A}(\xi)$ does not belong to $\Ass(\mc E'')$. Then $\Ann_{\mc A}(\xi)\neq\Ann_{\mc A}([\xi])$, otherwise $\Ann_{\mc A}([\xi])$ would be an associated prime of $\mc E''$. 
...
\begin{proof}
If $\mc N$ is a submodule of $\mc E$ and $\mc N\neq\mc E$, then by the fact that $\mc E/\mc N$ has at least one associated prime, we can find $\xi\in\mc E\setminus\mc N$ such that $\Ann_{\mc A}([\xi])$ is prime. Here $[\xi]$ denotes the residue class of $\xi$ in $\mc N$. Let $\mc N_1$ be generated by $\mc N$ and $\xi$. Then $\mc N_1/\mc N\simeq \mc A/\Ann_{\mc A}([\xi])$ has $\Ann_{\mc A}([\xi])$ as its only associated prime (cf. Exp. \ref{lb292}).

The above discussion shows that we can find a chain of submodules $0=\mc E_0\subsetneq\mc E_1\subsetneq\mc E_2\subsetneq\cdots$ of $\mc E$ such that each $\mc E_{i+1}/\mc E_i$ is equivalent to $\mc A/\pk$ for some prime ideal $\pk$. Since $\mc E$ is finitely-generated and $\mc A$ is Noetherian, this chain must have finite length. So it is of the form $0=\mc E_0\subsetneq\mc E_1\subsetneq\mc E_2\subset\cdots\subsetneq\mc E_n=\mc E$.  We know that $\mc E_{i+1}/\mc E_i$ has only one associated prime. Thus, if $\mc E_i$ has finitely many associated primes, then so does $\mc E_{i+1}$ by Lemma \ref{lb293}. Thus, by induction, $\mc E=\mc E_n$ has finitely many associated primes.
\end{proof}
...
\end{align*}
It follows that $\Hom_{\mc A}(\mc A/\mk,\mc E)$ is non-zero iff $\mk$ is an associated prime of $\mc E$.

Assume that (2) does not hold. Then $\mk$ is an associated prime. Write $\mk=\Ann_{\mc A}(\xi)$ where $\xi\neq 0$. Then every element of $\mk$ annihilates $\xi$. So every element of $\mk$ is a zero-divisor of $\mc E$. So (1) does hot hold.

Assume that (2) holds. Then $\mk$ is not an associated prime. By Thm. \ref{lb295}, $\mc E$ has finitely may associated primes $\pk_1,\dots,\pk_N$, which are all proper subspaces of $\mk$. In the special case that $\mc A$ is a local $\Cbb$-algebra (which is indeed the only situation we will encounter in out notes), $\mk$ does not equal any finite union of proper subspaces. So we can find $a\in\mk$ not inside $\pk_1\cup\cdots\cup\pk_N$. Then by Cor. \ref{lb294}, $a$ is a non zero-divisor of $\mc E$. Therefore (1) holds. In the general case, that $\mk\neq\pk_1\cup\cdots\cup\pk_N$ follows from the prime avoidance property. We refer the readers to (for instance) \cite[Sec. 11.2]{Vak17} for details.  
\end{proof}
...
\begin{lm}\label{lb298}
Let $\mc E\in\Mod(\mc A)$, and let $a\in\mk$ be a non zero-divisor of $\mc E$. Then for each $n\in\Nbb$,
\begin{align*}
\Ext_{\mc A}^n(\mc A/\mk,\mc E/a\mc E)\simeq \Ext_{\mc A}^{n+1}(\mc A/\mk,\mc E)
\end{align*}
\end{lm}
...
\begin{align*}
&\cdots\rightarrow\Ext_{\mc A}^n(\mc A/\mk,\mc E)\rightarrow\Ext_{\mc A}^n(\mc A/\mk,\mc E)\rightarrow\Ext_{\mc A}^n(\mc A/\mk,\mc E/a\mc E)\rightarrow\Ext_{\mc A}^{n+1}(\mc A/\mk,\mc E)\\
&\rightarrow\Ext_{\mc A}^{n+1}(\mc A/\mk,\mc E)\rightarrow\Ext_{\mc A}^{n+1}(\mc A/\mk,\mc E)\rightarrow\cdots
\end{align*}
By Rem. \ref{lb297}, the $\Ext$ functor preserves multiplications on both components since $\Hom$ clearly does. This implies that the morphism $\Ext_{\mc A}^n(\mc A/\mk,\mc E)\rightarrow\Ext_{\mc A}^n(\mc A/\mk,\mc E)$ in the above long exact sequence, which is $\Ext_{\mc A}^n(\mc A/\mk,\times a)$, is simply the action of $a$ on the $\mc A$-module $\Ext_{\mc A}^n(\mc A/\mk,\mc E)$. It also equals $\Ext_{\mc A}^n(\times a,\mc E)$, which is indeed zero since $\mc A/\mk\xrightarrow{\times a}\mc A/\mk$ is zero. So $\Ext_{\mc A}^n(\mc A/\mk,\times a)$ is zero, and similarly $\Ext_{\mc A}^{n+1}(\mc A/\mk,\times a)$ is zero. Therefore, by the exactness of the above sequence, the connecting map $\Ext_{\mc A}^n(\mc A/\mk,\mc E/a\mc E)\rightarrow\Ext_{\mc A}^{n+1}(\mc A/\mk,\mc E)$ must be an isomorphism.
\end{proof}


Now we can generalize Lemma \ref{lb296} as follows.

\begin{pp}\label{lb299}
...
\item $\Ext_{\mc A}^k(\mc A/\mk,\mc E)=0$.
\end{enumerate}
\end{pp}
...
By Prop. \ref{lb298},
\begin{align*}
&\Ext_{\mc A}^k(\mc A/\mk,\mc E)\simeq\Ext_{\mc A}^{k-1}(\mc A/\mk,\mc E/a_1\mc E)\simeq\Ext_{\mc A}^{k-2}(\mc A/\mk,\mc E/(a_1\mc E+a_2\mc E))\\
\simeq&\cdots\simeq\Hom_{\mc A}(\mc A/\mk,\mc E/(a_1\mc E+\cdots+a_k\mc E))
\end{align*}
By Lemma \ref{lb296}, the last term is zero if and only if $\mc E/(a_1\mc E+\cdots+a_k\mc E)$ has a non zero-divisor in $\mk$. This is equivalent to saying that (1) holds.
\end{proof}
...
\begin{proof}
Apply Prop. \ref{lb299} to any longest $\mc E$-regular sequence in $\mk$.
\end{proof}
...
\begin{proof}
By induction, it suffices to prove the case $k=1$. Then the formula follows from Lemma \ref{lb298} and Thm. \ref{lb300}.
\end{proof}



\begin{co}
Let $\mc E\in\Mod(\mc A)$ be finitely generated with depth $n$. Then any $\mc E$-regular sequence in $\mk$ of length $k<n$ can be extended to one of length $n$.
\end{co}

\begin{proof}
Immediate from Cor. \ref{lb305}.
\end{proof}
