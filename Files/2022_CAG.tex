...
\begin{align}
6\varphi^*\psi_*\scr O_Y\simeq\wtd\psi_*\scr O_Z. \label{eq176}
\end{align}
...
Choose any precompact open subset $W\subset S$. Then there exists $q_0\in\Nbb$ independent of $\scr E\in\Coh(\scr O_X)$ such that for every $q> q_0$, $H^q(\varphi^{-1}(V),\scr E)=0$ and $H^q(X_t,\scr E|_{X_t})=0$ whenever $V$ is a Stein open subset of $W$ and $t\in W$. In particular, for each $t\in W$, $R^q\varphi_*(\scr E)_t=0$.
\end{rem}
