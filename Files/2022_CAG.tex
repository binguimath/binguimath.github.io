...
\begin{proof}
Choose any $x\in X$ and let $y=\varphi(x)$. To show that $f$ is open at $x$, we may shrink $Y$ to a neighborhood of $y$ biholomorphic to an open subset of $\Cbb$ and shrink $X$ to $f^{-1}(Y)$. So we may assume that $Y$ is open in $\Cbb$. Since $f$ is not constant, by Cor. \ref{lb240}, $f_x$ is not constant in $\scr O_{X,x}$. Thus $f$ is open at $x$ by Cor. \ref{lb241}. 
\end{proof}
