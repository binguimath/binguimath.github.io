...
Let $X$ be a complex space such that $\red(X)$ has local decomposition $\red(X)=X_1\cup\cdots\cup X_N$ at $x\in X$. Let $Y$ be an analytic subset of $X$ irreducible at $x$. Define the \textbf{codimension} of $(Y,x)$ in $X$ to be \index{00@Codimension} \index{codim@$\codim_{(Y,x)}X$}
\begin{align}
\codim_{(Y,x)}X=\sup_{
\begin{subarray}{c}
1\leq i\leq N\\
...
\item After shrinking $X$ to a neighborhood of $x$, there is a nowhere dense analytic subset $A\subset Y$ such that for each $p\in Y\setminus A$, the ideal $\scr I_{Y,p}$ of $\scr O_{X,p}$ is generated by $\codim_{(Y,x)}X$ elements.
\end{enumerate}
\end{thm}
...
\end{align}
Let $n=\dim_x B$ and $m=\dim_xY$. Then by Prop. \ref{lb146} and \eqref{eq196}, we have $\codim_{(Y,x)}X=n-m$. 
...
\begin{align}
\codim_{(Y,x)}X\leq\text{ the smallest number of generators of }\scr I_{Y,p}
\end{align}
...
We remark that in the viewpoint of algebraic geometry, the prime ideal $\scr I_{Y,x}$ is a point of the scheme $\mathrm{Spec}(\scr O_{X,x})$, and $\codim_{(Y,x)}X$ is the Krull dimension of the localization of $\scr O_{X,x}$ at $\scr I_{Y,x}$.   \hfill\qedsymbol
\end{rem}
...
Let $Y$ be an analytic subset of $X$, and choose $x\in X$. Assume that $\dim_xX=n$ and $\dim_x Y=n-1$, and that both $X$ and $Y$ are irreducible at $x$. Choose a nonzero $f\in\scr I_{Y,p}$, and let $(N(f),x)=\bigcup_k (Z_k,x)$ be the local decomposition  of $(N(f),x)$. Then $(Y,x)=(Z_k,x)$ for some $k$.
\end{lm}
