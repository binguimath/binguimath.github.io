...
\end{align*}
If $f:\scr E_1\rightarrow\scr E_2$ is a sheaf map, then we have a canonical $\varphi_*f:\varphi_*\scr E_1\rightarrow\varphi_*\scr E_2$.
...
Recall that the \textbf{support of an $X$-sheaf} $\scr E$, denoted by $\Supp(\scr E)$, \index{SE@$\Supp(\scr E)$, the support of a sheaf $\scr E$} is the subset of all $x\in X$ such that $\scr E_x\neq0$.
...
Assume that for each open $U\subset X$, each element $f\in\scr O_X(U)$ is a continuous function on $X$. Then for each ideal $\mc I$ of $\scr O_X$, the subset $N(\mc I)$ is closed in $X$.
\end{rem}


\begin{proof}
Suppose $x\neq N(\mc I)$. Then for some $f\in\mc I_x$ we have $f(x)\neq 0$. Choose a neighborhood $U$ of $x$ so that $f\in\scr O_X(U)$. Since $f$ is continuous, by shrinking $U$ we may assume $f(y)\neq 0$ for all $y\in U$. So $U\cap N(\mc I)=\emptyset$.
\end{proof}
...
\end{align}
where $\mc I$ is a finite type ideal of $\scr O_X$. (The closedness is justified by Rem. \ref{lb20}.) The stalk of the structure sheaf at $x\in N(\mc I)$ is a local $\Cbb$-algebra
\begin{align*}
...
\begin{enumerate}[label=(\arabic*)]
\item If $\Phi\big(f_i-f_i(x)\big)=\Psi\big(f_i-f_i(x)\big)$ for all $i=1,\dots,n$, then $\Phi=\Psi$.
...
\simeq&\coker(\scr E_x\otimes\fk m_x\rightarrow \scr E_x\otimes\scr O_{X,x})\simeq\coker(\scr E_x\otimes\fk m_x\rightarrow \scr E_x)=\scr E_x/{\fk m_x\scr E_x}
\end{align*}
...
In general, the stalks of $\shom_{\scr O_X}(\scr E,\scr F)$ cannot be identified with $\Hom_{\scr O_{X,x}}(\scr E_x,\scr F_x)$. But good things happen when $\scr E,\scr F$ are coherent, as we will see in the next chapter.
...
\begin{rem}
If $\iota:Y\rightarrow X$ is an embedding of closed complex subspace, one may view an $\scr O_Y$-module $\scr F$ as the corresponding $\scr O_X$-module $\iota_*\scr F$. In particular, we have natural isomorphisms
\begin{gather*}
\Hom_{\scr O_Y}(\scr F_1,\scr F_2)\simeq\Hom_{\scr O_X}(\iota_*\scr F_1,\iota_*\scr F_2),\\
...
\end{align}
Thus, the study of the restriction $\scr E|_Y$ can be turned into the study of an $\scr O_X$-module.
\end{rem}
...
\end{tikzcd}
\end{equation}
The commutative square diagram above involving $S,X,Y,X\times_SY$ is called a \textbf{Cartesian square}. \index{00@Cartesian square}   \hfill\qedsymbol 
\end{df}
...
\end{align*}
which equals $\scr O_X/\mc J\scr O_X$ since the term insider the last $\coker$ is the multiplication map. (Compare Rem. \ref{lb4}.)
\end{proof}
