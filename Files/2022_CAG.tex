...
Let $X$ be a reduced complex space, and $Y$ an analytic subset of $X$. Let $x\in Y$ such that $X$ and $Y$ are irreducible at $x$. The following are equivalent.
\begin{enumerate}[label=(\arabic*)]
\item $(Y,x)\nsubset(\Sg(X),x)$. Namely, every neighborhood of $x$ in $Y$ contains a smooth point of $X$.
\item After shrinking $X$ to a neighborhood of $x$, there is a nowhere dense analytic subset $A\subset Y$ such that for each $p\in Y\setminus A$, the ideal $\scr I_{Y,p}$ of $\scr O_{X,p}$ is generated by $\dim_x X-\dim_x Y$ elements.
\item Each neighborhood of $x\in Y$ contains a point $p$ such that the ideal $\scr I_{Y,p}$ of $\scr O_{X,p}$ is generated by $\dim_x X-\dim_x Y$ elements.
\end{enumerate}
\end{thm}
...
\begin{proof}
By Thm. \ref{lb170}, we can shrink $X$ to a neighborhood of $x$ such that $X$ and $Y$ have pure dimensions $n$ and $m$ respectively. (2)$\Rightarrow$(3) is obvious.

(1)$\Rightarrow$(2): Let $A$ be the analytic subset $(Y\cap\Sg(X))\cup\Sg(Y)$ of $Y$. Then by (1) and Cor. \ref{lb177} and Thm. \ref{lb179}, after shrinking $X$, $A$ is nowhere dense in $Y$. Since each $p\in Y\setminus A$ is a smooth point of both $X$ and $Y$, (2) follows immediately from Rem. \ref{lb191}.

$\neg$(1)$\Rightarrow$ $\neg$(3): We shrink $X$ to a neighborhood of $x$ so that $Y\subset \Sg(X)$ (by $\neg$(1)), and that $\Sg(Y)$ is nowhere dense in $Y$ (by Thm. \ref{lb179}). Let us disprove (3). Choose any neighborhood $V$ of $x$ in $Y$. Then $V\setminus\Sg(Y)$ is not empty. Choose $p\in V\setminus\Sg(Y)$. Suppose that $\scr I_{Y,p}$ is generated by $k=n-m$ elements $f_1,\dots,f_k\in\scr O_{X,p}$. By Prop. \ref{lb178}, we have
\begin{align*}
\emb_p Y+k\geq\emb_p X
\end{align*}
By Thm. \ref{lb184}, we have $\emb_p Y=m$ and $\emb_pX>n$. This is impossible.
\end{proof}

\begin{rem}
Let $X,Y,x$ be as in Thm. \ref{lb387}. By inequality \eqref{eq195}, the smallest number of generators of $\scr I_{Y,p}$ is no less than the codimension $\dim_p X-\dim_p Y$. By Thm. \ref{lb170}, after shrinking $X$ to a neighborhood of $x$, for each $p\in Y$ we have $\dim_p X-\dim_p Y=\dim_xX-\dim_xY$. So
\begin{align}
\dim_xX-\dim_xY\leq\text{ the smallest number of generators of }\scr I_{Y,p}
\end{align}
This inequality is similar to $\dim_xX\leq\emb_xX$, since $\emb_xX=\dim_\Cbb\mk_{X,x}/\mk_{X,x}^2$ is the smallest number of generators of the ideal $\mk_{X,x}$ by Nakayama's lemma. Thus, one may view Thm. \ref{lb387} as a codimensional version of Thm. \ref{lb184}. (As in algebraic geometry, $(Y,y)$ can be viewed as a point of the scheme $\mathrm{Spec}(\scr O_{X,x})$.)
\end{rem}
