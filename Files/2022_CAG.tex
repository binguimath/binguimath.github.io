...
This example suggests why Nullstellensatz is important. Nullstellensatz essentially says that in coherent sheaves, all poles are of finite orders. (If $\scr E\subset\scr F$ are coherent, we consider poles at the analytic subset $\{x:\scr E_x\subsetneq\scr F_x\}$.) In the following, we give another compelling example.  \hfill\qedsymbol
\end{rem}
...
Let $X$ be a complex space such that $\red(X)$ has local decomposition $\red(X)=X_1\cup\cdots\cup X_N$ at $x\in X$. Let $Y$ be an analytic subset of $X$ irreducible at $x$. Define the \textbf{codimension} of $(Y,x)$ in $(X,x)$ to be \index{00@Codimension} \index{codim@$\codim_x(Y,X)$}
\begin{align}
\codim_x(Y,X)=\sup_{
\begin{subarray}{c}
1\leq i\leq N\\
