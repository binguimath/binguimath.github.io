...
\begin{co}\label{lb144}
Let $\varphi:X\rightarrow Y$ be a finite holomorphic map and $A$ an analytic subset of $X$. Then the set $\varphi(A)$ is an analytic subset of $Y$.
\end{co}

\begin{proof}
$A$ is the underlying set of a closed subspace of $X$, which we also denote by $A$. Then the inclusion $\iota:A\hookrightarrow X$ is finite, and hence $\varphi\circ\iota:A\rightarrow Y$ is finite. So by Thm. \ref{lb74} and Def. \ref{lb76}, $\varphi(A)=\varphi\circ\iota(A)$ can be viewed as a closed subspace of $Y$ whose underlying set is the image of $\varphi\circ\iota$.
\end{proof}
...
\draw [shift={(315.48,138.01)}, rotate = 140.2] [color={rgb, 255:red, 245; green, 166; blue, 35 }  ,draw opacity=1 ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
...
\draw [shift={(316.54,146.67)}, rotate = 140.2] [color={rgb, 255:red, 245; green, 166; blue, 35 }  ,draw opacity=1 ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
...
\draw [shift={(328.72,146.16)}, rotate = 140.2] [color={rgb, 255:red, 245; green, 166; blue, 35 }  ,draw opacity=1 ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
...
\end{gather}
\end{subequations}
called the \textbf{prime decomposition of $\pk$}.
...
\begin{proof}
Assume (1) is true. Then by Prop. \ref{lb113}, after shrinking $X$ to a neighborhood of $x$, $f$ is a non-zero divisor of $\scr O_{X,x}$ for all $x\in X$. If $N(f)$ contains an open subset $V$ of $X$, then $f$ takes value zero everywhere on $V$. So $f|_V=0$ because $X$ is reduced, contradicting the fact that $f$ is a non zero-divisor of $\scr O_{V,x}$ when $x\in V$.  So (2) must be true.
...
\end{align*}
We say that $X$ is \textbf{pure dimensional} \index{00@Pure dimensional complex spaces} if $x\in X\mapsto\dim_xX$ is a constant function. \hfill\qedsymbol
\end{df}
...
\begin{proof}
(1)$\Rightarrow$(2): By shrinking $X$, we assume $x$ is the only point of $X$. Let $\{0\}$ be the reduced point (whose structure sheaf is $\Cbb$). Then the obvious holomorphic map $X\rightarrow\{0\}$ is finite. Therefore, by Thm. \ref{lb74}, $\scr O_{X,x}$ is $\Cbb$-coherent, i.e., $\Cbb$-finite-dimensional.
...
\begin{df}
An element $f\in\scr O(X)$ is called \textbf{active at $x$} \index{00@Active elements} if $f$ (or more precisely $\red(f)$) is a non zero-divisor of $\scr O_{\red(X),x}$.
\end{df}

Non zero-divisors are always active, but the converse is not true.

\begin{pp}
If $f\in\scr O(X)$ is a non zero-divisor of $\scr O_{X,x}$, then $f$ is active at $x$.
\end{pp}
...
Let us prove $m\leq n-1$. Let $A=N(f)$. By Prop. \ref{lb141}, we may shrink $X$ and find a finite holomorphic map $\varphi:X\rightarrow V$ sending $x$ to $0$, where $V\subset\Cbb^n$ is open. By Cor. \ref{lb144}, $\varphi(A)$ is an analytic subset of $V$. So $m\leq \dim_0\varphi(A)$ by Cor. \ref{lb143}. Therefore, it suffices to prove $\dim_0\varphi(A)\leq n-1$.
...
\subsection{Applications of Active lemma}
...
\begin{align}
\dim_xX=\max_{1\leq i\leq N}\dim_xA_i\label{eq69}
\end{align}
\end{pp}
...
\begin{proof}
We prove this by induction on $m=\dim_xX$. The case $\dim_xX=0$ is obvious. Suppose \eqref{eq70} holds whenever $\dim_xX=m-1$. In the case that $\dim_xX=m$, choose an active element $f\in\fk m_{X,x}$ (Rem. \ref{lb148}). By shrinking $X$ to a neighborhood of $x$ we may assume $f\in\scr O(X)$. By Prop. \ref{lb121}, we may shrink $X$ further so that $N(f)$ is nowhere dense in $X$. So $N(f\otimes 1)=N(f)\times Y$ is nowhere dense in $Y$. By Prop. \ref{lb121} again, $f\otimes 1$ is an active germ of $X\times Y$ at $x\times y$. Therefore, by Active lemma,  $\dim_{x\times y}N(f)\times Y=\dim_{x\times y}X\times Y-1$, and $\dim_x N(f)=\dim X-1$. By assumption, $\dim_{x\times y}N(f)\times Y=\dim_xN(f)+\dim_yY$. This proves \eqref{eq70}.
\end{proof}
...
\begin{align}
1+\dim\scr O_{X,x}/\pk\leq\dim\scr O_{X,x}/\pk'. \label{eq71}
\end{align}
\end{lm}
...
\section{Dimensions and openness}
