...
\date{\footnotesize Main body finished in 2022\\Last update: April 2025}
...
\begin{eg}\label{lb9}
Let $\scr F$ be an $\scr O_X$-module. Let $\fk S\subset\scr O_X(X)$. For each open $U\subset X$, let $\scr E(U)$ be the set of all $s\in\scr F(U)$ whose stalk $s_x$ at each $x\in U$ is a (finite) $\scr O_{X,x}$-linear combination of $\{\sigma_x:\sigma\in\fk S\}$. Then $\scr E$ is an $\scr O_X$-submodule of $\scr F$. Indeed, one checks easily that $\scr E$ is the \textbf{$\scr O_X$-submodule generated by $\fk S$}, i.e., its stalk at each $x\in X$ is $\scr O_{X,x}$-generated by $\fk S$.
\end{eg}
...
\begin{pp}\label{lb419}
Let $\scr F$ be an $\scr O_X$-module. Suppose that for each $x\in X$ an $\scr O_{X,x}$-submodule $\mc E_x$ of $\scr F_x$ is chosen. Then the following are equivalent.
\begin{enumerate}
\item[(a)] There is a (necessarily unique, cf. Rem. \ref{lb11}) finite-type $\scr O_X$-submodule $\scr E$ of $\scr F$ whose stalk $\scr E_x$ at each $x\in X$ equals $\mc E_x$.
\item[(b)] Each $x\in X$ is contained in a neighborhood $U_x$ such that there exist finitely many $s_1,s_2,\dots\in\scr F(U_x)$ whose stalks at $y$ generate $\mc E_y$ for each $y\in U_x$.
\end{enumerate}
\end{pp}

\begin{proof}
(a)$\Rightarrow$(b): This is obvious. (b)$\Rightarrow$(a): Assume (b). By Exp. \ref{lb9}, for each $x\in X$, we have a (clearly finite-type) $\scr O_{U_x}$-submodule $\scr E_{U_x}$ of $\scr F|_{U_x}$ whose stalk at each $y\in U_x$ is $\mc E_y$. (Namely, we let $\scr E_{U_x}$ be the submodule of $\scr F|_{U_x}$ generated by $s_1,s_2,\dots$.) It is clear that if $x_1,x_2\in X$ then $\scr E|_{U_{x_1}}$ agree with $\scr E|_{U_{x_2}}$ on $U_{x_1}\cap U_{x_2}$ (since their stalks over $U_{x_1}\cap U_{x_2}$ agree). Therefore, we have a unique finite-type $\scr O_X$-submodule $\scr E$ of $\scr F$ whose restriction to each $U_x$ is $\scr E|_{U_x}$. (Namely, for each open $V\subset X$, define $\scr E(V)$ to be the set of all $s\in\scr F(V)$ satisfying $s|_{V\cap U_x}\in\scr E|_{U_x}(V\cap U_x)$ for each $U_x$.)
\end{proof}
...
\begin{eg}
If $Y$ is a closed (resp. open) complex subspace of a complex space $X$, and if $Z$ is a closed (resp. open) complex subspace of $Y$, then $Z$ is a closed (resp. open) complex subspace of $X$.
\end{eg}
...
\begin{pp}\label{lb81}
If $\alpha:Y\rightarrow X$ and $\beta:Z\rightarrow Y$ are closed embeddings, then so is the composition $\alpha\circ\beta:Z\rightarrow X$.
\end{pp}


\begin{proof}
It suffices to assume that $\alpha,\beta$ are inclusions of closed complex subspaces. Write $Y=\Specan(\scr O_X/\mc I)$ and $Z=\Specan(\scr O_Y/\wtd{\mc J})$ where $\mc I$ is a finite-type ideal of $\scr O_X$, and $\wtd{\mc J}$ is a finite-type ideal of $\scr O_Y$. In particular, for each $x\in X$, $\mc I_x$ is an ideal of $\scr O_{X,x}$; for each $y\in Y$, $\wtd{\mc J}_y$ is an ideal of $\scr O_{Y,y}=\scr O_{X,y}/\mc I_y$. 

For each $y\in Y$, let $\mc J_y\subset\scr O_{X,y}$ be the preimage of $\wtd{\mc J}_y$ under the quotient map $\scr O_{X,y}\rightarrow\scr O_{X,y}/\mc I_y$. Using Prop. \ref{lb419}, one checks easily that there is a finite-type ideal $\mc J$ of $\scr O_X$ whose stalk at each $y\in Y$ is $\mc J_y$, and whose stalk at each $x\in X\setminus Y$ is $\scr O_{X,x}=\mc I_x$.

It is clear that $\alpha\circ\beta:Z\rightarrow X$ is an injective closed map of topological spaces, that $N(\mc J)$ equals $Z$ (the image of $\alpha\circ\beta$), and that for all $z\in Z$ we have that $\mc J_z$ is the kernel of the composition of $\alpha^\#:\scr O_{X,z}\rightarrow \scr O_{Y,z}=\scr O_{X,z}/\mc I_z$ and $\beta^\#:\scr O_{Y,z}\rightarrow \scr O_{Y,z}/\wtd{\mc J}_z\simeq \scr O_{X,z}/\mc J_z$. Therefore, by Prop. \ref{lb14}, $\alpha\circ\beta$ is a closed embedding.
\end{proof}
...
\end{align*}
is a finite-type ideal of $\scr O_Y$, and is the unique ideal whose stalk at each $x\in N(\mc I)$ equals $(\mc I_x+\mc J_x)/\mc I_x$. 

Let $Z=\Specan(\scr O_Y/\wtd{\mc J})$. By Prop. \ref{lb81}, the composition $\alpha\circ\beta:Z\rightarrow X$ of the inclusion maps $\alpha:Y\hookrightarrow X$ and $\beta:Z\hookrightarrow Y$ is a closed embedding. Moreover, from the proof of Prop. \ref{lb81}, we see that $\alpha\circ\beta$ restricts to a biholomorphism
\begin{align}
\Specan(\scr O_Y/\wtd{\mc J})\xlongrightarrow[\simeq]{\varphi} \Specan(\scr O_X/(\mc I+\mc J))
\end{align}
which equals $N(\mc I)\cap N(\mc J)\xrightarrow{=}N(\mc I+\mc J)$ as maps of topological spaces, and whose stalk maps are
\begin{align*}
\scr O_{X,x}/(\mc I_x+\mc J_x)\quad\xlongrightarrow{\simeq}\quad\frac{\scr O_{X,x}/\mc I_x}{(\mc I_x+\mc J_x)/\mc I_x}=\scr O_{Y,x}/\wtd{\mc J}_x
\end{align*}
\hfill\qedsymbol
\end{eg}
...
We give an application of analytically generating elements. The following proposition generalized the substitution rule (Rem. \ref{lb27}).
...
For each $x\in X$, we let $\mc I_x$ be the ideal of $\scr O_{X,x}$ defined by (a) and (b). We shall use Prop. \ref{lb419} to show that there is a (necessarily unique) finite-type ideal $\mc I\subset\scr O_X$ whose stalk at each $x\in X$ is $\mc I_x$.

If $x\in X\setminus\Delta$, then Condition (b) of Prop. \ref{lb419} is clearly satisfied. So we let $x\in \Delta$. Choose a neighborhood $V_y\subset Y$ of $y=\varphi(x)$ biholomorphic to a model space. So we can choose finitely many $f_1,\dots f_n\in\scr O_Y(V_y)$ embedding $V_y$ onto a closed subspace of an open subset of $\Cbb^n$. Then $U_x=\varphi^{-1}(V_y)\cap \psi^{-1}(V_y)$ is a neighborhood of $x$. 


We claim that for each $p\in U_x$, $\varphi^\#(f_1)-\psi^\#(f_1),\dots,\varphi^\#(f_n)-\psi^\#(f_n)$ (defined on $U_x$) generate $\mc I_p$; this will verify Prop. \ref{lb419}-(b).  If $p\in U_x\cap \Delta$, then, as $\varphi(p)=\psi(p)$, the claim follows from Prop. \ref{lb24} or from the substitution rule (Rem. \ref{lb27}). If $p\in U_x\setminus\Delta$, then $\varphi(p)\neq\psi(p)$. So there exists $1\leq i\leq n$ such that $g_i=\varphi^\#(f_i)-\psi^\#(f_i)$ is nonzero at $p$. So the stalk $g_{i,p}$ generates $\scr O_{X,p}=\mc I_x$. 



Finally, let us prove $\Delta=N(\mc I)$. If $\varphi(x)\neq\psi(x)$, then $\mc I_x=\scr O_{X,x}$ shows $x\notin N(\mc I)$. If $\varphi(x)=\psi(x)$, then for each $f\in\scr O_{Y,\varphi(x)}$, $\varphi^\#(f)-\psi^\#(f)$ vanishes at $x$ by \eqref{eq8}. So $\mc I_x$ vanishes at $x$. So $x\in N(\mc I)$. 
\end{proof}
...
\begin{pp}\label{lb32}
Let $\varphi:X\rightarrow S$ and $\psi:Y\rightarrow S$ be holomorphic, and let $X_0$ and $Y_0$ be closed complex subspaces of $X,Y$ respectively. Assume that $X\times_SY$ is a fiber product of $\varphi$ and $\psi$. Recall $\pr_X:X\times_SY\rightarrow X$ and $\pr_Y:X\times_SY\rightarrow Y$. Then the intersection
\begin{align*}
...
Using Thm. \ref{lb26}, one can give a more explicit description of the graph of $\varphi:X\rightarrow Y$. We write it as $\Specan(\scr O_{X\times Y}/\mc J)$ for a finite-type ideal $\mc J$ determined by the following facts. Let $x\in X,y\in Y$. If $y\neq\varphi(x)$ then $\mc J_{x\times y}=\scr O_{X\times Y,x\times y}$. If $y=\varphi(x)$ then $\mc J_{x\times y}$ is the ideal of $\scr O_{X\times Y,x\times y}$ generated by
\begin{align}
...
\end{tikzcd}
\end{equation*}
By Prop. \ref{lb80} and Prop. \ref{lb32}, $\id\vee\pi$ and $\iota_{X,R}\vee\id$ are closed embeddings. So their composition $\varphi$ is a closed embedding (Prop. \ref{lb81}). By Prop. \ref{lb30},
\begin{align*}
...
The discriminant $D(\red(p))$ is a non-zero element of $\Kbb$. Equivalently (by Cor. \ref{lb202}), the multiplicity of any zero of $\red(p)$ in $\ovl \Kbb$ is $1$.
\end{rem}
...
Clearly $\red(p)$ and $p$ have the same zero set in any field extension $\ovl \Kbb$ in which $p$ splits.  Thus, if $p(z)=(z-z_1)^{m_1}\cdots(z-z_k)^{m_k}$ in $\ovl\Kbb[z]$, then by Rem. \ref{lb203}, 
\begin{align*}
...
\end{align}
By Rem. \ref{lb316}, if $W\subset V$ is open, we have a morphism of exact sequences from \eqref{eq136} to a similar one about $\varphi^{-1}(W)$. Therefore, we have a sequence of morphisms in $\Mod(\scr O_Y)$
\begin{align}
...
\end{align}
By checking the stalk maps, one sees that this sequence is exact.
...
Claim: It suffices to show that for each $k\in\Nbb$ there exist $\lambda_1,\dots,\lambda_n\in\Nbb$ such that
\begin{align*}
...
R^q\varphi_*\big( g_\nu^{b_\nu}(\scr E/\scr G)\big) \arrow[rrr,"R^q\varphi_*(\times g_\nu^{d_\nu+k})"] &&& R^q\varphi_*(\scr E/\scr G) \arrow[r,"\rho"] \arrow[d,"\delta"']          & R^q\varphi_*\big(\scr E/(\scr G+g_\nu^{b_\nu+d_\nu+k}\scr E)\big)                \\
            &&& R^{q+1}\varphi_*(\scr G)                             &                  
\end{tikzcd}
\end{equation*}
...
\end{align*}
which is a natural $\scr O_Y$-module morphism defined as follows. Choose any open $W\subset S$. Then by \eqref{eq168}, we have the pullback map 
\begin{align*}
...
\end{tikzcd}
\end{equation*}
where the lower left arrow is surjective. By Grauert comparison Thm. \ref{lb341}, the lower right arrow is injective. Therefore the horizontal arrow is surjective. Thus, the base change map $R^q\varphi_*(\scr E)\otimes\mc M\rightarrow R^q\varphi_*(\scr E\otimes\mc M)$ is surjective by Prop. \ref{lb342}.
\end{proof}



\begin{pp}\label{lb342}
Let $(\mc A,\mk)$ be a Noetherian local ring. Let $\varphi:\mc M\rightarrow\mc N$ be a morphism of finitely-generated $\mc A$-modules. Consider the induced map
\begin{align*}
\wht\varphi:\varprojlim_{k\in\Nbb} \mc M/\mk^k\mc M \rightarrow \varprojlim_{k\in\Nbb} \mc N/\mk^k\mc N
\end{align*}
Then $\varphi$ is surjective iff $\wht\varphi$ is surjective, and $\varphi$ is injective iff $\wht\varphi$ is injective.
\end{pp}

We will only use the fact that $\wht\varphi$ being surjective (resp. injective) implies $\varphi$ being surjective (resp. injective).



\begin{proof}
Assume that $\varphi$ is surjective. Then for each $k\in\Zbb$, the map $\mc M/\mk^k\mc M\rightarrow\mc N/\mk^k\mc N$ is surjective. Therefore, by Prop. \ref{lb333}, $\wht\varphi$ is surjective.


Assume that $\wht\varphi$ is surjective. Since we have a commutative diagram
\begin{equation*}
\begin{tikzcd}
...
\end{tikzcd}
\end{equation*}
where the vertical arrows are surjective. So $\mc M/\mk\mc M\rightarrow\mc N/\mk\mc N$ is surjective. By Nakayama's lemma, $\varphi$ is surjective.

Assume that $\wht\varphi$ is injective. Choose $\xi\in\Ker(\varphi)$. Then $\xi$ corresponds to the zero element of $\varprojlim_k \mc M/\mk^k\mc M$. So $\xi=0$ by Krull's intersection Thm. \ref{lb162}. This proves that $\varphi$ is injective.


Assume that $\varphi$ is injective. We view $\mc M$ as a submodule of $\mc N$ and $\varphi$ as the inclusion map. Then for each $k\in\Nbb$, the canonical map
\begin{align*}
\mc M/{\mc M\cap\mk^k\mc N}\rightarrow\mc N/\mk^k\mc N
\end{align*}
is injective. Therefore, by Prop. \ref{lb333}, the canonical map
\begin{align*}
\varprojlim_{k\in\Nbb}\mc M/{\mc M\cap\mk^k\mc N}\rightarrow\varprojlim_{k\in\Nbb}\mc N/\mk^k\mc N  \tag{$\star$}\label{eq225}
\end{align*}
is injective. Clearly $\mk^k\mc M\subset\mc M\cap\mk^k\mc N$. By the Artin-Rees Lemma (Thm. \ref{lb266}), there exists $k_0\in\Nbb$ such that for all $k\geq k_0$ we have $\mk(\mc M\cap\mk^k\mc N)=\mc M\cap\mk^{k+1}\mc N$ and hence $\mk^l(\mc M\cap\mk^{k_0}\mc N)=\mc M\cap\mk^{k_0+l}\mc N$ for all $l\in\Nbb$. Thus, for all $k\geq k_0$ we have
\begin{align*}
\mk^k\mc M\subset\mc M\cap\mk^k\mc N\subset \mk^{k-k_0}\mc M
\end{align*}
Therefore, the canonical map
\begin{align*}
\varprojlim_{k\in\Nbb}\mc M/\mk^k\mc N\rightarrow \varprojlim_{k\in\Nbb}\mc M/{\mc M\cap\mk^k\mc N}
\end{align*}
is an isomorphism. Composing this isomorphism with \eqref{eq225}, we conclude that $\wht\varphi$ is injective.
\end{proof}
...
\end{tikzcd}
\end{equation*}
where the lower left arrow is an isomorphism.  By Grauert comparison Thm. \ref{lb341},  the lower right arrow is injective. Therefore the horizontal arrow is an isomorphism. Therefore, by Prop. \ref{lb342}, the stalk map
\begin{align*}
...
(a,b)$\Rightarrow$(c): $R^q\varphi_*(\scr E)_t\otimes \mc M$ is naturally equivalent to $R^q\varphi_*(\scr E\otimes\mc M)$, and the latter is exact over $\mc M$. So $R^q\varphi_*(\scr E)_t\otimes-$ is exact on $\Modf(\scr O_{S,t})$. By Exp. \ref{lb265}, we have $\Tor_1^{\scr O_{S,t}}(R^q\varphi_*(\scr E)_t,\scr O_{S,t}/I)=0$ for each ideal $I\subset\scr O_{S,t}$. Therefore, by Prop. \ref{lb343}, $R^q\varphi_*(\scr E)_t$ is a flat $\scr O_{S,t}$-module. So it is free by Thm. \ref{lb280}.
\end{proof}
