...
\end{tikzcd}
\end{equation}
where $\mc J\scr O_X$ is the (necessarily finite-type) ideal of $\scr O_X$ generated by $\mc J$ (more precisely, by $\varphi^\#(\mc J)$). $\varphi^{-1}(S_0):=\Specan(\scr O_X/\mc J\scr O_X)$ is called the \textbf{inverse image of $S_0=\Specan(\scr O_S/\mc J)$} along $\varphi$. \index{00@Inverse images of closed subspaces $\varphi^{-1}(S_0)$}
\end{pp}
...
\end{tikzcd}
\end{equation}
The image of $\id\vee\varphi$ as a closed subspace of $X\times Y$ (namely, the canonical equalizer of $X\times Y\rightrightarrows Y$) is called the \textbf{graph of $\varphi$}. \index{00@Graphs of holomorphic maps}
\end{pp}
