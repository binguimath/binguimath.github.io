...
Let $\pi:X\rightarrow S$ be a finite continuous map. Let $\scr E$ be an $\scr O_X$-module and $\scr M$ an $\scr O_S$-module. Then we have a (clearly natural) $\scr O_S$-module isomorphism 
\begin{gather}
\begin{gathered}
...
By applying Nullstellensatz (Rem. \ref{lb115}-3) to $\scr O_Xf/(\scr O_Xf\cap\scr O_X)$, we see that there is $n\in\Zbb_+$ such that $(g_i^nf)_x\in\scr O_{X,x}$ for all $i$, thus $f\scr I_{\Sg(X),x}^n\subset\scr O_{X,x}$. Since $f\notin\scr O_{X,x}$, we can find the smallest $n\in\Zbb_+$ such that $f\scr I_{\Sg(X),x}^n\subset\scr O_{X,x}$. Choose any $\wtd f\in f\scr I_{\Sg(X),x}^{n-1}$ not in $\scr O_{X,x}$. (But note that $\wtd f$ belongs to $\Owht_{X,x}$ since $f$ does.) Then $\wtd f\scr I_{\Sg(X),x}\subset\scr O_{X,x}$. We claim that $\wtd f\scr I_{\Sg(X),x}\subset\scr I_{\Sg(X),x}$. Then the multiplication of $\wtd f$ gives an element of $\End_{\scr O_{X,x}}(\scr I_{\Sg(X),x})$ not inside $\scr O_{X,x}$, which disproves the RHS of \eqref{eq95}.
...
\begin{proof}
Since $\tau$ is a non zero-divisor of $\mc B$, we have a short exact sequence
\begin{align*}
...
We close this section with another interesting application of Thm. \ref{lb274} which was used in the proof of Lemma \ref{lb137}. In this application, the $(\mc B,\nk)$ and $(\mc A,\mk)$ in Thm. \ref{lb274} are set to be equal.
...
\begin{gather}
\Ext_{\mc A}^n(\mc A/\mk,\mc E/a\mc E)\simeq \Ext_{\mc A}^{n+1}(\mc A/\mk,\mc E)  \label{eq133}\\[0.5ex]
\Tor_{\mc A}^{n+1}(\mc A/\mk,\mc E/a\mc E)\simeq \Tor_{\mc A}^n(\mc A/\mk,\mc E)\label{eq134}
\end{gather}
\end{pp}
...
\begin{proof}
By induction, it suffices to prove the case $k=1$. Then the formula follows from Prop. \ref{lb298} and Thm. \ref{lb300}.
\end{proof}
...
If $(\scr E_\alpha)_\alpha$ is a family of $\scr O_X$-modules, we can define the \textbf{direct product} $\prod_\alpha\scr E_\alpha$ \index{00@Direct product sheaf $\prod_\alpha\scr E_\alpha$} in a natural way, i.e. whose spaces of sections on each open subset $U\subset X$ is $\prod_\alpha\scr E_\alpha(U)$. (It is in general not true that the stalk of the direct product equals the direct product of stalks.)
...
\begin{proof}
For each $x\in X$, we choose a monomorphism $\scr E_x\hookrightarrow\scr I_x$ of $\scr O_{X,x}$-modules, which exists due to Prop. \ref{lb311}. Set
\begin{align*}
...
\begin{df}
Let $H^\blt(X,-)$ be the right derived functor (cf. Thm. \ref{lb253}) of the functor $H^0(X,-)$ from $\Mod(\scr O_X)$ to $\Mod(\scr O(X0)00)$, sending $\scr E$ to $H^0(X,\scr E)=\scr E(X)$. For each $q\in\Nbb$, $H^q(X,\scr E)$ \index{Hq@$H^q(X,\scr E)$, sheaf cohomology} is called the \textbf{$q$-th cohomology group}  of $X$ with coefficients in $\scr E$. 
...
\end{align*}
by our notations. For each $q\in\Nbb$, the sheafification of the presheaf of $\scr O_Y$-modules associated to each open $V\subset Y$ the $\scr O_Y(V)$-module
\begin{align*}
...
\end{align*}
where $\sgn(\sigma)$ denotes the sign of the permutation $\sigma$.\footnote{One can also define \v Cech cohomology without assuming the alternate condition. The resulting theory is equivalent to the alternate one. See \cite{Dem} Sec. IV.5.D.} The $q$-th \textbf{coboundary operator} $\delta=\delta^q:C^q(\fk U,\scr E)\rightarrow C^{q+1}(\fk U,\scr E)$ is an $\scr O(X)$-module morphism defined by
\begin{align}
...
\end{gathered}
\end{gather}
It is helpful to view $\psi_{\alpha_j}s\cdot d\alpha_0\cdots \wht{d\alpha_j}\cdots d\alpha_q$ as the multiplication of $s\in\scr E(U_{\alpha_0\cdots\alpha_q})$ and $d\alpha_0\cdots d\alpha_q\in C^q(\fk U,\scr O_X)$.
...
\end{align*}
and recall that $\wch H^q(\fk U,-)=\mc H^q(C^\blt(\fk U,-))$. Clearly, a morphism a short exact sequence \eqref{eq140} gives a morphism of long exact sequences from \eqref{eq139} to a similar one for $\scr E',\scr F',\scr G'$.
...
Assume that $\wtd\rho:J\rightarrow I$ satisfies $V_\beta\subset U_{\wtd \rho_i(\beta)}$ for all $\beta\in J$. Then $\rho^\blt$ and $\wtd\rho^\blt$ induce the same $\scr O(X)$-module morphism (called restriction map)
\begin{align}
...
\end{align}
which is functorial in the sense that a morphism a short exact sequence \eqref{eq140} gives a morphism of long exact sequences from \eqref{eq145} to a similar one for $\scr E',\scr F',\scr G'$.
...
Let $\fk V=(V_x)_{x\in X}$. For each $x\in X$, choose $\rho(x)\in I$ such that $V_x\subset U_{\rho(x)}$. By the end of Step 1, it suffices to show that for each $x_0,\dots,x_q\in X$, $c_{\rho(x_0)\cdots\rho(x_q)}|_{V_{x_0\cdots x_q}}$ lifts to an element of $\scr F(V_{x_0\cdots x_q})$, and hence is enough to show that $c_{\rho(x_0)\cdots\rho(x_q)}|_{V_{x_0}}$ lifts to an element of $\scr F(V_{x_0})$. By (b), it suffices to prove that
\begin{align*}
x_0\in U_{\rho(x_0)\cdots\rho(x_q)}\qquad (\forall x_0,\dots,x_q\in X\text{ such that }V_{x_0\cdots x_q}\neq\emptyset)
\end{align*}
Therefore, it suffices to prove that for each $x,y\in X$,
\begin{align}
...
Let $X$ be a paracompact (e.g. second countable)  topological manifold. For every open cover $\fk U$ of $X$ there is a continuous partition of unity of $X$ subordinated to $\fk U$. Therefore, every $X$-sheaf over the sheaf of germs of continuous functions on $X$ is a fine sheaf.
\end{eg}
...
\begin{proof}
The first equivalence is due to Thm. \ref{lb321}. Since $H^{>0}(X,\scr E^\blt)=0$ by Prop. \ref{lb317}, the second equivalence holds (cf. Subsec. \ref{lb310} and especially \eqref{eq150}).
\end{proof}
...
\begin{eg}
Let $X$ be a paracompact smooth manifold. Let $\Fbb$ be $\Rbb$ or $\Cbb$. Then we have a resolution of constant $\Fbb$ called \textbf{de Rham resolution}:  \index{00@De Rham resolution}
\begin{align}
...
\begin{align}
H^q_{\mathrm{dR}}(X,\Fbb):=\mc H^q\Big(\bigwedge\nolimits^\blt_\Fbb X\Big).
\end{align}
...
\begin{align*}
H^q_{\ovl\partial}(X,\scr E):=\mc H^q\Big(\scr E\otimes_{\scr O_X}\bigwedge\nolimits^{0,\blt}X \Big)
\end{align*}
...
\end{align*}
showing that $C^\blt_{\scr F}(\fk U,\scr G)=C^\blt(\fk U,\scr G)$ and hence $\wch H_{\scr F}^\blt(X,\scr G)=\wch H^\blt(X,\scr G)$. Therefore, by \eqref{eq145} and its functoriality, we have a morphism of exact sequences
...
\end{align*}
where the third term is zero by assumption \eqref{eq155} and the first term is zero when $p>0$ by Lemma \ref{lb320}. Therefore $\wch H^{>0}(U_{\alpha_0\cdots\alpha_n},\scr G)=0$. So by case $q$ of Leray's theorem, the middle vertical arrow of \eqref{eq156} is an isomorphism. So the third vertical arrow is also an isomorphism due to Five lemma. This proves case $q+1$ for $\scr E$.
\end{proof}




Without assuming that $X$ is paracompact, Thm. \ref{lb326} still holds if we replace $\wch H^{>0}(U_{\alpha_0\cdots\alpha_n},\scr E)$ with $H^{>0}(U_{\alpha_0\cdots\alpha_n},\scr E)$, replace $\wch H^q(X,\scr E)$ with $H^q(X,\scr E)$, and define the map \eqref{eq154} in an appropriate way. (Cf. \cite{Dem} Chapter IV Thm. 5.17.) Indeed, this sheaf cohomology version is the common one that people refer to when talking about Leray's theorem. This version is especially useful in algebraic geometry, since schemes are not even Hausdorff. It also allows us to  compute the sheaf cohomology of coherent sheaves over a complex space $X$ which is non-necessarily paracompact by computing the \v Cech cohomology of a Stein open cover of $X$. We present this version in the following subsection.
...
\end{gather}
Then $(\fk C^q(\fk U,\scr E),d)$ is a cochain complex of $\scr O_X$-modules.
...
Moreover, if $\scr F\in\Mod(\scr O_X)$, then a morphism $\varphi\in\Hom_{\scr O_X}(\scr E,\scr F)$ gives rise to a natural morphism of cochain complexes $C^\blt(\fk U,\scr E)\rightarrow C^\blt(\fk U,\scr F)$. Therefore, $\fk C^\blt(\fk U,-)$ is an (additive) functor from $\Mod(\scr O_X)$ to the category of complex of $\scr O_X$-modules.
\end{pp}
...
Suppose that $X$ is a Stein space. Then for each $\scr E\in\Coh(\scr O_X)$ and each $x\in X$, the stalks of $\scr E(X)$ at $x$ generate the $\scr O_{X,x}$-module $\scr E_x$.
\end{thm}
...
\end{enumerate}
Indeed, (d) is due to that $X\times Y$ is Stein and that $X\times_SY$ is a closed subspace of $X\times Y$ (by Prop. \ref{lb90}). To show (e), it suffices to assume $n=2$. Then (e) follows from (d) and Exp. \ref{lb36}. (f) follows from (d) because $U\cap\varphi^{-1}(W)$ is the fiber product of $\varphi\circ\iota_{U,Y}:U\rightarrow Y$ and $\iota_{W,Y}:W\rightarrow Y$. Furthermore, we have (cf. \cite[Sec. V.4.3]{GR-a})
...
Fix $t\in S$ and write $\mk_{S,t}$ as $\mk_t$ for simplicity. Let the $\scr M$ in \eqref{eq159} be $\scr O_{S,t}/\mk_{S,t}^k$ and $\scr O_{S,t}/\mk_{S,t}^l$ where $l\geq k$. Then we have a commutative diagram
\begin{equation*}
...
\end{tikzcd}
\end{equation*}
where the horizontal maps are defined by \eqref{eq159}. In other words, we get a morphism of inverse system $R^q\varphi_*(\scr E)_t/\mk_t^\blt R^q\varphi_*(\scr E)_t\rightarrow R^q\varphi_*(\scr E/\mk_t^\blt\scr E)_t$. Passing to the inverse limit gives an $\scr O_{S,t}$-module morphism
\begin{align}
...
Indeed, we will only use the injectivity of \eqref{eq161} in our notes. So we postpone the proof of surjectivity to the end of this section.
...
\end{align*}
Suppose the claim is proved. For each $k\in\Nbb$, choose the corresponding $\lambda_1,\dots,\lambda_n$, and let $l=n\cdot\max\{\lambda_1,\dots,\lambda_n\}$. Then
\begin{align*}
...
\end{tikzcd}
\end{equation}
Again we choose $\scr E\in\Mod(\scr O_X)$. Consider the base change maps
\begin{gather*}
...
\end{align*}
composed with $\Phi$ gives the base change map for $\scr E$ and the largest Cartesian square
\begin{align*}
...
\end{tikzcd}
\end{equation}
where the first column is exact.
...
\end{align*}
which is an isomorphism by (c). We claim that the base change map for $\scr E|_{\varphi^{-1}(T)}=\scr E\otimes_{\scr O_S}\scr O_T$ and the second cell is an isomorphism. Then the base change map for $\scr E$ and the largest rectangle (namely \eqref{eq177}) is an isomorphism by Rem. \ref{lb340}, which finishes the proof of (f).

To prove the claim, notice the commutative diagram
\begin{equation*}
\begin{tikzcd}[column sep=-2cm]
R^q\varphi_*(\scr E)\otimes_{\scr O_S}\scr O_T\otimes_{\scr O_S} \scr O_T/\mk_{T,t} \arrow[rr,"\simeq"] \arrow[rd,"\simeq"] &              & R^q\varphi_*(\scr E\otimes_{\scr O_S}\scr O_T)\otimes_{\scr O_S} \scr O_T/\mk_{T,t} \\
                        & R^q\varphi_*(\scr E\otimes_{\scr O_S}\scr O_T\otimes_{\scr O_S} \scr O_T/\mk_{T,t}) \arrow[from=ru] &  
\end{tikzcd}
\end{equation*}
where the horizontal and the lower left arrows are isomorphisms by (c). Thus the lower right arrow is an isomorphism. Thus $\scr E\otimes_{\scr O_S}\scr O_T$ satisfies base change property-condition (e) in order $q$ at $t$. Therefore, by Step 1, it satisfies base change property-condition (f). This proves the claim.\\
...
\begin{align*}
R^q\varphi_*(\scr E)|_{X_t}\rightarrow H^q(X_t,\scr E|_{X_t})
\end{align*}
...
\section{Semicontinuity and base change theorem}


In this section, we let $\varphi:X\rightarrow S$ be a proper holomorphic map of complex spaces, and let $\scr E\in\Coh(\scr O_X)$ be a $\varphi$-flat.
...
\end{tikzcd}
\end{equation}
where $p=q+1$. So by Four lemma, $\Phi$ is injective.
\end{proof}



\begin{proof}[\textbf{Proof of Thm. \ref{lb355} (b)$\Rightarrow$(a) when $S$ is smooth}]
We assume that $S$ is a pure-dimensional complex manifold. We assume for simplicity that $S$ is an open subset of $\Cbb^n$. Assume that (b) holds. The short exact sequence \eqref{eq185} gives a commutative diagram \eqref{eq186} with exact columns, where we choose $p=q$. We also have a long exact sequence
\begin{align*}
