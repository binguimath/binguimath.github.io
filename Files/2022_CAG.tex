...
\begin{enumerate}[label=(\arabic*)]
\item $X$ is irreducible.
\item Every coherent ideal $\mc I$ of $\scr O_X$ satisfying $\mc I_x\cap \Nzd(\scr O_{X,x})=\emptyset$ for some $x\in X$ is the zero ideal.
\item Every coherent ideal $\mc I$ of $\scr O_X$ satisfying $\mc I_x=0$ for some $x\in X$ is the zero ideal.
\end{enumerate}
\end{thm}
...
\begin{proof}
Assume (1). Let $\mc I$ be a coherent ideal and let $A=N(\mc I)$. If $\mc I\neq 0_X$, then $A\neq X$. So by Prop. \ref{lb239}, $A$ is nowhere dense in $X$. So by Prop. \ref{lb123}, for every $x\in X$, $\mc I_x$ contains a non zero-divisor of $\scr O_{X,x}$. This proves (2).

Clearly (2) implies (3).

Assume that (1) is not true. By Prop. \ref{lb239}, there is an analytic subset $A\subsetneq X$ containing a non-empty open subset $U\subset X$. The $\scr I_A$ is non-zero but $\scr I_{A,x}=0$ for all $x\in U$. (3) is not true.
\end{proof}
