...
\begin{eg}
Assume that $\varphi:X\rightarrow S$ is a finite and flat holomorphic map of complex spaces and $S$ is connected. (For instance, if $X$ and $S$ are complex manifolds of dimension $n$, and if $\varphi$ is finite, then the fibers have dimension $0$, and hence $\varphi$ is flat by Cor. \ref{lb360}.) Then for each $t\in S$ we have $H^{>0}(X_t,\scr O_{X_t})=0$. Therefore, by Cor. \ref{lb359}, the dimension $\dim_\Cbb H^0(X_t,\scr O_{X_t})$ (which is the number of points of $X_t$ ``counting multiplicities") is independent of $t$. 

Consider the special case that $S=\Pbb^1$ and $X$ is a connected compact Riemann surface. Then we have $\dim H^0(X_0,\scr O_{X_0})=\dim H^0(X_\infty,\scr O_{X_\infty})$. This means that the number of zeros equals the number of poles of $\varphi$.  \hfill\qedsymbol
\end{eg}
