...
Let $\scr F$ be an $\scr O_X$-module. Suppose that for each $x\in X$ an $\scr O_{X,x}$-module $\mc E_x$ is chosen. Then the following are equivalent.
\begin{enumerate}
...
\begin{proof}
It suffices to assume that $\alpha,\beta$ are inclusions of closed complex subspaces. Write $Y=\Specan(\scr O_Z/\mc I)$ and $X=\Specan(\scr O_Y/\wtd{\mc J})$ where $\mc I$ is a finite-type ideal of $\scr O_Z$, and $\wtd{\mc J}$ is a finite-type ideal of $\scr O_Y$. In particular, for each $p\in Z$, $\mc I_p$ is an ideal of $\scr O_{Z,p}$; for each $x\in Y$, $\wtd{\mc J}_x$ is an ideal of $\scr O_{Y,x}=\scr O_{Z,x}/\mc I_x$. 

For each $x\in Y$, let $\mc J_x\subset\scr O_{X,x}$ be the preimage of $\wtd{\mc J}_x$ under the quotient map $\scr O_{X,x}\rightarrow\scr O_{X,x}/\mc I_x$. Using Prop. \ref{lb419}, one checks easily that there is a finite-type ideal $\mc J$ of $\scr O_X$ whose stalk at each $x\in Y$ is $\mc J_x$, and whose stalk at each $p\in X\setminus Y$ is $\scr O_{X,p}=\mc I_p$.

It is clear that $\beta\circ\alpha:X\rightarrow Z$ is an injective closed map of topological spaces, that $N(\mc J)$ equals $X$ (the image of $\beta\circ\alpha$), and that for all $x\in X$ we have that $\mc J_x$ is the kernel of the composition of $\beta^\#:\scr O_{Z,x}\rightarrow \scr O_{Y,x}=\scr O_{Z,x}/\mc I_x$ and $\alpha^\#:\scr O_{Y,x}\rightarrow \scr O_{Y,x}/\wtd{\mc J}_x\simeq \scr O_{Z,x}/\mc J_x$. Therefore, by Prop. \ref{lb14}, $\beta\circ\alpha$ is a closed embedding.
\end{proof}
