...
\begin{df}
The \textbf{sheaf of (germs of) densely defined holomorphic functions} of $X$ is \index{MX@$\fk W_X$, sheaves of densely defined holomorphic functions} \index{MX@$\fk W_X^\pre$, presheaves of densely defined holomorphic functions} the sheaf $\fk W_X$ associated to presheaf $\fk W_X^\pre$ such that for each open $U\subset X$,
\begin{align*}
...
\begin{eg}
Let $X$ be a reduced complex space with local decomposition $X=X_1\cup\cdots\cup X_N$ at $x$ such that Thm. \ref{lb125} holds. Then we can define the \textbf{characteristic function} \index{00@$\chi_{X_k}$, the characteristic function}
\begin{gather}
\begin{gathered}
...
\begin{align}
\bigoplus_i\iota_i^\#:\quad\fk W_X\xlongrightarrow{\simeq}\bigoplus_{1\leq i\leq N}\fk W_{X_i}\\
\bigoplus_i\iota_i^\#:\quad\Owht_X\xlongrightarrow{\simeq}\bigoplus_{1\leq i\leq N}\Owht_{X_i}
\end{align}
\end{subequations}
\end{pp}
...
Consider the open subset $Y_1^\circ=Y_1\setminus(Y_2\cup\cdots\cup Y_N)$ of $Y_1$. Then $A\cap Y_1^\circ$ contains no open subsets of $Y_1^\circ$. If $\Omega$ is an open subset of $Y_1$ contained inside $A\cap Y_1$, then $\Omega\cap Y_1^\circ$ is an open subset of $Y_1^\circ$ contained inside $A\cap Y_1^\circ$, which is empty. Thus $\Omega\subset Y_1'$ where $Y_1'=Y_1\setminus Y_1^\circ$. But $Y_1'=\bigcup_{j>1}Y_1\cap Y_j$ is nowhere dense in $Y_1$ since $\bigcup_{j>1}X_1\cap X_j$ is nowhere dense in $X_1$. So $\Omega$ is empty. Thus $A\cap Y_1$ is nowhere dense in $Y_1$.

That \eqref{eq85} are isomorphisms is not hard to check and is left to the readers.
\end{proof}
...
\section{Riemann extension theorems; $\scr O_{\Cbb^n,0}$ is a UFD}
...
\begin{thm}[\textbf{Second Riemann extension theorem}]
...
\begin{df}
Let $f(z)=a_0+a_1z+\cdots+a_mz^m$ and $g(z)=b_0+b_1z+\cdots+b_nz^n$ be polynomials in $\mc A[z]$ of degree $m,n$ respectively. Then the \textbf{resultant $\res(f,g)$} \index{00@$\res(f,g)$: the resultant} of $f$ and $g$ is the determinant of the $(m+n)\times(m+n)$ matrix
...
Assume the setting of Def. \ref{lb57}. So $\pi:X\rightarrow S$ is a Weierstrass map defined by polynomials $p_1(z_1),\dots,p_k(z_k)$. We assume that $S$ is reduced. We do not assume at the beginning that $X$ is reduced. Then we have discriminants
\begin{align*}
D(p_i)\in\scr O(S).
\end{align*}
We set
