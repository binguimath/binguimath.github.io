...
\end{align*}
By assumption (1) and Lemma \ref{lb156}, $\varphi(X)$ contains a neighborhood of $y\in Y$. So
\begin{align*}
\dim_y N(g)=\dim_y Y-1.
\end{align*}
...
When $X$ is not necessarily locally irreducible, $\nu$ is not open. But we have:


\begin{pp}
$\nu:\wht X\rightarrow X$ is a quotient map of topological spaces. Namely, a subset $U\subset X$ is open if and only if $\nu^{-1}(U)$ is open in $\wht X$.
\end{pp}
...
\begin{proof}
Suppose that $\nu^{-1}(U)$ is open. Choose any $x\in U$. We need to show that $x$ is an interior point of $U$. Choose a neighborhood $V$ of $x\in X$ such that (by Thm. \ref{lb125}) $V$ has local decomposition $V=V_1\cup\cdots\cup V_N$ at $x$ and that $V_i\cap V_j$ is nowhere dense in $V_i$ for all $1\leq i\neq j\leq N$. Then by Rem. \ref{lb236}, $\nu^{-1}(V)=\coprod_{i=1}^N\wht V_i$. By Prop. \ref{lb246}, $\nu^{-1}(x)\cap \wht V_i$ has only one element, which we denote by $p_i$. Then by Prop. \ref{lb247}, $\nu$ is open at each $p_i$. Therefore, the image of $\nu^{-1}(U)\cap \wht V_i$ (which is an open subset of $\wht X$ containing $p_i$) under $\nu$ contains a neighborhood of $x$ in $V_i$, written as $W_i\cap V_i$ where $W_i$ is a neighborhood of $x$ in $X$. 

Clearly $W_i\cap  V_i$ is inside $U$. Let $W=W_1\cap\cdots\cap W_N$. Then $W\cap V_i\subset U$. So $W\cap V=W\cap \bigcup_i V_i\subset U$. Therefore $W\cap V$ is a neighborhood of $x\in X$ contained in $U$.
\end{proof}
