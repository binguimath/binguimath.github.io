...
The biggest difference between subnets and subsequences is that the index set of a subnet is not necessarily a subset of the index set of the original net. Indeed, subnets are defined in this way mainly because we want to have a net version of Pb. \ref{lb64} in any topological space. (See Pb. \ref{lb223}.) Let us see an elementary example of subnet whose index set is larger than that of the original net. Its importance is justified by the proofs of Exp. \ref{lb125} and Prop. \ref{lb126}.
...
Let $\mc B$ be a set of open sets of a metric space (or more generally, a topological space) $X$. We say that $\mc B$ is a \textbf{base for the topology} \index{00@Base for topology} $\mc T$ of $X$ if one of the following (clearly) equivalent statements holds:
\begin{itemize}
...
Thus, according to Def. \ref{lb168}, the set of open balls of a metric space $X$ form a base for the topology of $X$. Nevertheless, even in the case of metric spaces, we sometimes consider more convenient bases than the set of open balls. We will see this when we study the topologies of infinite product spaces.
...
When (1) or (2) holds, we call $\mc T$ the \textbf{topology generated by $\mc B$}. \index{00@Topology generated by the base} Clearly, $\mc B$ is a base for $\mc T$ (cf. Def. \ref{lb170}).
...
\begin{eg}
If $(X,\mc T)$ is a topological space, then $\mc T$ is a base for $\mc T$.
\end{eg}

\begin{eg}
Let $(X,d)$ be a metric space with topology $\mc T$. Let $\mc B=\{B_X(x,r):x\in X,0<r<+\infty\}$. Then $\mc B$ is a base for $\mc T$. For each $\eps>0$, the set $\mc B'=\{B_X(x,r):x\in X,0<r<\eps\}$ is also a base for $\mc T$.
\end{eg}
...
\end{gather}
Using Prop. \ref{lb171}, one easily checks that $\mc B$ is a base for a topology $\mc T$. We call this the \textbf{standard topology} of $\ovl\Rbb$. \index{00@Topology of $\ovl\Rbb$}
...
Let $A$ be a subset of a topological space $X$. Let $\mc B$ be a base for the topology of $X$. Show that $\{U\cap A:U\in\mc B\}$ is a base for the subspace topology of $A$.
\end{exe}
...
\end{itemize}
Clearly, if $\mc B$ is a base for the topology, then $x_\alpha\rightarrow x$ iff:
\begin{itemize}
...
\begin{df}
Let $X$ be a topological space with a base for the topology $\mc B$. We say that $X$ is a \textbf{Hausdorff space} if the following equivalent conditions are satisfied:
\begin{itemize}
...
\begin{exe}
Let $\mc B$ be a base for the topology of $X$. Show that $x\in X$ is a closure point of $A$ iff every $U\in\mc B$ containing $x$ must intersect $A$.
\end{exe}
...
It is clear that if $\mc B$ is a base for the topology, then $x\in\Int(A)$ iff there exists $U\in\mc B$ such that $x\in U\subset A$.
...
It is clear that ``for every $V\in\Nbh_{Y}(y)$" in (2) and (2') can be replaced by ``for every $V\in\mc B$ containing $y$" if $\mc B$ is a base for the topology of $Y$. 
...
If $f=g$ then clearly $f|_A=g|_A$. Assume that $f|_A=g|_A$. For each $x\in\ovl A$, choose a net $(x_\alpha)$ in $A$ converging to $x$. Then $f(x)=\lim f(x_\alpha)=\lim g(x_\alpha)=g(x)$. So $f=g$.
\end{proof}
...
\end{aligned}
\end{align}
is a base for a topology $\mc T$, called the \textbf{product topology} \index{00@Product topology} or \textbf{pointwise convergence topology} \index{00@Pointwise convergence topology} of $S$. We call $(S,\mc T)$ the \textbf{product topological space}. \index{00@Product topological space} 
...
\begin{proof}
Either prove this directly using the base for the topology, or prove that any net cannot converge to two different values using Thm. \ref{lb192}.
\end{proof}
...
\begin{df}
We say that $X$ satisfied the \textbf{intermediate value property} (abbreviated to \textbf{IVP}) \index{00@IVP=Intermediate value property} if for every continuous function $f:X\rightarrow\Rbb$ and every $x,y\in X$ we have
\begin{align}
...
\begin{proof}
First, assume that $X$ does not satisfy IVP. Choose a continuous $f:X\rightarrow\Rbb$ with real numbers $a<b<c$ such that $a,c\in f(X)$ but $b\notin f(X)$. So $U=f^{(-\infty,b)}$ and $V=f^{-1}(b,+\infty)$ are disjoint non-empty open subsets of $X$. They are open, because $f$ is continuous (cf. Prop. \ref{lb191}). So $X$ is not connected.
...
Let $A$ be a subset of $\ovl\Rbb$. Then $A$ is connected iff $A$ is an interval.
\end{thm}
...
\end{itemize}
Then $y_n-x_n$ converges to $0$. So $x_n$ and $y_n$ converge to the same point $\xi$. Since $V$ is open, $U=X\setminus V$ is closed. So $\xi\in\ovl U=U$. Similarly, $\xi\in\ovl V=V$. This is impossible.
\end{proof}
...
The following proposition can be used to decompose an open subset of $\Rbb^N$ into open connected subsets. (See Pb. \ref{lb221}.)


\begin{pp}\label{lb208}
Assume that $X=\bigcup_{\alpha\in\scr A}X_\alpha$ such that each $X_\alpha$ is connected, and that $\bigcap_{\alpha\in\scr A}X_\alpha\neq\emptyset$. Then $X$ is connected.
\end{pp}
...
\begin{exe}
Prove Exe. \ref{lb207} and Exe. \ref{lb208} directly using Def. \ref{lb212} (but not using IVP).
\end{exe}
...
By Exp. \ref{lb214}, the exponential function $\exp:\Rbb\rightarrow\Rbb$ is continuous. We claim that $\exp$ is a strictly increasing homeomorphism from $\Rbb$ to $\Rbb_{>0}$. Its inverse function is the \textbf{logarithmic function} \index{log@$\log$} 
\begin{align*}
...
\begin{proof}
From $e^x=\sum_{n=0}^\infty x^n/n!$ we clearly have $e^0=1$ and $e^x>1$ if $x>0$. From $e^{x+y}=e^xe^y$ proved in Cor. \ref{lb215}, we know that $\exp$ is strictly increasing. Thus, by Cor. \ref{lb213}, $\exp$ is a homeomorphism from $\Rbb$ to $J=\exp(\Rbb)$, and $J$ is an interval. When $x\geq 0$, we have $e^x\geq x$. So $\sup_{x\geq 0}e^x=+\infty$. When $x\leq 0$, since $e^xe^{-x}=e^0=1$, we have $\inf_{x\leq 0}e^x=1/\sup_{x\geq 0}e^x=0$. So $\sup J=+\infty$ and $\inf J=0$. Since $0\notin\exp(\Rbb)$ (if $e^x=0$, then $1=e^xe^{-x}=0$, impossible), we have $J=\Rbb_{>0}$.
\end{proof}
...
a^z=e^{z\log a}
\end{align}
By Exp. \ref{lb217},  the map
\begin{align}
\Rbb\rightarrow\Rbb_{>0}\qquad x\mapsto a^x
\end{align}
is a strictly increasing homeomorphism. By the proof of Exp. \ref{lb217}, we have 
\begin{align}
...
a^{\frac 1n}=\sqrt[n]{a}
\end{align*}
Also, by Exp. \ref{lb217}, for each $p\in\Rbb\setminus\{0\}$,
\begin{align}
\Rbb_{>0}\rightarrow\Rbb_{> 0} \qquad x\mapsto x^p=e^{p\log x}
\end{align}
is a homeomorphism (since it is the composition of the homeorphisms $x\in\Rbb_{>0}\rightarrow p\log x\in\Rbb$ and $\exp:\Rbb\rightarrow\Rbb_{>0}$).
\end{eg}
...
\end{enumerate}
Any $x\in X$ satisfying one of these two requirements is called a \textbf{cluster point} \index{00@Cluster point of a net} of $(x_\alpha)_{\alpha\in I}$. (Compare Pb. \ref{lb64}.)
\end{prob}
...
By Pb. \ref{lb223}-(3), the set of cluster points of $(x_\alpha)$ is a closed subset, since intersections of closed subsets are closed (Cor. \ref{lb186}).
\end{rem}

For the reader's convenience, we also present below the sequential version of Pb. \ref{lb223}. ``(1)$\Leftrightarrow$(2)" is due to Pb. \ref{lb64}. ``(2)$\Leftrightarrow$(3)" is due to Pb. \ref{lb223}.
...
\end{enumerate}
Any $x\in X$ satisfying one of these two requirements is called a \textbf{cluster point} of  $(x_n)_{n\in\Zbb_+}$.
\end{pp} 
...
It is not hard to show that \emph{Prop. \ref{lb256} holds more generally when $X$ is first countable} (cf. Def. \ref{lb262}). The proof is similar to that for metric spaces.
\end{srem}


\begin{rem}\label{lb267}
By Pb. \ref{lb223}-(2) and Pb. \ref{lb256}-(2), if $(x_n)$ is a sequence in a metric space $X$, then $(x_n)$ has a subsequence converging to $x\in X$ iff $(x_n)$ has a subnet converging to $x$. This is not necessarily true when $X$ is a general topological space. Note that in the general case, cluster points of a sequence $(x_n)$ mean cluster points of $(x_n)$ as a net. Thus, they are not the limits of convergent \emph{subsequences} of $(x_n)$. 
\end{rem}
...
\begin{enumerate}
\item Check that $\mc B$ (defined by \eqref{eq63}) is a base for a topology. (Therefore, $\mc B$ generates a topology $\mc T$ on $I^*$.) Moreover, show that $(I^*,\mc T)$ is Hausdorff.
...
\end{enumerate}
When $X$ is a metric space, the above three conditions and the following two are equivalent:
\begin{itemize}
...
\begin{exe}
Prove the equivalence in Def. \ref{lb197}. Note that when applying Def. \ref{lb188} to the extended function $f:A\cup\{x\}\rightarrow Y$, the two equivalent statements you get are not completely the same as (2) and (3) of Def. \ref{lb197}. (For example, you will get a statement about nets in $A\cup\{x\}$ rather than nets in $A$.) But the proof for Def. \ref{lb197} is similar to that for Def. \ref{lb188}.
\end{exe}
...
Let $X$ be a topological space. A set $\mc U_x$ of neighborhoods of $x$ is called a \textbf{neighborhood base} of $x$, if for every $V\in\Nbh_X(x)$ there exists $U\in\mc U_x$ such that $U\subset V$. We call $X$ \textbf{first countable} \index{00@First countable} if every point $x$ has a neighborhood base which is a countable set.  
\end{df}
...
\begin{df}
A topological space $X$ is called \textbf{locally connected} \index{00@Locally connected} if for every $x\in X$ there exists $U\in\Nbh_X(x)$ which is connected.
\end{df}
...
\begin{eg}
$\Rbb^N$ is locally connected, since every point is contained in an open ball which is path-connected and hence connected (by Thm. \ref{lb220}). Therefore, every open subset of $\Rbb^N$ is locally connected.
\end{eg}
...
\end{thm}
This is the first fundamental theorem we prove in this course. Its significance lies in the fact that it connects two seemingly very different notions and compactness, and hence two strikingly different intuitions. We hope that the reader can not only follow the logical chains of the proofs, but also understand the pictures behind the proof. More precisely, we hope that the readers can have an intuitive understanding of the following questions:
\begin{itemize}
\item Why are these two compactness both powerful in solving certain problems? What roles do they play in the proof? How are the roles played by these two compactness related? (This is more important than just knowing why these notions are \emph{logically} equivalent.)
...
Thus, I feel that it is better to look at some applications of compactness before we prove Thm. \ref{lb222} rigorously. For pedagogical purpose, we also introduce two related notions of compactness:

\begin{df}
We say that $X$ is \textbf{net-compact}, \index{00@Net-compact} if every net $(x_\alpha)_{\alpha\in I}$ in $X$ has at least one cluster point (recall Pb. \ref{lb223}), equivalently, at least one convergent subsequence. We say that $X$ is \textbf{countably compact}, \index{00@Countably compact} if every countable open cover of $X$ has a finite subcover.
\end{df}
...
The above proof is typical. It suggests that compactness is powerful for \ul{proving finiteness properties} rather than finding solutions of functions satisfying certain requirements. Thus, if you want to prove a finiteness property using sequential or net-compactness, you have to prove it indirectly. For example, you need to prove by contradiction:
...
\subsubsection{Uniform convergence in multivariable functions}
...
\begin{align*}
f_x:Y\rightarrow Y\qquad y\mapsto f(x,y)
\end{align*}
...
\begin{rem}
Note that since $Y$ is compact, each $g\in C(Y,V)$ is uniformly bounded by EVT (applied to $|g|$). So $C(Y,V)\subset l^\infty(Y,V)$.
\end{rem}
...
I have already touched on this phenomenon in Rem. \ref{lb229}: \emph{The reason that sequences and nets run in the opposite direction to that of open sets is because closed sets are opposite to open sets}, as proved in Thm. \ref{lb181}. Now, we have a more vivid sense of the following sentence: 
\begin{itemize}
...
\end{itemize}
Thus, you can expect that the transition between closed and open sets plays a crucial role in the following proof of Thm. \ref{lb222}. 
...
\begin{align}
\text{sequentially compact}\quad\Longleftrightarrow\quad \text{countably compact}\qquad(\text{first countable spaces})
\end{align}
\end{srem}
...
Part 2. To prove $\ovl X=X$, we choose any net $(x_\alpha)$ in $X$ converging to $p\in Y$ and show that $p\in X$. Indeed, since $X$ is compact, $(x_\alpha)$ has a subnet $(x_\beta')$ converging to some $x\in X$. Since $(x_\beta')$ also converges to $x$, we have $p=x$ because $Y$ is Hausdorff. So $p\in X$.
\end{proof}

I have mentioned that non-Hausdorff spaces are not of interest in analysis. Thus, we mainly use the following special case of Prop. \ref{lb233}:
...
Recall that a similar property holds for complex metric spaces, cf. Prop. \ref{lb86}.
...
\begin{df}
We say that a topological space $X$ is \textbf{separable} \index{00@Separable} if $X$ has a countable dense subset. We say that $X$ is \textbf{second countable} \index{00@Second countable} if the topology of $X$ has a countable base (i.e., a base $\mc B$ with countably many elements).
\end{df}
...
\begin{pp}
If $Y$ is an open subset of a second countable space $X$, then $Y$ is second countable.
\end{pp}

\begin{proof}
Let $\mc B$ be a countable base for the topology of $X$. Then $\{Y\cap U:U\in\mc B\}$ is a countable base for the topology of $Y$.
\end{proof}
...
Let $\mc B$ be a base for the topology of $X$. Then the following are equivalent.
\begin{enumerate}[label=(\arabic*)]
...
\begin{proof}
Let $\mc B$ be a countable base for the topology of $X$. Let $\fk U$ be an open cover of $X$ such that each member of $\fk U$ is in $\mc B$. Then by discarding duplicated terms, $\fk U$ becomes countable. This verifies (2) of Prop. \ref{lb249}.
\end{proof}
...
\begin{proof}
Assume that $X$ is second countable with countable base $\mc B$. For each $U\in\mc B$, choose $x_U\in U$. Then one checks easily that $\{x_U:U\in\mc B\}$ is dense by checking that it intersects every nonempty open subset of $X$.
...
\begin{proof}
We claim that for every $\delta>0$, there is a finite set $E\subset X$ such that for every $x\in E$, the distance $d(x,E)<\delta$. Suppose that there is no such finite set for a given $\delta$. Pick any $x_1\in X$. Suppose $x_1,\dots,x_k\in X$ have been constructed. Then there is a point $x_{k+1}\in X$ whose distance to $\{x_1,\dots,x_k\}$ is $\geq \delta$. This defines inductively a sequence $(x_k)$ in $X$ such that any two elements have distance $\geq \delta$. So $(x_k)$ has no convergent subsequence, contradicting the sequential compactness of $X$.
...
\subsection{Finale: uniform convergence and equicontinuity -- two faces of $C(X\times Y,V)\simeq C(X,C(Y,V))$}
...
\begin{prob}
Let $(x_\alpha)_{\alpha\in I}$ be a net in the compact space $\ovl\Rbb$. Let $S$ be the (automatically nonempty) set of cluster points of $\ovl\Rbb$. Recall that $S$ is a closed subset by Rem. \ref{lb238}. For each $\alpha\in I$, define
\begin{gather}
...
Assume that $Y$ is compact. Let $(x_\alpha,y_\alpha)_{\alpha\in I}$ be a net in $X$. Assume that $x\in X$ is a cluster point ot $(x_\alpha)$. Prove that there exists $y\in Y$ such that $(x,y)$ is a cluster point of $(x_\alpha,y_\alpha)$.
\end{prob}
...
\begin{proof}[Hint]
Let $(f_\alpha)_{\alpha\in I}$ be a net in $S$ where $f_\alpha=(f_\alpha(1),f_\alpha(2),\dots)$. Use Pb. \ref{lb240} to construct inductively an element $x=(x(1),x(2),\dots)\in S$ such that for every $n\in\Zbb_+$, the element $x(n)$ is a cluster point of $(f_\alpha(n))_{\alpha\in I}$ in $X_n$. Prove that $x$ is a cluster point of $(f_\alpha)_{\alpha\in I}$ in $S$.
\end{proof}
...
Let $X$ be an LCH space. Let $W$ be an open subset of $X$. Let $A$ be a compact subset of $W$. Then there is a precompact open set $U\subset X$ such that $A\subset U\subset \Cl_X(U)\subset W$.
\end{pp}

\begin{proof}
Case 1: $A=\{x\}$ and $X$ is compact Hausdorff.  Then $B=X\setminus W$ is compact by Cor. \ref{lb234}. Since $X$ is Hausdorff, for each $y\in B$ there exists $V_y\in\Nbh_X(y)$ such that $x\notin \ovl V_y$. (Recall Cor. \ref{lb243}.) Since $B$ is compact, there are $y_1,\dots,y_n\in B$ such that $B\subset V$ where $V=V_{y_1}\cup\cdots\cup V_{y_n}$. By \eqref{eq74}, $\ovl V=\ovl V_{y_1}\cup\cdots\cup \ovl V_{y_n}$. So $x\notin\ovl V$. Let $U=X\setminus \ovl V$. Then $x\in U$. Since $U\subset X\setminus V$ and $X\setminus V$ is closed, $\ovl U\subset X\setminus V$, and hence $\ovl U\subset W$. Since $X$ is compact, $U$ is precompact.\\[-1ex]

Case 2: $A=\{x\}$ and $X$ is LCH. Choose a precompact $\Omega\in\Nbh_X(x)$. By case 1 (applied to $\ovl\Omega$), and by the definition of subspace topology, there exists $V\in\Nbh_X(x)$ such that $x\in V\cap \ovl\Omega\subset\Cl_{\ovl\Omega}(V\cap \ovl\Omega)\subset W\cap\ovl\Omega$. By Rem. \ref{lb182}, we have $\Cl_{\ovl\Omega}(V\cap \ovl\Omega)= \ovl{V\cap\ovl\Omega}\cap\ovl\Omega$, which contains $\ovl{V\cap\Omega}$ since both $\ovl{V\cap\ovl\Omega}$ and $\ovl\Omega$ do. So $x\in U\subset\ovl U\subset W$ where $U=V\cap\Omega$. Since $\Omega$ is precompact, $U$ is precompact. \\[-1ex]

Case 3: General case. For each $x\in A$, Case 2 shows that there exists a precompact $U_x\in\Nbh_X(x)$ such that $x\in U_x\subset\ovl U_x\subset W$. Since $W$ is compact, there exist some $x_1,\dots,x_m\in A$ such that $A\subset U$ where $U= U_{x_1}\cup\cdots\cup U_{x_m}$. (Recall Rem. \ref{lb244}.) By \eqref{eq74}, we have $\ovl U=\ovl U_{x_m}\cup\cdots\cup\ovl U_{x_m}$. So $\ovl U$ is compact and $\ovl U\subset W$.
\end{proof}


\begin{exe}
Let $X$ be LCH. Let $\mc V$ be a normed vector space. Let $(f_\alpha)_{\alpha\in I}$ be a net in $C(X,\mc V)$ converging pointwisely to $f:X\rightarrow 
\mc V$. Assume that  net $(f_\alpha)$ converges uniformly on every precompact open subset $U$ of $X$. (Note that the limit function must be $f|_U$.) Prove that $f$ is continuous. We say that $(f_\alpha)$ converges \textbf{locally uniformly} \index{00@Locally uniform convergence} to $f$.
\end{exe}
...
\begin{prob}
Let $(X,\mc T)$ be a second countable LCH space. Prove that the topology $\mc T$ has a base $\mc B$ whose members are all precompact.
\end{prob}

\begin{proof}[Hint]
Choose a countable base $\{U_1,U_2,\dots\}$ of $\mc T$. Write each $U_m$ as a countable union of open sets with compact closures in $X$.
\end{proof}
...
\begin{proof}[Hint]
Part 1: Choose an infinite countable base $\mc B=(U_1,U_2,\dots)$ for the topology of $X$ where each $U_n$ is nonempty. (Why can you do so?) Use Urysohn functions (Rem. \ref{lb257}) to construct $f_n:X\rightarrow [0,1]$ such that $f^{-1}(0)=X\setminus U_n$.

Part 2: Notice Exe. \ref{lb258}.
\end{proof}
...
\item $X$ is homeomorphic to a closed subset of $[0,1]^{\Zbb_+}$.
\end{enumerate}
\end{thm}
