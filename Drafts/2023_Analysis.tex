...
Topics (except those in the problem sets) marked with $\star\star$ are technical and/or their methods are rarely used in later studies. You can skim or skip them on first reading. Topics (except those in the problem sets) marked with $\star$ are interesting, but not necessarily technical. They are not essential for understanding the later part of the notes.

Topics in the problem sets marked with $\heartsuit$ are important problems that will be used later in this course.
...
\end{itemize}
Equivalently, this means that every (nonempty) open ball centered at $x$ contains all but finitely many $x_n$. 
\end{df}
...
\begin{gather}
\text{Property $P$ holds for } \textbf{sufficiently large} \text{ $n$}
\end{gather}
This means that ``there exists $N\in\Zbb_+$ such that $P$ holds for all $n\geq N$". Then $\lim_{n\rightarrow\infty} x_n=x$ means that ``for every $\varepsilon>0$, we have $d(x_n,x)<\varepsilon$ for sufficiently large $n$".
...
The following are some elementary examples and properties of convergent sequences.
...
\begin{pp}
Let $\Fbb\in\{\Qbb,\Rbb\}$ and $(x_n),(y_n)$ be sequences in $\Rbb$ converging to $x,y\in\Rbb$. If $x_n\leq y_n$ for all $n$, then $x\leq y$.
\end{pp}
...
\begin{eg}\label{lb27}
Let $(x_n)_{n\in\Zbb_+}$ be a sequence in $X$, and let $x\in X$. It is easy to see that
\begin{align*}
...
We now give some criteria for the divergence of a sequence.
...
One may wonder if the two criteria in Rem. \ref{lb26} are complete in order to determine whether a sequence diverges. This is true for sequences in $\Rbb^n$. We will discuss this topic later. 
...
\begin{rem}
One can compare the definition of continuous maps to that of linear maps. A map is continuous if it \emph{preserves the convergence of sequences}, i.e., if it maps convergent sequences to convergent ones. A map (between vector spaces) is linear if it perserves the addition and the scalar multiplication of vectors. In general, a good map between two sets with ``structures" is a map which preserves the structures. (Thus, linear combinations encode the linear structures of vector spaces. Similarly, the convergence of sequences remembers the ``topological" structures of metric spaces.)
\end{rem}
...
\begin{co}[\textbf{Squeeze Theorem}]\index{00@Squeeze Theorem}
...
\begin{proof}
Let $a_n=y_n-x_n$ and $b_n=z_n-x_n$. Then $0\leq a_n\leq b_n$, and $\lim_n b_n=\lim_n z_n-\lim_n x_n= 0$ because the subtraction map is continuous (Prop. \ref{lb40}). By Exp. \ref{lb28}, $a_n\rightarrow 0$. Since $x_n\rightarrow A$, $y_n=x_n+a_n$ converges to $A$, since the addition map is continuous by Prop. \ref{lb40}.
\end{proof}
...
\begin{eg}
By Prop. \ref{lb41}, $f(x,y,z)=x^2y+5y^4z^7-3xyz^2$ is a continuous function on $\Cbb^3$. Clearly $z\in\Cbb\mapsto \ovl z\in\Cbb$ is continuous. So $g(x,y,z)=f(\ovl x,\ovl y,z)-5xy^{-2}\ovl{z^{-3}}$ is a continuous function on $\Cbb\times\Cbb^\times\times\Cbb^\times$.
\end{eg}
...
\begin{pp}\label{lb42}
Let $X$ be a metric space and $E\subset X$. Define \textbf{distance function} \index{00@Distance function $d_E=d(\cdot,E)$} \index{dE@$d_E(x)=d(x,E)$} 
\begin{gather*}
d_E:X\rightarrow\Rbb_{\geq0}\\
d_E(x)\equiv d(x,E)=\inf_{e\in E}d(x,e)
\end{gather*}
Then $d_E$ is has Lipschitz constant $1$. So $d_E$ is continuous.
\end{pp}
...
\end{align}
This proves that $d_E$ has Lipschitz constant $1$.
\end{proof}
