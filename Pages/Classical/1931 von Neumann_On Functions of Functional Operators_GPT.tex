...
\newcommand{\translatorfootnote}[2]{%
  \begingroup
  \renewcommand{\thefootnote}{#1}%
  \footnote{\hspace{0.8em}\textit{Translator's note:} #2}%
  \endgroup}
...
Before approaching our proper subject, however, we must prove an auxiliary theorem about Lebesgue integrals which, together with some consequences, may also be of some
...
\[
 \operatorname{meas}(\mathfrak Q_b+\mathfrak P_b)
 \geq
 \operatorname{outer\,meas}(\mathfrak Q_x+\mathfrak P_x).
\]
The set $\mathfrak Q_b+\mathfrak P_b$ contains every $I_n$; if we denote it again by $\mathfrak P_b$, we find that it suffices to consider the preceding relation for those $\mathfrak P_b$ which contain all the $I_n$.
...
\]
All these functions $f_n(x)$, $n=1,2,\ldots$, can now be combined into a single
measurable function $\overline f(x)$ defined on $0<x<1$ (at
$x=\tfrac12,\tfrac13,\ldots$, put, say, $\overline f(x)=0$). Thus the problem is to
choose $G(u)$ so that the set of $x$ for which $\overline f(x)\in\mathfrak P$ is a
Lebesgue null set. Let $\psi(b)$ be the measure function of the measure function of
$\overline f(x)$. By \hyperref[thm:1]{Theorem 1}, the set of $x$ for which
$\overline f(x)\in\mathfrak P$ is certainly a Lebesgue null set whenever the set of
$b$ for which $\psi(b)\in\mathfrak P$ is one (take $g(u)=1$ for
$u\in\mathfrak P$ and $g(u)=0$ otherwise). We may therefore replace
$\overline f(x)$ by $\psi(b)$.
...
\phantomsection\label{eq:LS-definition-finite}
\begin{equation*}
...
\begin{description}
 ...
 If $f(x)=g(x)$ on a set $\mathfrak M$ as in \hyperref[prop:I.4.e]{e)},
 then\translatorfootnote{T1}{In property f) on source p.~198, the right-hand integral
 is printed with $d\psi(x)$, whereas the left-hand integral and the definition both
 require $d\varphi(x)$.  The differential has been corrected accordingly.}
 \[
  \int f(x)\,d\varphi(x)=\int g(x)\,d\varphi(x).
 \]
...
\phantomsection\label{eq:LS-change-finite}
\begin{equation*}
...
\phantomsection\label{eq:LS-restricted}
\begin{equation*}
...
\begin{description}
 ...
 Let $\overline{\mathfrak M}_1,\overline{\mathfrak M}_2,\ldots$ be pairwise disjoint $\Phi$-measurable sets (finitely or countably many), and let $\overline{\mathfrak M}=\overline{\mathfrak M}_1+\overline{\mathfrak M}_2+\cdots$. Then
\end{description}

% Source PDF page 10 / journal p. 200
\phantomsection\label{eq:LS-countable-additivity}
\begin{equation*}
...
\[
 ...
 \qquad \varepsilon>0.
\]
If $\psi(y)$ is the measure function of $\varphi(x)$ on $0,\alpha$, then the measure
function of $\varphi(x-\varepsilon)$ on $0,\alpha+2\varepsilon$ equals
$\psi(y)+\varepsilon$ for $0\leq y<\varphi(\alpha)$, and equals
$\alpha+2\varepsilon$ (not $\psi(y)+\varepsilon=\alpha+\varepsilon$) for
$y=\varphi(\alpha)$; indeed, $0\leq y\leq\varphi(\alpha)$ is the
 domain.\translatorfootnote{T3}{In this sentence on source p.~200,
$\varphi(x+\varepsilon)$ and $0\leq\alpha\leq\varphi(\alpha)$ are printed.  The
surrounding construction and the adjacent formulas require
$\varphi(x-\varepsilon)$ and $0\leq y\leq\varphi(\alpha)$, which have been restored.}
It follows that
\phantomsection\label{eq:LS-finite-line-agreement}
\begin{equation*}
 \int_0^{\alpha+2\varepsilon}f(x-\varepsilon)\,d\varphi(x-\varepsilon)
...
\phantomsection\label{eq:LS-change-restricted}
\begin{equation*}
...
\phantomsection\label{eq:BV-decomposition}
\begin{equation*}
...
\phantomsection\label{eq:LS-BV-definition}
\begin{align*}
 \int_{\overline{\mathfrak M}}f(x)\,d\varphi(x)
 &={}
...
\begin{description}
 ...
 \phantomsection\label{eq:LS-substitution-nested}
 \begin{equation*}
...
\phantomsection\label{eq:nested-RS-left}\label{eq:nested-RS-right}
\begin{align*}
 \int f(x)\,d\left[\int^x g(y)\,d\varphi(y)\right]
 &=\int f(x)\,
...
\phantomsection\label{eq:Hilbert-spectral-form}
\begin{equation*}
 (Af,g)=\int \lambda\,d(E(\lambda)f,g),
\end{equation*}
where $\lambda$ is the variable of integration and $E(\lambda)$ is the resolution of the identity belonging to $A$. (The notation is that explained in \hyperref[ref:E]{E} and \hyperref[ref:A]{A} at the places cited in \noteref{24}.) By $f,g,\ldots$ and $\varphi,\psi,\ldots$ we shall henceforth understand
...
(\hyperref[prop:I.4.f]{I.4(f)} and \hyperref[sec:I.5]{I.5}, together with
...
\hyperref[prop:I.4.f]{I.4(f)}), and consequently even for those of the zeroth
class (\hyperref[prop:I.4.g]{I.4(g)}), that is, for continuous functions.  The
integrals are then Riemann--Stieltjes integrals and hence, respectively,
limits of\translatorfootnote{T4}{On source pp.~203--204, the three paired equal sums alternate $F(\lambda_n)$ and $F(x_n)$ although $x_n$ is undefined, and four integrals with differential in $\lambda$ print $F(x)$. The arguments have been normalized here to $\lambda_n$ and $\lambda$, respectively.}
\begin{align*}
...
\begin{align*}
 \int |F(\lambda)|\,d|E(\lambda)f|^2
 &\leq \int C\,d|E(\lambda)f|^2
  =C\bigl(|1f|^2-|0f|^2\bigr)=C|f|^2,\\
 \int |F(\lambda)|\,d|E(\lambda)g|^2
...
\[
 \int F(\lambda)\,d(E(\lambda)f,g)=L(g)
\]
...
\begin{description}
 ...
 \(\xi=\varrho(x)\), \(x=\varrho^{-1}(\xi)\), be a monotone increasing,
 continuous, and therefore one-to-one, mapping of an interval
 \(0<x<\alpha\) onto \(-\infty<\xi<\infty\).  The particular choice of
 \(\varrho(x)\) is immaterial, for, as is easily seen, the expression
 \(\varrho(\psi_f(y))\) to be defined is only apparently dependent on it.
 Indeed, \(\varrho(\psi_f(y))\) is to be understood as the inverse, in the
 sense sketched at the end of \hyperref[sec:I.1]{I.1}, of
 \(|E(\xi)f|^2\).  Form
 \[
   \Phi_f(x)=|E(\varrho(x))f|^2
 \]
 and its measure function \(\psi_f(y)\), and then form
 \(\varrho(\psi_f(y))\).  For every \(f\), the set of all \(y\) for which
...
\(0<x<\alpha\) by \(\lambda=\varrho(x)\), \(x=\varrho^{-1}(\lambda)\)
...
Likewise, \hyperref[prop:II.2.h]{(h)} follows immediately if the convergence
\(F_n(A)\to F(A)\) asserted there is understood
only in the sense of ``weak'' convergence in \(\mathbf B\)
(cf.\ \noteref{35}); that is, if one only needs to prove
\[
...
\[
 ...
 =\int H(\lambda)\,d\left[
  ...
  \right],
\]
and this is precisely the assertion of
\hyperref[prop:I.5.i]{I.5(i)}.\translatorfootnote{T5}{The source cites I.4(i) here,
but the identity used is property I.5(i); the reference and its hyperlink have been
corrected.}
...
\(f\) the set of those \(y\) for which \(\varrho(\psi_f(y))\), formed as in
...
\(y\) for which \(\varrho(\psi'_f(y))\), now formed as in
\hyperref[prop:II.2.g]{(g)} for \(A\),
belongs to the set on which \(F(x)\) is altered--that is,\translatorfootnote{T6}{In
the corresponding last expression on source p.~207, $H$ is applied directly to
$\psi'_f(y)$; the intervening map $\varrho$ is evidently omitted.  It has been
restored to give the spectral variable used throughout this argument.} for which
\(H(\varrho(\psi'_f(y)))\in\mathfrak A\)--must be a Lebesgue null set.  By
...
\[
 ...
 \qquad\text{(cf.\ \hyperref[prop:II.2.a]{(a)}).}
\]
...
\(\varrho(\psi_f(y))\), apart from a Lebesgue null set of \(y\)'s.

Suppose that for every \(\epsilon>0\) and \(\eta>0\) there is a
\(v_0=v_0(\epsilon,\eta)\) such that, for every fixed \(v\geq v_0\), the
set\translatorfootnote{T7}{On source p.~209, the defining inequality for
$\mathfrak M_v(\epsilon)$ is printed as $\geq\epsilon$.  The proof immediately uses
the complement as the $\geq\epsilon$ set, so the definition has been corrected to
$<\epsilon$.}
\[
 \mathfrak M_v(\epsilon)
 =\bigl\{x:|F_{m_v}(x)-F_{n_v}(x)|<\epsilon\bigr\}
\]
contains all \(\varrho(\psi_f(y))\), with the exception of a set of \(y\)'s
...
\]
This intersection contains all \(\varrho(\psi_f(y))\), apart from a set of
...
\(\varrho(\psi_f(y))\) belong to \(\mathfrak M\), apart from a set of
...
\(\varrho(\psi_f(y))\) on a set of \(y\)'s of Lebesgue measure at least
\(\eta\).

For these \(v\), however,\translatorfootnote{T8}{On source pp.~209--210, the
corresponding norm identity omits the second factor after the adjoint and prints the
scalar differences without absolute-value bars.  The translation restores the
factors required by $|Tf|^2=(T^*Tf,f)$.}
\begin{align*}
...
\begin{align*}
 &=\int_0^\alpha
   |F_{m_v}(\varrho(x))-F_{n_v}(\varrho(x))|^2\,
   d|E(\varrho(x))f|^2\\
 &=\int_0^{|f|^2}
   \left|F_{m_v}(\varrho(\psi_f(y)))
        -F_{n_v}(\varrho(\psi_f(y)))\right|^2\,dy\\
 &\geq
...
sequence \(F_1(x),\allowbreak F_2(x),\allowbreak\ldots\) occurring there is replaced by
\[
...
every given pair \(m,n\geq v_0\), the set\translatorfootnote{T9}{On source p.~211,
the defining inequality for $\mathfrak M_{m,n}(\epsilon)$ is printed as
$\geq\epsilon$.  The following argument requires this to be the complementary set,
so the definition has been corrected to $<\epsilon$.}
\[
 \mathfrak M_{m,n}(\epsilon)
 =\{x:|F_m(x)-F_n(x)|<\epsilon\}
\]
contains every \(\varrho(\psi_f(y))\), with the exception of a set of
...
\(y\)'s for which \(\varrho(\psi_f(y))\) does not belong to that set has
...
\(\Phi_f(x)=|E(\varrho(x))f|^2\).  The Arzel\`a--Young theorem\noteref{39}
...
\(x=\varrho(\psi_f(y))\), the inequality
\[
...
\(x=\varrho(\psi_f(y))\) produces a sequence
...
Then
\[
 |F_{n_{p_v}}(x)-F_{n_{p_{v+1}}}(x)|\leq\frac1{2^v}
\]
for every \(\varrho(\psi_f(y))\), with the exception of a set of \(y\)'s of
...
\(x=\varrho(\psi_f(y))\), if \(q>v\geq\mu\), then
...
The set of \(y\)'s for whose \(x=\varrho(\psi_f(y))\) it diverges thus has
...
every \(\varrho(\psi_{f_n}(y))\), except for a set of \(y\)'s of measure
...
\[
 ...
 \qquad(v\to\infty),
\]
both for every \(x=\varrho(\psi_{f_l}(y))\), except for a set of \(y\)'s of
...
\]
for every \(\varrho(\psi_{f_l}(y))\), with the exception of a set of \(y\)'s
of measure \(0\).  Hence \(F'(x)=F''(x)\) for every such\translatorfootnote{T10}{On
source p.~212, this conclusion is printed as applying to
$\psi_{f_l}(y)$.  The preceding sentence and the definition in II.2(g)
show that the intended spectral points are $\varrho(\psi_{f_l}(y))$; the
omitted $\varrho$ has been restored.}
\(\varrho(\psi_{f_l}(y))\), apart from a null set of \(y\)'s, and this holds
...
must hold at every \(\varrho(\psi_f(y))\), with the exception of a set of
...
\hyperref[thm:5]{Theorem~5}, gives a subsequence whose associated set
\(\mathfrak M\) has the desired property for every
...
\(\varrho(\psi_f(y))=0\) occurs only on a Lebesgue null set; in other words,
\(0\) is not the value of this function on an interval of constancy.
Equivalently, \(\varrho^{-1}(0)\) is not a constancy-interval value of
\(\psi_f(y)\), or \(\varrho^{-1}(0)\) is not a discontinuity point of
\[
 \Phi_f(x)=|E(\varrho(x))f|^2,
\]
...
$\mathbf R(A)$ (cf.\ \hyperref[ref:A]{A}, p.~401, Theorem~10), with $A$ Hermitian; as is easily
...
$\alpha(y_1,\ldots,y_n)$; that is,\translatorfootnote{T11}{On source p.~217, the two
occurrences following the initial definitions of $\alpha$ and the $\beta_i$
interchange those symbols.  The translation uses the consistent convention stated
in the definitions: substitute the $\beta_i(y)$, then replace $y$ by
$\alpha(y_1,\ldots,y_n)$.}
\[
...
linear operators (cf.\ \hyperref[ref:A]{A}, p.~389); that is, every $A_m$ is to commute with every $A_n$
...
The ring $\mathbf R(A_1,A_2,\ldots)$ is Abelian and hence, by \hyperref[ref:A]{A}, p.~401,
...
$B=K(A)$.  It follows that\translatorfootnote{T12}{On source p.~219, the second
left-hand member in the following display is printed simply as $G_2$.  The missing
argument $A$ has been restored, in agreement with the preceding identities and the
parallel formula for $G_1$.}
\[
...
\[
 ...
 \end{cases}
\]
Then $H(x)$ and $H'(x)$ become, respectively,\translatorfootnote{T13}{In the displayed
definition on source p.~219, the condition in the $H'_D(x)$ branch is printed as
$|H(x)|\leq D$.  Since this branch defines $H'_D$ and the surrounding argument uses
$H'$, the intended condition $|H'(x)|\leq D$ has been restored.}
\[
 H_D(x)=
 \begin{cases}
   H(x), & |H(x)|\leq D,\\
...
$R$ belongs to $\mathbf R(R)=\mathbf R(\mathbf M)$ and hence, by \hyperref[ref:A]{A}, p.~398,
...
\hyperref[sec:IV.3]{IV, 3, a)--d)},\translatorfootnote{T14}{On source p.~221, this reference is printed as IV.2(a)--(d). These properties are stated in IV.3; the section number and hyperlink have been corrected.} the $n$th term of our operator sequence is then
...
functions converging everywhere (cf. \hyperref[sec:IV.3]{IV, 3, f)}\translatorfootnote{T15}{On source p.~222, this reference is printed as IV.2(f). Property (f) is stated in IV.3; the section number and hyperlink have been corrected.}), three
...
\[
 a\,1+B,
\]
where $a$ is a complex number and $B\in \mathbf R(\mathbf M)$.  By \hyperref[ref:A]{A}, p.~397,
...
works by \hyperref[ref:E]{E} and \hyperref[ref:A]{A}, respectively.  The reference here is to
\hyperref[ref:A]{A}, in particular to the end
of Chapter~III, \S2.
...
\originalnote{2}The Hermitian (bounded; cf.\ \hyperref[ref:A]{A}, Introduction, 1) operators $A,B$ are
called commuting if $AB=BA$.

\originalnote{3}For the description of Hilbert space, cf.\ \hyperref[ref:E]{E}, Chapter~I (and also
Introduction I--IV and Appendix~I).  For operators, cf.\ \hyperref[ref:E]{E}, Chapter~II and Appendix~II,
\S1.  Everything needed is also collected once more in \hyperref[ref:A]{A}, Introduction, 1.  The ring
of all bounded operators mentioned immediately below is introduced in \hyperref[ref:A]{A},
Introduction, 2.  We denote points of Hilbert space by
$f,g,\ldots,\varphi,\psi,\ldots$; bounded operators by $A,B,\ldots$; and complex
numbers by $a,\ldots$ (as in \hyperref[ref:E]{E} and \hyperref[ref:A]{A}).

\originalnote{4}In matrix terminology, $A^*$ is the conjugate transpose of $A$; cf.
 \hyperref[ref:E]{E}, Appendix~II, \S1, or \hyperref[ref:A]{A}, end of Introduction, 1.

\originalnote{5}The definition is in \hyperref[ref:A]{A}, Chapter~II, \S1, Definition~1.  Cf. also
\hyperref[ref:A]{A},
Chapter~I, \S3, the supplement to Theorem~5.

\originalnote{6}Cf.\ \hyperref[ref:A]{A}, Chapter~I, \S1, Definition~2.

\originalnote{7}Cf.\ \hyperref[ref:A]{A}, end of Chapter~III, \S2.
...
mutually commuting operators.  Cf.\ \hyperref[ref:A]{A}, Chapter~III, beginning of \S1 and \S2.

\originalnote{9}Cf.\ \hyperref[ref:A]{A}, toward the end of Chapter~II, \S1.
...
some of the $I_n$, neither of these alternatives would have been required of
$\mathfrak{P}'_b$.
...
constancy, for it then contains all the values of $\psi(y)$.  Incidentally, it is
...
notation used in \hyperref[ref:E]{E} and \hyperref[ref:A]{A}.  Cf. in particular
\hyperref[ref:E]{E}, pp.~91--92, Definition~17 and
...
\]
is monotone nondecreasing in $\lambda$ and bounded (cf.\ \hyperref[ref:E]{E}, p.~114).

\originalnote{26}Cf.\ \hyperref[ref:A]{A}, p.~418, Appendix~I.

\originalnote{27}Cf.\ \hyperref[ref:E]{E}, p.~91, Definition~17.  $E\leq F$ means
$| Ef|\leq| Ff|$ for every $f$; $E_n\to E$ means
$E_nf\to Ef$ for every $f$ (in the sense of ``strong'' convergence).\translatorfootnote{T16}{On source p.~225, this notation is printed as $E_n\to F$, although the defining relation immediately following is $E_nf\to Ef$. The left-hand side has been corrected to $E_n\to E$.}
...
\originalnote{29}For $E\leq F$ one has in general\translatorfootnote{T17}{The displayed identity on source p.~225 lacks the minus sign on the right. Since its left-hand side is nonpositive for $E\leq F$, the missing sign has been restored.}
\[
 | Ef|^2-| Ff|^2
 =-|(E-F)f|^2;
\]
cf.\ \hyperref[ref:E]{E}, p.~78.
...
\[
 |(Ef,g)|\leq| Ef|\,| Eg|
\]
(cf.\ \hyperref[ref:E]{E}, p.~93).

\originalnote{31}Cf. the citation and proof in \hyperref[ref:E]{E}, p.~94, Note~52.
...
\originalnote{35}Cf.\ \hyperref[ref:A]{A}, p.~383.

\originalnote{36}Cf.\ \hyperref[ref:E]{E}, p.~65, condition C.
...
$f_n\to\overline{f}$ (for the terminology, cf., e.g., \hyperref[ref:A]{A}, pp.~378--379).  Indeed,
\begin{align*}
 | f_n-\overline{f}|^2
