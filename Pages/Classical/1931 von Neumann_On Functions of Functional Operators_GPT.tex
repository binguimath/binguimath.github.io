...
$y=\varphi(\alpha)$; indeed, $0\leq y\leq\varphi(\alpha)$ is the
 domain.\translatorfootnote{T2}{In this sentence on source p.~200,
...
(cf.\ \noteref{35}); that is, if one need only prove
\[
...
\hyperref[prop:I.5.i]{I.5(i)}.\translatorfootnote{T3}{The source cites I.4(i) here,
...
belongs to the set on which \(F(x)\) is altered--that is,\translatorfootnote{T4}{In
...
set\translatorfootnote{T5}{On source p.~209, the defining inequality for
...
For these \(v\), however,\translatorfootnote{T6}{On source pp.~209--210, the
...
every given pair \(m,n\geq v_0\), the set\translatorfootnote{T7}{On source p.~211,
...
of measure \(0\).  Hence \(F'(x)=F''(x)\) for every such\translatorfootnote{T8}{On
...
$\alpha(y_1,\ldots,y_n)$; that is,\translatorfootnote{T9}{On source p.~217, the two
...
$B=K(A)$.  It follows that\translatorfootnote{T10}{On source p.~219, the second
...
\[
 ...
 \end{cases}
\]
Then $H(x)$ and $H'(x)$ become, respectively,\translatorfootnote{T11}{In the displayed
...
\originalnote{15}Since one may put $\varphi(a)=a$, ordinary measurability follows from
