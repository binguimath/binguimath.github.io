...
\renewcommand{\thefootnote}{T\arabic{footnote}}
...
function \(P_n\) of \(n\), with \(P_1=0\) and \(P_n>0\) for \(n\geq2\),
such that
...
\begin{enumerate}[label=\(\arabic*^{\circ}\),leftmargin=2.4em]
 ...
 by taking \(P_1=0\) and \(P_n=1\) for \(n\geq2\);
 \item when \(\alpha_{n+1}\geq 1+\lambda_n\), for it is then enough to
 take \(P_1=0\) and \(P_n=\lambda_{n-1}\) for \(n\geq2\).
\end{enumerate}
...
Concerning the symbol \(\mathcal S\), we record the almost immediate
...
Consider a positive distribution of mass on an infinite straight line
\(OX\ldots\), with the mass \(m_i\) concentrated at distance \(\xi_i\)
from the origin \(O\). The sum
\[
 \sum_i m_i\xi_i^k
\]
...
\[
  ...
  {(u-x)^2+\varepsilon^2}.
\]
We shall see that as $\varepsilon\to0$ this expression has a finite limit. Split the integral into two parts,\translatornote{The source prints $0$ as the lower limit of the second integral in this display. We use $x$, as required by the stated decomposition and by the immediately following limit.}
\[
...
Suppose that $(a,b)$ is of the second kind and, in addition, that $a$ and $b$ are points of continuity of $\Phi(u)$ \phantomsection\label{erratum:j85}and of all the functions $\varphi_n(u)$. It follows that
\[
...
\[
 ...
 \qquad(a\leq u\leq b).
\]
This is an important result. It assumes that $(a,b)$ is of the second kind and that $a$ and $b$ are points of continuity of $\Phi(u)$ \phantomsection\label{erratum:j86}and of all the functions $\varphi_n(u)$.
...
\[
 \sum_{n=1}^{\infty}a_n
\]
diverges,\translatornote{The source prints $\sum_{n=0}^{\infty}a_n$. We use $n=1$, consistently with the indexing of the continued fraction throughout the paper.} this yields only one solution of the problem; and indeed we shall soon prove that in this case there are no others.
...
\]
and the cut extends only from $z=0$ to $z=-a$.\translatornote{The source writes $u=0$ and $u=-a$. We use $z$ because this is a cut in the complex $z$-plane.} All the moments are then finite as soon as this is the case for $c_0=\psi(a)$. The integral evidently admits the asymptotic expansion
\[
  \frac{c_0}{z}-\frac{c_1}{z^2}+\frac{c_2}{z^3}-\cdots,
\]
...
% Source page J.98
We shall resume this study here, focusing especially on the question of convergence, which was scarcely considered in the earlier works just mentioned, and in which the continued fraction was always taken in the form $(\mathrm{I}^{d})$ (see the Introduction).
...
The minimum of the expression\translatornote{After $d\psi(u)$, the source also prints a final $du$. We omit this evident duplicate differential.}
\[
  \int_0^\infty
...
\[
  \frac{c_0}{z}-\frac{c_1}{z^2}+\frac{c_2}{z^3}-\cdots
\]
is always divergent. Indeed, this occurs when
\[
  \frac{c_{n+1}}{c_n}
\]
...
\[
  \frac{1}{\displaystyle\sum_{k=0}^\infty a_{2k+1}}.
\]
This is impossible, which proves the stated proposition.
\end{proof}
...
\[
  \mu_1>\mu.
\]
Remove from $\omega_1$ the mass $\mu$, which can be done without introducing a negative mass; one obtains a distribution $\omega'_1$ equivalent to $\omega'$. Thus, if $\omega'$ is determinate, $\omega'_1$ and $\omega'$ must be identical, and consequently $\omega$ and $\omega_1$ must also be identical.
...
\]
Next let $f(\bar u)$ be a function which is zero in the interval
$(0,\alpha)$ and equal to $f(u)$ for $u>\alpha$. We shall have
\[
  \{f(\bar u)\,du\}_n
  =\int_0^\infty f(\bar u)\,\mathcal L(\bar u)^2\,du
  =\int_\alpha^\infty f(u)\,\mathcal L(\bar u)^2\,du,
\]
where $\mathcal L(\bar u)$ is a certain polynomial of degree $n$ in $u$
...
\[
  \{f(\bar u)\,du\}_n
  <\int_0^\infty f(\bar u)\,\mathcal L(u)^2\,du
...
we shall also have
\[
  \{f(\bar u)\,du\}_\infty=0.
\]
Next I observe that
\[
  \{f_1(u)\,du\}_n
  <\int_0^\infty f_1(u)\,\mathcal L(\bar u)^2\,du
  =\int_0^\alpha f_1(u)\,\mathcal L(\bar u)^2\,du
   +\int_\alpha^\infty f_1(u)\,\mathcal L(\bar u)^2\,du.
\]

% Source page J.118
Now the polynomial $\mathcal L(\bar u)$ is determined, apart from a
factor, by the conditions
\[
  \int_0^\infty f(\bar u)\,\mathcal L(\bar u)u^k\,du=0
  \qquad(k=1,2,\ldots,n),
\]
or
\[
  \int_\alpha^\infty f(u)\,\mathcal L(\bar u)u^k\,du=0.
\]
...
$\mathcal L(\bar 0)=1$, we see that in the interval $(0,\alpha)$
\[
  0<\mathcal L(\bar u)\leq1,
\]
and consequently
\[
  \int_0^\alpha f_1(u)\,\mathcal L(\bar u)^2\,du
...
\[
  ...
  <M_\alpha\int_\alpha^\infty
       f(u)\,\mathcal L(\bar u)^2\,du
  =M_\alpha\{f(\bar u)\,du\}_n;
\]
hence
\[
  \{f_1(u)\,du\}_n
  <\frac12\varepsilon+M_\alpha\{f(\bar u)\,du\}_n.
\]
But there exists a number $\nu$ such that, for $n>\nu$,
\[
  M_\alpha\{f(\bar u)\,du\}_n<\frac12\varepsilon,
\]
...
\origsection{65}
We conclude here our study of the integral
\[
  \int_0^\infty\frac{d\psi(u)}{z+u}
\]
% Source page J.121
...
\[
  \int_0^\infty\frac{d\psi(u)}{z+u}
\]
gives a convergent continued fraction, then the expression
\[
  \frac{\mu}{z}+\int_0^\infty\frac{d\psi(u)}{z+u},
\]
where $\mu$ is a positive constant, also has a convergent continued
fraction, except in one singular case. This
...
\noindent \hyperref[erratum:j85]{Page J.85, line 11 from the top}: after \emph{of $\Phi(u)$}, add
\emph{and of all the functions $\varphi_n(u)$}.

\noindent \hyperref[erratum:j86]{Page J.86, line 11 from the top}: after \emph{of $\Phi(u)$}, add
\emph{and of all the functions $\varphi_n(u)$}.
