...
\newcommand{\SectionStart}[1]{%
  \par\noindent\SectionTarget{#1}\hspace*{\parindent}\ignorespaces
}
...
\par\noindent\hypertarget{sec:introduction}{}\hspace*{\parindent}%
...
\hspace*{4em}Uniqueness. Properties III, IV.\\
...
\hspace*{4em}$\alpha\geq1$. Unrestricted weak continuity of $\TrM(A)$. Theorems II, III, IV.\\
...
\chapterhead{II}{Immediate consequences}
...
Then from the proof of Lemma 11.4.3, we can conclude that
...
$(*)$ $\TrM(A)$ agrees with the previous definition of $\TrM(A)$ if $A$ is Hermitian.

$(*\,*)$ For uniformly bounded or for definite $A$'s, $\TrM(A)$ is weakly continuous.
\begin{enumerate}[label=(\roman*)]
 \item $\TrM(1)=1$;
...
Now, $(*)$ and (v) are immediate consequences of the definition. (i), (iv),
and (iv)$'$ follow from $(*)$ and \RO{}, Theorem XIII. $A^*$ is a weakly continuous
additive function of $A$. Thus $B$ and $C$ are too, and we may infer (iii) and
$(*\,*)$ from \StatementRef{property:I}{Properties I} and
...
weak) convergence by Property III, (**). Thus our relation holds for all
...
\SectionStart{3.2}3.2. We return now to the normalization of
...
Property II or III, (**), gives $\TrM(B_0)=\lambda$.
Finally
...
\SectionStart{3.3}3.3. We hold $g$ fixed, and make the following
...
$(\mathrm A_G)$ (cf. the corollary to
...
\SectionStart{3.4}3.4. We still suppose that $g$ is held fixed.
...
corollary to \StatementRef{lemma:3.2.3}{Lemma~3.2.3}. This $E_0$ is a
...
Lemma~1.2 there exist two $F_3$ and $F_4$ (both
...
\SectionStart{3.5}3.5. We can now take the decisive step.
...
Property II, $A=A_1-A_2$, $A_1$ and $A_2$, in $M$ and positive definite.
Property I of \SectionRef{2.1} then yields
...
$M'_{(\mathfrak M_i)}=M_i'$ of Definition~11.3.1. By Lemma~11.3.2, these
...
respectively (Lemma~11.3.6).

The ranges of $D_{M_{(\mathfrak M)}}$ and $D_{M'_{(\mathfrak M)}}$ are by
Lemma~11.3.7, the intervals $(0,1/m)$, $(0,\alpha)$ respectively. So we can
...
\SectionStart{4.1}4.1. We assume that $M,M'$ is in case
...
% supplies the stated unitary for the given f_0 and g_0; retained verbatim.
(cf. \StatementRef{lem:4.2.1}{Lemma 4.2.1}). Then if \(X=YU\), \(X\) and
\(Y\in\Q(M)\), we have
\[
...
% the numbered corollary appears under Theorem X; retained verbatim.
\StatementRef{thm:XI}{Theorem XI}). Now (by the same
...
\SectionStart{4.6}\StatementRef{thm:XI}{Theorem XI} could be extended to cover cases
...
(cf. \StatementRef{def:4.3.1}{Definition 4.3.1}) is determined by the
...
We can use the results of \hyperlink{sec:4.2}{\(\S\S\)4.2--4.4}, and thus we may form the
