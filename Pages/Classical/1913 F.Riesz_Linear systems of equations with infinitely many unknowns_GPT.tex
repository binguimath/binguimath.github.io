...
\newcommand{\translatornote}[1]{%
  ...
  \begingroup
    ...
    \renewcommand{\thefootnote}{\alph{translatornote}}%
...
satisfy system~\eqref{eq:6:6}, and this fact is connected with the holomorphy, for $z=1$, of the function $f(z)$ defined above.
...
\end{align*}
Since $s(x)$ is odd together with $f(x)$, one has
\[
  C_1=s(0)=0;
\]
...
Since the quotient of the odd functions $\theta_1$ and $\theta$ is even, $\Lambda_{-\mu}=\Lambda_\mu$; consequently the system reduces to
\[
 (-1)^n=\Lambda_0+
...
\[
  F(z)=\prod_{m=1}^{\infty}\left[1-\frac z{(2m-1)^2}\right]
      =\cos\frac\pi2\sqrt z.
\]
The residues of $1/F(z)$ are\translatornote{The printed text gives the factor $\pi/4$ in both places. Direct differentiation of $F(z)=\cos((\pi/2)\sqrt z)$ instead gives the factor $4/\pi$ in both places.}
\[
 B_m=\frac1{F'(b_m)}
 =\frac{-\frac{\pi}{4}\sqrt{b_m}}{\sin(\frac\pi2\sqrt{b_m})}
 =\pm\frac{\pi}{4}(2m-1).
\]
...
\[
  A_k=\sum_{i=1}^{\infty}|a_{ik}|;
\]
by hypothesis $\sum_{k=1}^{\infty}A_k$ converges. A well-known convergence rule then implies that the products
\[
  \Pi_n=\prod_{k=1}^{n}(1+A_k)
\]
...
$\left(\begin{smallmatrix}i\\k\end{smallmatrix}\right)$ of the determinant $\Delta$. Similarly we shall call $\Delta^{(k)}$ the minor $\left(\begin{smallmatrix}i\\k\end{smallmatrix}\right)$ of $\Delta$. It follows at once from the analogous fact for finite determinants that the minor $\left(\begin{smallmatrix}i\\k\end{smallmatrix}\right)$ is unchanged if the elements of row $i$, except the $k$th, are replaced by zeros or by arbitrary quantities. This observation shows that rows and columns play the same role in forming minors.

We shall prove the following theorem, whose first part is the familiar Laplace rule.

\begin{theorem}\label{thm:21:laplace}
...
We suppose that $\sum_{i,k}|a_{ik}|=\sum_kA_k=A$ converges, and ask whether system~\eqref{eq:23:10} can be satisfied by quantities $x_k$ that in addition remain bounded in absolute value. Under these conditions the left-hand sides of equations~\eqref{eq:23:10} are absolutely convergent series.
...
\begin{theorem}[von Koch]\label{thm:24:koch}
...
Choose $m$ such that
\[
  R_m=\sum_{i=m+1}^{\infty}A_i<\frac12.
\]
Then
\[
 \left|
 \left(\begin{matrix}1,\ldots,m\\1,\ldots,m\end{matrix}\right)
 \right|
 \geq1-\sum_{i=m+1}^{\infty}A_i
       -\sum_{i,k=m+1}^{\infty}A_iA_k-\cdots
...
\begin{theorem}\label{thm:28:cycles}
...
\begin{theorem}\label{thm:29:squares}
...
\begin{proof}
The minors belong to the same type as $\Delta$, so it suffices to prove absolute convergence of $\Delta$. By Theorem~\ref{thm:28:cycles}, it is enough to prove convergence of the sum of the moduli of the cyclic products. Those of first order have been covered explicitly by the hypothesis; below we consider only orders greater than $1$.

Following von Koch's proof, put, in the notation of the original,
\[
...
\begin{align*}
 ...
 &\quad\leq\sigma^{2n}\sum_k|i_1,k|^2.
\end{align*}
...
\]
and then choose a sufficiently small positive $t$ so that
\[
...
As for the data, we make only one hypothesis. Under this hypothesis the equations~\eqref{eq:35:8} must have a meaning. But before carrying out the solution, we know nothing about the quantities $x_k$. We are therefore led to suppose that the left-hand sides of~\eqref{eq:35:8} converge for every system $(x_k)$ for which~\eqref{eq:35:9} exists. This is equivalent to assuming that, for every $i$, the sums
\[
...
\end{equation}
cannot converge for every system $(x_k)$ for which~\eqref{eq:35:9} exists unless
\begin{equation}
...
If the series~\eqref{eq:35:10} converges for every system $(x_k)$ for which~\eqref{eq:35:9} exists, then the series~\eqref{eq:35:11} converges.
\end{theorem}
\begin{proof}
...
\]
whatever the values $\mu_1,\ldots,\mu_n$. Since, on the other hand, by~\eqref{eq:37:15},
\[
...
\end{theorem}
\begin{proof}
Consider the sequence of quantities $y_1^{(n)}$ $(n=1,2,\ldots)$. Their absolute values remain at most $G$. Hence, by Weierstrass's theorem, the sequence has one or more limiting values. Let $y_1^*$ be any one of them, for example the least of these limits, and let $n_1',n_2',\ldots$ be a sequence of indices such that
\[
...
\end{equation}
Now the Hermitian form
\[
  \sum_{i,j=1}^{n}\alpha_{ij}\nu_i\overline{\nu_j}
  =\sum_{k=1}^{\infty}\left|\sum_{i=1}^{n}a_{ik}\nu_i\right|^2
\]
...
\eqref{eq:52:55}. On the other hand, for every $p>1$,
\[
...
\begin{equation}
  ...
  \tag{1}\label{eq:55:1}
\end{equation}
Indeed, if no such constant existed, one could choose a sequence of
elements for which the quotient of the left-hand side by the right-hand
side tends to infinity. After multiplying the successive elements by
suitable constants and applying the selection procedure of
\hyperref[sec:41]{no.~41}, one would obtain a sequence tending to zero whose
transforms do not tend to zero, contrary to continuity.
...
coordinates vanish, distributivity and the preceding observation give
\begin{equation}
  \sum_{i=1}^{\infty}
...
$A^{-1}$: the two quotients displayed in \eqref{eq:60:13} and
\eqref{eq:60:14} must remain above some fixed number $m^2>0$. We shall show
...
$S_n=E+A+\cdots+A^{n-1}$ and suppose $n>m$. Then
\[
...
resulting elements are linear expressions in a finite number of those elements.
...
\emph{Annales de la Faculté des Sciences de Toulouse}, vol.~XI (1908), pp.~5--98.}
...
unlimited process.  On the one hand, one must sum a countable infinity of terms
...
extreme values of \(f(\mu)\) on the interval \((m,M)\).}
...
\(f(\mu^*)\ge f_m(\mu^*)-\varepsilon\ge g(\mu^*)\), contrary to the hypothesis.
Thus for all sufficiently large \(n\), \(f_m-\varepsilon<g_n\), and therefore
\(f_m(A)-\varepsilon E<g_n(A)\).  This implies
\(f_m(A)-\varepsilon E\le g(A)\), and since \(m\) and \(\varepsilon\) are
...
for every \(k\).  Since \(f'\) is the limit of an increasing sequence of
continuous functions, the established correspondence attaches to it a uniquely
determined form \(f'(A)\).  Introduce also the functions \(f_\xi(\mu)\), one for
...
intégrales,'' \emph{Annales de l'École Normale} (1911), pp.~31--62, and, in
...
January 12, 1910.}  In particular, putting
...
\(A_\xi^+(x,x)\); by this we mean the form corresponding to the function
\(f(\mu)=1\) for \(\mu\le\xi\) and \(f(\mu)=0\) for \(\mu>\xi\).  Plainly,
\[
  P_\xi=A_\xi^+-A_\xi.
\]
Regard \(A_\xi\) and \(A_\xi^+\) as functions of \(\xi\).  They are monotone:
as \(\xi\) increases, they too increase, or at least do not decrease.  Thus
the four limits
\(A_{\xi-0},A_{\xi+0},A^+_{\xi-0},A^+_{\xi+0}\) exist.  Moreover, since
\(A_\xi^+\ge A_\xi\) always, while for \(\varepsilon>0\)
\[
  A^+_{\xi-\varepsilon}\le A_\xi,
  \qquad A_{\xi+\varepsilon}\ge A_\xi^+,
\]
we have
\[
  A_{\xi-0}\le A^+_{\xi-0}\le A_\xi,
  \qquad
  A^+_{\xi+0}\ge A_{\xi+0}\ge A_\xi^+.
\]
Finally, the proposition stated at the end of
\hyperref[sec:91]{no.~91} gives
\(A_{\xi-0}=A_\xi\) and \(A^+_{\xi+0}=A_\xi^+\).  Consequently,
\[
  A_{\xi-0}=A^+_{\xi-0}=A_\xi,
  \qquad
  A_{\xi+0}=A^+_{\xi+0}=A_\xi^+.
\]
In other words, the forms \(A_\xi\) and \(A_\xi^+\), considered as functions
...
\]
can be made arbitrarily small by a suitable choice of \(n\) and the \(c_k\).
...
the spectral forms.  Let \(\mathcal G_\xi,\mathcal G_\xi^+,\mathcal G'_\xi\)
denote the sets of values \(\mu\) for which, respectively,
\(u(\mu)<\xi\), \(u(\mu)\le\xi\), and \(u(\mu)=\xi\).  The coefficients of
\(A_\xi,A_\xi^+,P_\xi\) are given by
\[
  \int \varphi_i(\mu)\varphi_k(\mu)\,d\mu,
\]
where the integral extends over \(\mathcal G_\xi,\mathcal G_\xi^+\), and
\(\mathcal G'_\xi\), respectively.  If, for example, \(u(\mu)\) is continuous
...
\[
  u_1(\mu)=\frac{u(\mu)+u(-\mu)}2,
\]
and \(A^{(2)}\) to the odd function
\[
  u_2(\mu)=\frac{u(\mu)-u(-\mu)}2.
\]
...
The extreme values of \(u(\mu)=\mu\) on \((-\pi,\pi)\) are \(-\pi\) and
\(\pi\)\translatornote{The printed text writes \(u(\mu)=\mu\) here, although
the function just chosen is the preceding piecewise function.  Both functions
have lower and upper bounds \(-\pi\) and \(\pi\).}; hence these are also the
...
expressions fail to represent bounded forms.  Consider, for example,
\[
  \sum_{i,k=1}^{\infty}\frac{x_ix_k}{(i+k)^\alpha},
  \qquad 0<\alpha<1.
\]
Put \(x_1=x_2=\cdots=x_n=1/\sqrt n\) and
\(x_{n+1}=x_{n+2}=\cdots=0\), consistently with \(E(x,x)=1\).  All terms for
which \(i+k\le n+1\) are at least \(1/[n(n+1)^\alpha]\), and there are more
than \(n(n+1)/2\) of them; thus the value of the expression exceeds
\(\tfrac12(n+1)^{1-\alpha}\), and grows without bound with \(n\).  Likewise,
...
\[
  ...
  =\Delta_i(\rho',z)
\]
converges whenever \(r<|z|<R\).  Therefore
\[
  z^{\rho'}\Delta_i(\rho',z)
\]
...
true of every admissible solution \((\psi_k)\) of \eqref{eq:110:14}.  The
function
\[
...
\(\varphi(s)\) only up to the addition of a function whose integral is zero.
...
the form \(cE+K(A)A\).  I cannot enter into details without going deeply into
...
Given a sequence extending indefinitely in both directions \((a_i)\), attach
to it a matrix \((a_{ik})\) by putting \(a_{ik}=a_{k-i}\).  This matrix belongs
...
\end{equation}
Let \(\mathcal C\) be the set of values \(s\) for which
\(|\alpha(-s)|>M_A\), and put \(x(s)=1\) on this set and zero elsewhere.
Inequality \eqref{eq:114:19} gives
\[
  \int_{\mathcal C}|\alpha(-s)|^2ds\le M_A^2\int_{\mathcal C}ds.
\]
Since \(|\alpha(-s)|>M_A\) on \(\mathcal C\), the set \(\mathcal C\) must
...
the dummy variable \(t\) in the standard coefficient formulas.}
\[
...
\[
  \lim_{n\to\infty}[A(x^{(n)},x^{(n)})]_n=A(x,x)
\]
shows that, for sufficiently large \(n\),
\([A(x^{(n)},x^{(n)})]_n>M-2\varepsilon\).  Consequently
\(M_{n-l}\) increases to \(M\) as \(n-l\to\infty\).  Similarly,
\(m_{n-l}\to m\), or equivalently
\[
