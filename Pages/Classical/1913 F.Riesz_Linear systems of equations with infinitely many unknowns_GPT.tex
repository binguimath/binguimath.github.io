...
\newcommand{\translatornotelabel}{T\arabic{translatornote}}
\newcommand{\translatornote}[1]{%
  \stepcounter{translatornote}%
  \begingroup
    \edef\savedfootnotenumber{\number\value{footnote}}%
    \renewcommand{\thefootnote}{\translatornotelabel}%
    \footnote{\emph{Translator's note:} #1}%
    \setcounter{footnote}{\savedfootnotenumber}%
  \endgroup
}
\newcommand{\translatornotemark}{%
  \stepcounter{translatornote}%
  \begingroup
    \edef\savedfootnotenumber{\number\value{footnote}}%
    \renewcommand{\thefootnote}{\translatornotelabel}%
    \footnotemark
    \xdef\pendingtranslatorfootnotenumber{\number\value{footnote}}%
    \setcounter{footnote}{\savedfootnotenumber}%
  \endgroup
}
\newcommand{\translatornotetext}[1]{%
  \begingroup
    \edef\savedfootnotenumber{\number\value{footnote}}%
    \setcounter{footnote}{\pendingtranslatorfootnotenumber}%
    \renewcommand{\thefootnote}{\translatornotelabel}%
    \footnotetext{\emph{Translator's note:} #1}%
    \setcounter{footnote}{\savedfootnotenumber}%
  \endgroup
}
...
\]
has the power-series expansion\translatornote{The printed series has \(x^4\)
in the term involving \(\Delta_3\). The power \(x^3\), required by the
power-series expansion, is used here.}
\[
...
In particular, put $c_i=1$ and $c_j=0$ for $j\ne i$; $\Delta_n^{(k)}$ then becomes what is called the minor
...
\]
whatever the values $\mu_1,\ldots,\mu_n$. Since, on the other hand, by~\eqref{eq:38:20},\translatornote{The French original refers here to equation~\eqref{eq:37:15}; the asserted equality is equation~\eqref{eq:38:20}.}
\[
...
\eqref{eq:52:55}.\translatornote{The French text refers here to
system~\eqref{eq:52:56}, evidently a misprint for the reduced
system~\eqref{eq:52:55}.} On the other hand, for every $p>1$,
\[
...
coordinate is $1$ when $j=i$; the column $(a_{ik})_i$ and
formula~\eqref{eq:56:4} show
...
\end{equation}
These inequalities\translatornote{The printed right-hand side of
\eqref{eq:60:14} is raised to the third power. The square required by the
norm and by the two quotients immediately below is used here.} give a
...
\end{equation}
This formula may also be verified by a straightforward symbolic calculation.

Analogous considerations applied to systems \eqref{eq:68:25} and
\eqref{eq:68:26} lead to the determination of a substitution \(\mathfrak F\)
such that
\[
  (E-\mathfrak A)\mathfrak F=E.
\]
There is no need to calculate \(\mathfrak F\) explicitly. It is enough to
observe that it exists; from this we shall conclude that the substitution
\(E-A\) has an inverse and that this inverse is furnished by the substitution
\[
  E-E_n+B+C_n^{-1}D+Q_nC_n^{-1}D+BQ_nC_n^{-1}D
\]
which we have just calculated. Indeed, let \(F\) be the transpose of
\(\mathfrak F\). Then
\[
  F(E-A)=E.
\]
Multiplying both sides of \eqref{eq:69:27} on the left by \(F\), we obtain
\[
  E-E_n+B+C_n^{-1}D+Q_nC_n^{-1}D+BQ_nC_n^{-1}D=F,
\]
and consequently
\[
  \bigl(E-E_n+B+C_n^{-1}D+Q_nC_n^{-1}D
    +BQ_nC_n^{-1}D\bigr)(E-A)=E.
\]
...
\hyperref[sec:67]{no.~67}. Indeed, $A_n+P_n+Q_n$\translatornotemark{} is always completely
...
\translatornotetext{In this footnote the French original prints
...
But $A^{(0)}$ is completely continuous, and it is easily seen that, if
% End inlined .parts/pages_061_120.tex
% Begin inlined .parts/pages_121_180.tex
% [Source printed page 121]
a completely continuous substitution is applied to Hilbert space, the
...
for the proof.\footnote{\phantomsection\label{fn:riesz-letter-hilbert}See my letter to Hilbert, ``Ueber quadratische Formen von
...
intégrales,'' \emph{Annales de l'École Normale} (1911), pp.~33--62,\translatornotemark{} and, in
particular for the correspondence considered here,
\hyperref[fn:riesz-letter-hilbert]{my note cited on source p.~124}.
...
January 12, 1910.}\translatornotetext{The French original prints pp.~31--62 for
...
The lower and upper bounds of the piecewise-defined function\translatornote{The French text
mistakenly writes \(u(\mu)=\mu\) here; the statement concerns the piecewise
function just defined.} \(u(\mu)\) on \((-\pi,\pi)\) are \(-\pi\) and
\(\pi\); hence these are also the
...
vol.~XII\translatornotemark{} (1911), pp.~135--164.}\translatornotetext{The French original prints
...
true of every admissible solution \((\psi_k)\) of \eqref{eq:110:14}.\translatornote{The French original refers here to equation~\eqref{eq:111:15}; equation~\eqref{eq:110:14} is meant.}  The
function
\[
