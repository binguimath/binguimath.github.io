...
% ============================================================================
% USER-CUSTOMIZABLE TYPOGRAPHY AND PAGE LAYOUT
% Change only the values in this block to personalize the document.  They are
% \providecommand's so a wrapper file may override them before \input'ing this
% file.  The defaults below reproduce the requested 13pt B5 layout exactly.
\providecommand{\UserBodyFontSize}{13pt}
% The baseline skip tracks the font size automatically (6/5 of the body size).
% It may still be overridden independently before this file is input.
\providecommand{\UserBodyBaselineSkip}{\dimexpr\UserBodyFontSize*6/5\relax}
\providecommand{\UserLineSpread}{1.1}
\providecommand{\UserLeftMargin}{0.7cm}
\providecommand{\UserRightMargin}{0.7cm}
\providecommand{\UserTopMargin}{0.91cm}
\providecommand{\UserBottomMargin}{0.91cm}
\providecommand{\UserHeaderHeight}{13.6pt}
\providecommand{\UserHeaderSeparation}{0.25cm}
% ============================================================================
...
\usepackage[
  ...
  includehead,
  left=\UserLeftMargin,
  right=\UserRightMargin,
  top=\UserTopMargin,
  bottom=\UserBottomMargin,
  headheight=\UserHeaderHeight,
  headsep=\UserHeaderSeparation
]{geometry}
...
\makeatother
\linespread{\UserLineSpread}
\makeatletter
\renewcommand{\normalsize}{%
  \@setfontsize\normalsize{\UserBodyFontSize}{\UserBodyBaselineSkip}%
  \abovedisplayskip .70\baselineskip plus .18\baselineskip minus .40\baselineskip%
  \abovedisplayshortskip 0pt plus .18\baselineskip%
  \belowdisplayshortskip .40\baselineskip plus .18\baselineskip minus .23\baselineskip%
...
\makeatletter
\newcommand\vnpartheading{\@startsection{vnpartheading}{0}{0pt}%
...
\newcommand{\vnpart}[2]{%
  ...
  \vnpartheading*{Part #1: #2}%
}
\makeatother
\newcommand{\partref}[1]{\hyperref[part:#1]{\mbox{Part~#1}}}
...
\newcommand{\RePart}{\Re}
\newcommand{\ImPart}{\Im}
\newcommand{\tnormleft}{\lvert\lvert\lvert}
\newcommand{\tnormright}{\rvert\rvert\rvert}
...
\thispagestyle{empty}
% The title page has no running header.  Reclaim exactly the header area rather
% than a fixed distance, so this remains correct after layout customization.
...
\item[\tocsec{16}] The operator $\tnormleft A\tnormright\leq1$ in the weak
...
$E\sim\sum E(\lambda)$, such that $M(\lambda)\ne N(\lambda)$ implies
...
$\Delta_{M(\lambda)}(E)$. The set $\bar N$.
...
$r-\bar N$ and $C_{(III,III)}$. Role of condition (iii) in
...
\bibitem{ref14} N. Lusin: \textit{Lecons sur les ensembles analytiques et
...
$f(\lambda)$'s too, if they are $\gtreqless0$!). In this case $f(\lambda)$ is
...
\mbox{\cite[p. 227, Theorem 6.3]{ref11}}. Therefore all $\rho_f(\lambda)$ are
...
\begin{gather*}
\int_{-}\alpha f(\lambda)\sqrt{d\sigma(\lambda)}
 =\alpha\int_{-}f(\lambda)\sqrt{d\sigma(\lambda)},\\[0.18\baselineskip]
\int_{-}(f(\lambda)+g(\lambda))\sqrt{d\sigma(\lambda)}
 =\int_{-}f(\lambda)\sqrt{d\sigma(\lambda)}
  +\int_{-}g(\lambda)\sqrt{d\sigma(\lambda)}.
\end{gather*}
...
f(\lambda_n)=
\begin{cases}
\dfrac{1}{\sqrt{a_m}}f^{(m)},&n=m,\\[0.23\baselineskip]
0,&n\ne m.
\end{cases}
\end{equation}
...
$\mathfrak H_2\subset[\mathfrak H_1^*,\mathfrak H_2^*,\ldots]$
$=$ the linear, closed set determined by $\mathfrak H_1^*,\mathfrak H_2^*,\ldots$. Now the two
...
\noindent\textup{(I)} determines $g(\lambda)$ uniquely, except for an arbitrary
...
(f(\lambda),\alpha g(\lambda))
  &=\overline\alpha(f(\lambda),g(\lambda)),\\
...
\section{Measurable families and existence and uniqueness of generalized direct
sums}\label{sec:6}
...
($l=\operatorname{Min}(k(\lambda),p)$). These form a normalized, orthogonal set, so
\[
...
($\alpha$), ($\gamma$), ($\delta$) of \hyperref[def:2]{Definition 2}.
Defining for
...
$\sigma(\lambda)$-measure 0,
and therefore it can be neglected. (We can add
...
\,d\sigma(\lambda)=0
\]
for every $\mu>-\infty$, $<+\infty$.
Thus $(f'(\lambda),g'(\lambda))=(f''(\lambda),g''(\lambda))$ except perhaps for a
...
fixed!), let the corresponding $''$-function be $f_m''(\lambda)$.
Define similarly
\[
g_n''(\lambda)=
\begin{cases}
...
fixed!), let the corresponding $'$-function be $g_n'(\lambda)$.
Then we have:
\begin{align*}
\tag{1}\label{eq:7.1}
...
$\lambda\notin N$.
As $N$ has the $\sigma(\lambda)$-measure 0, we can change
the $\mathfrak H'^{(\lambda)}$, $\lambda\in N$, arbitrarily. We make use of
 ...
 $\mathfrak H''^{(\lambda)}$, so that $k'(\lambda)=k''(\lambda)$,
and choose
 \[
...
$d\sigma_1(\lambda)/d\sigma_2(\lambda)$ by \secref{sec:2}, ($\gamma$).
Now we have by \cite{ref11},
\[
...
these definitions, because it is the generalized direct sum of the
% pp. 428--455 of the original journal pagination.
% This portion begins in the final paragraph of Section 8.
% p. 428
$\mathfrak H^{(\lambda)}$ with the weight-function $\sigma _1(\lambda)$
using the original definitions.
The essential fact to be used, in order to
establish this, is
\begin{align*}
 &\int\left(
...
Thus $E'(\mu)=E(\mu)$.
In other words: With the new definitions $\mathfrak H$
...
\item
$f_2(\lambda)$ ($\in\mathfrak H_2^{(\lambda)}$) is
...
Apply now \defref{def:3}.  As $\mu_{\sigma _1(\lambda)}(N_1)=0$, no $\lambda_n$
...
$b_n=\mu_{\sigma _2(\lambda)}(\mu_n)>0$, as in \secref{sec:4},
% p. 431
the \hyperref[rem:9.2]{Remark 2} of \secref{sec:9} gives for the sets
...
$\sigma(\lambda)$-measurable $\alpha(\lambda)$'s.
So we see: The ring $P$ contains all $\alpha(\Lambda)$'s.
Conversely: If a ring contains all
...
operator-functions in the usual sense.
Now apply Lemma~\ref{lem:4}: $\alpha(\lambda)=\lambda$ is bounded if we disregard the set $\lambda<-2$ or
...
&=\int\gamma(\lambda)\lVert\varphi_n(\lambda)\rVert^2d\tau _1(\lambda)\\
 ...
 &=(\gamma(\Lambda_1)\varphi_n,\varphi_n)\Bigr)
\end{align*}
% p. 435
and similarly
\[
...
a subset of $N'_2$ and thus has the $\tau _2(\lambda)$ measure $0$.  The latter
 is the image of a Borel-subset of $\operatorname{Complement}N'_1$,
 thus a Borel set,
 with the $\tau _2(\lambda)$-measure
...
\begin{align*}
 &\int\alpha(\lambda)(\psi_m(\lambda),\psi_n(\lambda))d\tau _2(\lambda)\\
 &\quad=\begin{cases}
   0,&m\ne n,\\
...
$\lambda\rightleftarrows m(\lambda)$ is a one-to-one mapping of the complement
of $N_1$ on the complement of $N_2$,
its inverse being there $\lambda\rightleftarrows n(\lambda)$.
% p. 437
...
$E_1(\lambda),\tau _1(\lambda)$-representation, and one
...
&\text{where }k_1(\lambda)=k_2(m(\lambda)).
\end{align*}
...
\texorpdfstring{$A\sim\sum A(\lambda)$}{A as a sum of A(lambda)}.
\texorpdfstring{\\}{ }Necessary and sufficient conditions and uniqueness}
\label{sec:13}
...
$\omega_m=\int_{-}\varphi_m(\lambda)\sqrt{d\sigma(\lambda)}$.
Form now the $f_m(\lambda)$ with
\[
 A^*\omega_m=\int_{-}f_m(\lambda)\sqrt{d\sigma(\lambda)}
\]
($A^*$ is the adjoint of $A$, cf. \cite[p.~112]{ref1} and
\cite[p.~295]{ref5}), and define
$\overline{a_{mn}(\lambda)}=(f_m(\lambda),\varphi_n(\lambda))$, so that
$f_m(\lambda)=\sum_n\overline{a_{mn}(\lambda)}\varphi_n(\lambda)$.
...
\]
except perhaps for a set $\bar N^{(m)}$ of $\sigma(\lambda)$-measure $0$
(cf. the argument of footnote~\ref{fn:7}).  The sum of all $\bar N^{(m)}$, say
$\bar{\bar N}$, is a fixed set of $\sigma(\lambda)$-measure $0$ (depending of course
...
\begin{equation}
 ...
 \label{eq:13.1}
\end{equation}
except for a set of $\sigma(\lambda)$-measure $0$.
(The series on the right side converges absolutely in any event, because
\[
...
$\lVert f\rVert^2=\int\lVert\bar f(\lambda)\rVert^2d\sigma(\lambda)
 =\mu_{\sigma(\lambda)}(S_p(x_1,x_2,\ldots))$.
For
...
\[
 ...
 =\tnormleft A(\lambda)\tnormright\leq C
...
The results obtained in the first part of this section put us in the
position to prove:
...
\item The numbers
$\tnormleft A(\lambda)\tnormright$ are bounded, if a
...
section, we obtain
$\tnormleft A(\lambda)\tnormright
\leq\tnormleft A\tnormright$ everywhere.  Therefore
we have in general
$\tnormleft A(\lambda)\tnormright
\leq\tnormleft A\tnormright$, except for a set of
$\sigma(\lambda)$-measure $0$.

\emph{Sufficiency of the condition:} All functions
$(A(\lambda)\varphi_m(\lambda),\varphi_n(\lambda))$ are
$\sigma(\lambda)$-measurable, and
$\tnormleft A(\lambda)\tnormright\leq C$, except for a
...
decompositions}. Convergence criteria.\texorpdfstring{\\}{ } Operator
functions. Behavior under equivalences}
\label{sec:14}
...
$\tnormleft A\tnormright\leq C$.

This means:

\emph{$\tnormleft A\tnormright$ is the smallest number
$C$ for which
$\tnormleft A(\lambda)\tnormright\leq C$ holds,
disregarding a suitable set of $\sigma(\lambda)$-measure $0$.  In other words:
$\tnormleft A\tnormright$ is the ``least upper bound up
...
$\sigma(\lambda)$-measurable,
$\tnormleft\alpha(\lambda)1\tnormright
...
\begin{enumerate}
\item[(I)] The
$\tnormleft A_n(\lambda)\tnormright$ are uniformly
  bounded, disregarding a suitable $\lambda$-set of $\sigma(\lambda)$-measure $0$.
...
$\tnormleft A_n\tnormright$ are uniformly bounded
\cite[p.~382]{ref2},
$\tnormleft A_n\tnormright\leq C$.  Therefore
$\tnormleft A_n(\lambda)\tnormright\leq C$, except for
...
$\tnormleft A_n\tnormright\leq C$,
$\tnormleft A_n(\lambda)\tnormright\leq C$
...
% The repeated word "assuming" is printed in the original p. 443.
(II).  (But we continue assuming assuming
$\tnormleft A_n\tnormright\leq C$,
$\tnormleft A_n(\lambda)\tnormright\leq C$.)  Observe,
...
$\tnormleft A_n\tnormright\leq C$,
$\tnormleft A_n(\lambda)\tnormright\leq C$, then $Af$
...
prove its sufficiency, assume that it is fulfilled.
We have then
\[
 \lim_{r\to\infty}
...
\cite[p.~227]{ref11}, is a ``null set with respect to $A$''.
Thus $\alpha_1(A)=\alpha_2(A)=\alpha(A)$.  Besides
...
M(lambda)}.\texorpdfstring{\\}{ } Discussion of
\texorpdfstring{$f(\lambda)$, $A(\lambda)$}{f(lambda),
A(lambda)}}
\label{sec:15}
...
$\sigma(\lambda)$-measurable, then $A(\lambda)B(\lambda)$ has that property
too:
Let $f(\lambda),g(\lambda)$ be $\sigma(\lambda)$-measurable, then
...
% The alpha below is the original p. 446 reading, despite the context.
$\alpha(\lambda)$, because
...
function $\tnormleft A(\lambda)\tnormright$: We know
...
$\mathfrak H$ with
$\tnormleft A\tnormright\leq1$, then there exists a
...
$\lim_{p\to\infty}(A_p\varphi_m,\varphi_n)=(A\varphi_m,\varphi_n)$
for all $m,n=1,2,\ldots$ (cf. \cite[pp.~385--386]{ref2}) and this is equivalent to the
...
and so $\lim_{p\to\infty}(A_p\varphi_m,\varphi_n)=a_{mn}$ exists.
If
...
$\tnormleft A\tnormright\leq1$, so $A\in S$.  Now as
...
=\operatorname{Max}(\operatorname{Dist}_1(a^{(1)},b^{(1)}),
...
$\sigma(\lambda)-\tau_1(\lambda)$-measurability,
and thus $\sigma(\lambda)$-measurability too.  Thus we may prove our lemma for
...
$[n_1,n_2,n_3,\ldots]\in T(\lambda)$ (for any $n_2,n_3,\ldots$)
by $n_1^0$, the smallest $n_2$ with
...
by \cite[p.~209, Theorem II]{ref15}, that $M(\alpha_0)$ is an analytic set,
...
We now return to the discussion of the $\sigma(\lambda)$-measurable rings of
$M(\lambda)$:
...
$\mathfrak H'_p$ with
$\tnormleft A\tnormright\leq1$ in their weak topology,
...
$\tnormleft A\tnormright\leq1$ in $\mathfrak H'_p$,
...
lies in $\tnormleft A\tnormright\leq1$.
...
$\tnormleft A\tnormright\leq1$.  If
$A\in M(\lambda)'$, $\tnormleft A\tnormright>1$, then
this ring will contain
$1/\tnormleft A\tnormright\cdot A$ and thus $A$ too.
...
$\tnormleft A\tnormright\leq1$ [which is separable].)
...
Thus the set
$D_p=N_p(M_p^{**}-M_p^*)\mathfrak E_p$ is a Borel-set too (in
$r\times S_p$), its elements are all pairs $(\lambda,E)$ with
$\lambda\in\Delta_p-\bar N_p-\bar{\bar N}_p$,
...
$K_p\subset\Delta_p-\bar N_p-\bar{\bar N}_p$.
...
% The following set relations reproduce the original p. 454 literally,
% despite their apparent logical inconsistency.
$\lambda\notin\bar N_p-\bar{\bar N}_p$, or
$\lambda\in\Delta-\bar N_p-\bar{\bar N}_p$, and every
...
$\lambda\in\Delta-\bar N_p-\bar{\bar N}_p$ with
$M(\lambda)\ne N(\lambda)$.
...
Put
$N=\bar N_1+\bar{\bar N}_1+\bar N_2
+\bar{\bar N}_2+\cdots+\bar N_\infty
+\bar{\bar N}_\infty$.  $N$ has the $\sigma(\lambda)$-measure $0$,
and
\[
 (\Delta_1-\bar N_1-\bar{\bar N}_1)
 +(\Delta_2-\bar N_2-\bar{\bar N}_2)+\cdots
 +(\Delta_\infty-\bar N_\infty-\bar{\bar N}_\infty)=r-N.
\]
% p. 455
...
$\tnormleft E(\lambda)\tnormright\leq1$.  Thus they
...
\texorpdfstring{$M\simeq\sum M(\lambda)$}{M as a sum of M(lambda)}}
\label{sec:18}
...
$M\simeq\sum M(\lambda)$, if $A\in M$ is equivalent to
$A\sim\sum A(\lambda)$, $A(\lambda)\in M(\lambda)$.
\end{definition}

\origpage{456}
If the $M(\lambda)$ are given, it is clear that there cannot be
$M\simeq\sum M(\lambda)$ for two different $M$'s; so there exists exactly
...
chosen that all $\tnormleft A_i(\lambda)\tnormright\leq 1$. In this
...
$\tnormleft A_i(\lambda)\tnormright$ are
$\sigma(\lambda)$-measurable along with $A_i(\lambda)$. Thus the operators
\[
A_i^0(\lambda)=
\begin{cases}
\dfrac{1}{\tnormleft A_i(\lambda)\tnormright}\,A_i(\lambda),
  &\text{for }\tnormleft A_i(\lambda)\tnormright\ne0,\\[0.35\baselineskip]
0,&\text{for }\tnormleft A_i(\lambda)\tnormright=0
...
$\tnormleft A_i^0(\lambda)\tnormright\leq1$. This proves the first
statement.
...
$\tnormleft A_i(\lambda)\tnormright\leq1$. Thus an
...
$\tnormleft A\tnormright\leq1$ is separable
(cf. \cite[p.~386]{ref2}), therefore the part of $\widetilde M'$ in it is
separable too.
Thus there exists a sequence of operators $B_j$,
$j=1,2,\ldots$, which is dense in it.
As all $B_j\in\widetilde M'$, therefore
$R(B_1,B_2,\ldots)\subset\widetilde M'$; and by our construction
$R(B_1,B_2,\ldots)$ contains all $A\in\widetilde M'$,
$\tnormleft A\tnormright\leq1$, thus all $A\in\widetilde M'$, that is
...
$\widetilde M\simeq\sum N(\lambda)'$.
...
$A_i,A_i^*$. By \hyperref[thm:V]{Theorem~V}, $F\in P'$, so $F$ commutes with all $P$ too.
Thus $F$ commutes with $\widetilde M$, hence with $E$, and therefore $F(\lambda)$
...
commutes with the ring generated by $M(\lambda)$ and $E(\lambda)$ too.
This means $F(\lambda)\in R(M(\lambda),E(\lambda))'$, and thus implies
...
$\widetilde M\simeq\sum N(\lambda)'$ implies
$\widetilde M\simeq\sum M(\lambda)$. Thus the proof is completed.
...
$M\simeq\sum M(\lambda)$. This $M$ is a ring containing $1$,
$P\subset M\subset P'$.
...
$M\simeq\sum M(\lambda)$. This system is unique, except for an arbitrary
$\lambda$-set of $\sigma(\lambda)$-measure $0$.
\end{itshape}
...
$R(P,A_1,A_2,\ldots)\simeq\sum M(\lambda)$. Now
$R(P,A_1,A_2,\ldots)\supset R(A_1,A_2,\ldots)=M$, and as $P\subset M$ and
all $A_i\in M$, $R(P,A_1,A_2,\ldots)\subset M$. Thus
$R(P,A_1,A_2,\ldots)=M$, and so $M\simeq\sum M(\lambda)$, proving the
existence of the desired $M(\lambda)$.

That the change of the $M(\lambda)$ on a set of $\sigma(\lambda)$-measure
$0$ does not affect $M\simeq\sum M(\lambda)$, is obvious. Thus it remains
to prove only this: If $M\simeq\sum M(\lambda)$ and
$M\simeq\sum N(\lambda)$, then $M(\lambda)=N(\lambda)$, except for a
$\sigma(\lambda)$-set of measure $0$.
Consider $M(\lambda)N(\lambda)$ and $N(\lambda)$, both are
$\sigma(\lambda)$-measurable
...
$\lambda$'s, therefore $M\simeq\sum N(\lambda)$ necessitates $E\in M$.
Now $M\simeq\sum M(\lambda)$ shows, that $E(\lambda)\in M(\lambda)$,
...
$M\simeq\sum M(\lambda)$. This is done in the three lemmas which follow.

\par\medskip\noindent\textbf{Lemma 13.}\phantomsection\label{lem:13}\quad
\begin{itshape}
$M\simeq\sum M(\lambda)$ implies
$M'\simeq\sum M(\lambda)'$.
\end{itshape}
...
\begin{itshape}
$M_s\simeq\sum M_s(\lambda)$, $s=1,2,\ldots$ (the sequence of the
$s$ may be finite
\origpage{459}
or infinite) implies
\[
M_1\cdot M_2\cdot\ldots\simeq
\sum M_1(\lambda)\cdot M_2(\lambda)\cdot\ldots
\]
and
\[
R(M_1,M_2,\ldots)\simeq
...
remember \hyperref[def:6]{Definition~6}.
Now this gives, together with
\hyperref[lem:13]{Lemma 13}, the statement
\[
R(M_1,M_2,\ldots)'
 \simeq\sum R(M_1(\lambda),M_2(\lambda),\ldots)',
\]
because of
...
\begin{itshape}
If $M\simeq\sum M(\lambda)$, $N\simeq\sum N(\lambda)$, then
...
$M\cdot N\simeq\sum M(\lambda)\cdot N(\lambda)$. As
$M\simeq\sum M(\lambda)$, we see by \hyperref[thm:VI]{Theorem VI}, that
...
The behavior of the decomposition $M\simeq\sum M(\lambda)$ under
equivalence is this:

\textit{If $M\simeq\sum M(\lambda)$, and the representation of
...
of $M$ in this new representation will be $M\simeq\sum N(\lambda)$, where
...
$M\simeq\sum M(\lambda)$ in the sense of \hyperref[def:6]{Definition~6} is possible, are the
...
$M\simeq\sum M(\lambda)$ (except for a $\lambda$-set of
...
$M\cdot M'\simeq\sum M(\lambda)\cdot M(\lambda)'$ and by the second
statement of
\origpage{460}
\secref{sec:14}, $P\simeq\sum 0(\lambda)$. Thus
...
condition (ii) in \hyperref[def:7]{\mbox{Definition 7}}, by the weaker condition
(ii)$'$, which arises from (ii) by permitting two addends $E_j$ ($j=1,2$)
only. Thus it would not affect \hyperref[def:7]{\mbox{Definition 7}} essentially if
...
$M\simeq M(\lambda)$. As each $M(\lambda)$ is a factor, we can select in
...
as in the proof of \hyperref[lem:6]{\mbox{Lemma 6}}: Replace the operator equations
...
Introduce $\mathfrak H_p'$, the $\psi_1',\psi_2',\ldots$, and the
isomorphism $J^{(\lambda)}$ as there. Replace now in our
\origpage{464}
...
$\tnormleft\widetilde V_s\tnormright\leq1$.
...
$M_0=r-M'$, $M_\infty=0$, $M_r=M'$. Choose an $\bar E(\lambda)$ with
$\bar M_r=M$. Define now
\[
E(\lambda)=
\begin{cases}
\bar E(\lambda)&\text{for }\lambda\in M',\\
0&\text{for }\lambda\notin M'.
\end{cases}
\]
Then $E(\lambda)$ is $\sigma(\lambda)$-measurable along with
$\bar E(\lambda)$ and $M'$, and it obviously meets our requirements.
...
$\bar M=\sum_{j=1}^{\infty}M_j\in T$, so
$\mu_{\sigma(\lambda)}(\bar M)\leq\alpha$. But
$\mu_{\sigma(\lambda)}(\bar M)\geq\mu_{\sigma(\lambda)}(M_j)
\geq\alpha-1/j$ for every $j=1,2,\ldots$, thus necessarily
$\mu_{\sigma(\lambda)}(\bar M)=\alpha$. Now if $M\in T$, then by (IV)
$\bar M+M\in T$ too; thus
$\mu_{\sigma(\lambda)}(\bar M+M)=\alpha$. Thus
$\mu_{\sigma(\lambda)}(M-\bar M\cdot M)
=\mu_{\sigma(\lambda)}(\bar M+M)
-\mu_{\sigma(\lambda)}(\bar M)=0$. Thus $\bar M$ has the property
(ii) of our lemma.

\item Applying (II) to $\bar M\in T$ proves that $\bar M$ has the
...
$\bar M$ depend only apparently on the choice of
...
$\sigma(\lambda)$-measurable in $\bar M$
...
$r-\bar M$, if $E(\lambda)$ is $\sigma(\lambda)$-measurable. Choose any
...
$\lambda\in r-\bar M$ it implies
$\lambda\in M_r-M_r\cdot\bar M$. Thus we deduce from
\hyperref[lem:19]{Lemma 19}: $r-\bar M$ is
...
the set of all $\lambda\in r-\bar M$,
...
$r-\bar M$ will therefore be $\sigma(\lambda)$-measurable too.

Thus we can restrict ourselves from now on to the $\lambda\in\bar M$.

Choose an arbitrary $d_{M(\lambda)}(E)$, an $\bar E(\lambda)$ by
\hyperref[lem:19]{Lemma 19}, and define
\[
d^*_{M(\lambda)}(E)=a^*(\lambda)d_{M(\lambda)}(E),\qquad
a^*(\lambda)=
\begin{cases}
\dfrac{1}{d_{M(\lambda)}(\bar E(\lambda))},
   &\text{for }\lambda\in\bar M,\\[0.35\baselineskip]
1,&\text{for }\lambda\notin\bar M.
\end{cases}
\]
If $\lambda\in\bar M$, then
\[
d^*_{M(\lambda)}(E(\lambda))
=\frac{d_{M(\lambda)}(E(\lambda))}
       {d_{M(\lambda)}(\bar E(\lambda))},
\]
which is a $\sigma(\lambda)$-measurable function along with $E(\lambda)$, by
\hyperref[lem:18]{Lemma 18}. Thus $d^*_{M(\lambda)}(E(\lambda))$ is
$\sigma(\lambda)$-measurable in $\bar M$, and therefore in all $r$, in
...
suffices to establish this in $\bar M$. Thus the
$\sigma(\lambda)$-measurability of $a(\lambda)$ in $\bar M$ is
sufficient. But it is necessary too: Put $E(\lambda)=\bar E(\lambda)$ in
\eqref{eq:21.1}, for $\lambda\in\bar M$ we have
$d^*_{M(\lambda)}(E(\lambda))>0$, $<+\infty$, and thus we can divide by it,
obtaining
\[
a(\lambda)=
\frac{\Delta_{M(\lambda)}(\bar E(\lambda))}
     {d^*_{M(\lambda)}(\bar E(\lambda))},
\]
which proves the $\sigma(\lambda)$-measurability of $a(\lambda)$ in
$\bar M$. Thus (ii) too is established, and the proof is completed.---
...
$\bar M$, and not in all $r$! Now we remarked in the discussion which
...
\secref{sec:22}, and in particular \origpage{467}in
\hyperref[def:9]{\mbox{Definition~9}}.) Besides
$\bar M\subset r-C_{III}$, because $\lambda\in\bar M$ implies
$d_{M(\lambda)}(\bar E(\lambda))>0$, $<+\infty$, and therefore
$\lambda\notin C_{III}$. Thus our result would be very natural, if
$\bar M$ coincided with $r-C_{III}$ up to a set of
$\sigma(\lambda)$-measure $0$. But this, that is
$\mu_{\sigma(\lambda)}(r-C_{III}-\bar M)=0$, is unproved! In fact we
cannot prove at present even the $\sigma(\lambda)$-measurability of $C_{III}$
(which is equivalent to that one of $r-C_{III}-\bar M$). (Cf.
\hyperref[thm:IX]{Theorem IX} and the observations following it.).

A natural attempt to prove
$\mu_{\sigma(\lambda)}(r-C_{III}-\bar M)=0$ would be this: If
...
$M_r=r-C_{III}$. Then $M_r-M_r\cdot\bar M=r-C_{III}-\bar M$, and
therefore \hyperref[lem:19]{Lemma 19} gives the desired
$\mu_{\sigma(\lambda)}(r-C_{III}-\bar M)=0$. But at present we are not
...
$M^*,M^{*'}$ of $\mathfrak H^*$, cf. \mbox{\cite[Definition~3.1.3]{ref10}}, but we
...
by its image under this mapping. The same is true for the set $\bar M$
...
$q=1,2,\ldots$, the same for $M(\lambda)'$.
Let the $\bar A_1,\bar A_2,\ldots$ be operators in $\mathfrak H^*$
generating the ring $M^*$, and
\origpage{469}
$\bar B_1,\bar B_2,\ldots$ the ring $M^{*'}$. Choose a set
...
% The prime on the following M(lambda) is present in the original p. 469,
% although it conflicts with the immediately following sentence and (22.1).
$M(\lambda)'$. This is equivalent to this: that the $J^*$-image of every
$\bar A_r$ commutes with every $B_q(\lambda)$, and similarly for every
$\bar B_r$ and $A_q(\lambda)$, $r,q=1,2,\ldots$. If we now replace
...
\left\{
\begin{aligned}
U\bar A_rU^*J^{(\lambda)}B_q(\lambda)J^{(\lambda)-1}
 &=J^{(\lambda)}B_q(\lambda)J^{(\lambda)-1}U\bar A_rU^*,\\
U\bar B_rU^*J^{(\lambda)}A_q(\lambda)J^{(\lambda)-1}
 &=J^{(\lambda)}A_q(\lambda)J^{(\lambda)-1}U\bar B_rU^*,
 &&r,q=1,2,\ldots,\\
U^*U&=UU^*=1.
\end{aligned}
\right.
...
\hyperref[lem:17]{\mbox{Lemmas 17}} and \hyperref[lem:18]{18}, that $C_p$ is a
Borel-set in $r\times S_p$. $\lambda\in\Delta_p'D_{M^*}$ means that there
exists a $U$ with $(\lambda,U)\in C_p$. From this we infer as in
\hyperref[lem:17]{\mbox{Lemmas 17}} and \hyperref[lem:18]{18} that
...
Theorem~VIII]{ref10}.)
We know from \cite[Lemma~8.6.1]{ref10} that
...
The connection of these sets with the set $\bar M$ of
\hyperref[lem:19]{Lemma 19} is of interest, too. We have already proved
$\bar M\subset r-C_{III}=r-C_{(III,III)}$. Consider now
$E(\lambda)=1$, for this $M_r=C_f$, and so by \hyperref[lem:19]{Lemma 19}
$\mu_{\sigma(\lambda)}(C_f-C_f\bar M)=0$. In other words:
If we disregard a set of $\sigma(\lambda)$-measure $0$, then
$\bar M\supset C_f$. As $\bar M$ is only defined up to such a set,
we may assume $\bar M\supset C_f$. One can prove furthermore that
$\bar M\supset C_{(I_\infty,I_\infty)}$, and that $\bar M$ is
unchanged, if we replace $M$ by $M'$. We will not give the details here. Now
we have $\bar M\supset C_f\supset C_{(I_m,I_\infty)}$,
$m=1,2,\ldots$, and $\supset C_{(II_1,II_\infty)}$; and as we can replace
$M$ by $M'$ without affecting $\bar M$, but thereby changing
$C_{(I_m,I_\infty)}$ into $C_{(I_\infty,I_m)}$, and
$C_{(II_1,II_\infty)}$ into $C_{(II_\infty,II_1)}$, this implies
$\bar M\supset C_{(I_\infty,I_m)}$, $m=1,2,\ldots$, and
$\supset C_{(II_\infty,II_1)}$. Thus we have
\begin{align*}
\bar M\supset{}&C_f+C_{(I_\infty,I_1)}+C_{(I_\infty,I_2)}+\cdots
...
\[
r-\bigl(C_{(II_\infty,II_\infty)}+C_{(III,III)}\bigr)
\subset\bar M\subset r-C_{(III,III)}.
\]
\origpage{471}
...
\mbox{\hyperref[lem:17]{Lemmas 17}, \hyperref[lem:18]{18}, and
...
$\tnormleft\widetilde V\tnormright\leq1$,
$\tnormleft\widetilde V'\tnormright\leq1$.
Denote the sets of all pairs $(\lambda,\widetilde V)$ and triplets
...
the proofs of \mbox{\hyperref[lem:17]{Lemmas 17}, \hyperref[lem:18]{18} and
...
\end{cases}
\]
and the operator $1_K$ (cf. \secref{sec:7}).
As $1_K(\lambda)^2=1_K(\lambda)=\overline{1_K(\lambda)}$, therefore $1_K$ is a
...
For every $A\in M$ form an operator $A_u\in M$ as follows:
Form
...
$\tnormleft A_u(\lambda)\tnormright=1$) and in $M(\lambda)$. Thus we
...
\begin{gather*}
1,\quad A_m^*,\quad (A_m)_u,\quad A_m\pm A_n,\\
A_mA_n,\quad A_m\mathbin{\#}A_n.
\end{gather*}
Then, correspondingly for all $\lambda$,
\begin{align*}
A_p(\lambda)={}&1,\quad A_m(\lambda)^*,\quad (A_m)_u(\lambda),\\
&A_m(\lambda)\pm A_n(\lambda),\quad A_m(\lambda)A_n(\lambda),\\
&A_m(\lambda)\mathbin{\#}A_n(\lambda),
\end{align*}
except for certain $\lambda$-sets of
$\sigma(\lambda)$-measure $0$, say
$N_3$, $N_4(m)$, $N_5(m)$, $N_6(m,n)$,
$N_7(m,n)$, $N_8(m,n)$, $N_9(m,n)$.
...
projections $1-E_m(\lambda)$ resp. $1-E_q(\lambda)$.
Therefore if $A_s=V_1$
...
\mathrel{\leq\text{ or }\geq}\theta d_{M(\lambda)}(E_n(\lambda))$
($d_{M(\lambda)}(E_m)$ or
...
$\sigma(\lambda)$-measurable by its construction. Let $\bar K_0$ be the
set of those $\lambda$, where $\gamma(\lambda)=0$. If the projection
$\bar E$ of \hyperref[lem:19]{Lemma 19} is substituted for $E_m$, then
we have for $\lambda\in\bar N$
$d_{M(\lambda)}(\bar E(\lambda))>0$, $<+\infty$, so for
$\lambda\in\bar K_0\cdot\bar N$, we have
$\delta_M^{(\lambda)}(\bar E)=0$.

Thus
\[
\Delta_M(1_{\bar K_0\cdot\bar N}(\lambda)\bar E)
=\int_{\bar K_0\cdot\bar N}
 \delta_M^{(\lambda)}(\bar E)\,d\sigma(\lambda)=0,
\]
and therefore $1_{\bar K_0\cdot\bar N}(\lambda)\bar E=0$.
But
$1_{\bar K_0\cdot\bar N}(\lambda)\bar E
\sim\sum1_{\bar K_0\cdot\bar N}(\lambda)\bar E(\lambda)$, so
$1_{\bar K_0\cdot\bar N}(\lambda)\bar E(\lambda)=0$, except
for a $\lambda$-set of $\sigma(\lambda)$-measure $0$. Now for
$\lambda\in\bar K_0\cdot\bar N$,
\origpage{479}
$1_{\bar K_0\cdot\bar N}(\lambda)=1$, and as
$\lambda\in\bar N$, $\bar E(\lambda)\ne0$, therefore
$1_{\bar K_0\cdot\bar N}(\lambda)\bar E(\lambda)\ne0$. All
of this implies
$\mu_{\sigma(\lambda)}(\bar K_0\cdot\bar N)=0$. Putting
$N_{12}=N_{11}+\bar K_0\cdot\bar N$, we still have
$\mu_{\sigma(\lambda)}(N_{12})=0$, and
we can state: For $\lambda\notin N_{12}$,
$\lambda\in\bar N$ implies
$\gamma(\lambda)\notin\bar K_0$, that is $\gamma(\lambda)>0$.
So $0<\gamma(\lambda)\leq+\infty$, and as
$0<d_{M(\lambda)}(E_m(\lambda))\leq+\infty$ too (if
$E_m(\lambda)\ne0$), we can write for \eqref{eq:23.6}:
\begin{equation}
\delta_M^{(\lambda)}(E_m)
=\gamma(\lambda)d_{M(\lambda)}(E_m(\lambda))
\quad\text{if }\lambda\notin N_{12},\ \lambda\in\bar N,
...
\eqref{eq:23.7} holds in this case too,
if we use again the convention
$0\cdot+\infty=0$. If $d_{M(\lambda)}(E_m(\lambda))=+\infty$, then we have
$d_{M(\lambda)}(E_m(\lambda))\geq
\theta d_{M(\lambda)}(\bar E(\lambda))$ for all $\theta>0$, so
\[
\delta_M^{(\lambda)}(E_m)
\geq\theta\delta_M^{(\lambda)}(\bar E)
=\theta\gamma(\lambda)d_{M(\lambda)}(\bar E(\lambda))
\]
...
$\bar N$, if we substitute the projection $\bar E$ of
\hyperref[lem:19]{Lemma 19} for $E_m$. Then we see: $\gamma(\lambda)$ is a
$\sigma(\lambda)$-measurable function in $\bar N$, and, disregarding a
$\lambda$-set of $\sigma(\lambda)$-measure $0$, we have:
\begin{equation}
\gamma(\lambda)=
\frac{\delta_M^{(\lambda)}(\bar E)}
     {d_{M(\lambda)}(\bar E(\lambda))}
\qquad\text{for }\lambda\in\bar N.
\tag{23.7$_1$}\label{eq:23.7.1}
\end{equation}

\item We next discuss the behavior of $\delta_M^{(\lambda)}(E_m)$ for
$\lambda\notin\bar N$. We again assume $\lambda\notin N_{12}$, too.
...
\cite[Lemma~7.2.3]{ref10}. Now form $A_p=V_3$
(in the sense of \hyperref[lem:21]{Lemma 21} for $E=E_m$).
As $A_p$ is partially isometric,
...
\[
E_n=E_t+E_w,\qquad
E_n(\lambda)\sim E_t(t)\sim E_w(\lambda)\vnmod{M(\lambda)}.
\]
...
\hyperref[lem:21]{\mbox{Lemma 21}}, with $E=E_q$ resp. $1-E_q$, and $F=E_t$ resp.
...
$\bar K_1$, it is $\sigma(\lambda)$-measurable along with
$\delta_M^{(\lambda)}(1)$. Then \eqref{eq:23.3} gives
\[
\Delta_M(1_{\bar K_1}(\lambda))
=\int_{\bar K_1}\delta_M^{(\lambda)}(1)\,d\sigma(\lambda)=0,
\qquad1_{\bar K_1}(\lambda)=0,
\]
and therefore $\mu_{\sigma(\lambda)}(\bar K_1)=0$. Put
$N_{15}=N_{14}+\bar K_1$, then
...
$\sigma(\lambda)$-measurable in $\bar N$. It is natural to ask, how far
$\Delta_M(E)$ determines $a(\lambda)$.

If $K\subset\bar N$ and $\sigma(\lambda)$-measurable, then
\[
\Delta_M(1_K(\lambda)\bar E)
=\int_Ka(\lambda)d_{M(\lambda)}(\bar E(\lambda))\,d\sigma(\lambda),
\]
thus
$\int_Ka(\lambda)d_{M(\lambda)}(\bar E(\lambda))\,d\sigma(\lambda)$ is
determined for all these sets $K$. Thus
$a(\lambda)d_{M(\lambda)}(\bar E(\lambda))$ is uniquely determined in
$\bar N$, up to a set of $\sigma(\lambda)$-measure $0$; and as
$d_{M(\lambda)}(\bar E(\lambda))>0$, $<+\infty$ for all
$\lambda\in\bar N$, therefore $a(\lambda)$ is determined to the same
extent. In $r-\bar N$ we have by \hyperref[lem:19]{Lemma 19} always
$d_{M(\lambda)}(\bar E(\lambda))=0$ or $+\infty$, except for a set of
$\sigma(\lambda)$-measure $0$, and therefore the numerical value of
$a(\lambda)$ (which must be $>0$, $\leq+\infty$) does not matter at all.
So we see:

\begin{itshape}
$a(\lambda)$ may be any function which fulfills
$0<a(\lambda)\leq+\infty$ and which is $\sigma(\lambda)$-measurable in
$\bar N$. $\Delta_M(\lambda)$ determines $a(\lambda)$ uniquely in
$\bar N$, up to a set of $\sigma(\lambda)$-measure $0$, while it leaves
it entirely arbitrary in $r-\bar N$.
\end{itshape}
...
surprising, that this is the case even for $\lambda\in r-\bar N$, where
$r-\bar N\subset C_{(III,III)}$. We discussed the relationship of
$r-\bar N$ and $C_{(III,III)}$ loc. cit.
...
that is if $\alpha(\lambda)=0$ or $1$.
In other words:
\[
...
\]
Then denote for each $K\subset r$ the set of all $\gamma=1,2,\ldots$ with
$\lambda_\gamma\in K$ by $L_K$, and define
\[
...
But it is sufficient, too, because it involves
...
$\bar N$ does not matter. (Concerning the latter, cf. $(\alpha)$ above.)
...
\eqref{eq:24.2} is finite for all $\lambda\in\bar N$, except for a
$\lambda$-set of $\sigma(\lambda)$-measure $0$.
\end{itshape}
