...
\newcommand{\modcondref}[1]{\hyperref[cond:2.1:#1]{(#1)}}
...
\end{center}
\begin{description}
\item[\secref{1}] \hyperref[sec:1]{Preliminaries}
\item[\secref{2}] \hyperref[sec:2]{Modular Hilbert Algebras}
\item[\secref{3}] \hyperref[sec:3]{Generalized Hilbert Algebras}
\item[\secref{4}] \hyperref[sec:4]{The Commutation Theorem for Modular Hilbert Algebras}
\item[\secref{5}] \hyperref[sec:5]{Self-Adjoint Subalgebras of Generalized Hilbert Algebras}
\item[\secref{6}] \hyperref[sec:6]{The Spectral Algebra}
\item[\secref{7}] \hyperref[sec:7]{The Modular Operator $\Delta$}
\item[\secref{8}] \hyperref[sec:8]{The Resolvent of the Modular Operator $\Delta$}
\item[\secref{9}] \hyperref[sec:9]{The One-Parameter Automorphisms defined by the Modular Operator $\Delta$}
\item[\secref{10}] \hyperref[sec:10]{Formulation of the Modular Hilbert Algebra}
\item[\secref{11}] \hyperref[sec:11]{Tensor product and Direct Sum of Modular Hilbert Algebras}

% Source PDF p. 5 / printed p. 4
\item[\secref{12}] \hyperref[sec:12]{The Standard Representation of von Neumann Algebras}
\item[\secref{13}] \hyperref[sec:13]{The Modular Automorphism Group and the Kubo--Martin--Schwinger Boundary Condition}
\item[\secref{14}] \hyperref[sec:14]{Semi-Finiteness and the Modular Automorphism Group}
\item[\secref{15}] \hyperref[sec:15]{The Radon--Nikodym Theorem and the Modular Automorphism Group}
\end{description}
...
\begin{enumerate}[label=(\Roman*)]
  \item\label{cond:2.1:I} $(\xi\eta\mid\zeta)=(\eta\mid\xi^\#\zeta)$;
  \item\label{cond:2.1:II} For each $\xi\in\mathfrak A$, the map
  $\eta\in\mathfrak A\longmapsto\xi\eta\in\mathfrak A$ is continuous;

% Source PDF p. 7 / printed p. 6
  \item\label{cond:2.1:III} The subalgebra $\mathfrak A^2$ of $\mathfrak A$, spanned by the
  elements $\xi\eta$ with $\xi,\eta\in\mathfrak A$, is dense in
  $\mathfrak A$;
  \item\label{cond:2.1:IV} $(\Delta(\alpha)\xi)^\#=\Delta(-\bar\alpha)\xi^\#$,
  $\xi\in\mathfrak A$, $\alpha\in\mathbb C$;
  \item\label{cond:2.1:V} $(\Delta(\alpha)\xi\mid\eta)
  =(\xi\mid\Delta(\bar\alpha)\eta)$;
  \item\label{cond:2.1:VI} $(\Delta(1)\xi^\#\mid\eta^\#)=(\eta\mid\xi)$;
  \item\label{cond:2.1:VII} $(\Delta(\alpha)\xi\mid\eta)$, $\xi,\eta\in\mathfrak A$, is an
  analytic function of $\alpha$ on $\mathbb C$;
  \item\label{cond:2.1:VIII} For every real number $t$, the set
  $(1+\Delta(t))\mathfrak A$ is dense in $\mathfrak A$.
\end{enumerate}
If an involutive algebra $\mathfrak A$ over $\mathbb C$ admits an inner
product satisfying conditions \modcondref{I}--\modcondref{III} and the following condition
\modcondref{IX}, then $\mathfrak A$ is called a \emph{generalized Hilbert algebra}:
\begin{enumerate}[label=(IX)]
  \item\label{cond:2.1:IX} The involution
...
\]
By conditions \modcondref{I} and \modcondref{III}, $\pi(\mathfrak A)$, the set of $\pi(\xi)$,
...
\begin{proof}
By condition \modcondref{V}, $\{\Delta(it):-\infty<t<+\infty\}$ is a one-parameter
...
$\mathcal H$.  By assumption \modcondref{IX}, there exists a real linear densely
...
Let $\mathfrak A$ be a generalized Hilbert algebra and $\mathfrak B$ be
  a self-adjoint subalgebra.  Then postulates \modcondref{I}, \modcondref{II} and \modcondref{IX} for the
...
sequence of polynomials defined by equality \eqref{eq:3.7}, so that
  $\mathfrak B$ satisfies postulate \modcondref{III}.  Therefore $\mathfrak B$ is a
generalized Hilbert algebra.  This completes the proof.
\end{proof}

It is not clear whether the corresponding assertion remains true for
  modular Hilbert algebras, because of postulate \modcondref{VIII}.
...
\hyperref[lem:5.2]{Lemma~5.2}.  Therefore, $\mathfrak B$ satisfies all
  ...
  \modcondref{VII}, and is equivalent to $\mathfrak A$.
...
\begin{enumerate}[label=(\alph*)]
 \item\label{cond:10:c:a} $\xi$ is in $\bigcap_\alpha\mathcal D(\Delta^\alpha)$;
 \item\label{cond:10:c:b} $\Delta^\alpha\xi$ is in $\mathfrak A^{\#}$ for every complex
 number $\alpha$;
 \item\label{cond:10:c:c} the function $\alpha\mapsto(\Delta^\alpha\xi\mid\eta)$ is
...
analytic.  Hence $\xi_1\xi_2$ satisfies condition
\hyperref[cond:10:c:c]{(c)} as well as conditions
\hyperref[cond:10:c:a]{(a)} and \hyperref[cond:10:c:b]{(b)}, which means that $\mathfrak C$ is an algebra
...
satisfies the postulates for modular Hilbert algebras except possibly
  postulate \modcondref{VIII}, so we shall only prove postulate \modcondref{VIII}.  Let
...
% Source PDF page 61 (printed page 60)
  \modcondref{VIII}.  However, since for each real number $t$,
...
$(1+\Delta(t))\mathfrak A$ is dense in $\mathfrak A$; hence
  postulate \modcondref{VIII} is also satisfied.  Therefore we get the following:
...
Then it is clear that $\mathfrak A$ satisfies the postulates for
  generalized Hilbert algebras except possibly postulate \modcondref{IX}.
...
$y\xi_0\mapsto y^*\xi_0$, $y\in M'$, with dense domain $M'\xi_0$.
  Therefore postulate \modcondref{IX} holds.  This completes the proof.
\end{proof}
...
\begin{equation}
  ...
  \label{eq:13.1}
\end{equation}
\refstepcounter{equation}\label{eq:13.2}
\noindent
\begin{minipage}[c]{\dimexpr\linewidth-4.2em\relax}
\setlength{\parindent}{0pt}%
There is a constant $\beta>0$ and, for each pair $a,b$ in $M$, there
exists a function $F$ holomorphic in $0<\operatorname{Im}z<\beta$ with
boundary values:
\setlength{\abovedisplayskip}{0pt}% Keep the two parts of (13.2) together.
\setlength{\abovedisplayshortskip}{0pt}%
\[
  F(t)=\varphi_0(\sigma_t(a)b)
  \quad\text{and}\quad
...
\]
\Cref{lem:14.3} means also that $\tau$ is a positive linear functional on
...
We have\ednote{The source prints the following equation number as
  ``(15.16)''; corrected to \eqref{eq:15.6}, as required by the sequence and by
