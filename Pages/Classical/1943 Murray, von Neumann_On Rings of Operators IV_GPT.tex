...
\makeatletter
\newcommand{\MvNSetCurrentLabel}[2]{%
  \edef\@currentlabel{#1}%
  \edef\@currentlabelname{#2}%
}
\newcommand{\MvNTargetLabel}[3]{%
  \phantomsection
  \begingroup
    \MvNSetCurrentLabel{#2}{#3}%
    \label{#1}%
  \endgroup
}
\newcommand{\FootnoteTarget}[1]{%
  \MvNTargetLabel{#1}{\thefootnote}{Footnote \thefootnote}}
\makeatother
...
\NewDocumentCommand{\PaperChapter}{m m m O{#3}}{%
  ...
  \MvNTargetLabel{#2}{\Roman{paperchapter}}{Chapter \Roman{paperchapter}}%
  \addcontentsline{toc}{section}{\texorpdfstring{Chapter \Roman{paperchapter}. #3}{Chapter \Roman{paperchapter}. #4}}%
...
\NewDocumentCommand{\IntroductionHeading}{}{%
  \MvNTargetLabel{chapter:introduction}{Introduction}{Introduction}%
...
\newcommand{\secref}[2]{\hyperref[#1]{\mbox{\S#2}}}
\newcommand{\chapterref}[2]{\hyperref[#1]{\mbox{Chapter~#2}}}
\newcommand{\thmref}[2]{\hyperref[#1]{\mbox{Theorem~#2}}}
\newcommand{\lemref}[2]{\hyperref[#1]{\mbox{Lemma~#2}}}
\newcommand{\defref}[2]{\hyperref[#1]{\mbox{Definition~#2}}}
...
% Three visibly distinct strokes are required here. Negative kerns collapse
% the three vertical delimiters into an apparent ordinary norm in newpxmath.
...
\MvNTargetLabel{contents}{Contents}{Contents}
...
We assume now that $\M$ is a factor in a finite case.\footnote{\FootnoteTarget{fn:1}A finite case is
...
$\mathfrak N=\Hilbert$.  Now $\mathfrak N\mathrel{\eta}\M'$ (cf. loc. cit.,
Lemma~5.1.1).  Hence $\mathfrak N\mathrel{\eta}\M$ and $E=P_{\mathfrak N}\in\M$.
...
\begin{align*}
  ...
  \Metric{U^\nu-V^\nu}
  &=\Metric{\sum_{\mu=1}^{\nu}U^{\mu-1}(U-V)V^{\nu-\mu}}\\
  % Editorial note E67: the source prints an equality sign in this estimate;
  % the triangle inequality requires the displayed \leq sign.
  &\leq\sum_{\mu=1}^{\nu}
...
\[
  ...
  \end{cases}
\]
It is clear that $\psi(\lambda)=\lambda\theta(\lambda)$.  Choose a
polynomial $p_\nu(\lambda)$ with
\[
  \theta(\lambda)\geq p_\nu(\lambda)
...
\[
  \OpNorm{q_\nu(A_{\nu'})-\psi(A_{\nu'})}\leq\frac1\nu,\footnote{\hypertarget{fn:11}{}The set
...
and
\begin{equation}
  \OpNorm{q_\nu(A_{\nu'})}\leq\OpNorm{A}.{}^{\hyperlink{fn:11}{11}}
...
\begin{description}
  ...
  \item[$\theta)$] Strong convergence closure
...
\lemref{lem:1.3.2}{1.3.2} yields $\alpha)\Leftrightarrow\theta)$.  From
...
$\theta)\Leftrightarrow\eta)$.  These equivalences give together
\[
  \alpha)\Leftrightarrow\beta)\Leftrightarrow\gamma)
  \Leftrightarrow\epsilon)\Leftrightarrow\theta)
...
algebraical type $\overline{\M}$ (and not $\M$ itself).\footnote{\FootnoteTarget{fn:12}This definition
...
$\mathfrak J^{\circ}$ then maps $\M$ in $\Hilbert$ on $\M_c$ in $\Hilbert_c$
and it is in this aspect a conjugate isomorphism of $\M$ and $\M_c$.

Consider next the mapping $\mathfrak J^*$, $A\mapsto A^*$.  This maps $\M$ in
$\Hilbert$ on $\M$ in $\Hilbert$ and it is, in this aspect, a conjugate dual
isomorphism of $\M$ and $\M$.  Consequently
$\mathfrak J^*\mathfrak J^{\circ}$ (i.e. $\mathfrak J^*$ in a new aspect) is a
...
of \eqref{eq:2.5.gamma} is necessarily $\Hilbert$ itself.
...
\begin{equation}
  ...
  $f\in\infty\otimes\Hilbert$ to an $A\in\M_1$ in the strong sense.
  \end{minipage}
  \right.
...
\footnote{\FootnoteTarget{fn:15}Observe that in this presentation $\overline{\N}$ is determined
...
$\overline{\M}^{\infty}$).}
\end{theorem}
(Concerning the omission of this extra condition, cf.
\hyperref[cor:2.6.1]{the corollary below}.)
\begin{proof}
...
\begin{corollary}\MvNTargetLabel{cor:2.6.1}{2.6.1}{Corollary to Lemma 2.6.1}
...
of \lemref{lem:2.6.2}{2.6.2} to find $E_2,\ldots,E_p$.
\mbox{The rest of that proof then applies to the present situation.}
\end{proof}
...
As we remarked in \hyperref[fn:15]{footnote~\ref*{fn:15}}, we may add
...
(i), (ii) in \lemref{lem:2.6.1}{2.6.1}.
...
If $\mathfrak M_1,\mathfrak M_2$ are two closed linear sets $\mathrel{\eta}\M$ with
...
$\mathfrak M_1,\mathfrak M_2$ be $\mathrel{\eta}\M_1,\mathrel{\eta}\M_2$ respectively, and such
...
\begin{corollary}\MvNTargetLabel{cor:2.10.3}{2.10.3}{Corollary to Lemma 2.10.3}
...
\hyperref[cor:2.10.3]{corollary} to \lemref{lem:2.10.3}{2.10.3}, and
\eqref{eq:2.4.alpha} or
\eqref{eq:2.9.alpha} imply $\overline{\M}^{c}=\overline{\M}$.

Ad $\mathfrak G=(1)$.  By \lemref{lem:2.10.3}{2.10.3} and
\hyperref[cor:2.10.3]{its corollary},
...
\(\mathfrak M\mathrel{\eta}\M\).

% Source PDF p. 28 (journal p. 743).
\begin{lemma}\phantomsection\label{lem:3.1.1}
If \(\mathfrak M\) is a closed linear set, \(\mathfrak M\mathrel{\eta}\M\), and such
that \(\mathfrak M\ne(0)\) and
...
If \(\mathfrak M\) is a closed linear set, \(\mathfrak M\mathrel{\eta}\M\) and
...
% Editorial note E64: the source prints ``Lemma 2.7.3''; corrected to
% ``Lemma 7.2.3'', the lemma actually appearing on the cited page.
...
\(\mathfrak M\mathrel{\eta}\M\) in the Hilbert space \(\Hilbert\) of \(\M\), then we
...
cases is readily shown by reference to \lemref{lem:2.10.3}{2.10.3} and
\hyperref[cor:2.10.3]{its corollary}.) \lemref{lem:3.1.4}{3.1.4}, with
...
\(\M\) and two closed linear sets \(\mathfrak M,\mathfrak N\mathrel{\eta}\M\), which
...
\(\mathrel{\eta}\M\) and \(\subseteq\mathfrak N\), \(D_{\M}(\mathfrak P)\) is a
...
\lemref{lem:2.10.3}{2.10.3} and
\hyperref[cor:2.10.3]{its corollary} being made in the discrete case.
...
spatial type (cf. \hyperref[fn:12]{footnote~\ref*{fn:12}}). We shall call the
abstraction of \(\M\) with
...
We shall now answer Question II in
\hyperref[intro:2]{\S2 in the introduction}, for the cases (I)
and (II)\footnote{In the cases (I) of course the answer is known. Nevertheless
we include them for the reason given in
\hyperref[fn:1]{footnote~\ref*{fn:1}}.}; i.e. determine when an
...
\(\Hilbert_2\) (cf.
\hyperref[intro:2]{Definition A in \S2 in the Introduction}) will also carry
...
\(\mathfrak M\mathrel{\eta}\M\) which is not \(=(0)\) and has a finite
...
\(\mathrel{\eta}\M\) and \(\mathrel{\eta}\M'\), respectively, both \(\ne(0)\), and
\(\mathfrak M_f^{\M'}\subseteq\mathfrak M\), since
\(f\in\mathfrak M\mathrel{\eta}\M\). Hence \(D_{\M}(\mathfrak M_f^{\M'})\) is
finite.
...
precisely the same proof. Our remark in
\hyperref[fn:1]{footnote~\ref*{fn:1}} could have been applied to
...
\begin{proof}
As in \hyperref[intro:2]{the introduction, \S2}, for each \(i=1,2\) a Hilbert space
...
\hyperref[intro:2]{Introduction, \S2}) which carries \(\M_1\) into \(\M_2\).
...
Choose accordingly a \(\mathfrak K_i\mathrel{\eta}\M_i\), with
...
\(\mathfrak L_1\mathrel{\eta}\M_1\), with
...
\(\mathfrak L_1\mathrel{\eta}\M_1\),
\(\mathfrak L_1\subseteq\mathfrak K_1\) imply
\(\mathfrak L_1\mathrel{\eta}\M_{1(\mathfrak K_1)}\). So \(\mathfrak I\) carries
\(\mathfrak L_1\) into an \(\mathfrak L_2\mathrel{\eta}\M_{2(\mathfrak K_2)}\). It
follows that \(\mathfrak L_2\mathrel{\eta}\M_2\) and
...
\(\mathfrak N_{i;u}\mathrel{\eta}\M_i\),
...
convergence of \(\sum_{u,v=1}^{\infty}\) in
\hyperlink{fn:24}{footnote~24} is readily seen to
...
\(\mathfrak K_1\mathrel{\eta}\M_1\), \(\mathfrak K_1\ne(0)\), and put
...
\(\mathfrak K_2\mathrel{\eta}\M_2\), \(\mathfrak K_2\ne(0)\). Then the isomorphism of
...
\(\mathfrak M_{f_0}^{\R'}\mathrel{\eta}\R\),
\(\mathfrak M_{f_0}^{\R}\mathrel{\eta}\R'\) (cf.
\cite[p.~143, Def.~5.1.1]{ref5}). We may assume that
\(\mathfrak M_{f_0}^{\R'}\) is finite dimensional. (Choose a finite
dimensional \(\mathfrak M\mathrel{\eta}\R\), \(\mathfrak M\ne(0)\), and then
...
\begin{equation}
 ...
 \quad\text{for a suitable }\mathfrak L\mathrel{\eta}\R.
...
\begin{equation}
 ...
 \quad\text{for a suitable }\mathfrak L'\mathrel{\eta}\R',\quad
...
\(\mathfrak L''\mathrel{\eta}\R_{(\mathfrak L)}\), and
\[
  \alpha
...
\]
(\(\mathfrak L''\mathrel{\eta}\R_{(\mathfrak L)}\) implies
\(\mathfrak L''\mathrel{\eta}\R\).) The membership
...
in the \hyperref[cor:2.10.3]{Corollary} to
\lemref{lem:2.10.3}{2.10.3}.
...
i.e. \hyperref[def:4.3.1]{Definitions 4.3.1},
\hyperref[def:4.5.2]{4.5.2} and \hyperref[def:4.6.1]{4.6.1}, below:
...
\[
  \Metric{F_1-F}
  =\bigl(\lvert D_{\M}(F_1)-D_{\M}(F)\rvert\bigr)^{1/2}.\footnote{\hypertarget{fn:35}{}
...
\begin{equation}\tag{4.4.\(\delta\)}\label{eq:4.4-delta}
 % Editorial note E65: the source prints the lower limit as i=0; corrected
 % to i=1, which is forced by the matrix-unit index range used here.
...
% Editorial note E68: the source prints q_s as the upper index range here;
% q is required for the q-order matrix base of O^+.
...
\begin{equation}\tag{4.5.\(\theta\)}\label{eq:4.5-theta}
 E_s^+=\sum_{u=l_{s-1}+1}^{l_s}W^+_{u,u}
...
\begin{equation}\tag{4.5.\(\iota\)}\label{eq:4.5-iota}
 ...
 &\quad\text{for }s=1,\ldots,\bar r,\quad u,v=1,\ldots,q_s.
 \end{aligned}\right.
 % Editorial note E69: the source prints u=1 as the lower summation index;
 % i=1 is required because i is the summation variable in the summand.
\end{equation}
Then one verifies by a direct computation that the \(E_s^+\) are mutually
orthogonal projections \(\ne0\), with \(\sum_{s=1}^{\bar r}E_s^+=1\), and
(using \defref{def:2.6.1}{2.6.1}) that the \(W^{+s}_{u,v}\) form, for any
fixed \(s\), a system of \(q_s\)-order matrix units with
\(\sum_{u=1}^{q_s}W^{+s}_{u,u}=E_s^+\). All these operators are in \(\M\).
...
\begin{equation}\tag{4.6.\(\pi\)}\label{eq:4.6-pi}
 ...
 % Editorial note E66: the source prints the upper index as \bar r;
 % corrected to m, the range of the given operators B_1,...,B_m.
\end{equation}
...
% Editorial note E70: the source prints E in M' dot N; the proof and
% equation (4.6.mu) require E in M dot N'.
...
\(\Metric{1-E}<\epsilon'^{1/2}\)\hspace{0.08em}%
\raisebox{0.45ex}{\textsuperscript{\hyperlink{fn:35}{35}}}, and therefore
\[
...
\(\N_1=\N_1(q,A_1,\ldots,A_m,\epsilon)=\mathbf O\).\footnote{\phantomsection\label{fn:38}Observe
...
\begin{proof}
Immediate by comparison of \hyperref[def:4.1.1]{\mbox{Definitions 4.1.1}}
and \hyperref[def:4.6.1]{\mbox{4.6.1}}.
\end{proof}
...
\]
were established in Lemmas \hyperref[lem:4.4.3]{4.4.3},
\hyperref[lem:4.5.1]{4.5.1}, and \hyperref[lem:4.6.2]{4.6.2}, respectively.
Consequently
\[
...
\]
by Lemmas \hyperref[lem:4.6.3]{4.6.3},
\hyperref[lem:4.6.4]{4.6.4}, respectively;
hence
\[
...
\hyperref[sec:5.2]{\S\S5.2}, \hyperref[sec:5.3]{5.3} and
\hyperref[sec:5.6]{5.6} we shall give various examples of such factors. Actually
...
\secref{sec:2.10}{2.10} will now be studied for the algebraical type
...
isomorphisms (cf. \(B^*\)) in \secref{sec:2.2}{2.2}) mentioned above.
\end{proof}
...
\begin{equation}\tag{4.8.\(\theta\)}\label{eq:4.8-theta}
 \Metric{A_{(\mathfrak M)}}_{\M_{(\mathfrak M)}}
 =\left(\frac1\alpha\right)^{1/2}\Metric{A}_{\M}.
 \text{\textsuperscript{\hyperref[fn:41]{\thefootnote}}}
\end{equation}
\footnotetext[\value{footnote}]{\phantomsection\label{fn:41}As a rule we did not indicate the factor (in a finite case) with
...
\(\M_{(\mathfrak M)}\) with
\(B_{1(\mathfrak M)},\ldots,B_{m(\mathfrak M)},
\epsilon/(2\alpha^{1/2})\) in
...
\[
 ...
 =\frac1{\alpha^{1/2}}\Metric{C_s-B_s}_{\M}
\]
...
We restate the results of \hyperref[lem:4.8.1]{\mbox{Lemmas 4.8.1}} and
\hyperref[lem:4.8.4]{\mbox{4.8.4}}.
...
Now we shall prove in \hyperref[thm:XVI]{\mbox{Theorems XVI}},
\hyperref[thm:XVI-prime]{\mbox{XVI\(^{\prime}\)}} that not approximately finite
...
However, as stated above, the opposite will be proven in
\hyperref[thm:XVI]{\mbox{Theorems XVI}},
\hyperref[thm:XVI-prime]{\mbox{XVI\(^{\prime}\)}}, and we
...
type \(\overline{\mathbf A}\)) by the indirect procedure of
\hyperref[thm:XIII]{\mbox{Theorems XIII}} and
\hyperref[thm:XIV]{\mbox{XIV}}.\footnote{The decisive
passage from \hyperref[thm:XIII]{\mbox{Theorem XIII}} to
\hyperref[thm:XIV]{\mbox{Theorem XIV}} makes use
...
\begin{equation}\tag{5.2.\(\delta\)}\label{eq:5.2-delta}
 ...
 a_0\in\mathfrak G_1,\ i=1,2,\ldots).
\end{equation}
...
\defref{def:4.1.1}{4.1.1}---in contrast with
\hyperref[fn:44]{footnote~\ref*{fn:44}}.}
...
\[
 W=W^{-1},\qquad WU_{a_0}W=V_{a_0}.\footnote{\hypertarget{fn:48}{}One also verifies immediately
\[
...
mentioned at the end of \hyperref[fn:49]{footnote~\ref*{fn:49}}). Thus
\(A\in J''\) and hence
...
examples at the end of \secref{sec:5.5}{5.5} and in
\secref{sec:5.6}{5.6}. Clearly no such group can be abelian.
...
\(\mathfrak G\) (cf. \hyperlink{fn:48}{footnote~48}), and so both \(\M=\R(I)\) and
...
computation rules indicated at the end of
\hyperref[fn:49]{footnote~\ref*{fn:49}}.) Hence
...
As we pointed out, the construction of \secref{sec:5.3}{5.3} is closely
...
\(x\) set of measure zero.} In \secref{sec:5.3}{5.3}, \(\mathfrak G\) had
to fulfill (i) of \lemref{lem:5.3.4}{5.3.4}, but the (iii) referred to was most
flagrantly violated.\textsuperscript{\hyperref[fn:56]{56},\,\hyperref[fn:57]{57}}
\addtocounter{footnote}{2}
\footnotetext[56]{\phantomsection\label{fn:56}We have \(x_0a=x_0\) and
\(\mu((x_0))=\mu(S)\ne0\).}
\footnotetext[57]{\phantomsection\label{fn:57}There is no difference between the two
...
\[
 ...
 \text{\textsuperscript{\hyperref[fn:62]{62}}}
\]
...
\refstepcounter{footnote}
\footnotetext[62]{\phantomsection\label{fn:62}Owing to
\(\bar x=[\bar\alpha_a;a\in\mathfrak G]\in\mathfrak S\)
...
\hyperref[fn:60]{footnote~\ref*{fn:60}}, we
...
(V) Apply the construction of \secref{sec:5.3}{5.3} to the group
...
\(\bar b\in\mathfrak G\). Cf.
\hyperref[fn:65]{footnote~\ref*{fn:65}}. Thus our statement is true for
...
\secref{sec:5.3}{5.3} to \(\{\mathfrak S,\mathfrak G\}\) (in place of its
\(\mathfrak G\)).
...
should like to apply the construction of \secref{sec:5.3}{5.3}. Since
...
\thmref{thm:I}{I} (of \secref{sec:1.6}{1.6} above) shows that our statement
...
\chapterref{chapter:I}{I} may be carried through on the assumption that
\(\M\) is a group
algebra.
...
\secref{sec:5.3}{5.3} too, and this time it is even easier to prove.

\begin{lemma}\phantomsection\label{lem:5.6.1}
The ring \(\M\) of \secref{sec:5.3}{5.3} is approximately finite if
...
The analysis of the invariants (1), (2) of \secref{sec:4.8}{4.8}, gives no
new information. As to (1), all examples of \cite{ref5} as well as
those of \secref{sec:5.3}{5.3} have
...
\[
  ...
  \text{\textsuperscript{\hyperref[fn:68]{68},\,\hyperref[fn:69]{69}}}
\]
\end{samepage}
\addtocounter{footnote}{2}
\footnotetext[68]{\phantomsection\label{fn:68}The requirement
$\operatorname{Tr}_{\M}(U)=0$ is necessary,
...
finite $\M$ would not have this property---but we need
\lemref{lem:6.1.2}{6.1.2}.}
\footnotetext[69]{\phantomsection\label{fn:69}This property could be further subdivided, according to the values
...
(cf. \hyperlink{fn:48}{footnote~48}) and so
\[
...
based on \hyperref[def:4.1.1]{Def.~4.1.1}.}
...
the use of the free group in \lemref{lem:6.2.2}{6.2.2} should be noted.}
...
Combining \lemref{lem:6.2.2}{6.2.2} with
\hyperref[cor:6.2.1]{Corollary 1} to
...
\hyperref[cor:6.2.2]{Corollary 2} of that lemma (for the same $\mathfrak F$).
\end{lemma}
...
To establish (i$_1$) of \lemref{lem:6.2.1}{6.2.1} and (i$_2'$) of
\hyperref[cor:6.2.2]{Corollary 2} of that lemma for the same $\mathfrak F$,
we define $\mathfrak F$ as the
...
\[
  \mathfrak F+c_1^{-1}\mathfrak F c_1=\mathfrak G-(1).
\]
One can establish (i$_2'$) of \hyperref[cor:6.2.2]{Corollary 2} of
\lemref{lem:6.2.1}{6.2.1} in
...
\lemref{lem:6.2.1}{6.2.1} or the (i$_2'$) of
\hyperref[cor:6.2.2]{its second corollary}. However,
...
\MvNTargetLabel{appendix}{Appendix}{Appendix}
...
\item Assume $r\in A$. Then $\mathfrak H_r$ satisfies the
  ...
  (i$_1$) and (i$_2'$) of \lemref{lem:6.2.1}{6.2.1} and
  \hyperref[cor:6.2.2]{its Corollary 2} and (i)
...
(i$_1$) and (i$_2'$) of \lemref{lem:6.2.1}{6.2.1} and
\hyperref[cor:6.2.2]{its Corollary 2} are
...
\begin{corollary}\MvNTargetLabel{cor:A.2}{}{Corollary}
...
this by taking $f=\varphi_1$ in the sense of \eqref{eq:5.3-alpha}. For if $A\in\M$,
...
$\varphi_a$, $a\in\mathfrak G$ of \eqref{eq:5.3-alpha} and hence for the linear
...
combinations of the $U_c$ and hence is $\mathbin{\eta}\M$. Cf.
\hyperref[lem:5.3.1]{Lemmas 5.3.1},
\hyperref[lem:5.3.2]{5.3.2} and \hyperref[lem:5.3.6]{5.3.6}.
...
Let $\mathfrak M^0$ be the set $\subseteq\Hilbert^{0,0}$ determined by the
$\varphi_a$ with $a\in\mathfrak G^0$ (cf. \eqref{eq:5.3-alpha}). It is clear that we
...
Ad (v). This is implied by \lemref{lem:A.1}{A.1} (vi) and
\hyperref[cor:6.2.2]{Corollary 2} of
\lemref{lem:6.2.1}{6.2.1}.
