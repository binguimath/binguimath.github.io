...
- 9/19 一致有界的（多重）线性映射的逐点收敛性由其在稠密子空间上的收敛决定，l^p上的弱星收敛，弱星收敛等价于矩阵收敛，弱星收敛与逐点收敛的联系，有界线性泛函被基本函数弱星逼近，用逼近论来证明对偶空间刻画定理（以Riesz-Fischer定理为例），Dirac测度线性组合的弱星稠密性（知道有这件事即可），从有限逼近到线性扩张的转换
...
- 12/26 有紧预解式的无界算子，无界算子的Hilbert-Schmidt定理与Fredhol Alternative，有界开集上Dirichlet Laplacian有紧预解式，Fredholm Alternative在椭圆方程的用法，Poincare不等式，Hahn-Banach定理的矩问题背景，矩问题的线性扩张解法与有限逼近解法

  讲义章节：8.8.2，8.8.3，8.9，9.1.1，9.2.1

- 12/30 矩问题的线性扩张解法，Hahn-Banach定理，复线性泛函与实线性泛函的等价，线性子空间先取annihilator再取pre-annihilator等于其闭包，稠密性等价于annihilator为零，赋范空间到其二次对偶空间的典范映射J，Hahn-Banach定理推出J保距，有界算子与其转置具有相同的范数，矩问题的有限逼近解法，商赋范空间，L(V/U,W)-->L(V,W)的等距性及其像的刻画
