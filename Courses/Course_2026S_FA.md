...
注：讲义7.A中对于向量值函数积分的处理比较一般，实际上这门课会遇到的积分都是紧支集连续函数的积分，因此可以用黎曼积分来理解。所以基本上只需要看7.A.1中的内容就行了。课上用到的$\langle \eta|\int fdt\rangle=\int \langle \eta|f\rangle dt$可以用黎曼和逼近来证明。
